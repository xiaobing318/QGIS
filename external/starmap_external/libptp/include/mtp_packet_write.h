 ...
 */
HANDLE64 mtp_create(const char *out_file, int tile_size, const char *mtp_id, const char *metadata,
 ...
 * 	失败 - 返回0，通过参数status可以了解到失败原因
 */
HANDLE64 mtp_wrtie_open(const char *filename, BOOL readonly, const char *private_key, uint32 private_key_len,
 ...
 * @param status		- 状态（错误状态：0 - 无错误，其它参考status.h）
 */
void add_tile_ranges(HANDLE64 handle, uint32 zoom, uint32 xmin, uint32 xmax, uint32 ymin, uint32 ymax, uint32 *status);
 ...
 * @param status		- 状态（错误状态：0 - 无错误，其它参考status.h）
 */
void add_tile(HANDLE64 handle, const char *filename, uint32 zoom, uint32 x, uint32 y, BOOL is_encrypt, uint32 *status);
 ...
 * @param status		- 状态（错误状态：0 - 无错误，其它参考status.h）
 */
void add_tile_with_data(HANDLE64 handle, char *buff, uint32 buff_size, uint32 zoom, uint32 x, uint32 y, BOOL is_encrypt, uint32 *status);
 ...
 * 	FALSE - 制定区域没有存在瓦片
 */
BOOL exist_tile(HANDLE64 handle, uint32 zoom, uint32 x, uint32 y, uint32 *status);
 ...
 * @param status		- 状态（错误状态：0 - 无错误，其它参考status.h）
 */
void mtp_write_close(HANDLE64 handle, uint32 *status);
