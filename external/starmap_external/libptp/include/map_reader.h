 ...
 *   供其它函数操作的句柄
 */
HANDLE64 open_map(const char *map_path, const char *map_id,
 ...
 *   地图的类型，见T_MAP_TYPE
 */
int get_production_type(HANDLE64 fd);
 ...
 *   json格式的元数据
 */
char *get_map_metadata(HANDLE64 fd);
 ...
 *   个数
 */
int get_map_legend_count(HANDLE64 fd);
 ...
 *   数据（图像）
 */
byte *get_map_legend(HANDLE64 fd, uint32 legend_index, uint32 *data_size, uint32 *status);
 ...
 *   个数
 */
int get_map_desc_count(HANDLE64 fd);
 ...
 *   数据（图像）
 */
byte *get_map_desc(HANDLE64 fd, uint32 desc_index, uint32 *data_size, uint32 *status);
 ...
 *   数据（图像）
 */
byte *get_map_thumb_max(HANDLE64 fd, uint32 *data_size, uint32 *status);
 ...
 *   数据（图像）
 */
byte *get_map_thumb_min(HANDLE64 fd, uint32 *data_size, uint32 *status);
 ...
 *   数据（图像）
 */
byte *get_map_thumb_fit(HANDLE64 fd, uint32 *data_size, uint32 *status);
 ...
 *   数据（图像）
 */
byte *get_map_thumb_micro(HANDLE64 fd, uint32 *data_size, uint32 *status);
 ...
 *   二进制数据
 */
byte *get_tile(HANDLE64 fd, uint32 zoom, uint32 x_index, uint32 y_index, BOOL crop, uint32 *data_len, uint32 *status);
 ...
 * 	   数据
 */
byte *get_file(HANDLE64 fd, const char *file_name, uint32 *data_size, uint32 *status);
 ...
 */
byte check_tile_edge(HANDLE64 fd, uint32 zoom, uint32 x_index, uint32 y_index, uint32 *status);
 ...
 * 	成功返回true，失败返回false
 */
BOOL get_all_guid(HANDLE64 fd, char **all_guid, uint32 **all_type, char **all_thumb_fit_id, char **all_thumb_max_id, char **all_thumb_min_id, char **all_thumb_micro_id, int32 *result_count);
