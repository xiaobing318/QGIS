...
#include "SW_NJBDX_Config_Reader.h"
#pragma endregion
...
/*---------------------------------------------*
 * 1. 读取 SW_Layers_Fields_Info.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 SW_Layers_Fields_Info.json 配置文件开始******************************************************************");
  logger->info("SW_Layers_Fields_Info.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  //  最终要返回的结构体
  SW_Layers_Fields_Info_Json_t data;
  ...
  //  3. 解析 "SW_Layers_Fields" 数组
  if (!j.contains("SW_Layers_Fields") || !j["SW_Layers_Fields"].is_array())
  {
    logger->error("配置文件中未找到 'SW_Layers_Fields' 字段或字段类型不正确，返回空结构体。");
    // 根据需求：也可以抛异常
    return data;
  }

  // 遍历 "SW_Layers_Fields"
  for (const auto& layerObj : j["SW_Layers_Fields"])
  {
    SW_Layers_Fields_t layerData;

    // 3.1 解析 "SW_Layer_Name"
    if (layerObj.contains("SW_Layer_Name") && layerObj["SW_Layer_Name"].is_string())
    {
      layerData.SW_Layer_Name = layerObj["SW_Layer_Name"].get<std::string>();
    }
    else
    {
      logger->warn("某个图层信息中缺少 'SW_Layer_Name' 字段或类型错误。");
    }
    ...
    {
      logger->warn("图层 '{}' 中缺少 'Size_Of_Fields' 字段或类型错误，默认为 0。", layerData.SW_Layer_Name);
    ...
    if (layerObj.contains("Fields") && layerObj["Fields"].is_array())
    {
      for (const auto& fld : layerObj["Fields"])
      {
        ...
        {
          logger->warn("图层 '{}' 有字段未提供 'chinese_field_name'，将使用空字符串。", layerData.SW_Layer_Name);
        }
        ...
        {
          logger->warn("图层 '{}' 有字段未提供 'english_field_name'，将使用空字符串。", layerData.SW_Layer_Name);
        }
        ...
        if (fld.contains("field_type") && fld["field_type"].is_string())
        {
          ...
          {
            logger->warn("图层 '{}' 中未知的字段类型 '{}', 将默认设为 OFTString。", layerData.SW_Layer_Name, typeStr);
            field.field_type = OFTString; // 默认
          }
        }
        else
        {
          logger->warn("图层 '{}' 有字段未提供 'field_type'，默认为 OFTString。", layerData.SW_Layer_Name);
          field.field_type = OFTString;
        }
        ...
        if (fld.contains("field_width"))
        {
          ...
          else if (fld["field_width"].is_string())
          {
            ...
            {
              logger->warn("图层 '{}' 中字段 'field_width' 无法解析为整数，使用默认值 0。", layerData.SW_Layer_Name);
              field.field_width = 0;
            }
          }
          else
          {
            logger->warn("图层 '{}' 中 'field_width' 类型不符合预期，使用默认值 0。", layerData.SW_Layer_Name);
            field.field_width = 0;
          }
        }
        else
        ...
        if (fld.contains("field_precision"))
        {
          ...
          else if (fld["field_precision"].is_string())
          {
            ...
            {
              logger->warn("图层 '{}' 中字段 'field_precision' 无法解析为整数，使用默认值 0。", layerData.SW_Layer_Name);
              field.field_precision = 0;
            }
          }
          else
          {
            logger->warn("图层 '{}' 中 'field_precision' 类型不符合预期，使用默认值 0。", layerData.SW_Layer_Name);
            field.field_precision = 0;
          }
        }
        else
    ...
    {
      logger->warn("图层 '{}' 缺少 'Fields' 字段或其类型不是数组，vFields 将为空。", layerData.SW_Layer_Name);
    }

    // 将处理好的图层信息放入 data 中
    data.vSW_Layers_Fields.push_back(layerData);
  }


  logger->info("已完成对 SW_Layers_Fields_Info.json 的解析，共解析到 {} 个图层。", data.vSW_Layers_Fields.size());
  //  日志结束
  logger->info("******************************************************************读取 SW_Layers_Fields_Info.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 2. 读取 SW_Layers_Mapping_NJBDX_Layers.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 SW_Layers_Mapping_NJBDX_Layers.json 配置文件开始******************************************************************");
  logger->info("SW_Layers_Mapping_NJBDX_Layers.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  //  最终要返回的结构体
  SW_Layers_Mapping_NJBDX_Layers_Json_t data;
  ...
  //  3. 解析 "SW_layers_mapping_NJBDX_layers" 数组
  if (!j.contains("SW_layers_mapping_NJBDX_layers") || !j["SW_layers_mapping_NJBDX_layers"].is_array())
  {
    logger->error("配置文件中缺少 'SW_layers_mapping_NJBDX_layers' 数组或格式不正确，返回空结构体。");
    return data; // 这里直接返回空结构体，也可根据需求抛异常
  }

  //  遍历 "SW_layers_mapping_NJBDX_layers"
  for (const auto& mappingObj : j["SW_layers_mapping_NJBDX_layers"])
  {
    // 用于存放单个映射关系
    SW_layers_mapping_NJBDX_layers_t oneMapping; 

    // 3.1 解析 "SW_Layer" 对象
    if (mappingObj.contains("SW_Layer") && mappingObj["SW_Layer"].is_object())
    {
      auto SWLayerObj = mappingObj["SW_Layer"];

      // 1、解析 "SW_layer_english_name"
      if (SWLayerObj.contains("SW_layer_english_name") && SWLayerObj["SW_layer_english_name"].is_string())
      {
        oneMapping.SW_Layer.SW_layer_english_name = SWLayerObj["SW_layer_english_name"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_english_name' 字段或类型错误。");
      }

      // 2、解析 "SW_layer_chinese_name"
      if (SWLayerObj.contains("SW_layer_chinese_name") && SWLayerObj["SW_layer_chinese_name"].is_string())
      {
        oneMapping.SW_Layer.SW_layer_chinese_name = SWLayerObj["SW_layer_chinese_name"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_chinese_name' 字段或类型错误。");
      }

      // 3、解析 "SW_layer_notes"
      if (SWLayerObj.contains("SW_layer_notes") && SWLayerObj["SW_layer_notes"].is_string())
      {
        oneMapping.SW_Layer.SW_layer_notes = SWLayerObj["SW_layer_notes"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_notes' 字段或类型错误。");
      }

      // 4、解析 "SW_layer_type"
      if (SWLayerObj.contains("SW_layer_type") && SWLayerObj["SW_layer_type"].is_string())
      {
        std::string typeStr = SWLayerObj["SW_layer_type"].get<std::string>();
        try
        {
          oneMapping.SW_Layer.SW_layer_type = stringToOGRGeometryType(typeStr);
        }
        catch (const std::invalid_argument& e)
        {
          logger->error("SW_layer_type 转换失败: {}", e.what());
          //  根据需求，可以选择抛出异常或设置为默认类型
          oneMapping.SW_Layer.SW_layer_type = wkbUnknown;
        }
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_type' 字段或类型错误。");
      }

      // 5、解析 "SW_layer_field_number"
      if (SWLayerObj.contains("SW_layer_field_number") && SWLayerObj["SW_layer_field_number"].is_number_integer())
      {
        oneMapping.SW_Layer.SW_layer_field_number = SWLayerObj["SW_layer_field_number"].get<int>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_field_number' 字段或类型错误，默认为 0。");
        oneMapping.SW_Layer.SW_layer_field_number = 0;
      }

      // 6、解析 "SW_layer_extention" (目前结构体内为空，可按需扩展)
      if (SWLayerObj.contains("SW_layer_extention") && SWLayerObj["SW_layer_extention"].is_object())
      {
        // 暂时无字段可解析，如需后续扩展可在此处添加解析逻辑
      }
      else
      {
        logger->warn("某个映射信息中缺少 'SW_layer_extention' 字段或类型错误（对象类型），但结构体中目前也没有扩展字段。");
      }
    }
    else
    {
      logger->warn("缺少 'SW_Layer' 对象或类型错误，该映射将仅保留默认空层信息。");
    }
    ...
    if (mappingObj.contains("NJBDX_Mapping_Layers") && mappingObj["NJBDX_Mapping_Layers"].is_array())
    {
      ...
      {
        SW_NJBDX_Mapping_Layer_t oneNJBDXLayer;
    ...
    // 将该映射结果放入最终 data 中
    data.vSW_layers_mapping_NJBDX_layers.push_back(oneMapping);
  }


  //  日志：完成解析
  logger->info("已完成对 SW_Layers_Mapping_NJBDX_Layers.json 的解析，共解析到 {} 条映射关系。", data.vSW_layers_mapping_NJBDX_layers.size());
  //  日志结束
  logger->info("******************************************************************读取 SW_Layers_Mapping_NJBDX_Layers.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 3. 读取 SW_Classification_Code_Conditional_Mapping_Lists.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 SW_Classification_Code_Conditional_Mapping_Lists.json 配置文件开始******************************************************************");
  logger->info("SW_Classification_Code_Conditional_Mapping_Lists.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  // 最终要返回的结构体
  SW_Classification_Code_Conditional_Mapping_Lists_Json_t data;
  ...
  // 3. 解析 "SW_Classification_Code_Conditional_Mapping_Lists" 数组
  if (!j.contains("SW_Classification_Code_Conditional_Mapping_Lists") || !j["SW_Classification_Code_Conditional_Mapping_Lists"].is_array())
  {
    logger->error("配置文件中缺少 'SW_Classification_Code_Conditional_Mapping_Lists' 数组或格式不正确，返回空结构体。");
    return data; // 根据需求：也可以抛异常
  }

  // 遍历 "SW_Classification_Code_Conditional_Mapping_Lists"
  for (const auto& itemObj : j["SW_Classification_Code_Conditional_Mapping_Lists"])
  {
    SW_Classification_Code_Conditional_Mapping_Lists_t oneItem;
    ...
    // (2) SW_Classification_Code_Mapping_Condition
    if (itemObj.contains("SW_Classification_Code_Mapping_Condition") && itemObj["SW_Classification_Code_Mapping_Condition"].is_string())
    {
      oneItem.SW_Classification_Code_Mapping_Condition = itemObj["SW_Classification_Code_Mapping_Condition"].get<std::string>();
    }
    else
    {
      logger->warn("有某个映射项缺少 'SW_Classification_Code_Mapping_Condition' 字段或类型错误，将使用空字符串。");
    }

    // (3) SW_Classification_Code_Mapping_Condition_Flag
    if (itemObj.contains("SW_Classification_Code_Mapping_Condition_Flag") && itemObj["SW_Classification_Code_Mapping_Condition_Flag"].is_string())
    {
      oneItem.SW_Classification_Code_Mapping_Condition_Flag = itemObj["SW_Classification_Code_Mapping_Condition_Flag"].get<std::string>();
    }
    else
    {
      logger->warn("有某个映射项缺少 'SW_Classification_Code_Mapping_Condition_Flag' 字段或类型错误，将使用空字符串。");
    }
    ...
    // 将解析到的结果加入集合
    data.vSW_Classification_Code_Conditional_Mapping_Lists.push_back(oneItem);
  }

  // 日志：完成解析
  logger->info("已完成对 SW_Classification_Code_Conditional_Mapping_Lists.json 的解析，共解析到 {} 条映射规则。", data.vSW_Classification_Code_Conditional_Mapping_Lists.size());

  //  日志结束
  logger->info("******************************************************************读取 SW_Classification_Code_Conditional_Mapping_Lists.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 4. 读取 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件开始******************************************************************");
  logger->info("SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  // 最终要返回的结构体
  SW_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t data;
  ...
  // 3. 解析 "SW_Layers_Other_Fields_Mapping" 数组
  if (!j.contains("SW_Layers_Other_Fields_Mapping") || !j["SW_Layers_Other_Fields_Mapping"].is_array())
  {
    logger->error("配置文件中缺少 'SW_Layers_Other_Fields_Mapping' 数组或格式不正确，返回空结构体。");
    return data; // 可按需求改为抛异常
  }

  //  遍历 "SW_Layers_Other_Fields_Mapping"
  for (const auto& layerObj : j["SW_Layers_Other_Fields_Mapping"])
  {
    SW_Layers_Other_Fields_Mapping_t layerMapping;
    ...
    if (layerObj.contains("Other_Fields_Mapping") && layerObj["Other_Fields_Mapping"].is_array())
    {
      ...
      {
        SW_Other_Fields_Mapping_t fieldMapping;
        ...
        // 2、解析 "SW_field_name"
        if (fieldObj.contains("SW_field_name") && fieldObj["SW_field_name"].is_string())
        {
          fieldMapping.SW_field_name = fieldObj["SW_field_name"].get<std::string>();
        }
        else
        {
          logger->warn("某个 Other_Fields_Mapping 缺少 'SW_field_name' 字段或类型错误，使用空字符串。");
        }
    ...
    // 将处理好的图层映射信息放入 data
    data.vSW_Layers_Other_Fields_Mapping.push_back(layerMapping);
  }

  //  日志：完成解析
  logger->info("已完成对 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 的解析，共解析到 {} 个图层。", data.vSW_Layers_Other_Fields_Mapping.size());

  //  日志结束
  logger->info("******************************************************************读取 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件结束******************************************************************");
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  //  最终要返回的结构体
  SW_NJBDX_Layers_Fields_Info_Json_t data;
  ...
  {
    SW_NJBDX_Layers_Fields_t layerData;
...
#pragma region "2 对五种配置文件中读取到的结构体进行正确性检查的接口"
 /**
  * @brief 检查 SW_Layers_Fields_Info_Json_t 的正确性
  */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 SW_Layers_Fields_Info.json 配置文件开始******************************************************************");


  // 1. Note 是否为空
  if (data.Note.empty())
  {
    logger->warn("[checkSW_Layers_Fields_Info_Json] Note 为空");
  }

  // 2. 遍历每一个图层，检查字段数量是否与数据实际一致
  for (const auto& layer : data.vSW_Layers_Fields)
  {
    // spdlog::debug(...) 可以输出更详细信息，或根据需要使用 spdlog::info/ spdlog::warn 等
    logger->debug("[checkSW_Layers_Fields_Info_Json] 检查的图层为: {}", layer.SW_Layer_Name);

    // Check Size_Of_Fields
    if (layer.Size_Of_Fields <= 0)
    {
      logger->warn("图层 '{}' 的Size_Of_Fields不合法--->{}", layer.SW_Layer_Name, layer.Size_Of_Fields);
      isOk = false;
    }
  ...
  //  日志结束
  logger->info("******************************************************************检查 SW_Layers_Fields_Info.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 SW_Layers_Mapping_NJBDX_Layers_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 SW_Layers_Mapping_NJBDX_Layers.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[checkSW_Layers_Mapping_NJBDX_Layers_Json] Note 为空");
  }

  for (const auto& mapping : data.vSW_layers_mapping_NJBDX_layers)
  {
    // 根据需要检查字段
    if (mapping.SW_Layer.SW_layer_english_name.empty())
    {
      logger->warn("SW_Layer 有一个空的english_name.");
      isOk = false;
    }
    ...
    {
      logger->warn("SW_Layer '{}' 没有NJBDX_Mapping_Layers映射", mapping.SW_Layer.SW_layer_english_name);
      isOk = false;
    }
  }

  //  日志结束
  logger->info("******************************************************************检查 SW_Layers_Mapping_NJBDX_Layers.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 SW_Classification_Code_Conditional_Mapping_Lists_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 SW_Classification_Code_Conditional_Mapping_Lists.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[SW_Classification_Code_Conditional_Mapping_Lists_Json] Note 为空。");
  }

  //  日志结束
  logger->info("******************************************************************检查 SW_Classification_Code_Conditional_Mapping_Lists.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 SW_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[checkSW_Other_Fields_Mapping_NJBDX_Other_Fields_Json] Note为空。");
  }

  for (const auto& layer : data.vSW_Layers_Other_Fields_Mapping)
  {
  ...
  //  日志结束
  logger->info("******************************************************************检查 SW_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件结束******************************************************************");
  return isOk;
}
