#ifndef JBHT_NJBDX_SEMANTIC_FUSION_H
#define JBHT_NJBDX_SEMANTIC_FUSION_H
...
/*-------------UI-------------*/
#include "ui_JBHT_NJBDX_semantic_fusion.h"
...
#pragma region "2 qgs_JBHT_NJBDX_semantic_fusion类"
class qgs_JBHT_NJBDX_semantic_fusion : public QDialog
{
	Q_OBJECT

public:
	qgs_JBHT_NJBDX_semantic_fusion(QWidget* parent = nullptr, Qt::WindowFlags fl = Qt::WindowFlags());
	~qgs_JBHT_NJBDX_semantic_fusion() override;
...
/**************************************************JBHT--->NJBDX内部函数******************************************************/
    ...
    //  JBHT--->NJBDX转化：单图幅
    int JBHT2NJBDX_Single_Frame_Data(
    ...
    //  根据JBHT数据集信息和NJBDX数据集信息、分类编码配置信息进行字段属性映射
    void JBHT_Mapping_NJBDX_Fields(
    ...
    //  从JBHT_Classification_Code_Conditional_Mapping_Lists.json表达式中提取NJBDX'分类编码'
    std::string JBHT_extractClassificationCode(const std::string& input);
    
    //  对单字段计算QGIS表达式
    std::string JBHT_execute_single_qgis_expression(
    ...
    //  对多字段计算QGIS表达式
    std::vector<std::string> JBHT_execute_multiple_qgis_expressions(
    ...
    //  根据要素来填充表达式上下文
    void JBHT_fillExpressionContextFromOGRFeature(
      OGRFeature* poSourceFeature,
    ...
    //  获取JBHT单个分幅数据集的信息
    void JBHT_Create_single_frame_data_all_layers_info(
    ...
    void CreateEmptyDatasetFromJBHTInfo(
        const JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity,
        ...
        std::shared_ptr<spdlog::logger> logger);
    //  检查JBHT数据集中的有效图层是否为空
    bool JBHT_Check_single_frame_data_all_layers_info(
      const JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity,
      std::shared_ptr<spdlog::logger> single_frame_data_logger);
    //  释放JBHT单个分幅数据集资源
    void JBHT_Close_single_frame_data_all_layers_info(
      JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity);
    //  删除空图层并且关闭非空图层
    void JBHT_Delete_And_Close_single_frame_data_all_layers_info(
        JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity);


    //  根据 JBHT 图层、图层映射关系和字段信息，创建对应的 NJBDX 图层
    bool JBHT_Create_NJBDX_all_layers_info(
      const JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity,
      const JBHT_Layers_Mapping_NJBDX_Layers_Json_t& JBHT_Layers_Mapping_NJBDX_Layers_Json_entity,
      const JBHT_Layers_Fields_Info_Json_t& JBHT_Layers_Fields_Info_Json_entity,
      const JBHT_NJBDX_Layers_Fields_Info_Json_t& NJBDX_Layers_Fields_Info_Json_entity,
    ...
    bool JBHT_Check_NJBDX_all_layers_info(
      const JBHT_NJBDX_single_frame_data_all_layers_info_t& NJBDX_single_frame_data_all_layers_info_entity);
    //  释放NJBDX单个分幅数据集资源
    void JBHT_Close_NJBDX_all_layers_info(
      JBHT_NJBDX_single_frame_data_all_layers_info_t& NJBDX_single_frame_data_all_layers_info_entity);



    //  提取图层名称
    std::string JBHT_Extract_LayerName(
      const std::string& layerName,
      const JBHT_Layers_Fields_Info_Json_t& JBHT_Layers_Fields_Info_Json_entity,
      std::shared_ptr<spdlog::logger> logger);

    //  辅助函数：将OGRwkbGeometryType映射到字符串
    std::string JBHT_OGRGeometryType2String(const OGRwkbGeometryType& GeoType);
...
private:
	...
	Ui_qgs_JBHT_NJBDX_semantic_fusion ui;
...
#endif // JBHT_NJBDX_SEMANTIC_FUSION_H
