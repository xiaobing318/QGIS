...
#include "JYDH_NJBDX_Config_Reader.h"
#pragma endregion
...
/*---------------------------------------------*
 * 1. 读取 JYDH_Layers_Fields_Info.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 JYDH_Layers_Fields_Info.json 配置文件开始******************************************************************");
  logger->info("JYDH_Layers_Fields_Info.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  //  最终要返回的结构体
  JYDH_Layers_Fields_Info_Json_t data;
  ...
  //  3. 解析 "JYDH_Layers_Fields" 数组
  if (!j.contains("JYDH_Layers_Fields") || !j["JYDH_Layers_Fields"].is_array())
  {
    logger->error("配置文件中未找到 'JYDH_Layers_Fields' 字段或字段类型不正确，返回空结构体。");
    // 根据需求：也可以抛异常
    return data;
  }

  // 遍历 "JYDH_Layers_Fields"
  for (const auto& layerObj : j["JYDH_Layers_Fields"])
  {
    JYDH_Layers_Fields_t layerData;

    // 3.1 解析 "JYDH_Layer_Name"
    if (layerObj.contains("JYDH_Layer_Name") && layerObj["JYDH_Layer_Name"].is_string())
    {
      layerData.JYDH_Layer_Name = layerObj["JYDH_Layer_Name"].get<std::string>();
    }
    else
    {
      logger->warn("某个图层信息中缺少 'JYDH_Layer_Name' 字段或类型错误。");
    }
    ...
    {
      logger->warn("图层 '{}' 中缺少 'Size_Of_Fields' 字段或类型错误，默认为 0。", layerData.JYDH_Layer_Name);
    ...
    if (layerObj.contains("Fields") && layerObj["Fields"].is_array())
    {
      for (const auto& fld : layerObj["Fields"])
      {
        ...
        {
          logger->warn("图层 '{}' 有字段未提供 'chinese_field_name'，将使用空字符串。", layerData.JYDH_Layer_Name);
        }
        ...
        {
          logger->warn("图层 '{}' 有字段未提供 'english_field_name'，将使用空字符串。", layerData.JYDH_Layer_Name);
        }
        ...
        if (fld.contains("field_type") && fld["field_type"].is_string())
        {
          ...
          {
            logger->warn("图层 '{}' 中未知的字段类型 '{}', 将默认设为 OFTString。", layerData.JYDH_Layer_Name, typeStr);
            field.field_type = OFTString; // 默认
          }
        }
        else
        {
          logger->warn("图层 '{}' 有字段未提供 'field_type'，默认为 OFTString。", layerData.JYDH_Layer_Name);
          field.field_type = OFTString;
        }
        ...
        if (fld.contains("field_width"))
        {
          ...
          else if (fld["field_width"].is_string())
          {
            ...
            {
              logger->warn("图层 '{}' 中字段 'field_width' 无法解析为整数，使用默认值 0。", layerData.JYDH_Layer_Name);
              field.field_width = 0;
            }
          }
          else
          {
            logger->warn("图层 '{}' 中 'field_width' 类型不符合预期，使用默认值 0。", layerData.JYDH_Layer_Name);
            field.field_width = 0;
          }
        }
        else
        ...
        if (fld.contains("field_precision"))
        {
          ...
          else if (fld["field_precision"].is_string())
          {
            ...
            {
              logger->warn("图层 '{}' 中字段 'field_precision' 无法解析为整数，使用默认值 0。", layerData.JYDH_Layer_Name);
              field.field_precision = 0;
            }
          }
          else
          {
            logger->warn("图层 '{}' 中 'field_precision' 类型不符合预期，使用默认值 0。", layerData.JYDH_Layer_Name);
            field.field_precision = 0;
          }
        }
        else
    ...
    {
      logger->warn("图层 '{}' 缺少 'Fields' 字段或其类型不是数组，vFields 将为空。", layerData.JYDH_Layer_Name);
    }

    // 将处理好的图层信息放入 data 中
    data.vJYDH_Layers_Fields.push_back(layerData);
  }


  logger->info("已完成对 JYDH_Layers_Fields_Info.json 的解析，共解析到 {} 个图层。", data.vJYDH_Layers_Fields.size());
  //  日志结束
  logger->info("******************************************************************读取 JYDH_Layers_Fields_Info.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 2. 读取 JYDH_Layers_Mapping_NJBDX_Layers.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 JYDH_Layers_Mapping_NJBDX_Layers.json 配置文件开始******************************************************************");
  logger->info("JYDH_Layers_Mapping_NJBDX_Layers.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  //  最终要返回的结构体
  JYDH_Layers_Mapping_NJBDX_Layers_Json_t data;
  ...
  //  3. 解析 "JYDH_layers_mapping_NJBDX_layers" 数组
  if (!j.contains("JYDH_layers_mapping_NJBDX_layers") || !j["JYDH_layers_mapping_NJBDX_layers"].is_array())
  {
    logger->error("配置文件中缺少 'JYDH_layers_mapping_NJBDX_layers' 数组或格式不正确，返回空结构体。");
    return data; // 这里直接返回空结构体，也可根据需求抛异常
  }

  //  遍历 "JYDH_layers_mapping_NJBDX_layers"
  for (const auto& mappingObj : j["JYDH_layers_mapping_NJBDX_layers"])
  {
    // 用于存放单个映射关系
    JYDH_layers_mapping_NJBDX_layers_t oneMapping; 

    // 3.1 解析 "JYDH_Layer" 对象
    if (mappingObj.contains("JYDH_Layer") && mappingObj["JYDH_Layer"].is_object())
    {
      auto JYDHLayerObj = mappingObj["JYDH_Layer"];

      // 1、解析 "JYDH_layer_english_name"
      if (JYDHLayerObj.contains("JYDH_layer_english_name") && JYDHLayerObj["JYDH_layer_english_name"].is_string())
      {
        oneMapping.JYDH_Layer.JYDH_layer_english_name = JYDHLayerObj["JYDH_layer_english_name"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_english_name' 字段或类型错误。");
      }

      // 2、解析 "JYDH_layer_chinese_name"
      if (JYDHLayerObj.contains("JYDH_layer_chinese_name") && JYDHLayerObj["JYDH_layer_chinese_name"].is_string())
      {
        oneMapping.JYDH_Layer.JYDH_layer_chinese_name = JYDHLayerObj["JYDH_layer_chinese_name"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_chinese_name' 字段或类型错误。");
      }

      // 3、解析 "JYDH_layer_notes"
      if (JYDHLayerObj.contains("JYDH_layer_notes") && JYDHLayerObj["JYDH_layer_notes"].is_string())
      {
        oneMapping.JYDH_Layer.JYDH_layer_notes = JYDHLayerObj["JYDH_layer_notes"].get<std::string>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_notes' 字段或类型错误。");
      }

      // 4、解析 "JYDH_layer_type"
      if (JYDHLayerObj.contains("JYDH_layer_type") && JYDHLayerObj["JYDH_layer_type"].is_string())
      {
        std::string typeStr = JYDHLayerObj["JYDH_layer_type"].get<std::string>();
        try
        {
          oneMapping.JYDH_Layer.JYDH_layer_type = stringToOGRGeometryType(typeStr);
        }
        catch (const std::invalid_argument& e)
        {
          logger->error("JYDH_layer_type 转换失败: {}", e.what());
          //  根据需求，可以选择抛出异常或设置为默认类型
          oneMapping.JYDH_Layer.JYDH_layer_type = wkbUnknown;
        }
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_type' 字段或类型错误。");
      }

      // 5、解析 "JYDH_layer_field_number"
      if (JYDHLayerObj.contains("JYDH_layer_field_number") && JYDHLayerObj["JYDH_layer_field_number"].is_number_integer())
      {
        oneMapping.JYDH_Layer.JYDH_layer_field_number = JYDHLayerObj["JYDH_layer_field_number"].get<int>();
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_field_number' 字段或类型错误，默认为 0。");
        oneMapping.JYDH_Layer.JYDH_layer_field_number = 0;
      }

      // 6、解析 "JYDH_layer_extention" (目前结构体内为空，可按需扩展)
      if (JYDHLayerObj.contains("JYDH_layer_extention") && JYDHLayerObj["JYDH_layer_extention"].is_object())
      {
        // 暂时无字段可解析，如需后续扩展可在此处添加解析逻辑
      }
      else
      {
        logger->warn("某个映射信息中缺少 'JYDH_layer_extention' 字段或类型错误（对象类型），但结构体中目前也没有扩展字段。");
      }
    }
    else
    {
      logger->warn("缺少 'JYDH_Layer' 对象或类型错误，该映射将仅保留默认空层信息。");
    }
    ...
    if (mappingObj.contains("NJBDX_Mapping_Layers") && mappingObj["NJBDX_Mapping_Layers"].is_array())
    {
      ...
      {
        JYDH_NJBDX_Mapping_Layer_t oneNJBDXLayer;
    ...
    // 将该映射结果放入最终 data 中
    data.vJYDH_layers_mapping_NJBDX_layers.push_back(oneMapping);
  }


  //  日志：完成解析
  logger->info("已完成对 JYDH_Layers_Mapping_NJBDX_Layers.json 的解析，共解析到 {} 条映射关系。", data.vJYDH_layers_mapping_NJBDX_layers.size());
  //  日志结束
  logger->info("******************************************************************读取 JYDH_Layers_Mapping_NJBDX_Layers.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 3. 读取 JYDH_Classification_Code_Conditional_Mapping_Lists.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 JYDH_Classification_Code_Conditional_Mapping_Lists.json 配置文件开始******************************************************************");
  logger->info("JYDH_Classification_Code_Conditional_Mapping_Lists.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  // 最终要返回的结构体
  JYDH_Classification_Code_Conditional_Mapping_Lists_Json_t data;
  ...
  // 3. 解析 "JYDH_Classification_Code_Conditional_Mapping_Lists" 数组
  if (!j.contains("JYDH_Classification_Code_Conditional_Mapping_Lists") || !j["JYDH_Classification_Code_Conditional_Mapping_Lists"].is_array())
  {
    logger->error("配置文件中缺少 'JYDH_Classification_Code_Conditional_Mapping_Lists' 数组或格式不正确，返回空结构体。");
    return data; // 根据需求：也可以抛异常
  }

  // 遍历 "JYDH_Classification_Code_Conditional_Mapping_Lists"
  for (const auto& itemObj : j["JYDH_Classification_Code_Conditional_Mapping_Lists"])
  {
    JYDH_Classification_Code_Conditional_Mapping_Lists_t oneItem;
    ...
    // (2) JYDH_Classification_Code_Mapping_Condition
    if (itemObj.contains("JYDH_Classification_Code_Mapping_Condition") && itemObj["JYDH_Classification_Code_Mapping_Condition"].is_string())
    {
      oneItem.JYDH_Classification_Code_Mapping_Condition = itemObj["JYDH_Classification_Code_Mapping_Condition"].get<std::string>();
    }
    else
    {
      logger->warn("有某个映射项缺少 'JYDH_Classification_Code_Mapping_Condition' 字段或类型错误，将使用空字符串。");
    }

    // (3) JYDH_Classification_Code_Mapping_Condition_Flag
    if (itemObj.contains("JYDH_Classification_Code_Mapping_Condition_Flag") && itemObj["JYDH_Classification_Code_Mapping_Condition_Flag"].is_string())
    {
      oneItem.JYDH_Classification_Code_Mapping_Condition_Flag = itemObj["JYDH_Classification_Code_Mapping_Condition_Flag"].get<std::string>();
    }
    else
    {
      logger->warn("有某个映射项缺少 'JYDH_Classification_Code_Mapping_Condition_Flag' 字段或类型错误，将使用空字符串。");
    }
    ...
    // 将解析到的结果加入集合
    data.vJYDH_Classification_Code_Conditional_Mapping_Lists.push_back(oneItem);
  }

  // 日志：完成解析
  logger->info("已完成对 JYDH_Classification_Code_Conditional_Mapping_Lists.json 的解析，共解析到 {} 条映射规则。", data.vJYDH_Classification_Code_Conditional_Mapping_Lists.size());

  //  日志结束
  logger->info("******************************************************************读取 JYDH_Classification_Code_Conditional_Mapping_Lists.json 配置文件结束******************************************************************");

  return data;
}

/*---------------------------------------------*
 * 4. 读取 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json
 *---------------------------------------------*/
...
{
  //  日志开始
  logger->info("******************************************************************读取 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件开始******************************************************************");
  logger->info("JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件路径: {}", filePath.toUtf8().toStdString());

  // 最终要返回的结构体
  JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t data;
  ...
  // 3. 解析 "JYDH_Layers_Other_Fields_Mapping" 数组
  if (!j.contains("JYDH_Layers_Other_Fields_Mapping") || !j["JYDH_Layers_Other_Fields_Mapping"].is_array())
  {
    logger->error("配置文件中缺少 'JYDH_Layers_Other_Fields_Mapping' 数组或格式不正确，返回空结构体。");
    return data; // 可按需求改为抛异常
  }

  //  遍历 "JYDH_Layers_Other_Fields_Mapping"
  for (const auto& layerObj : j["JYDH_Layers_Other_Fields_Mapping"])
  {
    JYDH_Layers_Other_Fields_Mapping_t layerMapping;
    ...
    if (layerObj.contains("Other_Fields_Mapping") && layerObj["Other_Fields_Mapping"].is_array())
    {
      ...
      {
        JYDH_Other_Fields_Mapping_t fieldMapping;
        ...
        // 2、解析 "JYDH_field_name"
        if (fieldObj.contains("JYDH_field_name") && fieldObj["JYDH_field_name"].is_string())
        {
          fieldMapping.JYDH_field_name = fieldObj["JYDH_field_name"].get<std::string>();
        }
        else
        {
          logger->warn("某个 Other_Fields_Mapping 缺少 'JYDH_field_name' 字段或类型错误，使用空字符串。");
        }
    ...
    // 将处理好的图层映射信息放入 data
    data.vJYDH_Layers_Other_Fields_Mapping.push_back(layerMapping);
  }

  //  日志：完成解析
  logger->info("已完成对 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 的解析，共解析到 {} 个图层。", data.vJYDH_Layers_Other_Fields_Mapping.size());

  //  日志结束
  logger->info("******************************************************************读取 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件结束******************************************************************");
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  //  最终要返回的结构体
  JYDH_NJBDX_Layers_Fields_Info_Json_t data;
  ...
  {
    JYDH_NJBDX_Layers_Fields_t layerData;
...
#pragma region "2 对五种配置文件中读取到的结构体进行正确性检查的接口"
 /**
  * @brief 检查 JYDH_Layers_Fields_Info_Json_t 的正确性
  */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 JYDH_Layers_Fields_Info.json 配置文件开始******************************************************************");


  // 1. Note 是否为空
  if (data.Note.empty())
  {
    logger->warn("[checkJYDH_Layers_Fields_Info_Json] Note 为空");
  }

  // 2. 遍历每一个图层，检查字段数量是否与数据实际一致
  for (const auto& layer : data.vJYDH_Layers_Fields)
  {
    // spdlog::debug(...) 可以输出更详细信息，或根据需要使用 spdlog::info/ spdlog::warn 等
    logger->debug("[checkJYDH_Layers_Fields_Info_Json] 检查的图层为: {}", layer.JYDH_Layer_Name);

    // Check Size_Of_Fields
    if (layer.Size_Of_Fields <= 0)
    {
      logger->warn("图层 '{}' 的Size_Of_Fields不合法--->{}", layer.JYDH_Layer_Name, layer.Size_Of_Fields);
      isOk = false;
    }
  ...
  //  日志结束
  logger->info("******************************************************************检查 JYDH_Layers_Fields_Info.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 JYDH_Layers_Mapping_NJBDX_Layers_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 JYDH_Layers_Mapping_NJBDX_Layers.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[checkJYDH_Layers_Mapping_NJBDX_Layers_Json] Note 为空");
  }

  for (const auto& mapping : data.vJYDH_layers_mapping_NJBDX_layers)
  {
    // 根据需要检查字段
    if (mapping.JYDH_Layer.JYDH_layer_english_name.empty())
    {
      logger->warn("JYDH_Layer 有一个空的english_name.");
      isOk = false;
    }
    ...
    {
      logger->warn("JYDH_Layer '{}' 没有NJBDX_Mapping_Layers映射", mapping.JYDH_Layer.JYDH_layer_english_name);
      isOk = false;
    }
  }

  //  日志结束
  logger->info("******************************************************************检查 JYDH_Layers_Mapping_NJBDX_Layers.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 JYDH_Classification_Code_Conditional_Mapping_Lists_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 JYDH_Classification_Code_Conditional_Mapping_Lists.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[JYDH_Classification_Code_Conditional_Mapping_Lists_Json] Note 为空。");
  }

  //  日志结束
  logger->info("******************************************************************检查 JYDH_Classification_Code_Conditional_Mapping_Lists.json 配置文件结束******************************************************************");

  return isOk;
}

/**
 * @brief 检查 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t 的正确性
 */
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  bool isOk = true;
  //  日志开始
  logger->info("******************************************************************检查 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件开始******************************************************************");

  if (data.Note.empty())
  {
    logger->warn("[checkJYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json] Note为空。");
  }

  for (const auto& layer : data.vJYDH_Layers_Other_Fields_Mapping)
  {
  ...
  //  日志结束
  logger->info("******************************************************************检查 JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json 配置文件结束******************************************************************");
  return isOk;
}
