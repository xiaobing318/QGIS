...
qgs_JYDH_NJBDX_semantic_fusion::qgs_JYDH_NJBDX_semantic_fusion(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl),
  m_input_data_path(""),
  m_output_data_path(""),
  m_counter_total(0),
  m_counter_succeeded(0)
{
	...
	connect(ui.Button_input_data_view, &QPushButton::clicked, this, &qgs_JYDH_NJBDX_semantic_fusion::Button_input_data_view);
	connect(ui.Button_output_data_view, &QPushButton::clicked, this, &qgs_JYDH_NJBDX_semantic_fusion::Button_output_data_view);
	connect(ui.Button_OK, &QPushButton::clicked, this, &qgs_JYDH_NJBDX_semantic_fusion::Button_OK);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &qgs_JYDH_NJBDX_semantic_fusion::Button_Cancel);

  restore_states();
}

qgs_JYDH_NJBDX_semantic_fusion::~qgs_JYDH_NJBDX_semantic_fusion()
{
  save_states();
}
#pragma endregion
...
//  获取输入数据目录成员函数
QString qgs_JYDH_NJBDX_semantic_fusion::get_m_input_data_path()
{
  //  从输入框中获取当前内容
...
//  获取输出数据目录成员函数
QString qgs_JYDH_NJBDX_semantic_fusion::get_m_output_data_path()
{
  //  从输入框中获取当前内容
...
void qgs_JYDH_NJBDX_semantic_fusion::set_m_input_data_path(const std::string& input_data_path)
{
  m_input_data_path = QString::fromStdString(input_data_path);
}

//  设置输出数据目录成员函数（目前还没有用到）
void qgs_JYDH_NJBDX_semantic_fusion::set_m_output_data_path(const std::string& output_data_path)
{
  m_output_data_path = QString::fromStdString(output_data_path);
}

//  恢复保存参数
void qgs_JYDH_NJBDX_semantic_fusion::restore_states()
{
  //  从上一次保存的路径数据中得到输入输出数据
  QgsSettings settings;
  //  指定从Plugins节读取设置
  settings.beginGroup("vector_data_JYDH_NJBDX_semantic_fusion");
  this->m_input_data_path = settings.value(QStringLiteral("vector_data_JYDH_NJBDX_semantic_fusion/m_input_data_path"), QDir::homePath()).toString();
  this->m_output_data_path = settings.value(QStringLiteral("vector_data_JYDH_NJBDX_semantic_fusion/m_output_data_path"), QDir::homePath()).toString();
...
//  保存参数
void qgs_JYDH_NJBDX_semantic_fusion::save_states()
{
  QgsSettings settings;
  // 指定设置保存到Plugins节
  settings.beginGroup("vector_data_JYDH_NJBDX_semantic_fusion");
  settings.setValue(QStringLiteral("vector_data_JYDH_NJBDX_semantic_fusion/m_input_data_path"), m_input_data_path);
  settings.setValue(QStringLiteral("vector_data_JYDH_NJBDX_semantic_fusion/m_output_data_path"), m_output_data_path);
  // 结束分组，非常重要，确保设置正确保存在指定的节中
  settings.endGroup();
}
#pragma endregion
...
void qgs_JYDH_NJBDX_semantic_fusion::Button_input_data_view()
{
  //  选择文件夹
  QString curPath = m_input_data_path;
  QString dlgTile = QObject::tr("JYDH--->NJBDX：请选择语义融合工具输入数据源所在的目录位置");
...
void qgs_JYDH_NJBDX_semantic_fusion::Button_output_data_view()
{
  //  选择文件夹
  QString curPath = m_output_data_path;
  QString dlgTile = QObject::tr("JYDH--->NJBDX：请选择语义融合处理后数据存放的目录位置");
  QString selectedDir = QFileDialog::getExistingDirectory(
    this,
    dlgTile,
    curPath,
...
void qgs_JYDH_NJBDX_semantic_fusion::Button_OK()
{
  /*
  功能说明：
    1、当UI界面中的“确定”按钮被点击的时候，对JYDH--->NJBDX处理开始
    2、UI界面中的输入和输出路径作为该工具的输入、输出路径参数

  算法流程：
    1. 验证用户输入的数据源路径和保存转换后数据的路径。
    2. 加载JYDH--->NJBDX涉及到的四个配置文件。
    3. 使用提取和验证的数据执行最终的转换过程。
  */

  /*
    在点击确定的时候说明JYDH--->NJBDX转化开始了，在后面的语义融合过程中可能会出现异常导致程序不能顺利进行到最后,
  因此首先在这里将界面参数保存下来。
  */
  save_states();
...
#pragma region "（第一步）判断输入数据路径、输出数据路径在文件系统中是否存在"
  ...
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("输入数据目录不存在，请重新输入！"));
    return;
  }
  if (!is_exist_directory(get_m_output_data_path()))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("输出数据目录不存在，请重新输入！"));
    return;
  }
#pragma endregion 
...
#pragma region "（第三步）读取JYDH--->NJBDX配置文件，并且检查五个配置文件是否有效"
  ...
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置目录 'DLHJ_Vector_Data_Semantic_Fusion_Config' 不存在！"));
    return;
  }

  //  拼接各个配置文件的完整路径
  QString configFilePath1 = configDir.filePath("JYDH/JYDH_Layers_Fields_Info.json");
  QString configFilePath2 = configDir.filePath("JYDH/JYDH_Layers_Mapping_NJBDX_Layers.json");
  QString configFilePath3 = configDir.filePath("JYDH/JYDH_Classification_Code_Conditional_Mapping_Lists.json");
  QString configFilePath4 = configDir.filePath("JYDH/JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json");
  QString configFilePath5 = configDir.filePath("NJBDX/NJBDX_Layers_Fields_Info.json");
  //  检查配置文件路径是否存在
  if (!is_exist_file(configFilePath1))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置文件JYDH_Layers_Fields_Info.json不存在！"));
    return;
  }
  if (!is_exist_file(configFilePath2))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置文件JYDH_Layers_Mapping_NJBDX_Layers.json不存在！"));
    return;
  }
  if (!is_exist_file(configFilePath3))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置文件JYDH_Classification_Code_Conditional_Mapping_Lists.json不存在！"));
    return;
  }
  if (!is_exist_file(configFilePath4))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置文件JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json不存在！"));
    return;
  }
  if (!is_exist_file(configFilePath5))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置文件NJBDX_Layers_fields_Info.json不存在！"));
    return;
  }
...
#pragma region "处理日志文件路径的字符集编码问题，在windows平台上文件系统使用的是UTF-16字符集编码"
    ...
    if (!outputDir.exists())
    {
        showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("配置目录DLHJ_Vector_Data_Semantic_Fusion_Config不存在！"));
        return;
    }
...
#pragma endregion
  ...
  //  读取配置文件内容
  JYDH_Layers_Fields_Info_Json_t JYDH_Layers_Fields_Info_Json_entity = readJYDH_Layers_Fields_Info_Json(
    normalizePath(configFilePath1).toUtf8(),
    read_config_logger);
  //  检查配置文件是否有效
  if (!checkJYDH_Layers_Fields_Info_Json(JYDH_Layers_Fields_Info_Json_entity, read_config_logger))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("JYDH_Layers_Fields_Info.json配置文件存在问题，请检查！"));
    return;
  }

  JYDH_Layers_Mapping_NJBDX_Layers_Json_t JYDH_Layers_Mapping_NJBDX_Layers_Json_entity = readJYDH_Layers_Mapping_NJBDX_Layers_Json(
    normalizePath(configFilePath2).toUtf8(),
    read_config_logger);
  if (!checkJYDH_Layers_Mapping_NJBDX_Layers_Json(JYDH_Layers_Mapping_NJBDX_Layers_Json_entity, read_config_logger))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("JYDH_Layers_Mapping_NJBDX_Layers.json配置文件存在问题，请检查！"));
    return;
  }


  JYDH_Classification_Code_Conditional_Mapping_Lists_Json_t JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity = readJYDH_Classification_Code_Conditional_Mapping_Lists_Json(
    normalizePath(configFilePath3).toUtf8(),
    read_config_logger);
  if (!checkJYDH_Classification_Code_Conditional_Mapping_Lists_Json(JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity, read_config_logger))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("JYDH_Classification_Code_Conditional_Mapping_Lists.json配置文件存在问题，请检查！"));
    return;
  }

  JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity = readJYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json(
    normalizePath(configFilePath4).toUtf8(),
    read_config_logger);
  if (!checkJYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json(JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity, read_config_logger))
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json配置文件存在问题，请检查！"));
    return;
  }

  JYDH_NJBDX_Layers_Fields_Info_Json_t NJBDX_Layers_Fields_Info_Json_entity = JYDH_readNJBDX_Layers_Fields_Info_Json(
  ...
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("NJBDX_layers_fields_info.json配置文件存在问题，请检查！"));
    return;
  }

#pragma endregion

#pragma region "（第四步）循环从分幅数据中处理JYDH--->NJBDX语义融合"
...
#pragma region "2、JYDH--->NJBDX语义融合处理"
  ...
  try
  {
      ...
      JYDH_Layers_Fields_Info_Json_entity,
      JYDH_Layers_Mapping_NJBDX_Layers_Json_entity,
      JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity,
      JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity,
      NJBDX_Layers_Fields_Info_Json_entity);
  }
  catch (const std::exception& e)
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), QString::fromStdString(e.what()));
    return;
  }
  catch (...)
  {
    showMessageBox(tr("矢量数据语义融合（JYDH--->NJBDX）"), tr("发生未知异常！"));
    return;
  }
  ...
  //  显示统计信息
  {
    //  记录日志
    QMessageBox msgBox1;
    msgBox1.setWindowTitle(tr("矢量语义融合处理（JYDH--->NJBDX）"));
    //  拼接显示信息
...
void qgs_JYDH_NJBDX_semantic_fusion::Button_Cancel()
{
  reject();
}
...
//  判断目录是否存在
bool qgs_JYDH_NJBDX_semantic_fusion::is_exist_directory(const QString& qstr_directory_path)
{
...
//  判断文件是否存在
bool qgs_JYDH_NJBDX_semantic_fusion::is_exist_file(const QString& qstr_file_path)
{
  return QFile::exists(qstr_file_path);
}

//  获取指定目录中的多个子目录
QStringList qgs_JYDH_NJBDX_semantic_fusion::get_sub_directories(const QString& file_directory_path)
{
...
//  自定义信息提示框
void qgs_JYDH_NJBDX_semantic_fusion::showMessageBox(const QString& title, const QString& text)
{
  QMessageBox msgBox;
...
//  辅助函数：转换路径为统一格式
QString qgs_JYDH_NJBDX_semantic_fusion::normalizePath(const QString& path)
{
  // 将反斜杠替换为正斜杠
...
#pragma region "7 JYDH--->NJBDX内部函数"

void qgs_JYDH_NJBDX_semantic_fusion::JYDH2NJBDX(
  const std::vector<std::string>& vstr_input_frame_data_path,
  const std::string& str_output_data_path,
  const JYDH_Layers_Fields_Info_Json_t& JYDH_Layers_Fields_Info_Json_entity,
  const JYDH_Layers_Mapping_NJBDX_Layers_Json_t& JYDH_Layers_Mapping_NJBDX_Layers_Json_entity,
  const JYDH_Classification_Code_Conditional_Mapping_Lists_Json_t& JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity,
  const JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_t& JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity,
  const JYDH_NJBDX_Layers_Fields_Info_Json_t& NJBDX_Layers_Fields_Info_Json_entity)
{
  /*
  Note:杨小兵-2025-01-15
  ...
  */
  ...
  for (size_t i = 0; i < size; i++)
  {
    try {
       ...
       //  对当前分幅数据进行语义融合处理
       iresult_single_frame_data = JYDH2NJBDX_Single_Frame_Data(
...
int qgs_JYDH_NJBDX_semantic_fusion::JYDH2NJBDX_Single_Frame_Data(
...
#pragma region "第一步：创建输出目录、日志器、日志文件"
  ...
  try {
    ...
    if (!inputDir.exists() || !QFileInfo(qSingleFrameDataPath).isDir())
    {
      ...
      QgsMessageLog::logMessage(qstrMsg, "JYDH--->NJBDX", Qgis::Critical);
      return -1;
    }
    ...
    QString SuccessfulSemanticFusionOutputDirName = "SuccessfulSemanticFusion_JYDH2NJBDX_SRCDATA_" + frame_dir_name;
    QString FailedSemanticFusionOutputDirName = "FailedSemanticFusion_JYDH2NJBDX_SRCDATA_" + frame_dir_name;
...
#pragma region "第三步：将JYDH数据所有图层的相关信息读取到自定义数据结构中"

  //  利用矢量驱动器打开当前分幅数据目录路径下的矢量数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameJYDHDataSet = (GDALDataset*)GDALOpenEx(single_frame_data_path.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  //  文件不存在或打开失败
  if (poSingleFrameJYDHDataSet == NULL)
  {
  ...
  //	创建变量用来存储JYDH分幅数据相关信息
  JYDH_single_frame_data_all_layers_info_t JYDH_single_frame_data_all_layers_info_entity;
  //  读取当前分幅数据矢量图层的信息：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考
  JYDH_Create_single_frame_data_all_layers_info(
    poSingleFrameJYDHDataSet,
    JYDH_single_frame_data_all_layers_info_entity,
    single_frame_data_successful_logger);
  //  检查数据集中的图层是否为空
  if (!JYDH_Check_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_entity, single_frame_data_successful_logger))
  {
    single_frame_data_successful_logger->error("函数[{}]：分幅数据{}中的有效矢量图层为空。", __FUNCTION__, single_frame_data_path);
    //  需要将已经打开的数据集关闭以释放内存资源
    JYDH_Close_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_entity);
    single_frame_data_successful_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
    return -8;
  }
#pragma endregion

#pragma region "第四步：将JYDH数据所有图层的相关信息读取到自定义数据结构中，创建空图层信息"
  //  创建变量用来存储JYDH分幅数据空数据相关信息
  JYDH_single_frame_data_all_layers_info_t JYDH_single_frame_data_all_layers_info_empty_entity;
  //  根据JYDH单个分幅数据中所有图层的信息创建一个空的输出数据集
  CreateEmptyDatasetFromJYDHInfo(
      JYDH_single_frame_data_all_layers_info_entity,
      JYDH_single_frame_data_all_layers_info_empty_entity,
      strSingleFailedSemanticFusionOutputDataPath,
      poShpDriver,
      single_frame_data_failed_logger);
  //  检查数据集中的图层是否为空
  if (!JYDH_Check_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_empty_entity, single_frame_data_failed_logger))
  {
...
#pragma region "第五步：根据JYDH实际数据和图层映射关系创建NJBDX数据集信息到自定义数据结构中"
  //  创建变量用来存储NJBDX分幅数据相关信息
  JYDH_NJBDX_single_frame_data_all_layers_info_t NJBDX_single_frame_data_all_layers_info_entity;
  //  使用JYDH数据、图层映射关系、NJBDX字段信息创建NJBDX数据集，即创建图层
  JYDH_Create_NJBDX_all_layers_info(
    JYDH_single_frame_data_all_layers_info_entity,
    JYDH_Layers_Mapping_NJBDX_Layers_Json_entity,
    JYDH_Layers_Fields_Info_Json_entity,
    NJBDX_Layers_Fields_Info_Json_entity,
    ...
    single_frame_data_successful_logger);
  //  检查数据集中的图层是否为空
  if (!JYDH_Check_NJBDX_all_layers_info(NJBDX_single_frame_data_all_layers_info_entity))
  {
    ...
    //  需要将两个数据集关闭以释放内存资源
    JYDH_Close_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_entity);
    JYDH_Close_NJBDX_all_layers_info(NJBDX_single_frame_data_all_layers_info_entity);
    single_frame_data_successful_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
    return -9;
  }
...
#pragma region "第六步：根据JYDH数据集信息和NJBDX数据集信息、分类编码配置信息进行字段属性映射"
  JYDH_Mapping_NJBDX_Fields(
    JYDH_single_frame_data_all_layers_info_entity,
    JYDH_single_frame_data_all_layers_info_empty_entity,
    NJBDX_single_frame_data_all_layers_info_entity,
    JYDH_Layers_Fields_Info_Json_entity,
...
#pragma region "第七步：释放JYDH分幅数据、NJBDX分幅数据中创建的内存资源"
  //  需要将三个数据集关闭以释放内存资源
  JYDH_Close_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_entity);
  //  删除空图层以及关闭非空图层
  JYDH_Delete_And_Close_single_frame_data_all_layers_info(JYDH_single_frame_data_all_layers_info_empty_entity);
  //  删除空图层以及关闭非空图层
  JYDH_Close_NJBDX_all_layers_info(NJBDX_single_frame_data_all_layers_info_entity);
  single_frame_data_successful_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
  single_frame_data_failed_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
...
#pragma region "JYDH2NJBDX_Single_Frame_Data中第六步对应函数"

// -----------------------------------------------------------------------------
//  主要实现函数：将 JYDH 数据源中的要素按分类编码映射到 NJBDX 数据源中
// -----------------------------------------------------------------------------
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_Mapping_NJBDX_Fields(
  const JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity,
  const JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_empty_entity,
  const JYDH_NJBDX_single_frame_data_all_layers_info_t& NJBDX_single_frame_data_all_layers_info_entity,
  const JYDH_Layers_Fields_Info_Json_t& JYDH_Layers_Fields_Info_Json_entity,
  const JYDH_NJBDX_Layers_Fields_Info_Json_t& NJBDX_Layers_Fields_Info_Json_entity,
  const JYDH_Layers_Mapping_NJBDX_Layers_Json_t& JYDH_Layers_Mapping_NJBDX_Layers_Json_entity,
  const JYDH_Classification_Code_Conditional_Mapping_Lists_Json_t& JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity,
...
{
  single_frame_data_successful_logger->info("<------------------函数[{}]日志开始--->进行 JYDH -> NJBDX 字段属性值映射------------------>",
    __FUNCTION__);

  //  循环处理 JYDH 图层
  int JYDH_layer_counter = 1;
  for (const auto& JYDH_single_layer : JYDH_single_frame_data_all_layers_info_entity.vJYDH_single_frame_data_single_layer_info)
  {
    //  日志中提示当前正在处理的JYDH图层
    single_frame_data_successful_logger->info("<----------函数[{}]：开始处理 JYDH 中第 {} 个图层：{} (该图层GDAL几何类型：{} )---------->",
      __FUNCTION__,
      JYDH_layer_counter,
      JYDH_single_layer.layer_name,
      OGRGeometryTypeToName(JYDH_single_layer.layer_geo_type));

#pragma region "1 在 JYDH_Layers_Mapping_NJBDX_Layers_Json_t 中找出当前JYDH图层对应的映射(一对多)"

    //  在 JYDH_Layers_Mapping_NJBDX_Layers_Json_t 中找出当前JYDH图层对应的映射(一对多)
    const auto& allMappings = JYDH_Layers_Mapping_NJBDX_Layers_Json_entity.vJYDH_layers_mapping_NJBDX_layers;
    //  使用 std::find_if 查找匹配的映射
    auto itFound = std::find_if(allMappings.begin(), allMappings.end(),
      [&JYDH_single_layer, JYDH_Layers_Fields_Info_Json_entity, this, single_frame_data_successful_logger](const JYDH_layers_mapping_NJBDX_layers_t& item) -> bool
      {
        //  解析当前图层的名称（例如D）
        std::string parsedName = this->JYDH_Extract_LayerName(
          JYDH_single_layer.layer_name,
          JYDH_Layers_Fields_Info_Json_entity,
          single_frame_data_successful_logger);
        ...
        if (parsedName.empty())
        {
          single_frame_data_successful_logger->info("函数[{}]：当前 JYDH 图层{}名称类型解析失败。", __FUNCTION__, JYDH_single_layer.layer_name);
          return false;
        }

        //  比较解析后的名称与映射中的名称
        bool nameMatches = (item.JYDH_Layer.JYDH_layer_english_name == parsedName);

        //  比较几何类型
        bool geometryMatches = (item.JYDH_Layer.JYDH_layer_type == JYDH_single_layer.layer_geo_type);
    ...
    {
      single_frame_data_successful_logger->warn("函数[{}]：无法在JYDH_Layers_Mapping_NJBDX_Layers.json中找到与图层 [{}] 对应的映射关系，跳过当前图层的处理。",
        __FUNCTION__,
        JYDH_single_layer.layer_name);
      continue;
    }

    //  在上一步中找到了当前JYDH图层对应映射的NJBDX图层，获取JYDH当前 OGRLayer* 图层指针
    OGRLayer* poJYDHLayer = JYDH_single_layer.polayer;
    //  判断JYDH图层指针是否有效
    if (!poJYDHLayer)
    {
      single_frame_data_successful_logger->warn("函数[{}]：JYDH 图层 [{}] 的 OGRLayer 指针为空，跳过当前图层的处理。",
        __FUNCTION__,
        JYDH_single_layer.layer_name);
      continue;
    }
...
#pragma region "2 遍历分类编码条件映射表"
    ...
    //  首先需要查询到JYDH空数据图层名称同JYDH非空数据图层名称相同的图层指针（对于遍历分类编条件映射表只需要查找一次）
    OGRLayer* pDestLayer = NULL;
    for (const auto& JYDH_current_single_empty_layer : JYDH_single_frame_data_all_layers_info_empty_entity.vJYDH_single_frame_data_single_layer_info)
    {
      if (JYDH_current_single_empty_layer.layer_name == JYDH_single_layer.layer_name)
      {
        pDestLayer = JYDH_current_single_empty_layer.polayer;
        break;
      }
    }
    if (!pDestLayer)
    {
      single_frame_data_successful_logger->warn("函数[{}]：当前处理的JYDH图层名称{}在语义融合失败图层名称集合中没有找到相同的图层名称，跳过语义融合失败图层要素写入操作。",
        __FUNCTION__,
        JYDH_single_layer.layer_name);
      continue;
    }

    // 在遍历当前JYDH图层前增加一个记录处理要素ID的集合
    ...
    //  遍历分类编码条件映射表
    for (const auto& codeMapping : JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity.vJYDH_Classification_Code_Conditional_Mapping_Lists)
    {
#pragma region "2.1 分类编码条件映射表项必要检查"
      //  判断当前‘分类编码条件映射表项’是否需要映射
      if (codeMapping.JYDH_Classification_Code_Mapping_Condition_Flag != "yes")
      {
        ...
        single_frame_data_successful_logger->info("\"JYDH_Classification_Code_Mapping_Condition\":\"{}\"", codeMapping.JYDH_Classification_Code_Mapping_Condition);
        single_frame_data_successful_logger->info("\"JYDH_Classification_Code_Mapping_Condition_Flag\":\"{}\"", codeMapping.JYDH_Classification_Code_Mapping_Condition_Flag);
      ...
      //  TODO:检查表达式合法性
      //if (!is_valid_qgis_expression(codeMapping.JYDH_Classification_Code_Mapping_Condition))
      //{
      //  single_frame_data_successful_logger->info("函数[{}]：当前'分类编码条件映射表项'中的'JYDH_Classification_Code_Mapping_Condition'无效。具体的'分类编码条件映射表项'如下所示：", __FUNCTION__);
      //  single_frame_data_successful_logger->info("\"Note\":\"{}\"", codeMapping.Note);
      //  single_frame_data_successful_logger->info("\"JYDH_Classification_Code_Mapping_Condition\":\"{}\"", codeMapping.JYDH_Classification_Code_Mapping_Condition);
      //  single_frame_data_successful_logger->info("\"JYDH_Classification_Code_Mapping_Condition_Flag\":\"{}\"", codeMapping.JYDH_Classification_Code_Mapping_Condition_Flag);
      //  single_frame_data_successful_logger->info("\"NJBDX_Classification_Code_Mapping_Condition\":\"{}\"", codeMapping.NJBDX_Classification_Code_Mapping_Condition);
...
#pragma region "2.2 根据 JYDH_Classification_Code_Mapping_Condition 对 JYDH 要素进行筛选"

      /*
      Notes:杨小兵-2025-03-05

      1、在JYDH_Classification_Code_Mapping_Condition其中添加了JYDH图层名称信息，这里需要将JYDH图层名称和筛选条件两者进行区分（抽取出来）
      2、获取得到JYDH的图层名，判断当前处理的图层名称是否同当前矢量图层名称是否相同，如果相同则继续进行处理，反之则跳过
      3、JYDH_Classification_Code_Mapping_Condition字符串示例：ACHARE|\"CODE\"=140403 AND \"TYPE\"!='栈道'(需要单独提取ACHARE和\"CODE\"=140403 AND \"TYPE\"!='栈道')
      */

      //  判断当前图层名称是否同当前分类编码映射条件中指定的图层名称相同，需要针对不同的数据源进行不同的图层名称的抽取规则，并且设置成通用和非通用两种方式

      //  首先获取到分类编码中指定图层的名称
      std::string Current_JYDH_single_layer_name = extractBeforeDelimiter(codeMapping.JYDH_Classification_Code_Mapping_Condition);
      //  需要对当前图层名称进行过滤抽取，例如从DN101202_B_point中抽取出B
      std::string Current_JYDH_single_extract_layer_name = JYDH_Extract_LayerName(JYDH_single_layer.layer_name, JYDH_Layers_Fields_Info_Json_entity, single_frame_data_successful_logger);

      //  判断当前图层名称是否为空
      if (Current_JYDH_single_layer_name == "")
      {
        single_frame_data_successful_logger->error("函数[{}]：分类编码映射配置文件过滤条件 [{}] 中的图层名称为空，在分类编码映射配置文件过滤条件 [{}] 中应该设置为 CommonLayerName（表示图层名称通用）或者设置为具体的图层名称，请重新检查并且设置分类编码映射配置文件。",
              __FUNCTION__, codeMapping.JYDH_Classification_Code_Mapping_Condition, codeMapping.JYDH_Classification_Code_Mapping_Condition);
          //  跳过当前分类编码映射表项的处理，继续处理下一个分类编码表项。
          continue;
      }
      //  判断当前图层是否为通用形式
      else if (Current_JYDH_single_layer_name == "CommonLayerName")
      {
      ...
      else if (Current_JYDH_single_layer_name != Current_JYDH_single_extract_layer_name)
      {
          //  跳过当前分类编码映射表项的处理，继续处理下一个分类编码表项。
          continue;
      }
      ...
      //  根据 JYDH_Classification_Code_Mapping_Condition 对 JYDH 要素进行筛选，例如可能为："\"CODE\"=140403 AND \"TYPE\"!='栈道'"
      const std::string ogrAttributeFilter = extractAfterDelimiter(codeMapping.JYDH_Classification_Code_Mapping_Condition);
      ...
      OGRFeatureDefn* poJYDHDefn = poJYDHLayer->GetLayerDefn();
      for (const auto& fieldName : fieldNames)
      {
        if (poJYDHDefn->GetFieldIndex(fieldName.c_str()) == -1)
        {
          single_frame_data_successful_logger->warn("函数[{}]：图层 [{}] 中不存在字段 [{}]，过滤条件 [{}] 将不会生效。",
            __FUNCTION__, JYDH_single_layer.layer_name, fieldName, ogrAttributeFilter);
      ...
      if (allFieldsExist)
      {
        if (poJYDHLayer->SetAttributeFilter(ogrAttributeFilter.c_str()) != OGRERR_NONE)
        {
      ...
      else
      {
        ...
        poJYDHLayer->SetAttributeFilter(nullptr);
        continue;
      }
      poJYDHLayer->ResetReading();
      //  对符合过滤条件的要素进行处理，这里需要将语义融合不成功的记录在pDestLayer图层中
      while ((poFeature = poJYDHLayer->GetNextFeature()) != nullptr)
      {
        //  先计算 NJBDX_Classification_Code_Mapping_Condition 的结果，例如 "11010202"，然后取其前两位 "11" 来决定映射到哪个 NJBDX 图层
        std::string classificationCodeResult = JYDH_extractClassificationCode(codeMapping.NJBDX_Classification_Code_Mapping_Condition);
        ...
        for (auto& nLayerInfo : NJBDX_single_frame_data_all_layers_info_entity.vNJBDX_single_frame_data_single_layer_info)
        {
          ...
          //  这里需要根据几何类型来拼接图层名称
          const std::string NJBDXLayerName = targetLayerEnglishName + "_" + JYDH_OGRGeometryType2String(poJYDHLayer->GetGeomType());
          //  比较解析后的名称与映射中的名称
          bool nameMatches = (nLayerInfo.layer_name == NJBDXLayerName);

          //  比较几何类型（NJBDX图层的几何类型应该同JYDH图层的几何类型是相同的，不应该存在类型不同的情况）
          bool geometryMatches = (nLayerInfo.layer_geo_type == poJYDHLayer->GetGeomType());
        ...
        //  将JYDH当前图层要素的几何定义赋值到新创建的NJBDX要素中
        JYDH_copy_geometry_from_source_to_target(poFeature, poNewNjbdxFeature, single_frame_data_successful_logger);



        //  日志说明现在开始处理JYDH数据源中当前图层的‘分类编码’字段属性值映射
        single_frame_data_successful_logger->info("<-----------------------------------第一步----------------------------------->");
        single_frame_data_successful_logger->info("函数[{}]：第一步--->开始处理 JYDH 图层：{} 中要素ID为 {} 的'分类编码'字段属性值映射。",
          __FUNCTION__,
          JYDH_single_layer.layer_name,
          poFeature->GetFID() + 1);

        //  写入分类编码字段（例如 NJBDX 图层中专门有 "CODE" 字段，即分类编码字段）
        JYDH_mapping_classification_code_field(poNewNjbdxFeature, "CODE", classificationCodeResult);

        //  日志说明现在结束处理JYDH数据源中当前图层的‘分类编码’字段属性值映射
        single_frame_data_successful_logger->info("函数[{}]：第一步--->结束处理 JYDH 图层：{} 中要素ID为 {} 的'分类编码'字段属性值映射。", __FUNCTION__, JYDH_single_layer.layer_name, poFeature->GetFID() + 1);
        single_frame_data_successful_logger->info("<-----------------------------------第一步----------------------------------->\n");
        ...
        //  日志说明现在开始处理JYDH数据源中当前图层的‘其他’字段属性值映射
        single_frame_data_successful_logger->info("<-----------------------------------第二步----------------------------------->");
        single_frame_data_successful_logger->info("函数[{}]：第二步--->开始处理 JYDH 图层：{} 中要素ID为 {} 的'其他'字段属性值映射。", __FUNCTION__, JYDH_single_layer.layer_name, poFeature->GetFID() + 1);

        //  获取JYDH_Classification_Code_Conditional_Mapping_Lists_Json_entity对应NJBDX的特定图层
        JYDH_Layers_Other_Fields_Mapping_t JYDH_Layers_Other_Fields_Mapping_entity;
        for (int i = 0; i < JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity.vJYDH_Layers_Other_Fields_Mapping.size(); i++)
        {
          //  这里的图层标识符可以是11、12、13等等
          if (JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity.vJYDH_Layers_Other_Fields_Mapping[i].NJBDX_Layer_English_Name == targetLayerEnglishName)
          {
            JYDH_Layers_Other_Fields_Mapping_entity = JYDH_Other_Fields_Mapping_NJBDX_Other_Fields_Json_entity.vJYDH_Layers_Other_Fields_Mapping[i];
            break;
          }
        }
        //  检查是否找到其他字段映射的配置信息
        if (!(JYDH_Layers_Other_Fields_Mapping_entity.vOther_Fields_Mapping.size()))
        {
          single_frame_data_successful_logger->error("函数[{}]：在JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json配置文件中没有找到NJBDX图层[{}]配置信息，跳过该要素的'其他'字段属性值映射。",
            __FUNCTION__,
            targetLayerEnglishName);
          ...
          //  日志说明现在结束处理JYDH数据源中当前图层的‘其他’字段属性值映射
          single_frame_data_successful_logger->info("函数[{}]：第二步--->结束处理 JYDH 图层：{} 中要素ID为 {} 的'其他'字段属性值映射。",
            __FUNCTION__,
            JYDH_single_layer.layer_name,
            poFeature->GetFID() + 1);
          single_frame_data_successful_logger->info("-----------------------------------第二步----------------------------------->\n");
        ...
        //  映射JYDH图层的其他字段属性
        JYDH_mapping_other_fields(
          poFeature,
          poNewNjbdxFeature,
          JYDH_Layers_Other_Fields_Mapping_entity,
          single_frame_data_successful_logger);

        //  日志说明现在结束处理JYDH数据源中当前图层的‘其他’字段属性值映射
        single_frame_data_successful_logger->info("函数[{}]：第二步--->结束处理 JYDH 图层：{} 中要素ID为 {} 的'其他'字段属性值映射。", __FUNCTION__, JYDH_single_layer.layer_name, poFeature->GetFID() + 1);
        single_frame_data_successful_logger->info("<-----------------------------------第二步----------------------------------->\n");
...
#pragma endregion
    ...
    if (!is_exist_classification_code_mapping_condition_flag)
    {
      ...
      single_frame_data_successful_logger->warn("函数[{}]：JYDH_Classification_Code_Conditional_Mapping_Lists.json配置文件中没有 JYDH 图层：{} 的映射信息。",
            __FUNCTION__, 
            JYDH_single_layer.layer_name);
    }
    ...
    while ((poRemainingFeature = poJYDHLayer->GetNextFeature()) != nullptr)
    {
...
#pragma endregion
    ...
    //  日志中提示当前正在处理的JYDH图层
    single_frame_data_successful_logger->info("<----------函数[{}]：结束处理 JYDH 中第 {} 个图层：{} (该图层GDAL几何类型：{} )---------->\n",
      __FUNCTION__,
      JYDH_layer_counter,
      JYDH_single_layer.layer_name,
      OGRGeometryTypeToName(JYDH_single_layer.layer_geo_type));
    JYDH_layer_counter++;
  } // end for JYDH_single_layer
 ...
 */
std::string qgs_JYDH_NJBDX_semantic_fusion::JYDH_extractClassificationCode(const std::string& input)
{
  // 定义匹配8位数字的正则表达式
 ...
 */
std::string qgs_JYDH_NJBDX_semantic_fusion::JYDH_execute_single_qgis_expression(
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  JYDH_fillExpressionContextFromOGRFeature(poSourceFeature, context);
  ...
  if (result.isNull())
  {
    logger->debug("函数[{}]：表达式结果为 NULL，跳过字段更新（为了满足 NJBDX 中的字段属性值来自 JYDH 多个不同字段的属性值，而不是唯一指定的字段，该调试信息可以直接忽略。）：{}，表达式：{}",
      __FUNCTION__,
      leftField,
      rightExpr);
    return std::string();
...
std::vector<std::string> qgs_JYDH_NJBDX_semantic_fusion::JYDH_execute_multiple_qgis_expressions(
 ...
 */
QgsFeature qgs_JYDH_NJBDX_semantic_fusion::convertOGRFeatureToQgsFeature(OGRFeature* poSourceFeature)
{
...
/**
 * @brief 将源要素(来自JYDH)的几何复制到新要素(将写入NJBDX)
 *
 * @param poSourceFeature 源 OGRFeature 指针
 * @param poTargetFeature 目标 OGRFeature 指针
 */
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_copy_geometry_from_source_to_target(
  OGRFeature* poSourceFeature,
  OGRFeature* poTargetFeature,
  std::shared_ptr<spdlog::logger> single_frame_data_logger)
{
  ...
  {
    //  记录错误信息
    single_frame_data_logger->info("函数[{}]：将JYDH要素的几何信息复制给NJBDX要素出现了错误。", __FUNCTION__);
  }
}
 ...
 */
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_mapping_classification_code_field(
  OGRFeature* poTargetFeature,
...
/**
 * @brief 将其他字段（非分类编码字段）从JYDH映射到NJBDX
 *
 * @param poSourceFeature 源 OGRFeature（JYDH）
 * @param poTargetFeature 目标 OGRFeature（NJBDX）
 * @param vOtherFieldsMapping 当前图层的“其他字段映射”规则
 * @param single_frame_data_logger 日志记录器
 */
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_mapping_other_fields(
  OGRFeature* poSourceFeature,
  OGRFeature* poTargetFeature,
  const JYDH_Layers_Other_Fields_Mapping_t& JYDH_Layers_Other_Fields_Mapping_entity,
  std::shared_ptr<spdlog::logger> single_frame_data_logger)
{
  ...
  //  遍历当前图层在 “JYDH_Other_Fields_Mapping_NJBDX_Other_Fields.json” 中配置的映射关系
  for (const auto& mappingItem : JYDH_Layers_Other_Fields_Mapping_entity.vOther_Fields_Mapping)
  {
    ...
    */
    std::string evalResult = JYDH_execute_single_qgis_expression(
      mappingItem.field_mapping_codition,
      poSourceFeature,
      poTargetFeature,
 ...
 */
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_fillExpressionContextFromOGRFeature(
  OGRFeature* poSourceFeature,
  QgsExpressionContext& context)
{
  if (!poSourceFeature)
...
void qgs_JYDH_NJBDX_semantic_fusion::CopyFeaturesToEmptyLayer(
    OGRLayer* pSrcLayer, 
    OGRLayer* pDestLayer, 
    std::shared_ptr<spdlog::logger> logger)
{
    ...
    {
        logger->info("函数[{}]：开始处理 JYDH 图层：{} 中要素ID为 {} 的要素。", 
        __FUNCTION__, 
        pSrcLayer->GetName(), 
        ...
        if (poDestFeature == nullptr)
        {
            ...
            logger->info("函数[{}]：结束处理 JYDH 图层：{} 中要素ID为 {} 的要素。", 
            __FUNCTION__, 
            pSrcLayer->GetName(), 
            poSrcFeature->GetFID() + 1);
...
void qgs_JYDH_NJBDX_semantic_fusion::CopyOneFeatureToEmptyLayer(
...
//  获取JYDH单个分幅数据集的信息
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_Create_single_frame_data_all_layers_info(
  GDALDataset* poSingleFrameJYDHDataSet,
  JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity,
  std::shared_ptr<spdlog::logger> single_frame_data_logger)
{

  if (!poSingleFrameJYDHDataSet)
  {
    //  数据集指针为空则设置成默认值
    JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS = NULL;
  ...
  {
    //  设置数据集指针
    JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS = poSingleFrameJYDHDataSet;
  }

  //  获取并设置数据集路径
  const char* dataset_path = poSingleFrameJYDHDataSet->GetDescription();
  if (!dataset_path)
  {
    //  获取不到则设置成默认值
    JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_path = "";
  ...
  {
    //  获取得到则赋值
    JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_path = dataset_path;
  }

  //  获取图层数量
  int layer_count = poSingleFrameJYDHDataSet->GetLayerCount();

  //  遍历每个图层
  for (int i = 0; i < layer_count; ++i)
  {
    //  获取图层指针
    OGRLayer* layer = poSingleFrameJYDHDataSet->GetLayer(i);
    if (!layer)
    {
    ...
    //  创建单个图层信息结构体变量
    JYDH_single_frame_data_single_layer_info_t layer_info;
    //  获取并设置图层名称
    ...
    //  将图层信息添加到向量中
    JYDH_single_frame_data_all_layers_info_entity.vJYDH_single_frame_data_single_layer_info.push_back(layer_info);
  }
}

/**
 * @brief 根据JYDH单个分幅数据中所有图层的信息创建一个空的输出数据集，
 *        新数据集中每个图层拥有相同的图层名称、几何类型、空间参考和字段定义，但不包含任何要素。
 *
 * @param srcData 已经读取到的JYDH单个分幅数据的所有图层信息
 ...
 */
void qgs_JYDH_NJBDX_semantic_fusion::CreateEmptyDatasetFromJYDHInfo(
  const JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity,
  JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_empty_entity,
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  //  数据集创建成功之后写入到自定义结构体中
  JYDH_single_frame_data_all_layers_info_empty_entity.JYDH_single_frame_data_path = newDatasetPath;
  JYDH_single_frame_data_all_layers_info_empty_entity.JYDH_single_frame_data_poDS = pNewDataset;

  JYDH_single_frame_data_single_layer_info_t JYDH_current_single_frame_data_single_layer_info_entity;
  //  遍历源数据中的每个图层，按原有结构创建对应的空图层
  for (const auto& layerInfo : JYDH_single_frame_data_all_layers_info_entity.vJYDH_single_frame_data_single_layer_info)
  {
    //  判断原始图层指针是否有效
    ...
    //  将图层信息写入到自定义结构体中
    JYDH_current_single_frame_data_single_layer_info_entity.layer_name = layerInfo.layer_name;
    JYDH_current_single_frame_data_single_layer_info_entity.layer_geo_type = layerInfo.layer_geo_type;
    JYDH_current_single_frame_data_single_layer_info_entity.poSRS = layerInfo.poSRS;
    ...
    if (pSrcFDefn)
    {
      ...
      JYDH_current_single_frame_data_single_layer_info_entity.polayer = pNewLayer;
    }
    else
    {
    ...
    JYDH_single_frame_data_all_layers_info_empty_entity.vJYDH_single_frame_data_single_layer_info.push_back(JYDH_current_single_frame_data_single_layer_info_entity);
  }
...
//  检查JYDH数据集中的有效图层是否为空
bool qgs_JYDH_NJBDX_semantic_fusion::JYDH_Check_single_frame_data_all_layers_info(
  const JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity,
  std::shared_ptr<spdlog::logger> single_frame_data_logger)
{
  //  TODO：添加更为详细的检查
  if (JYDH_single_frame_data_all_layers_info_entity.vJYDH_single_frame_data_single_layer_info.size() == 0)
  {
    return false;
  }
  return true;
...
//  释放JYDH单个分幅数据集资源
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_Close_single_frame_data_all_layers_info(
  JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity)
{
  if (JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS)
  {
    GDALClose(JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS);
  }
  return;
}

void qgs_JYDH_NJBDX_semantic_fusion::JYDH_Delete_And_Close_single_frame_data_all_layers_info(
    JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity)
{
    GDALDataset* poDataset = JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS;
    if (poDataset)
    {
        ...
        // 最后关闭数据集并将指针置空
        GDALClose(poDataset);
        JYDH_single_frame_data_all_layers_info_entity.JYDH_single_frame_data_poDS = nullptr;
    }
}
...
//  创建NJBDX单个分幅数据集的信息
bool qgs_JYDH_NJBDX_semantic_fusion::JYDH_Create_NJBDX_all_layers_info(
  const JYDH_single_frame_data_all_layers_info_t& JYDH_single_frame_data_all_layers_info_entity,
  const JYDH_Layers_Mapping_NJBDX_Layers_Json_t& JYDH_Layers_Mapping_NJBDX_Layers_Json_entity,
  const JYDH_Layers_Fields_Info_Json_t& JYDH_Layers_Fields_Info_Json_entity,
  const JYDH_NJBDX_Layers_Fields_Info_Json_t& NJBDX_Layers_Fields_Info_Json_entity,
...
#pragma region "2 根据JYDH图层信息遍历图层信息"
  //  用于存储已创建的 “NJBDX 图层英文名 -> GDALDataset*” 以避免重复创建
  std::map<std::string, GDALDataset*> createdDatasets;

  //  遍历所有 JYDH 图层信息
  for (const auto& JYDHLayerInfo : JYDH_single_frame_data_all_layers_info_entity.vJYDH_single_frame_data_single_layer_info)
  {
    //  获取得到当前JYDH单个图层的名称例如（DN09490622_D_line）
    const std::string& JYDHLayerName = JYDHLayerInfo.layer_name;
    //  获取得到当前JYDH单个图层的几何类型
    OGRwkbGeometryType JYDH_layer_geo_type = JYDHLayerInfo.layer_geo_type;

#pragma region "2.1 在JYDH_Layers_Mapping_NJBDX_Layers.json中找到对应的映射项"
    //  2.1 在映射表中查找对应的 NJBDX 图层集合
    JYDH_layers_mapping_NJBDX_layers_t MappingItem;
    //  提取JYDH图层名称(DN09490622_D_line--->D，根据不同的数据源将会有不同的抽取规则)
    std::string JYDH_extracted_layername = JYDH_Extract_LayerName(JYDHLayerName, JYDH_Layers_Fields_Info_Json_entity, single_frame_data_logger);

    //  循环遍历所有 JYDH_mapping_NJBDX 图层信息项
    for (auto& mapItem : JYDH_Layers_Mapping_NJBDX_Layers_Json_entity.vJYDH_layers_mapping_NJBDX_layers)
    {
      //  每次迭代都重新初始化匹配标志
      ...
      //  使用图层的英文名称判断是否匹配
      if (mapItem.JYDH_Layer.JYDH_layer_english_name == JYDH_extracted_layername)
      {
        layer_name_match_flag = true;
      }
      //  判断图层的几何类型是否匹配
      if (mapItem.JYDH_Layer.JYDH_layer_type == JYDH_layer_geo_type)
      {
    ...
    if (MappingItem.vNJBDX_Mapping_Layers.size() == 0)
    {
      ...
      single_frame_data_logger->info("函数[{}]：JYDH 图层[{}]在映射表中找到了空的对应 NJBDX 图层，已跳过该图层的处理", __FUNCTION__, JYDHLayerName);
      continue;
    }
#pragma endregion

#pragma region "2.2 根据JYDH图层的映射关系创建新的NJBDX图层"
    //  循环遍历该 JYDH 图层映射到的多个 NJBDX 图层
    for (auto& singleNJBDXLayerMapping : MappingItem.vNJBDX_Mapping_Layers)
    {
      ...
      const std::string& NJBDXLayerEnglishName_GeoType = NJBDXLayerEnglishName + "_" + JYDH_OGRGeometryType2String(singleNJBDXLayerMapping.NJBDX_layer_type);
      ...
      if (!poExistingDataset)
      {
        ...
        //  设置空间参考（沿用JYDH图层的 poSRS）
        OGRSpatialReference* poRef = nullptr;
        if (JYDHLayerInfo.poSRS)
        {
          poRef = JYDHLayerInfo.poSRS->Clone();
        }

        //  在该 Shapefile 数据集中创建图层
        OGRLayer* poNewLayer = poNewDS->CreateLayer(NJBDXLayerEnglishName_GeoType.c_str(), poRef, JYDH_layer_geo_type, nullptr);
        if (!poNewLayer)
        {
        ...
        {
          JYDH_NJBDX_single_frame_data_single_layer_info_t newLayerInfo;
          newLayerInfo.layer_name = NJBDXLayerEnglishName_GeoType;
          newLayerInfo.layer_geo_type = JYDH_layer_geo_type;
          ...
          single_frame_data_logger->info("函数[{}]：创建NJBDX图层 [{}] 成功 (来自 JYDH图层 [{}])",
            __FUNCTION__, NJBDXLayerEnglishName_GeoType, JYDHLayerName);
        }
      }
      else
      {
...
#pragma endregion
  ...
  } // end for (auto& JYDHLayerInfo ...)
#pragma endregion

#pragma region "3 关闭所有的数据集然后再以目录的方式打开"
  //  对 createdDatasets 中的数据集执行 GDALClose()
  for (const auto& JYDH_createdDataset : createdDatasets)
  {
    GDALClose(JYDH_createdDataset.second);
  }
...
bool qgs_JYDH_NJBDX_semantic_fusion::JYDH_Check_NJBDX_all_layers_info(
  const JYDH_NJBDX_single_frame_data_all_layers_info_t& NJBDX_single_frame_data_all_layers_info_entity)
{
...
void qgs_JYDH_NJBDX_semantic_fusion::JYDH_Close_NJBDX_all_layers_info(
  JYDH_NJBDX_single_frame_data_all_layers_info_t& NJBDX_single_frame_data_all_layers_info_entity)
{
...
//  提取图层名称
std::string qgs_JYDH_NJBDX_semantic_fusion::JYDH_Extract_LayerName(
    ...
    std::shared_ptr<spdlog::logger> logger)
{
    ...
    //  遍历配置文件中所有图层的配置信息
    for (const auto& layerFields : JYDH_Layers_Fields_Info_Json_entity.vJYDH_Layers_Fields)
    {
        //  如果配置中的 JYDH_Layer_Name 为空，直接跳过
        if (layerFields.JYDH_Layer_Name.empty())
        {
            logger->warn("函数[{}]：JYDH_Layers_Fields_Info.json配置文件中某个 JYDH_Layer_Name 为空字符串，跳过矢量图层名称提取处理。", __FUNCTION__);
            continue;
        }
        ...
        Notes:杨小兵-2025-02-16
          ...
          //std::regex re(layerFields.JYDH_Layer_Name, std::regex::icase);
        */
        std::regex re(layerFields.JYDH_Layer_Name);

        //  使用 std::regex_search 来检查 layerName 是否包含该模式
        if (std::regex_search(layerName, re))
        {
            //  TODO:如果匹配成功，如果这里将匹配成功的提示写入日志，那么将会造成日志占用的空间变大，所以只需要将没有匹配到的提示信息输出到日志中。
            logger->info("函数[{}]：图层名称[{}] 与JYDH_Layers_Fields_Info.json配置文件中的[{}] 匹配成功。",
                __FUNCTION__, layerName, layerFields.JYDH_Layer_Name);

            //  例如可以直接返回匹配到的 JYDH_Layer_Name
            return layerFields.JYDH_Layer_Name;
        }
    }

    //  如果遍历完也没有找到匹配的配置信息
    logger->info("函数[{}]：图层名称[{}] 未在JYDH_Layers_Fields_Info.json配置文件中找到匹配项。", __FUNCTION__, layerName);
    return "";
}

//  辅助函数：将OGRwkbGeometryType映射到字符串
std::string qgs_JYDH_NJBDX_semantic_fusion::JYDH_OGRGeometryType2String(const OGRwkbGeometryType& GeoType)
{
...
std::vector<std::string> qgs_JYDH_NJBDX_semantic_fusion::extractFieldNames(const std::string& filter)
{
...
std::string qgs_JYDH_NJBDX_semantic_fusion::extractBeforeDelimiter(const std::string& str)
{
    // lambda表达式：去除左右两边空格
    auto trim = [](std::string& s) {
        s.erase(0, s.find_first_not_of(" \t\r\n"));
...
std::string qgs_JYDH_NJBDX_semantic_fusion::extractAfterDelimiter(const std::string& str)
{
    // lambda表达式：去除左右两边空格
    auto trim = [](std::string& s) {
        s.erase(0, s.find_first_not_of(" \t\r\n"));
