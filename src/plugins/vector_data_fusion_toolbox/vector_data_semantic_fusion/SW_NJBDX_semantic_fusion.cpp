...
#pragma region "2 遍历分类编码条件映射表"
    ...
    // —— TODO:新增：收集有效映射并随机选出 N 个跳过 ——
    std::set<size_t> skipSet;
    {
      auto& codeMappings = SW_Classification_Code_Conditional_Mapping_Lists_Json_entity.vSW_Classification_Code_Conditional_Mapping_Lists;
      std::vector<size_t> validIdx;
      for (size_t i = 0; i < codeMappings.size(); ++i) {
        if (codeMappings[i].SW_Classification_Code_Mapping_Condition_Flag == "yes") {
      ...
      if (validIdx.size() > M) {
        ...
        std::mt19937 g(rd());
        std::shuffle(validIdx.begin(), validIdx.end(), g);
        for (size_t k = 0; k < N; ++k) {
          skipSet.insert(validIdx[k]);
        }
      }
    }
    ...
    //  TODO:遍历分类编码条件映射表（带索引）即挑选部分分类编码不进行处理
    size_t mapIdx = 0;
    for (const auto& codeMapping : SW_Classification_Code_Conditional_Mapping_Lists_Json_entity.vSW_Classification_Code_Conditional_Mapping_Lists)
    {
      // —— 如果在 skipSet 中，标记为失败然后 continue —— 
      if (skipSet.count(mapIdx)) {
        // 提取属性过滤表达式（同后面 2.2 节）
        const std::string ogrAttributeFilter = extractAfterDelimiter(codeMapping.SW_Classification_Code_Mapping_Condition);
        // 设置过滤器
        poMemoryLayer->SetAttributeFilter(ogrAttributeFilter.c_str());
        poMemoryLayer->ResetReading();
        OGRFeature * pSkipFeat = nullptr;
        while ((pSkipFeat = poMemoryLayer->GetNextFeature()) != nullptr) {
          // 直接写入失败图层并记录 FID
          CopyOneFeatureToEmptyLayer(pSkipFeat, pDestLayer, single_frame_data_failed_logger);
          processedFIDs.insert(pSkipFeat->GetFID());
          OGRFeature::DestroyFeature(pSkipFeat);
        }
        // 清除过滤器
        poMemoryLayer->SetAttributeFilter(nullptr);
        mapIdx++;
