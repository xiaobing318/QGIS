...
#pragma region "第一步：创建输出目录、日志器、日志文件"
  ...
  try {
    ...
    QString SuccessfulSemanticFusionOutputDirName = "SuccessfulSemanticFusion_JBHT2NJBDX_SRCDATA_" + frame_dir_name;
    QString FailedSemanticFusionOutputDirName = "FailedSemanticFusion_JBHT2NJBDX_SRCDATA_" + frame_dir_name;
...
#pragma region "第三步：将JBHT数据所有图层的相关信息读取到自定义数据结构中"
  ...
  //  检查数据集中的图层是否为空
  if (!JBHT_Check_single_frame_data_all_layers_info(JBHT_single_frame_data_all_layers_info_entity, single_frame_data_successful_logger))
  {
...
#pragma region "第四步：将JBHT数据所有图层的相关信息读取到自定义数据结构中，创建空图层信息"
  //  创建变量用来存储JBHT分幅数据空数据相关信息
  JBHT_single_frame_data_all_layers_info_t JBHT_single_frame_data_all_layers_info_empty_entity;
  //  根据JBHT单个分幅数据中所有图层的信息创建一个空的输出数据集
  CreateEmptyDatasetFromJBHTInfo(
      JBHT_single_frame_data_all_layers_info_entity,
      JBHT_single_frame_data_all_layers_info_empty_entity,
      strSingleFailedSemanticFusionOutputDataPath,
      poShpDriver,
      single_frame_data_failed_logger);
  //  检查数据集中的图层是否为空
  if (!JBHT_Check_single_frame_data_all_layers_info(JBHT_single_frame_data_all_layers_info_empty_entity, single_frame_data_failed_logger))
  {
...
#pragma region "第五步：根据JBHT实际数据和图层映射关系创建NJBDX数据集信息到自定义数据结构中"
...
#pragma region "第六步：根据JBHT数据集信息和NJBDX数据集信息、分类编码配置信息进行字段属性映射"
...
#pragma region "第七步：释放JBHT分幅数据、NJBDX分幅数据中创建的内存资源"
  //  需要将三个数据集关闭以释放内存资源
  JBHT_Close_single_frame_data_all_layers_info(JBHT_single_frame_data_all_layers_info_entity);
  //  删除空图层以及关闭非空图层
  JBHT_Delete_And_Close_single_frame_data_all_layers_info(JBHT_single_frame_data_all_layers_info_empty_entity);
  //  删除空图层以及关闭非空图层
  JBHT_Close_NJBDX_all_layers_info(NJBDX_single_frame_data_all_layers_info_entity);
  single_frame_data_successful_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
  single_frame_data_failed_logger->info("*****************************************结束处理分幅数据：{}*****************************************", single_frame_data_path);
...
#pragma region "JBHT2NJBDX_Single_Frame_Data中第六步对应函数"
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  if (result.isNull())
  {
    logger->debug("函数[{}]：表达式结果为 NULL，跳过字段更新（为了满足 NJBDX 中的字段属性值来自 JBHT 多个不同字段的属性值，而不是唯一指定的字段，该调试信息可以直接忽略。）：{}，表达式：{}",
      __FUNCTION__,
      leftField,
      rightExpr);
    return std::string();
...
void qgs_JBHT_NJBDX_semantic_fusion::CopyFeaturesToEmptyLayer(
    OGRLayer* pSrcLayer, 
    OGRLayer* pDestLayer, 
    std::shared_ptr<spdlog::logger> logger)
{
    ...
    {
        logger->info("函数[{}]：开始处理 JBHT 图层：{} 中要素ID为 {} 的要素。", 
        __FUNCTION__, 
        pSrcLayer->GetName(), 
        ...
        if (poDestFeature == nullptr)
        {
            ...
            logger->info("函数[{}]：结束处理 JBHT 图层：{} 中要素ID为 {} 的要素。", 
            __FUNCTION__, 
            pSrcLayer->GetName(), 
            poSrcFeature->GetFID() + 1);
...
void qgs_JBHT_NJBDX_semantic_fusion::CopyOneFeatureToEmptyLayer(
...
/**
 * @brief 根据JBHT单个分幅数据中所有图层的信息创建一个空的输出数据集，
 *        新数据集中每个图层拥有相同的图层名称、几何类型、空间参考和字段定义，但不包含任何要素。
 *
 * @param srcData 已经读取到的JBHT单个分幅数据的所有图层信息
 ...
 */
void qgs_JBHT_NJBDX_semantic_fusion::CreateEmptyDatasetFromJBHTInfo(
  const JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity,
  JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_empty_entity,
  ...
  std::shared_ptr<spdlog::logger> logger)
{
  ...
  //  数据集创建成功之后写入到自定义结构体中
  JBHT_single_frame_data_all_layers_info_empty_entity.JBHT_single_frame_data_path = newDatasetPath;
  JBHT_single_frame_data_all_layers_info_empty_entity.JBHT_single_frame_data_poDS = pNewDataset;

  JBHT_single_frame_data_single_layer_info_t JBHT_current_single_frame_data_single_layer_info_entity;
  //  遍历源数据中的每个图层，按原有结构创建对应的空图层
  for (const auto& layerInfo : JBHT_single_frame_data_all_layers_info_entity.vJBHT_single_frame_data_single_layer_info)
  {
    //  判断原始图层指针是否有效
    ...
    //  将图层信息写入到自定义结构体中
    JBHT_current_single_frame_data_single_layer_info_entity.layer_name = layerInfo.layer_name;
    JBHT_current_single_frame_data_single_layer_info_entity.layer_geo_type = layerInfo.layer_geo_type;
    JBHT_current_single_frame_data_single_layer_info_entity.poSRS = layerInfo.poSRS;
    ...
    if (pSrcFDefn)
    {
      ...
      JBHT_current_single_frame_data_single_layer_info_entity.polayer = pNewLayer;
    }
    else
    {
    ...
    JBHT_single_frame_data_all_layers_info_empty_entity.vJBHT_single_frame_data_single_layer_info.push_back(JBHT_current_single_frame_data_single_layer_info_entity);
  }
...
void qgs_JBHT_NJBDX_semantic_fusion::JBHT_Delete_And_Close_single_frame_data_all_layers_info(
    JBHT_single_frame_data_all_layers_info_t& JBHT_single_frame_data_all_layers_info_entity)
{
    GDALDataset* poDataset = JBHT_single_frame_data_all_layers_info_entity.JBHT_single_frame_data_poDS;
    if (poDataset)
    {
        ...
        // 最后关闭数据集并将指针置空
        GDALClose(poDataset);
        JBHT_single_frame_data_all_layers_info_entity.JBHT_single_frame_data_poDS = nullptr;
    }
}
...
std::string qgs_JBHT_NJBDX_semantic_fusion::extractBeforeDelimiter(const std::string& str)
{
    // lambda表达式：去除左右两边空格
    auto trim = [](std::string& s) {
        s.erase(0, s.find_first_not_of(" \t\r\n"));
...
std::string qgs_JBHT_NJBDX_semantic_fusion::extractAfterDelimiter(const std::string& str)
{
    // lambda表达式：去除左右两边空格
    auto trim = [](std::string& s) {
        s.erase(0, s.find_first_not_of(" \t\r\n"));
