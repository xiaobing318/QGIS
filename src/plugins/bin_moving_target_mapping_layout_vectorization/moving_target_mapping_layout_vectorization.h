#ifndef QGS_MOVING_TARGET_MAPPING_LAYOUTVECTORIZATION_DIALOG_H
#define QGS_MOVING_TARGET_MAPPING_LAYOUTVECTORIZATION_DIALOG_H
...
#include "ui_moving_target_mapping_layout_vectorization.h"
...
/*属性字段属性值对照结构体*/
struct FieldNameAndValue_MovingTarget
{
    string strFieldName;
    string strFieldValue;
    FieldNameAndValue_MovingTarget()
    {
        strFieldName = "";
        strFieldValue = "";
    }
...
#pragma region "3、动目标制图类"
class QgsMovingTargetMappingLayoutVectorizationDialog : public QDialog
{
    Q_OBJECT

public:
    QgsMovingTargetMappingLayoutVectorizationDialog(QWidget* parent = nullptr, Qt::WindowFlags fl = Qt::WindowFlags());
    ~QgsMovingTargetMappingLayoutVectorizationDialog() override;

private:

    Ui_QgsMovingTargetMapping_LayoutVectorizationDlg ui;
    ...
    // 鹰眼图数据文件路径
    QString m_qstrEagleEyeDataPath;
...
private slots:
    ...
    // 设置鹰眼图简单渲染方式
    void SetSimpleRenderRadioButton();

    // 设置鹰眼图分类渲染方式
    void SetCategoryRenderRadioButton();
...
private:
    ...
    int Get_Point(OGRFeature* poFeature, SE_DPoint& coordinate, vector<FieldNameAndValue_MovingTarget>& vFieldvalue);
    ...
    int Set_Point(OGRLayer* poLayer, double x, double y, double z, vector<FieldNameAndValue_MovingTarget>& vFieldAndValue);

    bool CreateShapefileCPG(string strCPGFilePath, string strEncoding);

    // 获取整饰点要素qml文件路径
    void GetLayoutDecPointQmlFilePath(QString& qstrLayoutDecPointQmlPath);

    // 获取整饰面要素qml文件路径
    void GetLayoutDecPolygonQmlFilePath(QString& qstrLayoutDecPolygonQmlPath);

    // 创建多边形要素
    int Set_Polygon(OGRLayer* poLayer, vector<SE_DPoint> OuterRing, vector<FieldNameAndValue_MovingTarget>& vFieldAndValue);

    // 裁剪影像图
    bool ChunkImageDataToJPEG(string strRasterFullName,
        double dLeft,
        double dTop,
        double dRight,
        double dBottom,
    ...
    */
    ...
    // 布局坐标到投影坐标的转换
    void TransformLayoutToProjCRS(SE_DPoint dRefPoint,
        SE_DPoint dRefLayoutPoint,
        SE_DPoint dLayoutPoint,
        double dMapUnitToLayoutUnit,
...
private:
    ...
    QColor		m_qEagleEyeLayerColor;			// 鹰眼图地图渲染颜色
    bool		m_bSetEagleEyeLayerColor;		// 是否设置鹰眼图渲染颜色

};
#pragma endregion

#endif // QGS_MOVING_TARGET_MAPPING_LAYOUTVECTORIZATION_DIALOG_H
