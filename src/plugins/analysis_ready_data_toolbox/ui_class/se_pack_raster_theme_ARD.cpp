...
#include "se_pack_raster_theme_ARD.h"
...
/*内部*/
#include "../ui_task/se_pack_raster_theme_ARD_task.h"
...
SE_PackRasterThemeARDDlg::SE_PackRasterThemeARDDlg(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	connect(ui.Button_OK , &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_OK_clicked);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_Cancel_clicked);
	connect(ui.pushButton_OpenTerrainFactorsMetaData, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_OpenTerrainMetaData_clicked);
	connect(ui.pushButton_OpenRasterizeMetaData, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_OpenRasterizeMetaData_clicked);

	connect(ui.pushButton_OpenRasterARDPath, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_Open_clicked);
	connect(ui.pushButton_SaveRasterARDPackage, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_Save_clicked);
	connect(ui.pushButton_Task_Model_Data_Path, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_OpenConfigFile_clicked);
	connect(ui.Button_SaveLogFilePath, &QPushButton::clicked, this, &SE_PackRasterThemeARDDlg::pushButton_SaveLog_clicked);
	...
	ui.lineEdit_InputRasterARDPath->setText(m_qstrInputDataPath);
...
SE_PackRasterThemeARDDlg::~SE_PackRasterThemeARDDlg()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/InputDataPath"), m_qstrInputDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/OutputDataPath"), m_qstrOutputDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/InputTerrainPath"), m_qstrInputTerrainPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/InputRasterizePath"), m_qstrInputRasterizePath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/OutputLogPath"), m_qstrOutputLogPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_RASTER_THEME_ARD/ConfigFilePath"), m_qstrConfigFilePath, QgsSettings::Section::Plugins);

}

void SE_PackRasterThemeARDDlg::restoreState()
{
	const QgsSettings settings;
	m_qstrInputDataPath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/InputDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputDataPath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/OutputDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrInputTerrainPath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/InputTerrainPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrInputRasterizePath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/InputRasterizePath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputLogPath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/OutputLogPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrConfigFilePath = settings.value(QStringLiteral("PACK_RASTER_THEME_ARD/ConfigFilePath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();

}



void SE_PackRasterThemeARDDlg::pushButton_OpenConfigFile_clicked()
{
	// 选择xml属性配置文件
...
void SE_PackRasterThemeARDDlg::pushButton_Save_clicked()
{
	// 选择文件夹
	QString curPath = m_qstrOutputDataPath;
	QString dlgTile = tr("请选择专题栅格分析就绪型数据包保存路径");
...
void SE_PackRasterThemeARDDlg::pushButton_OpenTerrainMetaData_clicked()
{
	QString curPath = QCoreApplication::applicationDirPath();
...
void SE_PackRasterThemeARDDlg::pushButton_OpenRasterizeMetaData_clicked()
{
	QString curPath = QCoreApplication::applicationDirPath();
...
void SE_PackRasterThemeARDDlg::pushButton_OK_clicked()
{
	// 日志级别
	// 0——错误；1——信息；2——调试
...
#pragma endregion
	...
	// 如果文件夹不存在
	if (!CheckFileOrDirExist(ui.lineEdit_InputRasterARDPath->text()))
	{
		QMessageBox msgBox;
		msgBox.setWindowTitle(tr("数据包定制及标准化封装工具"));
		msgBox.setText(tr("文件夹：%1不存在！").arg(ui.lineEdit_InputRasterARDPath->text()));
		msgBox.setStandardButtons(QMessageBox::Yes);
		msgBox.setDefaultButton(QMessageBox::Yes);
		msgBox.exec();
		return;
	...
	// 输入数据路径
	string strInputPath = ui.lineEdit_InputRasterARDPath->text().toLocal8Bit();
	...
	// 创建任务
	SE_PackRasterThemeARDTask* task = new SE_PackRasterThemeARDTask(
		tr("专题栅格分析就绪型数据封装"),
		strInputPath,
		vARDIdentifys,
		iMinGridZ,
	...
	connect(task, &SE_PackRasterThemeARDTask::taskFinished, this, &SE_PackRasterThemeARDDlg::onTaskFinished);
...
#pragma region "生成栅格分析就绪数据包元数据"
	...
	*/
	...
	rasterARDPackageMetaData.strBSXX_SJSZFW = terrainMetaInfo.strBSXX_SJSZFW;
...
void SE_PackRasterThemeARDDlg::pushButton_Cancel_clicked()
{
	reject();
}

void SE_PackRasterThemeARDDlg::onTaskFinished(bool result)
{
	// 计算并打印总进度
	CalculateTotalProgress();
}

void SE_PackRasterThemeARDDlg::pushButton_SaveLog_clicked()
{
	// 选择文件夹
...
void SE_PackRasterThemeARDDlg::CalculateTotalProgress()
{
	int totalProgress = 0;
...
void SE_PackRasterThemeARDDlg::pushButton_Open_clicked()
{
	...
	if (!directory.isEmpty())
	{
		...
		ui.lineEdit_InputRasterARDPath->setText(directory);
	}
	else
	{
		return;
...
// 读取属性检查配置信息
bool SE_PackRasterThemeARDDlg::LoadXmlConfigFile(const char* szConfigXmlFile, vector<THEME_TaskInfo>& vTaskInfos)
{
	// 如果xml文件为空
	if (!szConfigXmlFile)
	{
...
// 解析每个task节点
void SE_PackRasterThemeARDDlg::Parse_task(
	xmlDocPtr doc,
	xmlNodePtr cur,
	THEME_TaskInfo& info)
{
...
// 解析每个model节点
void SE_PackRasterThemeARDDlg::Parse_model(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
// 解析每个data节点
void SE_PackRasterThemeARDDlg::Parse_data(
	xmlDocPtr doc,
	xmlNodePtr cur,
	THEME_ARDInfo& info)
{
...
// 判断vector中是否存在当前元素
bool SE_PackRasterThemeARDDlg::IsExistedInVector(vector<string>& vValues, string strValue)
{
...
/*获取当前目录下所有子目录*/
void SE_PackRasterThemeARDDlg::GetSubDirFromFilePath(const string& strFilePath,	vector<string>& vSubDir)
{
	// 获取当前工作目录
...
bool SE_PackRasterThemeARDDlg::ReadMetaDataJsonFile(const string& strFile, RasterDataMetadataInfo& metaInfo)
{
	// 读取JSON文件
	...
	metaInfo.dBSXX_FBL = dsl["BSXX_SJSZFW"];
...
void SE_PackRasterThemeARDDlg::WriteMetaDataToJsonFile(const string& strFile, RasterDataMetadataInfo& metaInfo)
{
...
bool SE_PackRasterThemeARDDlg::CheckFileOrDirExist(const QString& qstrFileDirOrPath)
{
