...
#include "se_pack_theme_display_ready_data.h"
...
#include <QDateTime>
/*内部*/
#include "../ui_task/se_pack_theme_display_ready_data_task.h"
...
SE_PackThemeDisplayReadyDataDlg::SE_PackThemeDisplayReadyDataDlg(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	connect(ui.Button_OK , &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_OK_clicked);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_Cancel_clicked);

	connect(ui.pushButton_SavePackage, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_Save_clicked);
	connect(ui.pushButton_Task_Model_Data_Path, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_OpenConfigFile_clicked);
	connect(ui.Button_SaveLogFilePath, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_SaveLog_clicked);
	connect(ui.pushButton_AddDataPath, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_AddDisplayReadyDataPath);
	connect(ui.pushButton_ClearDataPath, &QPushButton::clicked, this, &SE_PackThemeDisplayReadyDataDlg::pushButton_ClearDisplayReadyDataPath);
...
SE_PackThemeDisplayReadyDataDlg::~SE_PackThemeDisplayReadyDataDlg()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/OutputDataPath"), m_qstrOutputDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/OutputLogPath"), m_qstrOutputLogPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/ConfigFilePath"), m_qstrConfigFilePath, QgsSettings::Section::Plugins);

}

void SE_PackThemeDisplayReadyDataDlg::restoreState()
{
	const QgsSettings settings;
	m_qstrOutputDataPath = settings.value(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/OutputDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputLogPath = settings.value(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/OutputLogPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrConfigFilePath = settings.value(QStringLiteral("PACK_THEME_DISPLAY_READY_DATA/ConfigFilePath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();

}



void SE_PackThemeDisplayReadyDataDlg::pushButton_OpenConfigFile_clicked()
{
	// 选择xml属性配置文件
...
void SE_PackThemeDisplayReadyDataDlg::pushButton_Save_clicked()
{
	// 选择文件夹
...
void SE_PackThemeDisplayReadyDataDlg::pushButton_AddDisplayReadyDataPath()
{
	// 选择文件夹
	QString curPath = QCoreApplication::applicationDirPath();
...
void SE_PackThemeDisplayReadyDataDlg::pushButton_ClearDisplayReadyDataPath()
{
	// 删除所有行
...
void SE_PackThemeDisplayReadyDataDlg::pushButton_OK_clicked()
{
	// 日志级别
	// 0——错误；1——信息；2——调试
...
#pragma region "获取用户对象选择的数据类型"

	// 用户对象选择的数据类型，默认：矢量、影像、高程，其它类型待逐步细化
	vector<string> vUserDataTypes;
	vUserDataTypes.clear();
	GetUserDataTypeCheckStatus(vUserDataTypes);
...
#pragma endregion
	...
	*/
	...
	{
		// 创建任务
		SE_PackThemeDisplayReadyDataTask* task = new SE_PackThemeDisplayReadyDataTask(
			tr("专题显示就绪型数据封装"),
			strInputPath,
		...
		connect(task, &SE_PackThemeDisplayReadyDataTask::taskFinished, this, &SE_PackThemeDisplayReadyDataDlg::onTaskFinished);
...
void SE_PackThemeDisplayReadyDataDlg::pushButton_Cancel_clicked()
{
	reject();
}

void SE_PackThemeDisplayReadyDataDlg::onTaskFinished(bool result)
{
	// 计算并打印总进度
	CalculateTotalProgress();
}

void SE_PackThemeDisplayReadyDataDlg::pushButton_SaveLog_clicked()
{
	// 选择文件夹
...
void SE_PackThemeDisplayReadyDataDlg::CalculateTotalProgress()
{
	int totalProgress = 0;
...
// 读取属性检查配置信息
bool SE_PackThemeDisplayReadyDataDlg::LoadXmlConfigFile(const char* szConfigXmlFile, vector<THEME_TaskInfo>& vTaskInfos)
{
	// 如果xml文件为空
	if (!szConfigXmlFile)
	{
...
// 解析每个task节点
void SE_PackThemeDisplayReadyDataDlg::Parse_task(
	xmlDocPtr doc,
	xmlNodePtr cur,
	THEME_TaskInfo& info)
{
...
// 解析每个model节点
void SE_PackThemeDisplayReadyDataDlg::Parse_model(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
// 解析每个data节点
void SE_PackThemeDisplayReadyDataDlg::Parse_data(
	xmlDocPtr doc,
	xmlNodePtr cur,
	THEME_ARDInfo& info)
{
...
// 判断vector中是否存在当前元素
bool SE_PackThemeDisplayReadyDataDlg::IsExistedInVector(vector<string>& vValues, string strValue)
{
...
/*获取当前目录下所有子目录*/
void SE_PackThemeDisplayReadyDataDlg::GetSubDirFromFilePath(const string& strFilePath,	vector<string>& vSubDir)
{
	// 获取当前工作目录
...
bool SE_PackThemeDisplayReadyDataDlg::ReadMetaDataJsonFile(const string& strFile, RasterDataMetadataInfo& metaInfo)
{
	// 读取JSON文件
...
void SE_PackThemeDisplayReadyDataDlg::WriteMetaDataToJsonFile(const string& strFile, RasterDataMetadataInfo& metaInfo)
{
...
bool SE_PackThemeDisplayReadyDataDlg::CheckFileOrDirExist(const QString& qstrFileDirOrPath)
{
...
void SE_PackThemeDisplayReadyDataDlg::GetDataTypeCheckStatus(vector<string>& vDataType)
{
	vDataType.clear();
	// 矢量
...
void SE_PackThemeDisplayReadyDataDlg::GetFeatureTypeCheckStatus(vector<string>& vFeatureType)
{
    vFeatureType.clear();
...
void SE_PackThemeDisplayReadyDataDlg::GetUserDataTypeCheckStatus(vector<string>& vDataType)
{
	vDataType.clear();
	// 步兵
	if (ui.checkBox_BuBing->isChecked())
	{
		vDataType.push_back(SE_DATA_TYPE_SHILIANG);
		vDataType.push_back(SE_DATA_TYPE_YINGXIANG);
		vDataType.push_back(SE_DATA_TYPE_GAOCHENG);
	...
	// 陆军航空兵
	else if (ui.checkBox_LuJunHangKongBing->isChecked())
	{
		vDataType.push_back(SE_DATA_TYPE_SHILIANG);
		vDataType.push_back(SE_DATA_TYPE_YINGXIANG);
		vDataType.push_back(SE_DATA_TYPE_GAOCHENG);
	...
	// 装甲兵
	else if (ui.checkBox_ZhuangJiaBing->isChecked())
	{
		vDataType.push_back(SE_DATA_TYPE_SHILIANG);
		vDataType.push_back(SE_DATA_TYPE_YINGXIANG);
		vDataType.push_back(SE_DATA_TYPE_GAOCHENG);
	...
	// 通信兵
	else if (ui.checkBox_TongXinBing->isChecked())
	{
		vDataType.push_back(SE_DATA_TYPE_SHILIANG);
		vDataType.push_back(SE_DATA_TYPE_YINGXIANG);
		vDataType.push_back(SE_DATA_TYPE_GAOCHENG);
