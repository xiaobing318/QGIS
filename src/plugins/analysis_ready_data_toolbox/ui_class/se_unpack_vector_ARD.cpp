...
#include "se_unpack_vector_ARD.h"
#include "commontype/se_config.h"
#include "../ui_task/se_unpack_vector_ARD_task.h"
...
CSE_UnpackVectorARDDlg::CSE_UnpackVectorARDDlg(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	connect(ui.Button_Save, &QPushButton::clicked, this, &CSE_UnpackVectorARDDlg::Button_Save_clicked);
	connect(ui.Button_Open, &QPushButton::clicked, this, &CSE_UnpackVectorARDDlg::Button_Open_clicked);
	connect(ui.Button_OK, &QPushButton::clicked, this, &CSE_UnpackVectorARDDlg::Button_OK_accepted);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &CSE_UnpackVectorARDDlg::Button_Cancel_rejected);

	connect(ui.lineEdit_InputDataPath, &QLineEdit::textChanged, this, &CSE_UnpackVectorARDDlg::on_lineEdit_InputDataPath_TextChanged);
	connect(ui.lineEdit_OutputPath, &QLineEdit::textChanged, this, &CSE_UnpackVectorARDDlg::on_lineEdit_OutputPath_TextChanged);
	connect(ui.Button_SaveLogFilePath, &QPushButton::clicked, this, &CSE_UnpackVectorARDDlg::pushButton_SaveLog_clicked);
...
CSE_UnpackVectorARDDlg::~CSE_UnpackVectorARDDlg()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("UNPACK_VECTOR_ARD/InputDataPath"), m_qstrInputDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("UNPACK_VECTOR_ARD/OutputDataPath"), m_qstrOutputDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("UNPACK_VECTOR_ARD/OutputLogPath"), m_qstrOutputLogPath, QgsSettings::Section::Plugins);

}


void CSE_UnpackVectorARDDlg::Button_Save_clicked()
{
	// 选择文件夹
...
void CSE_UnpackVectorARDDlg::Button_Open_clicked()
{
	// 选择文件夹
...
/*获取在指定目录下的目录的路径*/
QStringList CSE_UnpackVectorARDDlg::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
void CSE_UnpackVectorARDDlg::Button_OK_accepted()
{
	// 日志级别
	// 0——错误；1——信息；2——调试
...
#pragma endregion
	...
	for (int i = 0; i < qstrList.length(); ++i)
	{
		...
		// 创建任务
		SE_UnpackVectorARDTask* task = new SE_UnpackVectorARDTask(
			tr("矢量分析就绪数据包解包"),
			strInputPath,
			dLeft,
			dTop,
		...
		connect(task, &SE_UnpackVectorARDTask::taskFinished, this, &CSE_UnpackVectorARDDlg::onTaskFinished);
...
void CSE_UnpackVectorARDDlg::Button_Cancel_rejected()
{
	reject();
}

void CSE_UnpackVectorARDDlg::on_lineEdit_InputDataPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void CSE_UnpackVectorARDDlg::on_lineEdit_OutputPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}
...
void CSE_UnpackVectorARDDlg::restoreState()
{
	const QgsSettings settings;
	m_qstrInputDataPath = settings.value(QStringLiteral("UNPACK_VECTOR_ARD/InputDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputDataPath = settings.value(QStringLiteral("UNPACK_VECTOR_ARD/OutputDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputLogPath = settings.value(QStringLiteral("UNPACK_VECTOR_ARD/OutputLogPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();

}

// 判断目录是否存在
bool CSE_UnpackVectorARDDlg::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool CSE_UnpackVectorARDDlg::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}


QString CSE_UnpackVectorARDDlg::GetInputPtpDataPath()
{
	return ui.lineEdit_InputDataPath->text();
}

QString CSE_UnpackVectorARDDlg::GetOutputDataPath()
{
	return ui.lineEdit_OutputPath->text();
}

// 获取指定目录的最后一级目录
void CSE_UnpackVectorARDDlg::GetFolderNameFromPath(QString qstrPath, QString& qstrFolderName)
{
...
QStringList CSE_UnpackVectorARDDlg::GetGpkgFileNames(const QString& path)
{
	QDir dir(path);
...
void CSE_UnpackVectorARDDlg::GetGpkgInfo(string strPtpFileName,
	int& minz,
	int& maxz,
	int& basez,
	int& x,
...
bool CSE_UnpackVectorARDDlg::isDirExist(QString fullPath)
{
	QDir dir(fullPath);
	if (dir.exists())
	{
...
bool CSE_UnpackVectorARDDlg::ClearEmptyFolder(const QString& qstrDirPath)
{
	bool bRet = true;
...
void CSE_UnpackVectorARDDlg::onTaskFinished(bool result)
{
	// 计算并打印总进度
	CalculateTotalProgress();
}

void CSE_UnpackVectorARDDlg::pushButton_SaveLog_clicked()
{
	// 选择文件夹
...
void CSE_UnpackVectorARDDlg::CalculateTotalProgress()
{
...
bool CSE_UnpackVectorARDDlg::CheckFileOrDirExist(const QString& qstrFileDirOrPath)
{
