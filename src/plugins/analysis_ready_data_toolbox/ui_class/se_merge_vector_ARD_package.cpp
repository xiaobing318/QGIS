...
/*--------------SE---------------*/
#include "se_merge_vector_ARD_package.h"
/*-------------------------------*/
...
#include "../ui_task/se_merge_vector_gpkg_task.h"


CSE_MergeVectorARDPackageDialog::CSE_MergeVectorARDPackageDialog(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	connect(ui.Button_OK, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::Button_OK_accepted);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::Button_Cancel_rejected);
	connect(ui.Button_Save, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::Button_Save_clicked);
	connect(ui.pushButton_AddARDPackPath, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::pushButton_AddARDPackPath);
	connect(ui.pushButton_ClearARDPackPath, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::pushButton_ClearARDPackPath);
	connect(ui.pushButton_Up, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::pushButton_Up);
	connect(ui.pushButton_Down, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::pushButton_Down);
	connect(ui.Button_SaveLogFilePath, &QPushButton::clicked, this, &CSE_MergeVectorARDPackageDialog::pushButton_SaveLog_clicked);
...
CSE_MergeVectorARDPackageDialog::~CSE_MergeVectorARDPackageDialog()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("MERGE_VECTOR_ARD_PACKAGE/OutputPtpPath"), m_qstrOutputPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("MERGE_VECTOR_ARD_PACKAGE/OutputLogPath"), m_qstrOutputLogPath, QgsSettings::Section::Plugins);

}

void CSE_MergeVectorARDPackageDialog::pushButton_AddARDPackPath()
{
	// 选择文件夹
	QString curPath = QCoreApplication::applicationDirPath();
	QString dlgTile = tr("请选择矢量分析就绪型数据包所在路径");
	...
	if (!selectedDir.isEmpty())
	{
		...
		for (int iRow = 0; iRow < ui.tableWidget_ARDPath->rowCount(); ++iRow)
		{
			...
			{
				QString qstrTitle = tr("矢量分析就绪型数据包合包");
...
void CSE_MergeVectorARDPackageDialog::pushButton_ClearARDPackPath()
{
	// 删除所有行
...
void CSE_MergeVectorARDPackageDialog::pushButton_Up()
{
	// 获取选中的行
...
void CSE_MergeVectorARDPackageDialog::pushButton_Down()
{
	// 获取选中的行
	QList<QTableWidgetItem*> selectedItems = ui.tableWidget_ARDPath->selectedItems();
	if (selectedItems.isEmpty()) 
...
void CSE_MergeVectorARDPackageDialog::Button_Save_clicked()
{
	// 选择文件夹
...
/*获取在指定目录下的目录的路径*/
QStringList CSE_MergeVectorARDPackageDialog::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
void CSE_MergeVectorARDPackageDialog::Button_OK_accepted()
{
	// 日志级别
	// 0——错误；1——信息；2——调试
	...
	{
		QString qstrTitle = tr("矢量分析就绪型数据包合包");
		QString qstrText = tr("矢量分析就绪数据包合包至少需要两个数据包路径，请添加数据包路径！");
		QMessageBox msgBox;
		msgBox.setWindowTitle(qstrTitle);
		msgBox.setText(qstrText);
		msgBox.setStandardButtons(QMessageBox::Yes);
	...
	// 创建任务
	SE_MergeVectorGpkgTask* task = new SE_MergeVectorGpkgTask(
		tr("矢量分析就绪型数据包合包"),
		vInputGpkgPath,
		m_MergeStrategy,
		strDataTime,
	...
	connect(task, &SE_MergeVectorGpkgTask::taskFinished, this, &CSE_MergeVectorARDPackageDialog::onTaskFinished);
...
void CSE_MergeVectorARDPackageDialog::Button_Cancel_rejected()
{
	reject();
}


void CSE_MergeVectorARDPackageDialog::restoreState()
{
	const QgsSettings settings;
	m_qstrOutputPath = settings.value(QStringLiteral("MERGE_VECTOR_ARD_PACKAGE/OutputPtpPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrOutputLogPath = settings.value(QStringLiteral("MERGE_VECTOR_ARD_PACKAGE/OutputLogPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();

}

// 判断目录是否存在
bool CSE_MergeVectorARDPackageDialog::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool CSE_MergeVectorARDPackageDialog::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}


QString CSE_MergeVectorARDPackageDialog::GetOutputPath()
{
	return ui.lineEdit_OutputPath->text();
}

// 获取指定目录的最后一级目录
void CSE_MergeVectorARDPackageDialog::GetFolderNameFromPath(QString qstrPath, QString& qstrFolderName)
{
...
/*获取指定目录的最后一级目录*/
void CSE_MergeVectorARDPackageDialog::GetFolderNameFromPath_string(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
...
void CSE_MergeVectorARDPackageDialog::GetPackageInfo(string strPtpFileName,
	int &minz,
	int &maxz,
	int &basez,
	int &x,
...
bool CSE_MergeVectorARDPackageDialog::IsExistedInVector(string strValue, vector<string>& vValue)
{
...
bool CSE_MergeVectorARDPackageDialog::isDirExist(QString fullPath)
{
	QDir dir(fullPath);
	if (dir.exists())
	{
...
void CSE_MergeVectorARDPackageDialog::onTaskFinished(bool result)
{
	// 计算并打印总进度
	CalculateTotalProgress();
}

void CSE_MergeVectorARDPackageDialog::pushButton_SaveLog_clicked()
{
	// 选择文件夹
...
void CSE_MergeVectorARDPackageDialog::CalculateTotalProgress()
{
	int totalProgress = 0;
...
bool CSE_MergeVectorARDPackageDialog::CheckFileOrDirExist(const QString& qstrFileDirOrPath)
{
