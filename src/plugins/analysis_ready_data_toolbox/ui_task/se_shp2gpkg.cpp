...
/*构造函数*/
SE_Shp2GpkgTask::SE_Shp2GpkgTask(const QString& name,
...
#pragma endregion
    ...
    // 获取当前目录下所有的shp文件
    QStringList qstrShpFileList = GetFileNames(QString::fromLocal8Bit(m_strInputPath.c_str()));

    memset(szLog, 0, 1000);
    sprintf(szLog, "当前目录:%s下共有%d个shp文件", m_strInputPath.c_str(), qstrShpFileList.length());
    file_logger->info(szLog);
    file_logger->debug(szLog);
    file_logger->flush();
    ...
    for (int i = 0; i < qstrShpFileList.length(); ++i)
    {
        string strShpFileName = qstrShpFileList[i].toLocal8Bit();
        string strShpFullPath = m_strInputPath + "/" + strShpFileName;
       
		ImportVectorLayerToGeoPackage(strShpFullPath, poGpkgDS);
		setProgress((i + 1) * 100.0 / qstrShpFileList.length());

        memset(szLog, 0, 1000);
        sprintf(szLog, "shp文件:%s入gpkg数据库完成！", strShpFullPath.c_str());
        file_logger->info(szLog);
        file_logger->debug(szLog);
        file_logger->flush();
    }
...
bool SE_Shp2GpkgTask::isCanceled()
{
    // 返回当前取消状态
    return mCanceled;           
}

void SE_Shp2GpkgTask::cancel()
{
    // 设置取消状态为 true
    mCanceled = true;           
}

int SE_Shp2GpkgTask::progress() const
{
    // 返回当前进度
    return mProgress;           
}

void SE_Shp2GpkgTask::finished(bool result)
{
    emit taskFinished(result);
}

QStringList SE_Shp2GpkgTask::GetFileNames(const QString& path)
{
    QDir dir(path);
...
bool SE_Shp2GpkgTask::ImportVectorLayerToGeoPackage(const string& strFilePath, GDALDataset* pGpkgDB)
{
	// 注册所有驱动
	GDALAllRegister();
