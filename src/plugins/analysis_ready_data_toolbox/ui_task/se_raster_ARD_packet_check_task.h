#ifndef SE_RASTER_ARD_PACKET_CHECK_TASK_H
#define SE_RASTER_ARD_PACKET_CHECK_TASK_H
...
/*栅格分析就绪数据包检查任务类*/
class SE_RasterARDPacketCheckTask : public QgsTask
{
    Q_OBJECT

public:

    SE_RasterARDPacketCheckTask(const QString& name,
...
private:
    ...
    RasterARDPacketCheckItems     m_sARDPacketCheckItems;            // 分析就绪数据包检查项
...
private:
    ...
    // 获取数据库中指定表名的ZXY记录
    bool GetRasterInfoInGpkg(sqlite3* db, string strTableName, vector<ZXY_INDEX>& vZXY_Index);

    // 获取数据库中的表名
    bool getTableNames(sqlite3* db, vector<string>& vTableNames);
...
#endif // SE_RASTER_ARD_PACKET_CHECK_TASK_H
