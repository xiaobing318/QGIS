...
#include "se_customize_vector_task.h"
...
/*构造函数*/
SE_CustomizeVectorTask::SE_CustomizeVectorTask(const QString& name,
...
bool SE_CustomizeVectorTask::run()
{
#pragma region "创建日志器"
...
#pragma endregion
    ...
    // 日志器名称
    string strLogFileName = "CustomizeVector_" + m_strARDIdentify + "_" + strLayerName;

    // 日志文件全路径
    string strLogFileFullPath = m_strOutputLogPath + "/System_Running_" + strLogLevel + "_" + strLogFileName + ".txt";

    // 日志器名称
    string strLoggerName = "CustomizeVector_" + m_strARDIdentify + "_" + strLayerName;
...
#pragma endregion
    ...
    {
        err = CSE_ZchjDataProcess::GenerateVectorAnalysisReadyDataFormatShp(
            vParam,
    ...
    {
        err = CSE_ZchjDataProcess::GenerateVectorAnalysisReadyDataFormatShp2(
            vParam,
            m_strOutputPath.c_str());
    }
...
bool SE_CustomizeVectorTask::isCanceled()
{
    // 返回当前取消状态
    return mCanceled;           
}

void SE_CustomizeVectorTask::cancel()
{
    // 设置取消状态为 true
    mCanceled = true;           
}

int SE_CustomizeVectorTask::progress() const
{
    // 返回当前进度
    return mProgress;           
}

void SE_CustomizeVectorTask::finished(bool result)
{
    emit taskFinished(result);
}

// 获取当前路径下所有shp文件路径
void SE_CustomizeVectorTask::GetShpFilesFromFilePath(
...
bool SE_CustomizeVectorTask::DeleteFilesWithSameNameDifferentExtension(const fs::path& currentFilePath) 
{
    if (!fs::exists(currentFilePath) || !fs::is_regular_file(currentFilePath)) 
    {
        //std::cerr << "The specified file does not exist or is not a regular file." << std::endl;
        return false;
    }

    fs::path currentDir = currentFilePath.parent_path(); // 获取当前文件所在目录
    std::string currentFileName = currentFilePath.stem().string(); // 获取文件名（不带扩展名）
    ...
    for (const auto& entry : fs::directory_iterator(currentDir)) 
    {
        if (entry.is_regular_file()) 
        {
            ...
            std::string fileName = filePath.stem().string(); // 文件名（不带扩展名）
            std::string fileExtension = filePath.extension().string(); // 文件扩展名

            // 检查文件名是否相同但扩展名不同
            if (fileName == currentFileName && filePath != currentFilePath) {
                //std::cout << "Deleting file: " << filePath << std::endl; // 打印删除信息
