...
#include "se_customize_terrain_factor_task.h"
...
/*构造函数*/
SE_CustomizeTerrainFactorTask::SE_CustomizeTerrainFactorTask(const QString& name,
    const std::string& strTifFilePath,
...
bool SE_CustomizeTerrainFactorTask::run()
{
#pragma region "创建日志器"
...
#pragma endregion
    ...
    // 日志器名称
    string strLogFileName = "TerrainFactor_" + m_strARDIdentify;

    // 日志文件全路径
    string strLogFileFullPath = m_strOutputLogPath + "/System_Running_" + strLogLevel + "_" + strLogFileName + ".txt";

    // 日志器名称
    string strLoggerName = "TerrainFactor_" + m_strARDIdentify;
...
#pragma endregion
    ...
    SE_Error err = CSE_ZchjDataProcess::GenerateRasterAnalysisReadyData(
            m_strTifFilePath.c_str(),
            m_Param,
...
bool SE_CustomizeTerrainFactorTask::isCanceled()
{
    // 返回当前取消状态
    return mCanceled;           
}

void SE_CustomizeTerrainFactorTask::cancel()
{
    // 设置取消状态为 true
    mCanceled = true;           
}

int SE_CustomizeTerrainFactorTask::progress() const
{
    // 返回当前进度
    return mProgress;           
}

void SE_CustomizeTerrainFactorTask::finished(bool result)
{
