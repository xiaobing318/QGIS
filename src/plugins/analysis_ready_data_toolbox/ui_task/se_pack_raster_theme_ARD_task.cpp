...
#include "se_pack_raster_theme_ARD_task.h"
#include <qdir.h>
#include <filesystem>
...
/*构造函数*/
SE_PackRasterThemeARDTask::SE_PackRasterThemeARDTask(const QString& name,
...
bool SE_PackRasterThemeARDTask::run()
{
    /*算法思路：
...
#pragma endregion
    ...
    // 输出一条启动信息
    char szLog[1000] = { 0 };
    sprintf(szLog, "正在执行专题栅格分析就绪型数据封装任务...");
...
#pragma endregion
    ...
    memset(szLog, 0, 1000);
    sprintf(szLog, "筛选出模型所需分析就绪数据的目录完成！");
    file_logger->info(szLog);
    file_logger->debug(szLog);
    file_logger->flush();
...
#pragma region "将 TIFF 数据写入 SQLite 数据库"
...
#pragma endregion
    ...
    memset(szLog, 0, 1000);
    sprintf(szLog, "专题栅格分析就绪型数据封装任务完成！");
    file_logger->info(szLog);
    file_logger->debug(szLog);
    file_logger->flush();
    // 卸载日志记录器
...
bool SE_PackRasterThemeARDTask::isCanceled()
{
    // 返回当前取消状态
    return mCanceled;           
}

void SE_PackRasterThemeARDTask::cancel()
{
    // 设置取消状态为 true
    mCanceled = true;           
}

int SE_PackRasterThemeARDTask::progress() const
{
    // 返回当前进度
    return mProgress;           
}

void SE_PackRasterThemeARDTask::finished(bool result)
{
    emit taskFinished(result);
}

QStringList SE_PackRasterThemeARDTask::GetFileNames(const QString& path)
{
    QDir dir(path);
...
/*获取当前目录下所有子目录*/
void SE_PackRasterThemeARDTask::GetSubDirFromFilePath(const string& strFilePath,
    vector<string>& vSubDir)
{
    // 获取当前工作目录
...
// 获取当前路径下所有tif文件路径
void SE_PackRasterThemeARDTask::GetTifFilesFromFilePath(
...
// 根据文件名提取Z、X、Y
bool SE_PackRasterThemeARDTask::extractZXY(
...
double SE_PackRasterThemeARDTask::GetGridResByGridLevel(int iLevel)
{
    switch (iLevel)
    {
    case 0:
...
SE_DRect SE_PackRasterThemeARDTask::CalGridExtentByZXY(int iLevel, int iX, int iY)
{
    // 根据格网获取对应的分辨率
...
void SE_PackRasterThemeARDTask::CalXRangeAndYRangeByGeoRectAndLevel(
    SE_DRect dRect,
    int iGridLevel,
    int& iMinX,
    int& iMaxX,
...
/*根据Z计算分包级别*/
void SE_PackRasterThemeARDTask::GetPackLevelByZ(vector<ARD_PACKAGE_INFO>& vInfo, int iZ, int& iPackLevel)
{
...
/*根据经纬度、打包级别计算行列索引*/
void SE_PackRasterThemeARDTask::CalXAndYByPointAndLevel(
    SE_DPoint dPoint,
    int iPackageLevel,
    int& iX,
    int& iY)
...
void SE_PackRasterThemeARDTask::GetIndexFromGpkgDB(vector<ZXY_INDEX>& vIndex, int iZ, int iX, int iY, int& iIndex)
{
...
void SE_PackRasterThemeARDTask::GetIndexFromSQLiteDB(vector<ZXY_INDEX>& vIndex, int iZ, int iX, int iY, int& iIndex)
{
    for (int i = 0; i < vIndex.size(); ++i)
    {
        if (vIndex[i].iZ == iZ
...
void SE_PackRasterThemeARDTask::createTable(sqlite3* db, const char* szTableName)
{
    char szSQL[500] = { 0 };
...
void SE_PackRasterThemeARDTask::insertData(
    sqlite3* db,
...
bool SE_PackRasterThemeARDTask::IsExistedInVector(string strValue, vector<string>& vValue)
{
