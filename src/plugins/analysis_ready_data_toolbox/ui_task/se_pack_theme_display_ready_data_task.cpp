...
#include "se_pack_theme_display_ready_data_task.h"
#include <qdir.h>
...
/*构造函数*/
SE_PackThemeDisplayReadyDataTask::SE_PackThemeDisplayReadyDataTask(
    const QString& name,
    const string& strInputPath,
    const vector<string>& vIdentifys,
    const vector<string>& vUserDataTypes,
    ...
    m_vUserDataTypes(vUserDataTypes),
...
bool SE_PackThemeDisplayReadyDataTask::run()
{
    /*算法思路：
...
#pragma endregion
    ...
    strLogFileName += "_PackThemeDisplayReadyData_";
    ...
    strLoggerName += "_PackThemeDisplayReadyData_";
    ...
    // 输出一条启动信息
    char szLog[1000] = { 0 };
    sprintf(szLog, "正在执行专题显示就绪型数据封装任务...");
...
#pragma region "遍历当前输入目录下所有的文件，判断是否符合封装要求"
        ...
        && dtop <= m_dRect.dtop)
    {
        ...
        // 判断数据类型是否在任务-模型-数据的类型列表、用户对象对应的数据类型列表、数据类型复选框列表中
        if (!IsExistedInVector(strGLXX_CPMC, m_vIdentifys)
            && !IsExistedInVector(strGLXX_CPMC, m_vDataTypes)
            && !IsExistedInVector(strGLXX_CPMC, m_vUserDataTypes))
        {
            memset(szLog, 0, 1000);
...
bool SE_PackThemeDisplayReadyDataTask::isCanceled()
{
    // 返回当前取消状态
    return mCanceled;           
}

void SE_PackThemeDisplayReadyDataTask::cancel()
{
    // 设置取消状态为 true
    mCanceled = true;           
}

int SE_PackThemeDisplayReadyDataTask::progress() const
{
    // 返回当前进度
    return mProgress;           
}

void SE_PackThemeDisplayReadyDataTask::finished(bool result)
{
    emit taskFinished(result);
}

QStringList SE_PackThemeDisplayReadyDataTask::GetFileNames(const QString& path)
{
    QDir dir(path);
...
bool SE_PackThemeDisplayReadyDataTask::IsExistedInVector(string strValue, vector<string>& vValue)
{
...
void SE_PackThemeDisplayReadyDataTask::GetJsonFilesFromFilePath(
...
bool SE_PackThemeDisplayReadyDataTask::ReadMetaDataJsonFile(const string& strFile,
...
// 根据元数据标识获取属性值
string SE_PackThemeDisplayReadyDataTask::GetValueByKey(const string& strKey,
    vector<MetaDataTemplateInfo>& vMetadatInfo)
{
...
string SE_PackThemeDisplayReadyDataTask::convertEncoding(const string& input,
...
// 封装文件拷贝函数
bool SE_PackThemeDisplayReadyDataTask::copyFile(const fs::path& sourceFilePath, 
...
// 递归拷贝目录函数（矢量数据类型使用）
bool SE_PackThemeDisplayReadyDataTask::copyDirectory_4_ShiLiang(
...
bool SE_PackThemeDisplayReadyDataTask::copyDirectory_4_OtherDataType(
    const fs::path& sourceDir,
    const fs::path& destinationDir,
    std::shared_ptr<spdlog::logger> file_logger)
{
...
std::string SE_PackThemeDisplayReadyDataTask::generateUUID() {
