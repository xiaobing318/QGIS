#include "se_feature_attribute_check_thread.h"
...
SE_FeatureAttributeThread::SE_FeatureAttributeThread(QObject *parent)
    : QThread(parent)
{
    restart = false;
    abort = false;
...
SE_FeatureAttributeThread::~SE_FeatureAttributeThread()
{
    mutex.lock();
    abort = true;
    condition.wakeOne();
...
void SE_FeatureAttributeThread::SetThreadParams(QString qstrInputShpDataPath, QString qstrInputXmlPath, QString qstrOutputLogPath)
{
...
void SE_FeatureAttributeThread::run()
{
    mutex.lock();
	...
	QString qstrResult;
	// ----------------------开始-------------------------//
	string strOutputLogPath = qstrOutputLogPath.toLocal8Bit();
	...
	if (!fpLog)
	{
		...
		msgbox.exec();*/
		dPercent = 0;
		qstrResult = QString::fromLocal8Bit("创建日志文件失败！");
	...
	if (qstrShpSubDir.size() == 0)
	{
		...
		else
		{
			...
			dPercent =  1.0;
			qstrResult = QString::fromLocal8Bit("线划图属性检查完成！");
			emit resultProcess(dPercent, qstrResult);
		}
...
// 判断目录是否存在
bool SE_FeatureAttributeThread::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool SE_FeatureAttributeThread::FileIsExisted(QString qstrFilePath)
{
...
/*获取在指定目录下的目录的路径*/
QStringList SE_FeatureAttributeThread::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
/*获取指定目录的最后一级目录*/
void SE_FeatureAttributeThread::GetFolderNameFromPath_string(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
...
SE_Error SE_FeatureAttributeThread::FeatureAttributeCheck(
...
// 读取属性检查配置信息
bool SE_FeatureAttributeThread::LoadFeatureAttrCheckXmlConfigFile(const char* szAttrCheckConfigXmlFile, vector<VectorLayerFieldInfo>& vLayerConfigFieldInfo)
{
	// 如果xml文件为空
...
/*UTF-8转GBK*/
string SE_FeatureAttributeThread::UTF8_To_GBK(const string& str)
{
...
void SE_FeatureAttributeThread::GetLayerInfo(string strLayerName,
	string& strSheetNumber,
	string& strLayerType,
	string& strGeometryType)
{
...
// 解析每个layer_fields节点
void SE_FeatureAttributeThread::Parse_layer_fields(
	xmlDocPtr doc,
	xmlNodePtr cur,
	VectorLayerFieldInfo& info)
{
...
// 解析每个fields节点
void SE_FeatureAttributeThread::Parse_fields(
	xmlDocPtr doc,
	xmlNodePtr cur,
	FieldInfo& info)
{
...
void SE_FeatureAttributeThread::Parse_field_enum_values_list_simple(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
void SE_FeatureAttributeThread::Parse_field_enum_values_list_complex(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
void SE_FeatureAttributeThread::Parse_field_enum_values(
	xmlDocPtr doc,
	xmlNodePtr cur,
	FieldEnumValue& enumValue)
{
...
void SE_FeatureAttributeThread::GetAttributeCheckInfoByLayerType(
	string strLayerType,
...
void SE_FeatureAttributeThread::LayerFieldInfoCheck(
	vector<LayerFieldInfo>& vLayerFieldInfo,
	VectorLayerFieldInfo& sLayerFieldCheckInfo,
...
// 获取要素属性信息
SE_Error SE_FeatureAttributeThread::GetFeatureAttr(
...
void SE_FeatureAttributeThread::LayerFieldAttrCheck(
...
bool SE_FeatureAttributeThread::FieldValueIsExistedInSimpleEnumList(
