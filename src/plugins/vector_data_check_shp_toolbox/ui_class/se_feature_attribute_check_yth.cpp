...
#include "se_feature_attribute_check_yth.h"
...
CSE_FeatureAttributeCheckYTHDialog::CSE_FeatureAttributeCheckYTHDialog(QgisInterface* qgisInterface, QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl), 
	m_qgisInterface(qgisInterface)
{
	...
	connect(ui.Button_SaveLog, &QPushButton::clicked, this, &CSE_FeatureAttributeCheckYTHDialog::Button_Save_clicked);
	connect(ui.Button_OpenXml, &QPushButton::clicked, this, &CSE_FeatureAttributeCheckYTHDialog::Button_OpenXml_clicked);
	connect(ui.Button_OpenShp, &QPushButton::clicked, this, &CSE_FeatureAttributeCheckYTHDialog::Button_OpenShp_clicked);
	connect(ui.Button_OK, &QPushButton::clicked, this, &CSE_FeatureAttributeCheckYTHDialog::Button_OK_accepted);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &CSE_FeatureAttributeCheckYTHDialog::Button_Cancel_rejected);

	connect(ui.lineEdit_InputShpDataPath, &QLineEdit::textChanged, this, &CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_InputShpDataPath_TextChanged);
	connect(ui.lineEdit_InputXmlPath, &QLineEdit::textChanged, this, &CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_InputXmlPath_TextChanged);
	connect(ui.lineEdit_OutputLogPath, &QLineEdit::textChanged, this, &CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_OutputLogPath_TextChanged);
	restoreState();
	// 初始化输入路径和输出路径
	...
	connect(&m_Thread, &SE_FeatureAttributeThreadYTH::resultProcess, this, &CSE_FeatureAttributeCheckYTHDialog::handleResults);
}

CSE_FeatureAttributeCheckYTHDialog::~CSE_FeatureAttributeCheckYTHDialog()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("FeatureAttributeCheckYTH/InputXmlPath"), m_qstrInputXmlPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("FeatureAttributeCheckYTH/InputShpDataPath"), m_qstrInputShpDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("FeatureAttributeCheckYTH/SaveLogPath"), m_qstrSaveLogPath, QgsSettings::Section::Plugins);
}

void CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_InputShpDataPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_InputXmlPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void CSE_FeatureAttributeCheckYTHDialog::on_lineEdit_OutputLogPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void CSE_FeatureAttributeCheckYTHDialog::handleResults(const double& dPercent, const QString& s)
{
	...
	if (dPercent == 1)
	{
		...
		QString qstrTitle = QString::fromLocal8Bit("一体化数据属性检查");
		QString qstrText = s;
		QMessageBox msgBox;
...
void CSE_FeatureAttributeCheckYTHDialog::Button_Save_clicked()
{
	// 选择文件夹
...
void CSE_FeatureAttributeCheckYTHDialog::Button_OpenXml_clicked()
{
	// 选择xml属性配置文件
	QString curPath = QCoreApplication::applicationDirPath();
...
void CSE_FeatureAttributeCheckYTHDialog::Button_OpenShp_clicked()
{
	// 选择文件夹
	QString curPath = QCoreApplication::applicationDirPath();
...
/*获取在指定目录下的目录的路径*/
QStringList CSE_FeatureAttributeCheckYTHDialog::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
void CSE_FeatureAttributeCheckYTHDialog::Button_OK_accepted()
{
	ui.progressBar->reset();
...
void CSE_FeatureAttributeCheckYTHDialog::Button_Cancel_rejected()
{
	reject();
}

void CSE_FeatureAttributeCheckYTHDialog::restoreState()
{
	const QgsSettings settings;
	m_qstrInputXmlPath = settings.value(QStringLiteral("FeatureAttributeCheckYTH/InputXmlPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrInputShpDataPath = settings.value(QStringLiteral("FeatureAttributeCheckYTH/InputShpDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrSaveLogPath = settings.value(QStringLiteral("FeatureAttributeCheckYTH/SaveLogPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
}

// 判断目录是否存在
bool CSE_FeatureAttributeCheckYTHDialog::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool CSE_FeatureAttributeCheckYTHDialog::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}


QString CSE_FeatureAttributeCheckYTHDialog::GetInputXmlPath()
{
	return ui.lineEdit_InputXmlPath->text();
}

QString CSE_FeatureAttributeCheckYTHDialog::GetInputShpDataPath()
{
	return ui.lineEdit_InputShpDataPath->text();
}

QString CSE_FeatureAttributeCheckYTHDialog::GetOutputLogPath()
{
	return ui.lineEdit_OutputLogPath->text();
}

// 获取指定目录的最后一级目录
void CSE_FeatureAttributeCheckYTHDialog::GetFolderNameFromPath(QString qstrPath, QString& qstrFolderName)
{
...
/*获取指定目录的最后一级目录*/
void CSE_FeatureAttributeCheckYTHDialog::GetFolderNameFromPath_string(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
...
int CSE_FeatureAttributeCheckYTHDialog::GBK_2_UTF8(char* utfstr, const char* srcstr, int maxutfstrlen)
{
	if (NULL == srcstr)
	{
...
SE_Error CSE_FeatureAttributeCheckYTHDialog::FeatureAttributeCheck(
...
// 读取属性检查配置信息
bool CSE_FeatureAttributeCheckYTHDialog::LoadFeatureAttrCheckXmlConfigFile(const char* szAttrCheckConfigXmlFile, vector<VectorLayerFieldInfo>& vLayerConfigFieldInfo)
{
	// 如果xml文件为空
...
/*UTF-8转GBK*/
string CSE_FeatureAttributeCheckYTHDialog::UTF8_To_GBK(const string& str)
{
...
void CSE_FeatureAttributeCheckYTHDialog::GetLayerInfo(string strLayerName,
	string& strSheetNumber,
	string& strLayerType,
	string& strGeometryType)
{
...
// 解析每个layer_fields节点
void CSE_FeatureAttributeCheckYTHDialog::Parse_layer_fields(
	xmlDocPtr doc,
	xmlNodePtr cur,
	VectorLayerFieldInfo& info)
{
...
// 解析每个fields节点
void CSE_FeatureAttributeCheckYTHDialog::Parse_fields(
	xmlDocPtr doc,
	xmlNodePtr cur,
	FieldInfo& info)
{
...
void CSE_FeatureAttributeCheckYTHDialog::Parse_field_enum_values_list_simple(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
void CSE_FeatureAttributeCheckYTHDialog::Parse_field_enum_values_list_complex(
	xmlDocPtr doc,
	xmlNodePtr cur,
...
void CSE_FeatureAttributeCheckYTHDialog::Parse_field_enum_values(
	xmlDocPtr doc,
	xmlNodePtr cur,
	FieldEnumValue& enumValue)
{
...
void CSE_FeatureAttributeCheckYTHDialog::GetAttributeCheckInfoByLayerType(
	string strLayerType,
...
void CSE_FeatureAttributeCheckYTHDialog::LayerFieldInfoCheck(
	vector<LayerFieldInfo>& vLayerFieldInfo,
	VectorLayerFieldInfo& sLayerFieldCheckInfo,
...
// 获取要素属性信息
SE_Error CSE_FeatureAttributeCheckYTHDialog::GetFeatureAttr(
...
void CSE_FeatureAttributeCheckYTHDialog::LayerFieldAttrCheck(
...
bool CSE_FeatureAttributeCheckYTHDialog::FieldValueIsExistedInSimpleEnumList(
