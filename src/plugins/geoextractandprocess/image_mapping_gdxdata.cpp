...
const double MOSAIC_MAP_LEFT_X = 61;		// 镶嵌线地图窗口左边界布局坐标，单位：毫米
...
QgsImageMappingGdxDataDialog::QgsImageMappingGdxDataDialog(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	// 初始化所有要素类列表为不选中状态
	m_vStrLayerName.push_back("RAILN");		// 铁路
	m_vStrLayerName.push_back("ROALN");		// 公路
	m_vStrLayerName.push_back("BOULN");		// 境界
	m_vStrLayerName.push_back("AA0PL");		// 国家政区
	m_vStrLayerName.push_back("AA1PL");		// 一级政区
	m_vStrLayerName.push_back("AA2PL");		// 二级政区
	m_vStrLayerName.push_back("AA3PL");		// 三级政区
	...
	connect(ui.pushButton_ImageMapping, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::ImageMapping_GdxData);
	
	connect(ui.pushButton_Close, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::Close);

	connect(ui.pushButton_OpenVectorData, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::OpenVectorData);
	connect(ui.pushButton_OpenImageData, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::OpenImageData);
	connect(ui.pushButton_OpenMosaicData, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::OpenMosaicData);
	connect(ui.pushButton_Save, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::ImageMappingDataSave);
	...
	connect(ui.radioButton_LonLatRange, &QRadioButton::clicked, this, &QgsImageMappingGdxDataDialog::LonLatRadioButton_Click);
	connect(ui.radioButton_SheetList, &QRadioButton::clicked, this, &QgsImageMappingGdxDataDialog::SheetListRadioButton_Click);

	connect(ui.pushButton_LoadSheetList, &QPushButton::clicked, this, &QgsImageMappingGdxDataDialog::LoadSheetListFile);
...
QgsImageMappingGdxDataDialog::~QgsImageMappingGdxDataDialog()
{
	m_vImageMappingSchema.clear();
	m_mCode_Names.clear();
	m_mName_Codes.clear();
...
void QgsImageMappingGdxDataDialog::Close()
{
	accept();
}


void QgsImageMappingGdxDataDialog::OpenVectorData()
{
	// 选择文件夹
...
void QgsImageMappingGdxDataDialog::OpenImageData()
{
	// 选择文件夹
...
void QgsImageMappingGdxDataDialog::OpenMosaicData()
{
	// 选择文件夹
...
void QgsImageMappingGdxDataDialog::ImageMappingDataSave()
{
	// 选择文件夹
...
void QgsImageMappingGdxDataDialog::UpdateUIByImageMappingSchema(const QModelIndex& index)
{
	//点击获取当前索引index的内容
...
void QgsImageMappingGdxDataDialog::LonLatRadioButton_Click()
{
	ui.lineEdit_Left->setEnabled(true);
	ui.lineEdit_Top->setEnabled(true);
	ui.lineEdit_Right->setEnabled(true);
...
void QgsImageMappingGdxDataDialog::SheetListRadioButton_Click()
{
...
void QgsImageMappingGdxDataDialog::SaveImageMappingSchema()
{
}

QgsLayoutItem* QgsImageMappingGdxDataDialog::GetLayoutItemByName(QList<QgsLayoutItem*>& itemList, const QString qstrName)
{
...
QString QgsImageMappingGdxDataDialog::GetMainTitleSheet()
{
...
QString QgsImageMappingGdxDataDialog::GetMainTitleName()
{
...
QString QgsImageMappingGdxDataDialog::GetSubTitleProvinceName()
{
...
QString QgsImageMappingGdxDataDialog::GetSubTitleCountyName()
{
...
QString QgsImageMappingGdxDataDialog::GetSecurityClassification()
{
...
void QgsImageMappingGdxDataDialog::SetListWidgetAllUnSelectedStatus(const QString& strLayerName)
{
	// 存储list控件
	...
	// 铁路
	if (strLayerName == "RAILN")
	{
		vQListWidget.push_back(ui.listWidget_TieLu);
	}

	// 公路
	if (strLayerName == "ROALN")
	{
		vQListWidget.push_back(ui.listWidget_GongLu);
	}

	// 境界
	if (strLayerName == "BOULN")
	{
		vQListWidget.push_back(ui.listWidget_JingJie);
	}

	// 国家政区
	if (strLayerName == "AA0PL")
	{
		vQListWidget.push_back(ui.listWidget_GuoJiaZhengQu);
	}

	// 一级政区
	if (strLayerName == "AA1PL")
	{
		vQListWidget.push_back(ui.listWidget_YiJiZhengQu);
	}

	// 二级政区
	if (strLayerName == "AA2PL")
	{
		vQListWidget.push_back(ui.listWidget_ErJiZhengQu);
	}

	// 三级政区
	if (strLayerName == "AA3PL")
	{
		vQListWidget.push_back(ui.listWidget_SaniJiZhengQu);
	}
...
// 初始化要素类名称和编码的映射表
void QgsImageMappingGdxDataDialog::InitFeatureClassNameCodeTable()
{
	// 获取映射表配置参数
	...
	qstrPath += "\\config\\featureclass_name_code_gdx.cfg";
...
// 获取图层字段选择状态
void QgsImageMappingGdxDataDialog::GetLayerFeatureClassChecked(string strLayerType, vector<string>& vFeatureClassCode)
{
	// RAILN、ROALN、BOULN、AA0PL、AA1PL、AA2PL、AA3PL
	...
	// 铁路
	if (strLayerType == "RAILN")
	{
		vQListWidget.push_back(ui.listWidget_TieLu);
	}

	// 公路
	if (strLayerType == "ROALN")
	{
		vQListWidget.push_back(ui.listWidget_GongLu);
	}

	// 境界
	if (strLayerType == "BOULN")
	{
		vQListWidget.push_back(ui.listWidget_JingJie);
	}

	// 国家政区
	if (strLayerType == "AA0PL")
	{
		vQListWidget.push_back(ui.listWidget_GuoJiaZhengQu);
	}

	// 一级政区
	if (strLayerType == "AA1PL")
	{
		vQListWidget.push_back(ui.listWidget_YiJiZhengQu);
	}

	// 二级政区
	if (strLayerType == "AA2PL")
	{
		vQListWidget.push_back(ui.listWidget_ErJiZhengQu);
	}

	// 三级政区
	if (strLayerType == "AA3PL")
	{
		vQListWidget.push_back(ui.listWidget_SaniJiZhengQu);
	}
...
void QgsImageMappingGdxDataDialog::InitImageMappingSchema()
{
	// 获取运行环境路径
	QString qstrExePath = QCoreApplication::applicationDirPath();

	// 国地信数据模板路径
	QString qstrSchemaPath = qstrExePath + "/layout/schema_gdx";
	...
	for (int i = 0; i < xmlFileNameList.size(); i++)
	{
		...
		vector<ImageMappingSchema_Gdx> vSchemaList;
		vSchemaList.clear();
	...
	for (int i = 0; i < m_vImageMappingSchema.size(); i++)
	{
		ImageMappingSchema_Gdx param = m_vImageMappingSchema[i];
		// 设置对话框中各部件的内容
...
QStringList QgsImageMappingGdxDataDialog::GetFileNamesByExtName(const QString& path, const QString& extName)
{
	QDir dir(path);
...
bool QgsImageMappingGdxDataDialog::LoadImageMappingSchemaXml(const QString& xmlPath, vector<ImageMappingSchema_Gdx>& vSchema)
{
	QDomDocument doc;
	QFile file(xmlPath);
...
void QgsImageMappingGdxDataDialog::SetUIByImageMappingSchema(const ImageMappingSchema_Gdx& schema)
{
	// 获取运行环境路径
	QString qstrExePath = QCoreApplication::applicationDirPath();
	QString qstrSchemaPath = qstrExePath + "/layout/schema_gdx";
	QString qstrTemplatePath = qstrExePath + "/layout/template_gdx";
	...
	{
		FeatureClassParam_Gdx param = schema.vFeatureClass[i];
...
// 设置各图层要素类列表框状态
void QgsImageMappingGdxDataDialog::SetListWidgetStatus(FeatureClassParam_Gdx& param)
{
	// 存储list控件
	vector<QListWidget*> vQListWidget;
	vQListWidget.clear();
	...
	// 铁路
	if (param.qstrLayerName == "RAILN")
	{
		vQListWidget.push_back(ui.listWidget_TieLu);
	}

	// 公路
	if (param.qstrLayerName == "ROALN")
	{
		vQListWidget.push_back(ui.listWidget_GongLu);
	}

	// 境界
	if (param.qstrLayerName == "BOULN")
	{
		vQListWidget.push_back(ui.listWidget_JingJie);
	}

	// 国家政区
	if (param.qstrLayerName == "AA0PL")
	{
		vQListWidget.push_back(ui.listWidget_GuoJiaZhengQu);
	}

	// 一级政区
	if (param.qstrLayerName == "AA1PL")
	{
		vQListWidget.push_back(ui.listWidget_YiJiZhengQu);
	}

	// 二级政区
	if (param.qstrLayerName == "AA2PL")
	{
		vQListWidget.push_back(ui.listWidget_ErJiZhengQu);
	}

	// 三级政区
	if (param.qstrLayerName == "AA3PL")
	{
		vQListWidget.push_back(ui.listWidget_SaniJiZhengQu);
	}
...
void QgsImageMappingGdxDataDialog::GetListWidgetStatus(const QString& qstrLayerType, FeatureClassParam_Gdx& param)
{
	...
	// 铁路
	if (qstrLayerType == "RAILN")
	{
		vQListWidget.push_back(ui.listWidget_TieLu);
	}

	// 公路
	if (qstrLayerType == "ROALN")
	{
		vQListWidget.push_back(ui.listWidget_GongLu);
	}

	// 境界
	if (qstrLayerType == "BOULN")
	{
		vQListWidget.push_back(ui.listWidget_JingJie);
	}

	// 国家政区
	if (qstrLayerType == "AA0PL")
	{
		vQListWidget.push_back(ui.listWidget_GuoJiaZhengQu);
	}

	// 一级政区
	if (qstrLayerType == "AA1PL")
	{
		vQListWidget.push_back(ui.listWidget_YiJiZhengQu);
	}

	// 二级政区
	if (qstrLayerType == "AA2PL")
	{
		vQListWidget.push_back(ui.listWidget_ErJiZhengQu);
	}

	// 三级政区
	if (qstrLayerType == "AA3PL")
	{
		vQListWidget.push_back(ui.listWidget_SaniJiZhengQu);
	}
...
// 判断列表中元素是否在要素类列表中
bool QgsImageMappingGdxDataDialog::bIsInFeatureClasses(QString qstrText, vector<QString>& vFeatureClassCode)
{
...
bool QgsImageMappingGdxDataDialog::ExportMapToFile(
...
void QgsImageMappingGdxDataDialog::ConvertMilliMeterToPixel(double w, double h, int dpi, int& x, int& y)
{
	// x,y 为像素数，w,h为毫米
	// 水平方向：x = w * dpi / 25.4
	// 垂直方向：y = h * dpi / 25.4
...
void QgsImageMappingGdxDataDialog::ConvertPixeltoMilliMeter(int x, int y, int dpi, double& w, double& h)
{
	// x, y 为像素数，w, h为毫米
	// 水平方向：w = x * 25.4 / dpi
	// 垂直方向：h = y * 25.4 / dpi
...
void QgsImageMappingGdxDataDialog::restoreState()
{
...
void QgsImageMappingGdxDataDialog::GetSheetListByMappingRange(const QString& qstrTemplateName,
	double dLeft,
	double dTop,
	double dRight,
	double dBottom,
...
void QgsImageMappingGdxDataDialog::GetVectorLayerPathList(const ImageMappingSchema_Gdx& schema,
...
double QgsImageMappingGdxDataDialog::CalCenterMeridian(double dLong, double dScale)
{
...
void QgsImageMappingGdxDataDialog::GetMBR(int iCount, double* pPoints, double& dLeft, double& dTop, double& dRight, double& dBottom)
{
	double dMinX = 1e20;
	double dMaxX = -1e20;
	double dMinY = 1e20;
...
QString QgsImageMappingGdxDataDialog::GetEPSGCodeByCenterMeridianAndScale(double dLong, double dScale)
{
...
void QgsImageMappingGdxDataDialog::Degree2DMS(double degree, int& iDegree, int& iMinute, int& iSecond)
{
	iDegree = (int)degree;
...
bool QgsImageMappingGdxDataDialog::ExportImageMetaData(const QString& filePath,
	ImageMappingSchema_Gdx& schema,
	double dLeft,
	double dRight,
	double dTop,
	double dBottom,
...
QString QgsImageMappingGdxDataDialog::FormatDegree(int iDegree)
{
	QString qstrDegree;
	if (iDegree == 0)
	{
...
QString QgsImageMappingGdxDataDialog::FormatDegree2(int iDegree)
{
	QString qstrDegree;
	if (iDegree == 0)
	{
...
QString QgsImageMappingGdxDataDialog::FormatMinute(int iMinute)
{
	QString qstrMinute;
	if (iMinute == 0)
	{
...
QString QgsImageMappingGdxDataDialog::FormatMinute2(int iMinute)
{
	QString qstrMinute;
	if (iMinute == 0)
	{
...
QString QgsImageMappingGdxDataDialog::FormatSecond(int iSecond)
{
	QString qstrSecond;
	if (iSecond == 0)
	{
...
QString QgsImageMappingGdxDataDialog::FormatSecond2(int iSecond)
{
	QString qstrSecond;
	if (iSecond == 0)
	{
...
bool QgsImageMappingGdxDataDialog::LoadImageMappingMetaDataConfigFile(string& strCategory,
	string& strName,
	string& strType,
	string& strLength,
	string& strValue,
	ImageMetaData_Gdx& data)
{
	// 配置文件路径
	// 获取映射表配置参数
...
void QgsImageMappingGdxDataDialog::DrawGridOfNeighboringZone(QString qstrSheet,
...
QString QgsImageMappingGdxDataDialog::GetQmlFilePathFromLayerName(double dScale, const QString& qstrLayerName)
{
	QString qstrQmlPath = "";
	// 运行环境路径
	...
	qstrQmlPath = qstrRunDirPath + "\\style\\gdx\\" + qstrScale + "\\" + qstrLayerName + ".qml";
...
QString QgsImageMappingGdxDataDialog::GetMosaicQmlFilePath(double dScale)
{
	QString qstrQmlPath = "";
	// 运行环境路径
	QString qstrRunDirPath = QCoreApplication::applicationDirPath();
	...
	qstrQmlPath = qstrRunDirPath + "\\style\\gdx\\" + qstrScale + "\\" + "mosaic.qml";
...
void QgsImageMappingGdxDataDialog::CalGaussBorderCoords(const SE_DRect& dRange,
...
void QgsImageMappingGdxDataDialog::CalLayoutBorderPoints(
...
int QgsImageMappingGdxDataDialog::CalBandNumber(double dCenterMeridian, double dScale)
{
	// 根据中央经线计算带号
	int iBandNumber = 0;
	string strBandType;
...
int QgsImageMappingGdxDataDialog::bIsAdjProjectBandSheet(double dScale, const QString& qstrSheet, double dCenterMeridian)
{
	// 当前投影带左边界经度值，单位：度
...
void QgsImageMappingGdxDataDialog::DrawAdjKmNet(QgsPrintLayout* pPrintLayout,
...
#pragma region "绘制上边界"
	...
	font10km.setPixelSize(25);		// 8.52P≈25像素
...
#pragma endregion
	...
	for (int iPointIndex = iBeginMarkIndex_X_Top; iPointIndex <= iEndMarkIndex_X_Top; iPointIndex++)
	{
		...
		pTopBorderLabel_10km->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx, layoutTopBorderPoint.dy - 3.5, QgsUnitTypes::LayoutMillimeters));
		*/
			...
			iPointIndex == iEndMarkIndex_X_Top)
		{
			...
			pTopBorderLabel_100km->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx - 1.9, layoutTopBorderPoint.dy - 2.5, QgsUnitTypes::LayoutMillimeters));
			*/
			...
			pTopBorderLabel_BandNumber->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx - 4, layoutTopBorderPoint.dy - 3.3, QgsUnitTypes::LayoutMillimeters));
			*/
...
void QgsImageMappingGdxDataDialog::GetRadioChecked(bool& bLonLatRange, bool& bSheetList)
{
...
void QgsImageMappingGdxDataDialog::GetSheetListFromPlainTextEdit(vector<QString>& qstrSheetList)
{
	qstrSheetList.clear();
...
bool QgsImageMappingGdxDataDialog::LoadLayoutItemPositionsConfig(double dScale, vector<LayoutItemPositions_Gdx>& vLayoutItemPositions)
{
	// 配置文件路径
...
void QgsImageMappingGdxDataDialog::GetLayoutItemPositionByName(const QString& qstrName,
	vector<LayoutItemPositions_Gdx>& vItem,
	LayoutItemPositions_Gdx& item)
{
...
QgsLayoutItem::ReferencePoint QgsImageMappingGdxDataDialog::GetReferencePoint(const QString& qstrReferencePoint)
{
	// 左上角
...
bool QgsImageMappingGdxDataDialog::ReadSMSFile(string strSMSPath, int iKey, string& strValue)
{
...
void QgsImageMappingGdxDataDialog::TransformGaussToLayout(SE_DPoint dRefGaussPoint, SE_DPoint dRefLayoutPoint, SE_DPoint dGaussPoint, double dMapUnitToLayoutUnit, SE_DPoint& dLayoutPoint)
{
...
// 影像图制图——标准流程
void QgsImageMappingGdxDataDialog::ImageMapping_GdxData()
{
	/*判断数据路径的有效性*/
	//-------------------------------------//
...
#pragma region "获取布局元素位置配置信息"
	...
	/*获取布局元素位置配置信息*/
	vector<LayoutItemPositions_Gdx> vLayoutItemPositions;
	vLayoutItemPositions.clear();
...
#pragma region "读取左图幅图名"
		// 左图幅图名
		QString qstrLeftSheetMapName;

		// 左邻接图幅SMS路径
		QString qstrSMSFilePath_LeftSheet = m_ImageMappingSchema.qstrVectorDataPath + "/" + qstrLeftSheet + "/" + qstrLeftSheet + ".SMS";
		string strSMSFilePath_LeftSheet = qstrSMSFilePath_LeftSheet.toLocal8Bit();
		FILE* fpLeftSheet = fopen(strSMSFilePath_LeftSheet.c_str(), "r");
		if (!fpLeftSheet) {
			QgsMessageLog::logMessage(tr("读取元数据文件失败！:%1").arg(qstrSMSFilePath_LeftSheet), tr("影像图制图"), Qgis::Critical);
		}
		else
		{
			// 读取图名
			string strMapNameTemp;
			ReadSMSFile(strSMSFilePath_LeftSheet, 6, strMapNameTemp);
			qstrLeftSheetMapName = QString::fromLocal8Bit(strMapNameTemp.c_str());
		}
#pragma endregion

#pragma region "读取右图幅图名"
		// 右图幅图名
		QString qstrRightSheetMapName;

		// 右邻接图幅SMS路径
		QString qstrSMSFilePath_RightSheet = m_ImageMappingSchema.qstrVectorDataPath + "/" + qstrRightSheet + "/" + qstrRightSheet + ".SMS";
		string strSMSFilePath_RightSheet = qstrSMSFilePath_RightSheet.toLocal8Bit();
		FILE* fpRightSheet = fopen(strSMSFilePath_RightSheet.c_str(), "r");
		if (!fpRightSheet) {
			QgsMessageLog::logMessage(tr("读取元数据文件失败！:%1").arg(qstrSMSFilePath_RightSheet), tr("影像图制图"), Qgis::Critical);
		}
		else
		{
			// 读取图名
			string strMapNameTemp;
			ReadSMSFile(strSMSFilePath_RightSheet, 6, strMapNameTemp);
			qstrRightSheetMapName = QString::fromLocal8Bit(strMapNameTemp.c_str());
		}
#pragma endregion

#pragma region "读取上图幅图名"
		// 上图幅图名
		QString qstrTopSheetMapName;

		// 上邻接图幅SMS路径
		QString qstrSMSFilePath_TopSheet = m_ImageMappingSchema.qstrVectorDataPath + "/" + qstrTopSheet + "/" + qstrTopSheet + ".SMS";
		string strSMSFilePath_TopSheet = qstrSMSFilePath_TopSheet.toLocal8Bit();
		FILE* fpTopSheet = fopen(strSMSFilePath_TopSheet.c_str(), "r");
		if (!fpTopSheet) {
			QgsMessageLog::logMessage(tr("读取元数据文件失败！:%1").arg(qstrSMSFilePath_TopSheet), tr("影像图制图"), Qgis::Critical);
		}
		else
		{
			// 读取图名
			string strMapNameTemp;
			ReadSMSFile(strSMSFilePath_TopSheet, 6, strMapNameTemp);
			qstrTopSheetMapName = QString::fromLocal8Bit(strMapNameTemp.c_str());
		}
#pragma endregion

#pragma region "读取下图幅图名"
		// 下图幅图名
		QString qstrBottomSheetMapName;

		// 下邻接图幅SMS路径
		QString qstrSMSFilePath_BottomSheet = m_ImageMappingSchema.qstrVectorDataPath + "/" + qstrBottomSheet + "/" + qstrBottomSheet + ".SMS";
		string strSMSFilePath_BottomSheet = qstrSMSFilePath_BottomSheet.toLocal8Bit();
		FILE* fpBottomSheet = fopen(strSMSFilePath_BottomSheet.c_str(), "r");
		if (!fpBottomSheet) {
			QgsMessageLog::logMessage(tr("读取元数据文件失败！:%1").arg(qstrSMSFilePath_BottomSheet), tr("影像图制图"), Qgis::Critical);
		}
		else
		{
			// 读取图名
			string strMapNameTemp;
			ReadSMSFile(strSMSFilePath_BottomSheet, 6, strMapNameTemp);
			qstrBottomSheetMapName = QString::fromLocal8Bit(strMapNameTemp.c_str());
		}
#pragma endregion
...
#pragma region "图层列表定义"
		...
		qstrImageFilePath = m_qstrImageDataPath;
...
#pragma region "设置主地图范围及空间参考"
		...
		/*获取主地图位置配置信息*/
		LayoutItemPositions_Gdx mainMapItemPosition;        // 主地图窗口位置配置信息
...
#pragma region "【1】更新地图标题-图名位置"
		...
		// 获取位置配置信息
		LayoutItemPositions_Gdx textItem_MapTitleName;
...
#pragma region "【2】更新地图标题-图幅号位置"
		...
		// 获取位置配置信息
		LayoutItemPositions_Gdx textItem_MapTitleSheet;
...
#pragma region "【3】更新地图副标题-政区说明"
		...
		// 获取位置配置信息
		LayoutItemPositions_Gdx textItem_MapSubTileProvince;
...
#pragma region "【12】镶嵌线地图"
		...
		if (m_ImageMappingSchema.layoutItemParam.bMosaicMapChecked)
		{
			...
			pMosaicMapItem->attemptSetSceneRect(QRectF(m_dExteriorLeftTopLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + MOSAIC_MAP_LEFT_X,
				m_dExteriorLeftTopLayoutPoint.dy + m_dExteriorMapLayoutMBR.GetHeight() + 3,
				MOSAIC_MAP_WIDTH,
				MOSAIC_MAP_HEIGHT));
...
#pragma region "左下接图表"
			...
			pTextItem_LeftBottomName->setText("");
...
#pragma region "右上接图表"
			...
			pTextItem_RightTopName->setText("");
...
#pragma region "右下接图表"
			...
			pTextItem_RightBottomName->setText("");
...
#pragma region "【20】根据要素选择状态绘制图例"
		...
		{
			FeatureClassParam_Gdx param = m_ImageMappingSchema.vFeatureClass[iIndex];
...
#pragma region "主地图图例列表"		
		...
		// 图例名称列表
		vector<MainMapLegendInfo_Gdx> vMainMapLegends;
...
#pragma region "注记层"
		...
		// 首都：280111
		/*MainMapLegendInfo_Gdx legend_280111;
		...
		// 省：280121
		MainMapLegendInfo_Gdx legend_280121;
		...
		// 市：280131
		MainMapLegendInfo_Gdx legend_280131;
		...
		// 县：280141
		MainMapLegendInfo_Gdx legend_280141;
		...
		// 乡：280102
		MainMapLegendInfo_Gdx legend_280102;
...
#pragma region "颜色"
		...
		dMosaicLegendLeftTopCornerLayoutPoint.dx = MOSAIC_LEGEND_LEFT_TOP_X;
...
void QgsImageMappingGdxDataDialog::LoadSheetListFile()
{
	// 打开txt文件
	// 选择文件夹
...
/*根据两点确定的直线计算点坐标Y*/
void QgsImageMappingGdxDataDialog::CalCoordYByTwoPoint(SE_DPoint dPoint1, SE_DPoint dPoint2, double dInputX, double& dOutputY)
{
	// 如果两点Y坐标相同
...
/*根据两点确定的直线计算点坐标X*/
void QgsImageMappingGdxDataDialog::CalCoordXByTwoPoint(SE_DPoint dPoint1, SE_DPoint dPoint2, double dInputY, double& dOutputX)
{
	// 如果两点Y坐标相同
	if (fabs(dPoint1.dy - dPoint2.dy) <= IMAGE_MAPPING_ZERO)
	{
...
void QgsImageMappingGdxDataDialog::CalGaussBorderCoords_A_B(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
...
void QgsImageMappingGdxDataDialog::CalGaussBorderCoords_C_B(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
void QgsImageMappingGdxDataDialog::CalGaussBorderCoords_D_C(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
void QgsImageMappingGdxDataDialog::CalGaussBorderCoords_D_A(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
int QgsImageMappingGdxDataDialog::ClockWise(vector<SE_DPoint>& vPoints)
{
	int i, j, k;
	int count = 0;
	double z;
...
SE_DPoint QgsImageMappingGdxDataDialog::LineIntersect(SE_DPoint p0,SE_DPoint p1,SE_DPoint p2,SE_DPoint p3)
{
...
int QgsImageMappingGdxDataDialog::CalIndexInVector_XCoord(double dx, vector<SE_DPoint>& vPoints)
{
...
int QgsImageMappingGdxDataDialog::CalIndexInVector_YCoord(double dy, vector<SE_DPoint>& vPoints)
{
	for (int i = 0; i < vPoints.size(); i++)
	{
...
void QgsImageMappingGdxDataDialog::TransformLayoutToGauss(SE_DPoint dRefGaussPoint,
...
void QgsImageMappingGdxDataDialog::CalEndPointByStartPointAndAngle(
...
void QgsImageMappingGdxDataDialog::ConvertMilFromIntToString(int iMil, QString& qstrMil)
{
	// 如果密位数为个位数
	if (iMil < 10)
	{
...
QList<QgsStringMap> QgsImageMappingGdxDataDialog::GetAttributeValue(QgsVectorLayer* layer)
{
...
void QgsImageMappingGdxDataDialog::GetAcqDataAndSensorByCateName(QList<QgsStringMap>& qAttributeList,
...
// 判断目录是否存在
bool QgsImageMappingGdxDataDialog::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool QgsImageMappingGdxDataDialog::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}


void QgsImageMappingGdxDataDialog::InitLineEdit()
{
	// 上边界纬度
	...
	// 附注
	ui.lineEdit_Annotations->setValidator(new QRegExpValidator(QRegExp("^[\u4e00-\u9fa5a-zA-Z0-9_]+$")));
