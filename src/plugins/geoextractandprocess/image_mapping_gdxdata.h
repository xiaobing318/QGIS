#pragma once
#ifndef QGS_IMAGE_MAPPING_GDXDATA_DIALOG_H
#define QGS_IMAGE_MAPPING_GDXDATA_DIALOG_H
...
#include "ui_image_mapping_gdxdata.h"
...
/*要素选择参数结构体*/
struct FeatureClassParam_Gdx
{
	QString qstrLayerName;				// 图层名称：国地信标准图层名称，如：公路-"ROALN"、铁路-"RAILN"等
	vector<QString> vFeatureClassCode;	// 当前图层内要素类选择列表，通过类别代码区分，例如：普通铁路（11001）

	FeatureClassParam_Gdx()
	{
		qstrLayerName = "";
...
struct ImageMappingSchema_Gdx
{
	...
	/*矢量要素选择*/
	vector<FeatureClassParam_Gdx>        vFeatureClass;     // 图层要素类列表
	...
	ImageMappingSchema_Gdx()
	{
		...
		layoutItemParam = LayoutItemParam_Gdx();
...
/*制图布局元素位置结构体*/
struct LayoutItemPositions_Gdx
{
...
/*影像图元数据结构体*/
struct ImageMetaData_Gdx
{
	vector<ImageMetaDataItem_Gdx> vItems;       // 元数据项集合
...
/*镶嵌线图例信息结构体*/
struct MosaicMapLegendInfo_Gdx
{
	QColor					qColor;					// 颜色
	QString					qstrData;				// 摄影时间
	QString					qstrSensor;				// 传感器

	MosaicMapLegendInfo_Gdx()
	{
		qColor = QColor(0,0,0);
		qstrData = "";
		qstrSensor = "";
...
class QgsImageMappingGdxDataDialog : public QDialog
{
	Q_OBJECT

public:
	QgsImageMappingGdxDataDialog(QWidget* parent = nullptr, Qt::WindowFlags fl = Qt::WindowFlags());
	~QgsImageMappingGdxDataDialog() override;

private:

	Ui_QgsImageMappingDlg_GdxData ui;
	...
	// 图层RAILN、ROALN、BOULN、AA0PL、AA1PL、AA2PL、AA3PL
	...
	// 制图方案列表
	vector<ImageMappingSchema_Gdx> m_vImageMappingSchema;

	// 当前所选制图方案
	ImageMappingSchema_Gdx m_ImageMappingSchema;
...
private:
	...
	// 加载制图方案xml文件
	bool LoadImageMappingSchemaXml(const QString& xmlPath, vector<ImageMappingSchema_Gdx>& vSchema);

	// 根据制图方案xml文件设置界面状态
	void SetUIByImageMappingSchema(const ImageMappingSchema_Gdx& schema);

	// 设置要素选择列表框的状态
	void SetListWidgetStatus(FeatureClassParam_Gdx& param);

	// 获取要素选择列表框的状态
	void GetListWidgetStatus(const QString& qstrLayerType, FeatureClassParam_Gdx& param);
	...
	// 根据制图方案获取矢量图层全路径列表
	void GetVectorLayerPathList(const ImageMappingSchema_Gdx& schema,
	...
	*/
	...
	// 加载布局元素位置配置文件
	bool LoadLayoutItemPositionsConfig(double dScale, vector<LayoutItemPositions_Gdx>& vLayoutItemPositions);
...
#endif // QGS_IMAGE_MAPPING_GDXDATA_DIALOG_H
