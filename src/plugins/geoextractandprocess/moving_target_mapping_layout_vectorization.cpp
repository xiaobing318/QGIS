...
#pragma region "图层列表定义"
	...
	QList<QgsMapLayer*> qEagleEyeMapLayerList;    // 鹰眼地图加载图层列表，加载世界政区图
...
#pragma region "加载轨迹点图层"
	...
	if (QFileInfo::exists(qstrTracePointLayerFilePath))
	{
		...
		// 地图范围扩大，包括轨迹点边界
		dTracePointLayerRect.dleft = qgsPointLayerRect.xMinimum() - qgsPointLayerRect.width() * 0.02;
		dTracePointLayerRect.dright = qgsPointLayerRect.xMaximum() + qgsPointLayerRect.width() * 0.02;
		dTracePointLayerRect.dtop = qgsPointLayerRect.yMaximum() + qgsPointLayerRect.height() * 0.02;
		dTracePointLayerRect.dbottom = qgsPointLayerRect.yMinimum() - qgsPointLayerRect.height() * 0.02;
...
#pragma region "判断是否存在轨迹点标注图层，如果已存在，则直接加载；如果不存在，则复制轨迹点层"

	// 轨迹点标注图层默认不存在
	bool bTracePointLabelLayerExisted = false;

	// 获取输入轨迹点图层所在目录，轨迹点标注层也生成在此目录下
	string strTracePointLayerFileNameTemp = qstrTracePointLayerFilePath.toLocal8Bit();
	...
	QString qstrLabelPointShpFilePathTemp = QString::fromLocal8Bit(strLabelPointShpFilePathTemp.c_str());
...
#pragma region "加载影像图层"
	...
	// 先按照制图范围裁切影像数据
	string strImageDataPath = GetImageDataPath().toLocal8Bit();
	string strOutputDataPath = GetDataSavePath().toLocal8Bit();
...
#pragma region "加载轨迹点图层到鹰眼图地图控件中"
...
#pragma region "外图廓整饰面要素图层【生成地图说明、动目标说明】"
		...
		if (!bLayoutDecPolygonExisted)
		{
			...
			string strShpFilePath = qstrShpFilePath.toLocal8Bit();
...
#pragma region "【1-1】创建整饰要素-动目标地图图名"
		...
		if (!bLayoutDecPolygonExisted)
		{
			...
			fieldImp.strFieldValue = qstrMapName.toLocal8Bit();
			vFieldNameAndValue.push_back(fieldImp);
...
#pragma region "创建整饰要素-地图说明"
		...
		if (!bLayoutDecPolygonExisted)
		{
			...
			fieldImp.strFieldName = "VALUE";
			fieldImp.strFieldValue = qstrMapDetail.toLocal8Bit();
			vFieldNameAndValue.push_back(fieldImp);
...
#pragma region "【6-1】创建整饰要素-动目标说明"
		...
		if (!bLayoutDecPolygonExisted)
		{
			...
			fieldImp.strFieldName = "VALUE";
			fieldImp.strFieldValue = qstrMovingTargetDetail.toLocal8Bit();
			vFieldNameAndValue.push_back(fieldImp);
...
{
	// 选择文件夹
	QString curPath = m_qstrEagleEyeDataPath;
	QString fileName = QFileDialog::getOpenFileName(this, tr("加载鹰眼图数据"), curPath, "*.shp *.SHP");
...
bool QgsMovingTargetMappingLayoutVectorizationDialog::CreateShapefileCPG(string strCPGFilePath, string strEncoding /*= "System"*/)
{
...
void QgsMovingTargetMappingLayoutVectorizationDialog::GetLayoutDecPolygonQmlFilePath(QString& qstrLayoutDecPolygonQmlPath)
{
	...
	qstrLayoutDecPolygonQmlPath = qstrRunDirPath + "\\style\\theme\\moving_target\\layout_decpolygon.qml";

#else
	qstrLayoutDecPolygonQmlPath = qstrRunDirPath + "/style/theme/moving_target/layout_decpolygon.qml";
...
bool QgsMovingTargetMappingLayoutVectorizationDialog::ChunkImageDataToJPEG(string strRasterFullName,
	double dLeft,
	double dTop,
	double dRight,
	double dBottom,
	...
	QString &qstrOutputRasterFileFullName)
{
	...
	if (!pSrcDataSet)
	{
		...
		string strMapName = GetMapName().toLocal8Bit();
...
#define MODE (S_IRWXU | S_IRWXG | S_IRWXO)
		...
		qstrOutputRasterFileFullName = QString::fromLocal8Bit(strOutputFullName.c_str());
