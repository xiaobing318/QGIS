...
typedef struct YTH_single_feature
{
...
{
  std::vector<YTH_OSM_code_mapping_single_layer_t> v_YTH_layers;
...
#pragma region "11、YTH属性表结构配置映射表所涉及的自定义数据结构（代码重构）"
  ...
  {
    std::vector<YTH_single_layer_t> v_YTH_layers;
...
#pragma region "（4）转化时OSM相关信息"
typedef struct OSM_single_layer_info
{
  string layer_name;
  string layer_geo_type;
  OGRLayer* polayer;
  OGRSpatialReference* poSRS;
  OSM_single_layer_info()
  {
    layer_name = "";
    layer_geo_type = "";
    polayer = NULL;
...
}OSM_single_layer_info_t;
typedef struct single_frame_data_OSM_layers_info
{
  //	分幅数据目录路径
  std::string frame_data_path;
  GDALDataset* poDS;
  std::vector<OSM_single_layer_info_t> vOSMLayer_info;
...
}single_frame_data_OSM_layers_info_t;
#pragma endregion
...
#pragma region "（6）转化时一体化相关信息"
//	单个图幅的不同数据转化为一体化数据的时候需要维持的数据结构（保持输出图层中各种信息）
typedef struct single_YTHLayer_info
{
  std::string layer_name;
  std::string layer_geo_type;
  OGRLayer* polayer;
  OGRSpatialReference* poSRS;
  single_YTHLayer_info()
  {
    layer_name = "";
    layer_geo_type = "";
    polayer = NULL;
...
}single_YTHLayer_info_t;
typedef struct single_frame_YTHLayer_infos
{
  std::string frame_data_path;
  GDALDataset* poDS;
  std::vector<std::string> source_Layer_name;
  std::vector<single_YTHLayer_info_t> vYTHLayer_info;
  single_frame_YTHLayer_infos()
  {
    frame_data_path = "";
    poDS = NULL;
    source_Layer_name.clear();
    vYTHLayer_info.clear();
  }

}single_frame_YTHLayer_infos_t;
#pragma endregion
...
#pragma region "（4）OSM---->一体化（代码重构）"
  ...
  static void use_OSMData_create_single_frame_YTHLayer_infos(
    const single_frame_data_OSM_layers_info_t& single_frame_OSMLayer_infos,
    const YTH_fields4all_layers_t& YTH_fields4all_layers,
    const YTH_OSM_layer_mapping_table_t& YTH_OSM_layer_mapping_table,
    const string& strOutputDataPath,
