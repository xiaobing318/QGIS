...
QgsImageMappingDialog::QgsImageMappingDialog(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	...
	// 标准制图流程
	connect(ui.pushButton_ImageMapping, &QPushButton::clicked, this, &QgsImageMappingDialog::ImageMapping_StandardProcess);
	
	// 整饰要素矢量化流程
	//connect(ui.pushButton_ImageMapping, &QPushButton::clicked, this, &QgsImageMappingDialog::ImageMapping_LayoutVectorization);

	connect(ui.pushButton_Close, &QPushButton::clicked, this, &QgsImageMappingDialog::Close);

	connect(ui.pushButton_OpenVectorData, &QPushButton::clicked, this, &QgsImageMappingDialog::OpenVectorData);
	connect(ui.pushButton_OpenImageData, &QPushButton::clicked, this, &QgsImageMappingDialog::OpenImageData);
	connect(ui.pushButton_OpenMosaicData, &QPushButton::clicked, this, &QgsImageMappingDialog::OpenMosaicData);
	connect(ui.pushButton_Save, &QPushButton::clicked, this, &QgsImageMappingDialog::ImageMappingDataSave);
	...
	connect(ui.radioButton_LonLatRange, &QRadioButton::clicked, this, &QgsImageMappingDialog::LonLatRadioButton_Click);
	connect(ui.radioButton_SheetList, &QRadioButton::clicked, this, &QgsImageMappingDialog::SheetListRadioButton_Click);

	connect(ui.pushButton_LoadSheetList, &QPushButton::clicked, this, &QgsImageMappingDialog::LoadSheetListFile);
	connect(ui.pushButton_LoadMappingSchema, &QPushButton::clicked, this, &QgsImageMappingDialog::LoadMappingSchema);
	connect(ui.pushButton_SaveMappingSchema, &QPushButton::clicked, this, &QgsImageMappingDialog::SaveMappingSchema);


	/*设置制图范围页面控件状态*/
	ui.radioButton_LonLatRange->setEnabled(true);
	ui.radioButton_LonLatRange->setChecked(true);
...
QgsImageMappingDialog::~QgsImageMappingDialog()
{
	m_vImageMappingSchema.clear();
	m_mCode_Names.clear();
	m_mName_Codes.clear();
	...
	// 地图输出路径
	settings.setValue(QStringLiteral("QgsImageMappingDialog/ImageMappingDataSavePath"), m_qstrImageMappingDataSavePath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("QgsImageMappingDialog/ImageMappingSchemaSavePath"), m_qstrImageMappingSchemaSavePath, QgsSettings::Section::Plugins);
}

void QgsImageMappingDialog::Close()
{
	accept();
}

void QgsImageMappingDialog::ImageMapping()
{
	...
	*/
	...
	// 副标题是否绘制状态
	//m_ImageMappingSchema.layoutItemParam.bMapSubTitleChecked = ui.checkBox_MainMap_SubTitle->isChecked();

	// 副标题省名和县名从界面获取
	//m_ImageMappingSchema.layoutItemParam.qstrMapSubTitleProvince = GetSubTitleProvinceName();
	//m_ImageMappingSchema.layoutItemParam.qstrMapSubTitleCounty = GetSubTitleCountyName();
...
#pragma region "从元数据SMS文件中获取“图名”、“政区说明”、“磁偏角”、“坐标纵线偏角”、“磁坐偏角”"
		...
		else
		{
			...
			// 读取磁偏角
			string strCiPianJiaoValue;
			ReadSMSFile(strSMSPath, 34, strCiPianJiaoValue);
			dCiPianJiao = atof(strCiPianJiaoValue.c_str());

			// 读取坐标纵线偏角
			string strZuoBiaoZongXianPianJiao;
			ReadSMSFile(strSMSPath, 36, strZuoBiaoZongXianPianJiao);
			dZuoBiaoZongXianPianJiao = atof(strZuoBiaoZongXianPianJiao.c_str());

			// 读取磁坐偏角
			string strCiZuoPianJiao;
			ReadSMSFile(strSMSPath, 35, strCiZuoPianJiao);
			dCiZuoPianJiao = atof(strCiZuoPianJiao.c_str());
		}
...
#pragma endregion
		...
		qstrImageFilePath = m_ImageMappingSchema.qstrImageDataPath + "/" + qstrSheet + ".tif";
...
#pragma region "加载影像图层"
		...
		{
			// 记录日志
			QgsMessageLog::logMessage((QString(tr("栅格图层不合法:")) + fileNameRaster), tr("影像图制图"), Qgis::Critical);
			continue;
		}
		// 获取影像数据分辨率
		...
		if (!pLayerRaster)
		{
			delete rasterLayer;
			// 记录日志
			QgsMessageLog::logMessage((QString(tr("增加栅格图层失败:")) + fileNameRaster), tr("影像图制图"), Qgis::Critical);
			continue;
		}
...
#pragma region "加载矢量图层"
		...
		for (j = 0; j < vVectorLayerPathList.size(); j++)
		{
			...
			if (QFileInfo::exists(qstrVectorLayerFilePath))
			{
				...
				{
					// 记录日志
					QgsMessageLog::logMessage((QString(tr("矢量图层不合法:")) + fileNameVector), tr("影像图制图"), Qgis::Critical);
					continue;
				}
				...
				{
					// QgsMessageLog::logMessage((QString(tr("样式文件不存在:")) + qstrQmlFilePath), tr("影像图制图"), Qgis::Critical);
					continue;
				}
				...
				if (!pLayer)
				{
					delete pVecotrLayer;
					// 记录日志
					QgsMessageLog::logMessage((QString(tr("增加矢量图层失败:")) + fileNameVector), tr("影像图制图"), Qgis::Critical);
					continue;
				}
...
#pragma region "加载镶嵌线图层"
		...
		{
			// 记录日志
			QgsMessageLog::logMessage((QString(tr("矢量图层不合法:")) + fileNameMosaic), tr("影像图制图"), Qgis::Critical);
			continue;
		}

		/*图层已有的样式*/
		QgsMapLayerStyle oldMosaicStyle = vecLayerMosaic->styleManager()->style(vecLayerMosaic->styleManager()->currentStyle());
		QString message;
		bool defaultLoadedStyleFlag = false;

		// 镶嵌线样式文件路径
		QString qstrMosaicQmlFilePath = GetMosaicQmlFilePath(dScale);
		message = vecLayerMosaic->loadNamedStyle(qstrMosaicQmlFilePath, defaultLoadedStyleFlag, true, QgsMapLayer::Symbology | QgsMapLayer::Labeling | QgsMapLayer::Fields);

		if (!defaultLoadedStyleFlag)
		{
			QgsMessageLog::logMessage((QString(tr("镶嵌线样式文件不存在:")) + qstrMosaicQmlFilePath), tr("影像图制图"), Qgis::Critical);
		}
		...
		if (!pLayerMosaic)
		{
			...
			QgsMessageLog::logMessage((QString(tr("增加矢量图层失败:")) + fileNameMosaic), tr("影像图制图"), Qgis::Critical);
			continue;
		}
...
#pragma region "获取地图外图廓左上角点布局坐标（单位：mm）"

		/*获取地图外图廓元素*/
		QgsLayoutItemShape* pMapOuterLine = (QgsLayoutItemShape*)GetLayoutItemByName(qLayoutItemList, "Rectangle_Outline");
		if (!pMapOuterLine)
		{
			// 记录日志
			QgsMessageLog::logMessage(tr("地图外图廓获取失败！"), tr("影像图制图"), Qgis::Critical);
			continue;
		}

		/*获取地图外图廓的左上角点布局坐标*/
		m_MapOutlineLeftTopCornerLayoutPoint.dx = pMapOuterLine->positionWithUnits().x();
		m_MapOutlineLeftTopCornerLayoutPoint.dy = pMapOuterLine->positionWithUnits().y();
...
#pragma region "设置主地图范围及空间参考"
		...
		/*对图幅四个角点进行高斯投影正算，获取四个投影坐标的最小外接矩形*/
		// 图幅四个角点坐标
		...
		m_dInteriorMapLayoutMBR.dleft = dMBRLeft;
		m_dInteriorMapLayoutMBR.dtop = dMBRTop;
		m_dInteriorMapLayoutMBR.dright = dMBRRight;
		m_dInteriorMapLayoutMBR.dbottom = dMBRBottom;
		...
		double dMapItemRectX = pMainMapItem->rect().x();
		double dMapItemRectY = pMainMapItem->rect().y();

		/*重新设置地图范围*/
		pMainMapItem->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + mainMapItemPosition.dLeft_offset,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + mainMapItemPosition.dTop_offset,
			(dMBRRight - dMBRLeft) * dMapUnit2LayoutUnit,
			(dMBRTop - dMBRBottom) * dMapUnit2LayoutUnit));
		pMainMapItem->setExtent(rect);

		/*设置完比例尺后获取地图范围*/
		QgsRectangle qgsMapRect = pMainMapItem->extent();
		pMainMapItem->setLayers(qMainMapLayerList);
		/*地图图廓布局尺寸*/
		QRectF mapItemLayoutRect = pMainMapItem->rect();
		m_qMainMapItemLayoutRect = mapItemLayoutRect;
#pragma endregion

#pragma region "更新外图廓尺寸和位置"

		pMapOuterLine->setReferencePoint(QgsLayoutItem::UpperLeft);
		pMapOuterLine->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx,
			m_MapOutlineLeftTopCornerLayoutPoint.dy,
			mapItemLayoutRect.width() + 2 * OUTER_MAP_INTERIOR_MAP_DISTANCE,
			mapItemLayoutRect.height() + 2 * OUTER_MAP_INTERIOR_MAP_DISTANCE));

		m_qMapOutLineItemLayoutRect = pMapOuterLine->rect();
#pragma endregion

#pragma region "绘制方里网"
		/*获取地图内图廓四个角点的高斯坐标值*/
		// 左上角高斯坐标值
		SE_DPoint leftTopCornerGaussPoint;
		leftTopCornerGaussPoint.dx = qgsMapRect.xMinimum();
		leftTopCornerGaussPoint.dy = qgsMapRect.yMaximum();

		// 左下角高斯坐标值
		SE_DPoint leftBottomCornerGaussPoint;
		leftBottomCornerGaussPoint.dx = qgsMapRect.xMinimum();
		leftBottomCornerGaussPoint.dy = qgsMapRect.yMinimum();

		// 右上角高斯坐标值
		SE_DPoint rightTopCornerGaussPoint;
		rightTopCornerGaussPoint.dx = qgsMapRect.xMaximum();
		rightTopCornerGaussPoint.dy = qgsMapRect.yMaximum();

		// 右下角高斯坐标值
		SE_DPoint rightBottomCornerGaussPoint;
		rightBottomCornerGaussPoint.dx = qgsMapRect.xMaximum();
		rightBottomCornerGaussPoint.dy = qgsMapRect.yMinimum();

		/*获取地图内图廓四个角点布局坐标*/

		// 布局坐标，左上角为（0,0）点，从左往右依次增加，从上往下依次增加

		// 左上角布局坐标值
		SE_DPoint leftTopCornerLayoutPoint;
		leftTopCornerLayoutPoint.dx = pMainMapItem->pagePositionWithUnits().x();
		leftTopCornerLayoutPoint.dy = pMainMapItem->pagePositionWithUnits().y();

		// 左下角布局坐标值
		SE_DPoint leftBottomCornerLayoutPoint;
		leftBottomCornerLayoutPoint.dx = pMainMapItem->pagePositionWithUnits().x();
		leftBottomCornerLayoutPoint.dy = pMainMapItem->pagePositionWithUnits().y() + mapItemLayoutRect.height();

		// 右上角布局坐标值
		SE_DPoint rightTopCornerLayoutPoint;
		rightTopCornerLayoutPoint.dx = pMainMapItem->pagePositionWithUnits().x() + mapItemLayoutRect.width();
		rightTopCornerLayoutPoint.dy = pMainMapItem->pagePositionWithUnits().y();

		// 右下角布局坐标值
		SE_DPoint rightBottomCornerLayoutPoint;
		rightBottomCornerLayoutPoint.dx = pMainMapItem->pagePositionWithUnits().x() + mapItemLayoutRect.width();
		rightBottomCornerLayoutPoint.dy = pMainMapItem->pagePositionWithUnits().y() + mapItemLayoutRect.height();

		/*根据地图投影范围计算内图廓上、下、左、右边界整公里坐标点集合*/
		SE_DRect mainMapGaussRect;      // 内图廓高斯投影范围

		// 左边界高斯投影值
		mainMapGaussRect.dleft = qgsMapRect.xMinimum();

		// 右边界高斯投影值
		mainMapGaussRect.dright = qgsMapRect.xMaximum();

		// 上边界高斯投影值
		mainMapGaussRect.dtop = qgsMapRect.yMaximum();

		// 下边界高斯投影值
		mainMapGaussRect.dbottom = qgsMapRect.yMinimum();
		...
		CalGaussBorderCoords(mainMapGaussRect,
		...
		double dMapUnitToLayoutUnit = pMainMapItem->mapUnitsToLayoutUnits();

		CalLayoutBorderPoints(
			leftTopCornerGaussPoint,
			leftTopCornerLayoutPoint,
			leftBottomCornerGaussPoint,
			leftBottomCornerLayoutPoint,
			rightTopCornerGaussPoint,
			rightTopCornerLayoutPoint,
			rightBottomCornerGaussPoint,
			rightBottomCornerLayoutPoint,
			dMapUnitToLayoutUnit,
			vLeftBorderGaussCoords,
			vTopBorderGaussCoords,
			vRightBorderGaussCoords,
			vBottomBorderGaussCoords,
		...
		/*根据上、下、左、右布局整公里坐标点，自动创建方里网刻度线*/
		//----------------------------------------------------------//
		// 方里网刻度线的样式
		QVariantMap kmLineBorderMap;
		kmLineBorderMap["line_color"] = "137,137,137,255";
		kmLineBorderMap["line_style"] = "solid";
		kmLineBorderMap["line_width"] = "0.1";
		QgsLineSymbol* kmLineBorderLine = QgsLineSymbol::createSimple(kmLineBorderMap);
...
#pragma region "绘制上边界到下边界方里网及刻度线"
			...
			/*-------------------------------------*/
			// X方向方里网
			QgsLayoutItemPolyline* pKmLine_X = nullptr;
			for (int iPointIndex = 0; iPointIndex < vTopBorderLayoutCoords.size(); iPointIndex++)
			{
				...
				// 内图廓下边界整公里刻度点
				SE_DPoint layoutBottomBorderPoint = vBottomBorderLayoutCoords[iPointIndex];

				QPolygonF polygon;

				/*如果方里网刻度线与左右内图廓X坐标值差在KM_LINE_TOLERANCE_X毫米内，不绘制方里网刻度线，只绘制方里网*/
				if (fabs(layoutTopBorderPoint.dx - leftTopCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X
					|| fabs(layoutTopBorderPoint.dx - rightTopCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X)
				{
					// 整公里刻度线与外图廓上边界交叉点
				...
				{
					// 整公里刻度线与外图廓上边界交叉点
					polygon.append(QPointF(layoutTopBorderPoint.dx, layoutTopBorderPoint.dy - KM_LINE_LENGTH));

					// 整公里刻度线与外图廓下边界交叉点
					polygon.append(QPointF(layoutBottomBorderPoint.dx, layoutBottomBorderPoint.dy + KM_LINE_LENGTH));
				}

				pKmLine_X = new QgsLayoutItemPolyline(polygon, pPrintLayout);
				pKmLine_X->setSymbol(kmLineBorderLine);
				pPrintLayout->addLayoutItem(pKmLine_X);
			}
#pragma endregion
...
#pragma region "绘制左边界到右边界方里网及刻度线"
			...
			/*-------------------------------------*/
			// Y方向方里网
			QgsLayoutItemPolyline* pKmLine_Y = nullptr;
			for (int iPointIndex = 0; iPointIndex < vLeftBorderLayoutCoords.size(); iPointIndex++)
			{
				...
				// 内图廓右边界整公里刻度点
				SE_DPoint layoutRightBorderPoint = vRightBorderLayoutCoords[iPointIndex];

				QPolygonF polygon;

				/*如果方里网刻度线与上下内图廓Y坐标值差在KM_LINE_TOLERANCE_Y毫米内，不绘制方里网刻度线，只绘制方里网*/
				if (fabs(layoutLeftBorderPoint.dy - leftTopCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y
					|| fabs(layoutLeftBorderPoint.dy - rightTopCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y)
				{
					// 整公里刻度线与外图廓左边界交叉点
				...
				{
					// 整公里刻度线与外图廓左边界交叉点
					polygon.append(QPointF(layoutLeftBorderPoint.dx - KM_LINE_LENGTH, layoutLeftBorderPoint.dy));

					// 整公里刻度线与外图廓右边界交叉点
					polygon.append(QPointF(layoutRightBorderPoint.dx + KM_LINE_LENGTH, layoutRightBorderPoint.dy));
				}

				pKmLine_Y = new QgsLayoutItemPolyline(polygon, pPrintLayout);
				pKmLine_Y->setSymbol(kmLineBorderLine);
				pPrintLayout->addLayoutItem(pKmLine_Y);
			}
#pragma endregion
...
#pragma region "绘制内图廓上边界整公里刻度值"
			...
			int iBeginMarkIndex_X = 0;                                      // 默认索引为0
			int iEndMarkIndex_X = vTopBorderLayoutCoords.size() - 1;        // 默认索引为最后一个点
			for (int iPointIndex = 0; iPointIndex < vTopBorderLayoutCoords.size(); iPointIndex++)
			{
				SE_DPoint layoutTopBorderPoint = vTopBorderLayoutCoords[iPointIndex];
				if (fabs(layoutTopBorderPoint.dx - leftTopCornerLayoutPoint.dx) >= KM_LINE_TOLERANCE_X)
				{
					iBeginMarkIndex_X = iPointIndex;
					break;
				}
			}
			...
			{
				SE_DPoint layoutTopBorderPoint = vTopBorderLayoutCoords[iPointIndex];
				if (fabs(layoutTopBorderPoint.dx - rightTopCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X)
				{
					iEndMarkIndex_X = iPointIndex - 1;
					break;
				}
			}
			...
			for (int iPointIndex = 0; iPointIndex < vTopBorderLayoutCoords.size(); iPointIndex++)
			{
				// 内图廓上边界整公里刻度点
				SE_DPoint layoutTopBorderPoint = vTopBorderLayoutCoords[iPointIndex];
				SE_DPoint layoutTopBorderGaussPoint = vTopBorderGaussCoords[iPointIndex];
				/*如果方里网刻度线与左右内图廓X坐标值差在KM_LINE_TOLERANCE_X毫米内，不绘制刻度值*/
				if (fabs(layoutTopBorderPoint.dx - leftTopCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X
					|| fabs(layoutTopBorderPoint.dx - rightTopCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X)
				{
					continue;
				...
				else
				{
					...
					// 设置参考点
					/*pTopBorderLabel_10km->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx, layoutTopBorderPoint.dy - 3), false);
					pTopBorderLabel_10km->adjustSizeToText();*/

					//pTopBorderLabel_10km->setHAlign(Qt::AlignLeft);
					//pTopBorderLabel_10km->setVAlign(Qt::AlignBottom);
					...
					// 绘制百公里和带号
					if (iPointIndex == iBeginMarkIndex_X ||
						iPointIndex == iEndMarkIndex_X)
					{
						//-------------------------------------------------//
						// 百公里
						//-------------------------------------------------//
						...
						//pTopBorderLabel_100km->adjustSizeToText();
						//pTopBorderLabel_100km->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx - 1.875/4.0, layoutTopBorderPoint.dy - 1.875), false);
						...
						//pTopBorderLabel_BandNumber->adjustSizeToText();
						//pTopBorderLabel_BandNumber->attemptMove(QgsLayoutPoint(layoutTopBorderPoint.dx - 1.875 * 3 /4, layoutTopBorderPoint.dy - 3), false);
...
#pragma region "绘制内图廓下边界整公里刻度值"
			...
			/*绘制内图廓下边界整公里刻度值*/
			for (int iPointIndex = 0; iPointIndex < vBottomBorderLayoutCoords.size(); iPointIndex++)
			{
				// 内图廓下边界整公里刻度点
				SE_DPoint layoutBottomBorderPoint = vBottomBorderLayoutCoords[iPointIndex];
				SE_DPoint layoutBottomBorderGaussPoint = vBottomBorderGaussCoords[iPointIndex];
				/*如果方里网刻度线与左右内图廓X坐标值差在KM_LINE_TOLERANCE_X毫米内，不绘制刻度值*/
				if (fabs(layoutBottomBorderPoint.dx - leftBottomCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X
					|| fabs(layoutBottomBorderPoint.dx - rightBottomCornerLayoutPoint.dx) < KM_LINE_TOLERANCE_X)
				{
					continue;
				}
				else
				{
					...
					/*pBottomBorderLabel_10km->adjustSizeToText();
					pBottomBorderLabel_10km->attemptMove(QgsLayoutPoint(layoutBottomBorderPoint.dx, layoutBottomBorderPoint.dy, QgsUnitTypes::LayoutMillimeters));*/
						...
						iPointIndex == iEndMarkIndex_X)
					{
						...
						/*pBottomBorderLabel_100km->adjustSizeToText();
						pBottomBorderLabel_100km->attemptMove(QgsLayoutPoint(layoutBottomBorderPoint.dx - 1.9, layoutBottomBorderPoint.dy + 1, QgsUnitTypes::LayoutMillimeters));*/
						...
						/*pBottomBorderLabel_BandNumber->adjustSizeToText();
						pBottomBorderLabel_BandNumber->attemptMove(QgsLayoutPoint(layoutBottomBorderPoint.dx - 4, layoutBottomBorderPoint.dy + 0.3, QgsUnitTypes::LayoutMillimeters));*/
						// 带号
...
#pragma region "绘制左边界方里网数值"
			...
			// 第一个需要绘制千公里+百公里的左、右边界整公里方里网点索引
			int iBeginMarkIndex_Y = 0;                                       // 默认索引为0
			int iEndMarkIndex_Y = vLeftBorderLayoutCoords.size() - 1;        // 默认索引为最后一个点
			for (int iPointIndex = 0; iPointIndex < vLeftBorderLayoutCoords.size(); iPointIndex++)
			{
				SE_DPoint layoutLeftBorderPoint = vLeftBorderLayoutCoords[iPointIndex];
				if (fabs(layoutLeftBorderPoint.dy - leftBottomCornerLayoutPoint.dy) >= KM_LINE_TOLERANCE_Y)
				{
					iBeginMarkIndex_Y = iPointIndex;
					break;
				}
			}
			...
			for (int iPointIndex = 0; iPointIndex < vLeftBorderLayoutCoords.size(); iPointIndex++)
			{
				...
				// 内图廓左边界整公里高斯坐标值
				SE_DPoint layoutLeftBorderGaussPoint = vLeftBorderGaussCoords[iPointIndex];

				/*如果方里网刻度线与上、下内图廓Y坐标值差在KM_LINE_TOLERANCE_Y毫米内，不绘制刻度值*/
				if (fabs(layoutLeftBorderPoint.dy - leftTopCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y
					|| fabs(layoutLeftBorderPoint.dy - leftBottomCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y)
				{
					continue;
				}
				else
				{
					...
					/*pLeftBorderLabel_10km->adjustSizeToText();
					pLeftBorderLabel_10km->attemptMove(QgsLayoutPoint(layoutLeftBorderPoint.dx - 3.7, layoutLeftBorderPoint.dy - 3.5, QgsUnitTypes::LayoutMillimeters));
					*/
					...
					// 绘制千公里+百公里
					if (iPointIndex == iBeginMarkIndex_Y ||
						iPointIndex == iEndMarkIndex_Y)
					{
						//-------------------------------------------------//
						// 千公里+百公里
						//-------------------------------------------------//
						...
						/*pLeftBorderLabel_100km->adjustSizeToText();
						pLeftBorderLabel_100km->attemptMove(QgsLayoutPoint(layoutLeftBorderPoint.dx - 5, layoutLeftBorderPoint.dy - 3.5, QgsUnitTypes::LayoutMillimeters));
						*/
						
						// 100km刻度值
						pLeftBorderLabel_100km->attemptSetSceneRect(QRectF(layoutLeftBorderPoint.dx - 5.85,
							layoutLeftBorderPoint.dy - 3.3,
							3.5,
							3.5));
...
#pragma region "绘制右边界方里网数值"
			...
			/*绘制内图廓右边界整公里刻度值*/
			for (int iPointIndex = 0; iPointIndex < vRightBorderLayoutCoords.size(); iPointIndex++)
			{
				// 内图廓右边界整公里刻度点布局坐标值
				SE_DPoint layoutRightBorderPoint = vRightBorderLayoutCoords[iPointIndex];

				// 内图廓右边界整公里高斯坐标值
				SE_DPoint layoutRightBorderGaussPoint = vRightBorderGaussCoords[iPointIndex];

				/*如果方里网刻度线与上、下内图廓Y坐标值差在KM_LINE_TOLERANCE_Y毫米内，不绘制刻度值*/
				if (fabs(layoutRightBorderPoint.dy - rightTopCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y
					|| fabs(layoutRightBorderPoint.dy - rightBottomCornerLayoutPoint.dy) < KM_LINE_TOLERANCE_Y)
				{
					continue;
				}
				else
				{
					...
					/*pRightBorderLabel_10km->adjustSizeToText();
					pRightBorderLabel_10km->attemptMove(QgsLayoutPoint(layoutRightBorderPoint.dx + 1.3, layoutRightBorderPoint.dy - 3.5, QgsUnitTypes::LayoutMillimeters));
					*/

					// 10km刻度值
					pRightBorderLabel_10km->attemptSetSceneRect(QRectF(layoutRightBorderPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE - 3.9,
						...
						iPointIndex == iEndMarkIndex_Y)
					{
						...
						/*pRightBorderLabel_100km->adjustSizeToText();
						pRightBorderLabel_100km->attemptMove(QgsLayoutPoint(layoutRightBorderPoint.dx + 0.05, layoutRightBorderPoint.dy - 3.5, QgsUnitTypes::LayoutMillimeters));
						*/
						
						// 100km刻度值
						pRightBorderLabel_100km->attemptSetSceneRect(QRectF(layoutRightBorderPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE - 5.85,
							layoutRightBorderPoint.dy - 3.3,
							3.5,
							3.5));
...
#pragma region "绘制邻带方里网"
			...
			if (iAdjSheetType == 2)
			{
				...
				/*将上边界邻带投影点Y坐标赋值为地图投影上边界Y坐标+内外图廓宽度*/
				for (int iPointIndex = 0; iPointIndex < vTopBorderAdjCurrentBandGaussCoords.size(); iPointIndex++)
				{
					vTopBorderAdjCurrentBandGaussCoords[iPointIndex].dy = leftTopCornerGaussPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				}

				/*将下边界邻带投影点Y坐标赋值为地图投影下边界Y坐标-内外图廓宽度*/
				for (int iPointIndex = 0; iPointIndex < vBottomBorderAdjCurrentBandGaussCoords.size(); iPointIndex++)
				{
					vBottomBorderAdjCurrentBandGaussCoords[iPointIndex].dy = leftBottomCornerGaussPoint.dy - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				}

				/*将左边界邻带投影点X坐标赋值为地图投影左边界X坐标-内外图廓宽度*/
				for (int iPointIndex = 0; iPointIndex < vLeftBorderAdjCurrentBandGaussCoords.size(); iPointIndex++)
				{
					vLeftBorderAdjCurrentBandGaussCoords[iPointIndex].dx = leftTopCornerGaussPoint.dx - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				}

				/*将右边界邻带投影点X坐标赋值为地图投影右边界X坐标+内外图廓宽度*/
				for (int iPointIndex = 0; iPointIndex < vRightBorderAdjCurrentBandGaussCoords.size(); iPointIndex++)
				{
					vRightBorderAdjCurrentBandGaussCoords[iPointIndex].dx = rightTopCornerGaussPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				}
...
#pragma region "将邻带方里网上、下、左、右边界刻度值转化为布局坐标"
				...
				/*邻带方里网上边界刻度转布局坐标*/
				// 地图外图廓左上角高斯坐标
				SE_DPoint outerLeftTopCornerGaussPoint;
				outerLeftTopCornerGaussPoint.dx = leftTopCornerGaussPoint.dx - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				outerLeftTopCornerGaussPoint.dy = leftTopCornerGaussPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;

				// 地图外图廓左下角高斯坐标
				SE_DPoint outerLeftBottomCornerGaussPoint;
				outerLeftBottomCornerGaussPoint.dx = leftTopCornerGaussPoint.dx - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				outerLeftBottomCornerGaussPoint.dy = leftBottomCornerGaussPoint.dy - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;

				// 地图外图廓右上角高斯坐标
				SE_DPoint outerRightTopCornerGaussPoint;
				outerRightTopCornerGaussPoint.dx = rightTopCornerGaussPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				outerRightTopCornerGaussPoint.dy = leftTopCornerGaussPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;

				// 地图外图廓右下角高斯坐标
				SE_DPoint outerRightBottomCornerGaussPoint;
				outerRightBottomCornerGaussPoint.dx = rightTopCornerGaussPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;
				outerRightBottomCornerGaussPoint.dy = leftBottomCornerGaussPoint.dy - OUTER_MAP_INTERIOR_MAP_DISTANCE / dMapUnitToLayoutUnit;

				/*获取地图外图廓四个角点布局坐标*/
				/*地图图廓布局尺寸*/
				/*获取外图廓元素*/
				QgsLayoutItemShape* pRectOutline = (QgsLayoutItemShape*)GetLayoutItemByName(qLayoutItemList, "Rectangle_Outline");

				QRectF OutlineItemLayoutRect = pRectOutline->rect();

				// 布局坐标，左上角为（0,0）点，从左往右依次增加，从上往下依次增加

				// 外图廓左上角布局坐标值
				SE_DPoint outerLeftTopCornerLayoutPoint;
				outerLeftTopCornerLayoutPoint.dx = pRectOutline->pagePositionWithUnits().x();
				outerLeftTopCornerLayoutPoint.dy = pRectOutline->pagePositionWithUnits().y();

				// 外图廓左下角布局坐标值
				SE_DPoint outerLeftBottomCornerLayoutPoint;
				outerLeftBottomCornerLayoutPoint.dx = pRectOutline->pagePositionWithUnits().x();
				outerLeftBottomCornerLayoutPoint.dy = pRectOutline->pagePositionWithUnits().y() + OutlineItemLayoutRect.height();

				// 外图廓右上角布局坐标值
				SE_DPoint outerRightTopCornerLayoutPoint;
				outerRightTopCornerLayoutPoint.dx = pRectOutline->pagePositionWithUnits().x() + OutlineItemLayoutRect.width();
				outerRightTopCornerLayoutPoint.dy = pRectOutline->pagePositionWithUnits().y();

				// 外图廓右下角布局坐标值
				SE_DPoint outerRightBottomCornerLayoutPoint;
				outerRightBottomCornerLayoutPoint.dx = pRectOutline->pagePositionWithUnits().x() + OutlineItemLayoutRect.width();;
				outerRightBottomCornerLayoutPoint.dy = pRectOutline->pagePositionWithUnits().y() + OutlineItemLayoutRect.height();

				// 外图廓左边界布局点坐标
				vector<SE_DPoint> vOuterLeftBorderLayoutCoords;
				vOuterLeftBorderLayoutCoords.clear();

				// 外图廓右边界布局点坐标
				vector<SE_DPoint> vOuterRightBorderLayoutCoords;
				vOuterRightBorderLayoutCoords.clear();

				// 外图廓上边界布局点坐标
				vector<SE_DPoint> vOuterTopBorderLayoutCoords;
				vOuterTopBorderLayoutCoords.clear();

				// 外图廓下边界布局点坐标
				vector<SE_DPoint> vOuterBottomBorderLayoutCoords;
				vOuterBottomBorderLayoutCoords.clear();

				/*计算上、下、左、右边界点在布局中的位置，mm为单位,页面左上角为（0,0）*/
				CalLayoutBorderPoints(
					outerLeftTopCornerGaussPoint,
					outerLeftTopCornerLayoutPoint,
					outerLeftBottomCornerGaussPoint,
					outerLeftBottomCornerLayoutPoint,
					outerRightTopCornerGaussPoint,
					outerRightTopCornerLayoutPoint,
					outerRightBottomCornerGaussPoint,
					outerRightBottomCornerLayoutPoint,
					dMapUnitToLayoutUnit,
					vLeftBorderAdjCurrentBandGaussCoords,
					vTopBorderAdjCurrentBandGaussCoords,
					vRightBorderAdjCurrentBandGaussCoords,
					vBottomBorderAdjCurrentBandGaussCoords,
					vOuterLeftBorderLayoutCoords,
					vOuterTopBorderLayoutCoords,
					vOuterRightBorderLayoutCoords,
					vOuterBottomBorderLayoutCoords);

				/*绘制邻带方里网刻度*/
				DrawAdjKmNet(pPrintLayout,
					vOuterLeftBorderLayoutCoords,
					vOuterTopBorderLayoutCoords,
					vOuterRightBorderLayoutCoords,
					vOuterBottomBorderLayoutCoords,
...
#pragma region "镶嵌线地图设置"
		...
		pMosaicMapItem->setVisibility(true);
			pMosaicMapItem->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + 61,
				m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3,
				64,
				61));
...
#pragma region "设置外图廓上下左右四个角的经纬度值"
		/*------------------------------------*/
		/* 设置外图廓上下左右四个角的经纬度值 */
		/*------------------------------------*/
		/*将角点经纬度转换为DMS格式*/
...
#pragma region "地图左下角经纬度"
//---------------------------------------------//
/*TextItem_LeftBottomLatSecond	地图左下角纬度-秒
TextItem_LeftBottomLatDegree	地图左下角纬度-度
TextItem_LeftBottomLatMinute	地图左下角纬度-分
TextItem_LeftBottomLonSecond	地图左下角经度-秒
TextItem_LeftBottomLonMinute	地图左下角经度-分
TextItem_LeftBottomLonDegree	地图左下角经度-度
*/
...
//--------------------------------------------//
		...
		TextItem_LeftBottomLatSecond->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height()));

		// 自动调整尺寸
		TextItem_LeftBottomLatSecond->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLatSecond->setHAlign(Qt::AlignRight);
		...
		// 自动调整尺寸
		TextItem_LeftBottomLatMinute->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLatMinute->setHAlign(Qt::AlignRight);
		...
		TextItem_LeftBottomLatDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() - TextItem_LeftBottomLatMinute->rect().height()));

		// 自动调整尺寸
		TextItem_LeftBottomLatDegree->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLatDegree->setHAlign(Qt::AlignRight);
...
#pragma endregion
		...
		TextItem_LeftBottomLonDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() + TextItem_LeftBottomLatSecond->rect().height()));

		// 自动调整尺寸
		TextItem_LeftBottomLonDegree->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLonDegree->setHAlign(Qt::AlignRight);
		...
		// 自动调整尺寸
		TextItem_LeftBottomLonMinute->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLonMinute->setHAlign(Qt::AlignRight);
		...
		TextItem_LeftBottomLonSecond->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + TextItem_LeftBottomLonMinute->rect().width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() + TextItem_LeftBottomLatSecond->rect().height()));

		// 自动调整尺寸
		TextItem_LeftBottomLonSecond->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		//TextItem_LeftBottomLonSecond->setHAlign(Qt::AlignRight);
#pragma endregion


#pragma region "地图左上角经纬度"
		/*TextItem_LeftTopLatDegree	地图左上角纬度 - 度
			TextItem_LeftTopLatSecond	地图左上角纬度 - 秒
			TextItem_LeftTopLatMinute	地图左上角纬度 - 分
			TextItem_LeftTopLonSecond	地图左上角经度 - 秒
			TextItem_LeftTopLonMinute	地图左上角经度 - 分
			TextItem_LeftTopLonDegree	地图左上角经度 - 度*/
			//---------------------------------------------//
			// 地图左上角纬度 - 度
		...
		TextItem_LeftTopLatDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE));

		// 自动调整尺寸
		TextItem_LeftTopLatDegree->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLatDegree->setHAlign(Qt::AlignRight);
		...
		// 自动调整尺寸
		TextItem_LeftTopLatMinute->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLatMinute->setHAlign(Qt::AlignRight);
		...
		TextItem_LeftTopLatSecond->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + TextItem_LeftTopLatMinute->rect().height()));

		// 自动调整尺寸
		TextItem_LeftTopLatSecond->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLatSecond->setHAlign(Qt::AlignRight);
		...
		TextItem_LeftTopLonDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE - TextItem_LeftTopLatDegree->rect().height()));

		// 自动调整尺寸
		TextItem_LeftTopLonDegree->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLonDegree->setHAlign(Qt::AlignRight);
		...
		// 自动调整尺寸
		TextItem_LeftTopLonMinute->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLonMinute->setHAlign(Qt::AlignLeft);
		...
		TextItem_LeftTopLonSecond->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + TextItem_LeftTopLonMinute->rect().width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE - TextItem_LeftTopLatDegree->rect().height()));

		// 自动调整尺寸
		TextItem_LeftTopLonSecond->adjustSizeToText();

		// 设置水平对齐方式-右对齐
		TextItem_LeftTopLonSecond->setHAlign(Qt::AlignLeft);
#pragma endregion



#pragma region "地图右下角经纬度"
		//---------------------------------------------//

		/*TextItem_RightBottomLatSecond	地图右下角纬度-秒
		TextItem_RightBottomLatDegree	地图右下角纬度-度
		TextItem_RightBottomLatMinute	地图右下角纬度-分
		TextItem_RightBottomLonSecond	地图右下角经度-秒
		TextItem_RightBottomLonMinute	地图右下角经度-分
		TextItem_RightBottomLonDegree	地图右下角经度-度
		*/
		...
		TextItem_RightBottomLatMinute->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height()));

		// 自动调整尺寸
		TextItem_RightBottomLatMinute->adjustSizeToText();
		...
		// 自动调整尺寸
		TextItem_RightBottomLatSecond->adjustSizeToText();
		...
		TextItem_RightBottomLatDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() - TextItem_RightBottomLatMinute->rect().height()));

		// 自动调整尺寸
		TextItem_RightBottomLatDegree->adjustSizeToText();
		...
		TextItem_RightBottomLonMinute->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() + TextItem_RightBottomLatSecond->rect().height()));

		// 自动调整尺寸
		TextItem_RightBottomLonMinute->adjustSizeToText();
		//--------------------------------------------//
		// 地图右下角经度 - 秒
		...
		// 自动调整尺寸
		TextItem_RightBottomLonSecond->adjustSizeToText();
		...
		TextItem_RightBottomLonDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - TextItem_RightBottomLonMinute->rect().width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height() + TextItem_RightBottomLatSecond->rect().height()));

		// 自动调整尺寸
		TextItem_RightBottomLonDegree->adjustSizeToText();
#pragma endregion

#pragma region "地图右上角经纬度"
		//---------------------------------------------//

		/*TextItem_RightTopLatSecond	地图右上角纬度-秒
		TextItem_RightTopLatMinute	地图右上角纬度-分
		TextItem_RightTopLatDegree	地图右上角纬度-度
		TextItem_RightTopLonSecond	地图右上角经度-秒
		TextItem_RightTopLonMinute	地图右上角经度-分
		TextItem_RightTopLonDegree	地图右上角经度-度
		*/
		//---------------------------------------------//
		// 地图右上角纬度 - 度
		...
		// 自动调整尺寸
		TextItem_RightTopLatDegree->adjustSizeToText();
		//--------------------------------------------//
		// 地图右上角纬度 - 分
		...
		// 自动调整尺寸
		TextItem_RightTopLatMinute->adjustSizeToText();
		...
		TextItem_RightTopLatSecond->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + TextItem_RightTopLatMinute->rect().height()));

		// 自动调整尺寸
		TextItem_RightTopLatSecond->adjustSizeToText();
		...
		TextItem_RightTopLonMinute->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE - TextItem_RightTopLatDegree->rect().height()));

		// 自动调整尺寸
		TextItem_RightTopLonMinute->adjustSizeToText();
		//--------------------------------------------//
		// 地图右上角经度 - 秒
		...
		// 自动调整尺寸
		TextItem_RightTopLonSecond->adjustSizeToText();
		...
		TextItem_RightTopLonDegree->attemptMove(QgsLayoutPoint(
			m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - TextItem_RightTopLonMinute->rect().width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE - TextItem_RightTopLatDegree->rect().height()));

		// 自动调整尺寸
		TextItem_RightTopLonDegree->adjustSizeToText();
		//--------------------------------------------//
#pragma endregion

#pragma endregion

#pragma region "设置左上角、右下角图幅号"
...
#pragma region "根据布局整饰要素名称及制图方案设置相应名称"
		// 根据布局整饰要素名称及制图方案设置相应名称，如制图单位
		for (j = 0; j < qLayoutItemList.size(); j++)
		{
			QgsLayoutItem* pItem = qLayoutItemList[j];
			QString qstrName = pItem->displayName();

			// 【1】地图标题-图幅号TextItem_MapTitleSheet
 			if (qstrName == "TextItem_MapTitleSheet")
			{
				((QgsLayoutItemLabel*)pItem)->setText(qstrSheet);
			}

			// 【2】地图标题-地图名称 TextItem_MapTitleName
			else if (qstrName == "TextItem_MapTitleName")
			{
				// 从SMS文件中读取“图名”
				QString qstrMapTitleName = QString::fromLocal8Bit(strMapName.c_str());
				((QgsLayoutItemLabel*)pItem)->setText(qstrMapTitleName);
			}

			// 【3】地图副标题-省名称 TextItem_MapSubTileProvince
			else if (qstrName == "TextItem_MapSubTileProvince")
			{
				// 从SMS文件中读取“政区说明”
				QString qstrMapSubTitleProvince = QString::fromLocal8Bit(strMapSubTitleProvince.c_str());
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMapSubTitleChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(qstrMapSubTitleProvince);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
					((QgsLayoutItemLabel*)pItem)->setVisibility(false);
				}
			}

			// 【4】地图副标题-县名称 TextItem_MapSubTitleCounty
			/*else if (qstrName == "TextItem_MapSubTitleCounty")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMapSubTitleChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrMapSubTitleCounty);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【5】制图单位 TextItem_MappingAgency
			else if (qstrName == "TextItem_MappingAgency")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMapAgencyChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrMapAgency);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);

					// 设置位置
					((QgsLayoutItemLabel*)pItem)->setReferencePoint(QgsLayoutItem::Middle);

					// 设置参考点坐标
					((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
						m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2.0,
						m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 27.667));

					// 自动调整尺寸
					((QgsLayoutItemLabel*)pItem)->adjustSizeToText();

					// 设置水平对齐方式
					((QgsLayoutItemLabel*)pItem)->setHAlign(Qt::AlignHCenter);
			...
			// 【6】地图左上角图幅号 TextItem_LeftTopSheet
			else if (qstrName == "TextItem_LeftTopSheet")
			{
				// 如果是绘制状态
			...
			// 【7】地图右下角图幅号 TextItem_RightBottomSheet
			else if (qstrName == "TextItem_RightBottomSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bRightBottomCornerSheetChecked)
				{
					...
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
					// 设置位置
					((QgsLayoutItemLabel*)pItem)->setReferencePoint(QgsLayoutItem::LowerRight);

					// 设置参考点坐标
					((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
						m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() + 49,
						m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 30));

					// 自动调整尺寸
					((QgsLayoutItemLabel*)pItem)->adjustSizeToText();

					// 设置水平对齐方式-右对齐
					((QgsLayoutItemLabel*)pItem)->setHAlign(Qt::AlignLeft);
			...
			// 【8】主地图图例 Legend_MainMap
			else if (qstrName == "Legend_MainMap")
			{
				/*QgsLayoutItemLegend* pMainLegend = (QgsLayoutItemLegend*)pItem;
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMainMapLegendChecked)
				{
					pMainLegend->setLinkedMap(pMainMapItem);
					pMainLegend->setVisibility(true);
					// TODO：需要调试
					//pMainLegend->setAutoUpdateModel(true);
				}
				else
				{
					pMainLegend->setVisibility(false);
				}*/
			}

			// 【9】镶嵌线地图图例 Legend_Mosaic
			else if (qstrName == "Legend_Mosaic")
			{
				/*QgsLayoutItemLegend* pMosaicLegend = (QgsLayoutItemLegend*)pItem;
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMosaicMapLegendChecked)
				{
					pMosaicLegend->setLinkedMap(pMosaicMapItem);
					pMosaicLegend->setVisibility(true);
				}
				else
				{
					pMosaicLegend->setVisibility(false);
				}*/
			}

			// 【10】比例尺 PictureItem_Scale，使用svg比例尺表示
			else if (qstrName == "PictureItem_Scale")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bScaleChecked)
				{
					// 设置图片模式
					((QgsLayoutItemPicture*)pItem)->setMode(QgsLayoutItemPicture::FormatSVG);
					...
					// 设置图片路径
					((QgsLayoutItemPicture*)pItem)->setPicturePath(qstrSvgPath);
					((QgsLayoutItemPicture*)pItem)->setResizeMode(QgsLayoutItemPicture::Zoom);
					((QgsLayoutItemPicture*)pItem)->setVisibility(true);
					...
					((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
						m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2.0,
						m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 15));
				...
				{
					((QgsLayoutItemScaleBar*)pItem)->setVisibility(false);
				}
			}

			// 【11】接图表中间方框内名称
			else if (qstrName == "TextItem_CenterName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrMapTitleName);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【12】接图表中间方框内图幅号
			else if (qstrName == "TextItem_CenterSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
			...
			// 【13】接图表左边方框内名称-根据当前图幅左边图幅号获取对应矢量图层元数据信息，
			// 从SMS文件中读取邻接图幅图名
			else if (qstrName == "TextItem_LeftName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// TODO:计算当前图幅左边相邻图幅号，从对应图幅元数据中获取图幅名
					((QgsLayoutItemLabel*)pItem)->setText("");
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【14】接图表左边方框内图幅号
			else if (qstrName == "TextItem_LeftSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// 计算左边图幅名称
					((QgsLayoutItemLabel*)pItem)->setText(qstrLeftSheet);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【15】接图表右边方框内名称-根据当前图幅右边图幅号获取对应矢量图层元数据信息，
			// 从SMS文件中读取邻接图幅图名
			else if (qstrName == "TextItem_RightName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// TODO:计算当前图幅右边相邻图幅号，从对应图幅元数据中获取图幅名
					((QgsLayoutItemLabel*)pItem)->setText("");
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
			...
			// 【16】接图表右边方框内图幅号
			else if (qstrName == "TextItem_RightSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// 计算右边图幅号
					((QgsLayoutItemLabel*)pItem)->setText(qstrRightSheet);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【17】接图表上边方框内名称-根据当前图幅上边图幅号获取对应矢量图层元数据信息，
			// 从SMS文件中读取邻接图幅图名
			else if (qstrName == "TextItem_TopName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// TODO:计算当前图幅上边相邻图幅号，从对应图幅元数据中获取图幅名
					((QgsLayoutItemLabel*)pItem)->setText("");
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
			...
			// 【18】接图表上边方框内图幅号
			else if (qstrName == "TextItem_TopSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// 计算上边界图幅号
					((QgsLayoutItemLabel*)pItem)->setText(qstrTopSheet);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【19】接图表下边方框内名称-根据当前图幅下边图幅号获取对应矢量图层元数据信息，
			// 从SMS文件中读取邻接图幅图名
			else if (qstrName == "TextItem_BottomName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// TODO:计算当前图幅下边相邻图幅号，从对应图幅元数据中获取图幅名
					((QgsLayoutItemLabel*)pItem)->setText("");
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
			...
			// 【20】接图表下边方框内图幅号
			else if (qstrName == "TextItem_BottomSheet")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
					// 计算当前图幅下边相邻图幅号
					((QgsLayoutItemLabel*)pItem)->setText(qstrBottomSheet);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【21】接图表左下方方框内图名
			else if (qstrName == "TextItem_LeftBottomName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
			...
			// 【22】接图表左上方方框内图名
			else if (qstrName == "TextItem_LeftTopName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
			...
			// 【23】接图表右下方方框内图名
			else if (qstrName == "TextItem_RightBottomName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
			...
			// 【24】接图表右上方方框内图名
			else if (qstrName == "TextItem_RightTopName")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bSheetListChecked)
				{
			...
			// 【25】地图说明
			else if (qstrName == "TextItem_MapDetail")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bMapDetailsChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrMapDetails);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);

					// 调整尺寸
					((QgsLayoutItemLabel*)pItem)->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE,
						m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3,
						60,
						27));
				}
			...
			// 【26】地图附注
			else if (qstrName == "TextItem_Annotations")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bAnnotationsChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrMapAnnotations);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				}
				else
				{
			...
			// 【27】文字比例尺
			else if (qstrName == "TextItem_Scale")
			{
				// 如果是绘制状态
				if (m_ImageMappingSchema.layoutItemParam.bScaleChecked)
				{
					((QgsLayoutItemLabel*)pItem)->setText(m_ImageMappingSchema.layoutItemParam.qstrScale);
					((QgsLayoutItemLabel*)pItem)->setVisibility(true);

					// 设置位置
					((QgsLayoutItemLabel*)pItem)->setReferencePoint(QgsLayoutItem::LowerMiddle);

					// 设置参考点坐标
					((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
						m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2.0,
						m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 9));
			...
			// 【28】黑色圆点
			else if (qstrName == "Ellipse_Black")
			{
				((QgsLayoutItemLabel*)pItem)->setVisibility(true);
				...
				// 设置参考点坐标
				((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
					m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2.0 - 12,
					m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 21.5));
			...
			else if (qstrName == "Ellipse_Blue")
			{
				...
				// 设置参考点坐标
				((QgsLayoutItemLabel*)pItem)->attemptMove(QgsLayoutPoint(
					m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2.0 - 4,
					m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 21.5));
			...
			// 【31】黄色圆点
			else if (qstrName == "Ellipse_Yellow")
			{
				((QgsLayoutItemLabel*)pItem)->setVisibility(true);
...
#pragma region "更新地图标题-图幅号位置"
		...
		// 设置参考点坐标
		pTextItem_MapTitleSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + textItem_MapTitleSheet.dTop_offset));
...
#pragma region "更新地图标题-图幅名称位置"
		...
		// 设置参考点坐标
		pTextItem_MapTitleName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + textItem_MapTitleName.dTop_offset));
...
#pragma region "更新地图副标题-政区说明"
		...
		// 设置参考点坐标
		pTextItem_MapSubTileProvince->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() / 2,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + textItem_MapSubTileProvince.dTop_offset));
...
#pragma region "设置内外图廓之间四个角的矩形大小及位置"
		
		//---------------------------------------//
		// 左上角矩形
		QgsLayoutItemShape *pLeftTopRectangle = (QgsLayoutItemShape*)GetLayoutItemByName(
			qLayoutItemList,
			"Rectangle_LeftTop");

		pLeftTopRectangle->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx,
			m_MapOutlineLeftTopCornerLayoutPoint.dy,
			OUTER_MAP_INTERIOR_MAP_DISTANCE,
			OUTER_MAP_INTERIOR_MAP_DISTANCE));
		...
		//---------------------------------------//
		// 左下角矩形
		QgsLayoutItemShape* pLeftBottomRectangle = (QgsLayoutItemShape*)GetLayoutItemByName(
			qLayoutItemList,
			"Rectangle_LeftBottom");

		pLeftBottomRectangle->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height(),
			OUTER_MAP_INTERIOR_MAP_DISTANCE,
			OUTER_MAP_INTERIOR_MAP_DISTANCE));
		...
		//---------------------------------------//
		// 右上角矩形
		QgsLayoutItemShape* pRightTopRectangle = (QgsLayoutItemShape*)GetLayoutItemByName(
			qLayoutItemList,
			"Rectangle_RightTop");

		pRightTopRectangle->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy,
			OUTER_MAP_INTERIOR_MAP_DISTANCE,
			OUTER_MAP_INTERIOR_MAP_DISTANCE));
		...
		//---------------------------------------//
		// 右下角矩形
		QgsLayoutItemShape* pRightBottomRectangle = (QgsLayoutItemShape*)GetLayoutItemByName(
			qLayoutItemList,
			"Rectangle_RightBottom");

		pRightBottomRectangle->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.height(),
			OUTER_MAP_INTERIOR_MAP_DISTANCE,
			OUTER_MAP_INTERIOR_MAP_DISTANCE));
...
#pragma region "更新密级位置"
		...
		// 设置参考点坐标
		pTextItem_SecurityClassification->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy - 3));
...
#pragma region "图例标题"
		...
		// 设置参考点坐标
		pTextItem_MainMapLegend->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() + 3,
			m_MapOutlineLeftTopCornerLayoutPoint.dy));
...
#pragma region "左上接图表"
		...
		pTextItem_LeftTopName->setFrameEnabled(true);
		// 设置参考点位置
		pTextItem_LeftTopName->setReferencePoint(QgsLayoutItem::UpperRight);

		// 设置参考点坐标
		pTextItem_LeftTopName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx+ OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26 - 28,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3));
...
#pragma region "左中边框"
		...
		// 设置参考点坐标
		pTextItem_LeftRectangle->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26 - 28,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));
...
#pragma region "左中图幅号"
		...
		// 设置参考点坐标
		pTextItem_LeftSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26 - 28,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));

		// 自动调整尺寸
		pTextItem_LeftSheet->adjustSizeToText();
#pragma endregion
...
#pragma region "左中图名"
		...
		// 设置参考点坐标
		pTextItem_LeftName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26 - 28,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 4.5));

		// 自动调整尺寸
		pTextItem_LeftName->adjustSizeToText();
#pragma endregion
...
#pragma region "左下接图表"
		...
		// 设置参考点坐标
		pTextItem_LeftBottomName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26 - 28,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 9));
...
#pragma region "中上边框"
		...
		// 设置参考点坐标
		pTextItem_TopRectangle->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3));
...
#pragma region "中上图幅号"
		...
		// 设置参考点坐标
		pTextItem_TopSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3));

		// 自动调整尺寸
		pTextItem_TopSheet->adjustSizeToText();
#pragma endregion
...
#pragma region "中上图名"
		...
		// 设置参考点坐标
		pTextItem_TopName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 4.5));

		// 自动调整尺寸
		pTextItem_TopName->adjustSizeToText();
#pragma endregion
...
#pragma region "正中图框"
		...
		// 设置参考点坐标
		pTextItem_CenterRectangle->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));
...
#pragma region "正中图幅名"
		...
		// 设置参考点坐标
		pTextItem_CenterSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));

		// 自动调整尺寸
		pTextItem_CenterSheet->adjustSizeToText();
#pragma endregion	
...
#pragma region "正中图名"
		...
		// 设置参考点坐标
		pTextItem_CenterName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 4.5));

		// 自动调整尺寸
		pTextItem_CenterName->adjustSizeToText();
#pragma endregion
#pragma endregion
...
#pragma region "中下图框"
		...
		// 设置参考点坐标
		pTextItem_BottomRectangle->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 9));
...
#pragma region "中下图幅名"
		...
		// 设置参考点坐标
		pTextItem_BottomSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 9));

		// 自动调整尺寸
		pTextItem_BottomSheet->adjustSizeToText();
#pragma endregion
...
#pragma region "中下图名"
		...
		// 设置参考点坐标
		pTextItem_BottomName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width() - 26,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 9 + 4.5));

		// 自动调整尺寸
		pTextItem_BottomName->adjustSizeToText();
#pragma endregion
...
#pragma region "右上接图表"
		...
		// 设置参考点坐标
		pTextItem_RightTopName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3));
...
#pragma region "右中图框"
		...
		// 设置参考点坐标
		pTextItem_RightRectangle->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));
...
#pragma region "右中图幅名"
		...
		// 设置参考点坐标
		pTextItem_RightSheet->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9));

		// 自动调整尺寸
		pTextItem_RightSheet->adjustSizeToText();
#pragma endregion
...
#pragma region "右中图名"
		...
		// 设置参考点坐标
		pTextItem_RightName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 4.5));

		// 自动调整尺寸
		pTextItem_RightName->adjustSizeToText();
#pragma endregion
...
#pragma region "右下接图表"
		...
		// 设置参考点坐标
		pTextItem_RightBottomName->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + OUTER_MAP_INTERIOR_MAP_DISTANCE + m_qMainMapItemLayoutRect.width(),
			m_MapOutlineLeftTopCornerLayoutPoint.dy + m_qMapOutLineItemLayoutRect.height() + 3 + 9 + 9));
...
#pragma region "图例外框"
		QgsLayoutItemShape* pRectangle_MainMapLegend = (QgsLayoutItemShape*)GetLayoutItemByName(qLayoutItemList,
			"Rectangle_MainMapLegend");

		// TODO:高度80需要根据实际图例高度进行设置
		pRectangle_MainMapLegend->attemptSetSceneRect(QRectF(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() + 3,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE,
			39,
			80));
...
#pragma region MyRegion "附注"

		
		// 附注
		QgsLayoutItemLabel* pTextItem_Annotations = (QgsLayoutItemLabel*)GetLayoutItemByName(qLayoutItemList,
				"TextItem_Annotations");

		// 设置参考点位置
		pTextItem_Annotations->setReferencePoint(QgsLayoutItem::UpperLeft);

		// 设置参考点坐标
		pTextItem_Annotations->attemptMove(QgsLayoutPoint(m_MapOutlineLeftTopCornerLayoutPoint.dx + m_qMapOutLineItemLayoutRect.width() + 3,
			m_MapOutlineLeftTopCornerLayoutPoint.dy + OUTER_MAP_INTERIOR_MAP_DISTANCE + pRectangle_MainMapLegend->rect().height() + 15));

		// 自动调整尺寸
		pTextItem_Annotations->adjustSizeToText();

#pragma endregion



#pragma region "导出地图设置"
		/*【4】每个图幅输出工程文件、元数据文件、地图文件（pdf格式或tiff格式）*/
		...
		double width = 800;     // 单位：毫米
		double height = 650;    // 单位：毫米
...
#define MODE (S_IRWXU | S_IRWXG | S_IRWXO)
		...
		// 设置进度条
		ui.progressBar->setValue(100.0 * (i + 1) / vMapSheetList.size());
	}
...
void QgsImageMappingDialog::OpenVectorData()
{
	// 选择文件夹
...
void QgsImageMappingDialog::OpenImageData()
{
	// 选择文件夹
...
void QgsImageMappingDialog::OpenMosaicData()
{
	// 选择文件夹
...
void QgsImageMappingDialog::ImageMappingDataSave()
{
	// 选择文件夹
...
void QgsImageMappingDialog::UpdateUIByImageMappingSchema(const QModelIndex& index)
{
	//点击获取当前索引index的内容
...
void QgsImageMappingDialog::LonLatRadioButton_Click()
{
	ui.lineEdit_Left->setEnabled(true);
	ui.lineEdit_Top->setEnabled(true);
	ui.lineEdit_Right->setEnabled(true);
...
void QgsImageMappingDialog::SheetListRadioButton_Click()
{
...
QgsLayoutItem* QgsImageMappingDialog::GetLayoutItemByName(QList<QgsLayoutItem*>& itemList, const QString qstrName)
{
...
QString QgsImageMappingDialog::GetMainTitleSheet()
{
...
QString QgsImageMappingDialog::GetMainTitleName()
{
...
QString QgsImageMappingDialog::GetSubTitleProvinceName()
{
...
QString QgsImageMappingDialog::GetSubTitleCountyName()
{
...
QString QgsImageMappingDialog::GetSecurityClassification()
{
...
void QgsImageMappingDialog::SetListWidgetAllUnSelectedStatus(const QString& strLayerName)
{
	// D、F、J、K、R
	// 存储list控件
...
// 初始化要素类名称和编码的映射表
void QgsImageMappingDialog::InitFeatureClassNameCodeTable()
{
	// 获取映射表配置参数
...
// 获取图层字段选择状态
void QgsImageMappingDialog::GetLayerFeatureClassChecked(string strLayerType, vector<string>& vFeatureClassCode)
{
	// D、F、J、K、R
...
void QgsImageMappingDialog::InitImageMappingSchema()
{
	// 获取运行环境路径
...
QStringList QgsImageMappingDialog::GetFileNamesByExtName(const QString& path, const QString& extName)
{
	QDir dir(path);
...
bool QgsImageMappingDialog::LoadImageMappingSchemaXml(const QString& xmlPath, vector<ImageMappingSchema>& vSchema)
{
	QDomDocument doc;
	QFile file(xmlPath);
...
void QgsImageMappingDialog::SetUIByImageMappingSchema(const ImageMappingSchema& schema)
{
	// 获取运行环境路径
	QString qstrExePath = QCoreApplication::applicationDirPath();
	QString qstrSchemaPath = qstrExePath + "/layout/schema";
...
// 设置各图层要素类列表框状态
void QgsImageMappingDialog::SetListWidgetStatus(FeatureClassParam& param)
{
	// 图层：D、F、J、K、R
	// 存储list控件
	vector<QListWidget*> vQListWidget;
...
void QgsImageMappingDialog::GetListWidgetStatus(const QString& qstrLayerType, FeatureClassParam& param)
{
...
// 判断列表中元素是否在要素类列表中
bool QgsImageMappingDialog::bIsInFeatureClasses(QString qstrText, vector<QString>& vFeatureClassCode)
{
...
bool QgsImageMappingDialog::ExportMapToFile(
...
void QgsImageMappingDialog::ConvertMilliMeterToPixel(double w, double h, int dpi, int& x, int& y)
{
	// x,y 为像素数，w,h为毫米
	// 水平方向：x = w * dpi / 25.4
	// 垂直方向：y = h * dpi / 25.4
...
void QgsImageMappingDialog::ConvertPixeltoMilliMeter(int x, int y, int dpi, double& w, double& h)
{
	// x, y 为像素数，w, h为毫米
	// 水平方向：w = x * 25.4 / dpi
	// 垂直方向：h = y * 25.4 / dpi
...
void QgsImageMappingDialog::restoreState()
{
	const QgsSettings settings;
	m_qstrImageMappingDataSavePath = settings.value(QStringLiteral("QgsImageMappingDialog/ImageMappingDataSavePath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrImageMappingSchemaSavePath = settings.value(QStringLiteral("QgsImageMappingDialog/ImageMappingSchemaSavePath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();

}

void QgsImageMappingDialog::GetSheetListByMappingRange(const QString& qstrTemplateName,
	double dLeft,
	double dTop,
	double dRight,
	double dBottom,
...
void QgsImageMappingDialog::GetVectorLayerPathList(const ImageMappingSchema& schema,
...
double QgsImageMappingDialog::CalCenterMeridian(double dLong, double dScale)
{
...
void QgsImageMappingDialog::GetMBR(int iCount, double* pPoints, double& dLeft, double& dTop, double& dRight, double& dBottom)
{
	double dMinX = 1e20;
	double dMaxX = -1e20;
	double dMinY = 1e20;
...
QString QgsImageMappingDialog::GetEPSGCodeByCenterMeridianAndScale(double dLong, double dScale)
{
...
void QgsImageMappingDialog::Degree2DMS(double degree, int& iDegree, int& iMinute, int& iSecond)
{
	iDegree = (int)degree;
...
bool QgsImageMappingDialog::ExportImageMetaData(const QString& filePath,
...
QString QgsImageMappingDialog::FormatDegree(int iDegree)
{
	QString qstrDegree;
	if (iDegree == 0)
	{
...
QString QgsImageMappingDialog::FormatDegree2(int iDegree)
{
	QString qstrDegree;
	if (iDegree == 0)
	{
...
QString QgsImageMappingDialog::FormatMinute(int iMinute)
{
	QString qstrMinute;
	if (iMinute == 0)
	{
...
QString QgsImageMappingDialog::FormatMinute2(int iMinute)
{
	QString qstrMinute;
	if (iMinute == 0)
	{
...
QString QgsImageMappingDialog::FormatSecond(int iSecond)
{
	QString qstrSecond;
	if (iSecond == 0)
	{
...
QString QgsImageMappingDialog::FormatSecond2(int iSecond)
{
	QString qstrSecond;
	if (iSecond == 0)
	{
...
bool QgsImageMappingDialog::LoadImageMappingMetaDataConfigFile(string& strCategory,
	string& strName,
	string& strType,
	string& strLength,
	string& strValue,
...
void QgsImageMappingDialog::DrawGridOfNeighboringZone(QString qstrSheet,
...
QString QgsImageMappingDialog::GetQmlFilePathFromLayerName(double dScale, const QString& qstrLayerName)
{
	QString qstrQmlPath = "";
	// 运行环境路径
...
QString QgsImageMappingDialog::GetMosaicQmlFilePath(double dScale)
{
	QString qstrQmlPath = "";
	// 运行环境路径
	QString qstrRunDirPath = QCoreApplication::applicationDirPath();
...
void QgsImageMappingDialog::GetLayoutQmlFilePath(double dScale,
...
void QgsImageMappingDialog::GetLayoutDecPointQmlFilePath(double dScale,
...
void QgsImageMappingDialog::CalGaussBorderCoords(const SE_DRect& dRange,
...
void QgsImageMappingDialog::CalLayoutBorderPoints(
...
int QgsImageMappingDialog::CalBandNumber(double dCenterMeridian, double dScale)
{
	// 根据中央经线计算带号
	int iBandNumber = 0;
	string strBandType;
...
int QgsImageMappingDialog::bIsAdjProjectBandSheet(double dScale, const QString& qstrSheet, double dCenterMeridian)
{
	// 当前投影带左边界经度值，单位：度
...
void QgsImageMappingDialog::DrawAdjKmNet(QgsPrintLayout* pPrintLayout,
	vector<SE_DPoint>& vOuterLeftBorderLayoutCoords,
	vector<SE_DPoint>& vOuterTopBorderLayoutCoords,
	vector<SE_DPoint>& vOuterRightBorderLayoutCoords,
	vector<SE_DPoint>& vOuterBottomBorderLayoutCoords,
...
void QgsImageMappingDialog::DrawAdjKmNet_CreateLayer(
...
void QgsImageMappingDialog::GetRadioChecked(bool& bLonLatRange, bool& bSheetList)
{
...
void QgsImageMappingDialog::GetSheetListFromPlainTextEdit(vector<QString>& qstrSheetList)
{
	qstrSheetList.clear();
...
bool QgsImageMappingDialog::LoadLayoutItemPositionsConfig(double dScale, vector<LayoutItemPositions>& vLayoutItemPositions)
{
	// 配置文件路径
...
void QgsImageMappingDialog::GetLayoutItemPositionByName(const QString& qstrName,
...
QgsLayoutItem::ReferencePoint QgsImageMappingDialog::GetReferencePoint(const QString& qstrReferencePoint)
{
	// 左上角
...
bool QgsImageMappingDialog::ReadSMSFile(string strSMSPath, int iKey, string& strValue)
{
	FILE* fp = fopen(strSMSPath.c_str(), "r");
	if (!fp) {
		return false;
...
void QgsImageMappingDialog::TransformGaussToLayout(SE_DPoint dRefGaussPoint, SE_DPoint dRefLayoutPoint, SE_DPoint dGaussPoint, double dMapUnitToLayoutUnit, SE_DPoint& dLayoutPoint)
{
...
// 影像图制图——标准流程
// 2022-9-20
void QgsImageMappingDialog::ImageMapping_StandardProcess()
{
	/*判断数据路径的有效性*/
	//-------------------------------------//
...
#pragma region "加载矢量图层"
		...
		for (j = 0; j < vVectorLayerPathList.size(); j++)
		{
			...
			if (QFileInfo::exists(qstrVectorLayerFilePath))
			{
				...
				//=====================================================//
				// 修改时间：2023-03-08
				// 修改内容：根据矢量图层要素选择状态设置属性查询条件
				//=====================================================//
				//--------------------begin----------------------------//
				// 根据图层类型获取对应的属性查询条件，编码列表

				// 从图层名称中提取出图层类型_几何类型，如：DN02501024_D_line.shp中获取D
				/*int nPos = qstrBaseName.indexOf("_", 0, Qt::CaseInsensitive);
				QString qstrLayerType = qstrBaseName.mid(nPos + 1, 1);
				
				// 当前图层的类型编码集合
				vector<QString> vFeatureClass;
				vFeatureClass.clear();

				GetFeatureClassesByLayerType(m_ImageMappingSchema.vFeatureClass, qstrLayerType, vFeatureClass);

				//QString qstrSql = tr("编码") + "=" + "140305" + "OR" + tr("编码") + "=" + "140401" + "OR" + tr("编码") + "=" + "140403";
				QString qstrSql = tr("编码") + "=" + "140305" + tr("编码") + "=" + "140401"  + tr("编码") + "=" + "140403";
				pVecotrLayer->selectByExpression(qstrSql, Qgis::SelectBehavior::SetSelection);

				int iSelCount = pVecotrLayer->selectedFeatureCount();

				QgsFeatureIds qgsFeaIds = pVecotrLayer->selectedFeatureIds();

				pVecotrLayer->select(qgsFeaIds);*/
				...
				{ 
					...
					if (iIndexOfX != -1)
					{
						...
						/*if (fabs(layoutTopBorderPoint.dx - interiorMapLayoutPoint_A.dx) < KM_LINE_TOLERANCE_X
							|| fabs(layoutTopBorderPoint.dx - interiorMapLayoutPoint_B.dx) < KM_LINE_TOLERANCE_X)
						{
							// 整公里刻度线与外图廓上边界交叉点
							polygon.append(QPointF(layoutTopBorderPoint.dx, layoutTopBorderPoint.dy));
						...
						pKmLine_X_Top[iPointIndex]->setSymbol(kmLineBorderLineSymbol);
			...
			for (int iPointIndex = 0; iPointIndex < vBorderLayoutCoords_D_A.size(); iPointIndex++)
			{
				...
				if (iIndexOfY != -1)
				{
					...
					/*if (fabs(layoutLeftBorderPoint.dy - interiorMapLayoutPoint_A.dy) < KM_LINE_TOLERANCE_Y
						|| fabs(layoutLeftBorderPoint.dy - interiorMapLayoutPoint_D.dy) < KM_LINE_TOLERANCE_Y)
					{
						// 整公里刻度线与外图廓左边界交叉点
						polygon.append(QPointF(layoutLeftBorderPoint.dx, layoutLeftBorderPoint.dy));
					...
					pKmLine_Y_Left[iPointIndex]->setSymbol(kmLineBorderLineSymbol);
...
#pragma region "绘制内图廓下边界整公里刻度值"
			...
			for (int iPointIndex = iBeginMarkIndex_X_Bottom; iPointIndex <= iEndMarkIndex_X_Bottom; iPointIndex++)
			{
					...
					iPointIndex == iEndMarkIndex_X_Bottom)
				{
					...
					// 修改内容：调整下边界带号
					pBottomBorderLabel_BandNumber->attemptSetSceneRect(QRectF(layoutBottomBorderPoint.dx - 4,
						layoutBottomBorderPoint.dy + 0.2,
						4,
						4));
...
#pragma region "绘制内图廓左边界方里网数值"
			...
			// 时间：2023-02-23
			// 修改内容：内图廓左、右边界方里网刻度大小调整
			// 设置字体及大小
			QFont font100km_Y;      // 左右边界千公里+百公里
			font100km_Y.setFamily(QStringLiteral("等线"));
			font100km_Y.setPixelSize(19);
			font100km_Y.setBold(true);
...
#pragma region "依次绘制8条短线"
		...
		// 8条短线与内图廓线样式保持一致
		// 绘制A-(1)短线
		QgsLayoutItemPolyline* pLayoutLine_A_1 = nullptr;
		QPolygonF pPolygon_A_1;
		pPolygon_A_1.append(QPointF(interiorMapLayoutPoint_A.dx, interiorMapLayoutPoint_A.dy));
		pPolygon_A_1.append(QPointF(dEightCornerLayoutPoints[1].dx, dEightCornerLayoutPoints[1].dy));
		pLayoutLine_A_1 = new QgsLayoutItemPolyline(pPolygon_A_1, pPrintLayout);
		pLayoutLine_A_1->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_A_1);



		// 绘制A-(2)短线
		QgsLayoutItemPolyline* pLayoutLine_A_2 = nullptr;
		QPolygonF pPolygon_A_2;
		pPolygon_A_2.append(QPointF(interiorMapLayoutPoint_A.dx, interiorMapLayoutPoint_A.dy));
		pPolygon_A_2.append(QPointF(dEightCornerLayoutPoints[2].dx, dEightCornerLayoutPoints[2].dy));
		pLayoutLine_A_2 = new QgsLayoutItemPolyline(pPolygon_A_2, pPrintLayout);
		pLayoutLine_A_2->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_A_2);

		// 绘制B-(3)短线
		QgsLayoutItemPolyline* pLayoutLine_B_3 = nullptr;
		QPolygonF pPolygon_B_3;
		pPolygon_B_3.append(QPointF(interiorMapLayoutPoint_B.dx, interiorMapLayoutPoint_B.dy));
		pPolygon_B_3.append(QPointF(dEightCornerLayoutPoints[3].dx, dEightCornerLayoutPoints[3].dy));
		pLayoutLine_B_3 = new QgsLayoutItemPolyline(pPolygon_B_3, pPrintLayout);
		pLayoutLine_B_3->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_B_3);

		// 绘制B-(4)短线
		QgsLayoutItemPolyline* pLayoutLine_B_4 = nullptr;
		QPolygonF pPolygon_B_4;
		pPolygon_B_4.append(QPointF(interiorMapLayoutPoint_B.dx, interiorMapLayoutPoint_B.dy));
		pPolygon_B_4.append(QPointF(dEightCornerLayoutPoints[4].dx, dEightCornerLayoutPoints[4].dy));
		pLayoutLine_B_4 = new QgsLayoutItemPolyline(pPolygon_B_4, pPrintLayout);
		pLayoutLine_B_4->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_B_4);

		// 绘制C-(5)短线
		QgsLayoutItemPolyline* pLayoutLine_C_5 = nullptr;
		QPolygonF pPolygon_C_5;
		pPolygon_C_5.append(QPointF(interiorMapLayoutPoint_C.dx, interiorMapLayoutPoint_C.dy));
		pPolygon_C_5.append(QPointF(dEightCornerLayoutPoints[5].dx, dEightCornerLayoutPoints[5].dy));
		pLayoutLine_C_5 = new QgsLayoutItemPolyline(pPolygon_C_5, pPrintLayout);
		pLayoutLine_C_5->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_C_5);

		// 绘制C-(6)短线
		QgsLayoutItemPolyline* pLayoutLine_C_6 = nullptr;
		QPolygonF pPolygon_C_6;
		pPolygon_C_6.append(QPointF(interiorMapLayoutPoint_C.dx, interiorMapLayoutPoint_C.dy));
		pPolygon_C_6.append(QPointF(dEightCornerLayoutPoints[6].dx, dEightCornerLayoutPoints[6].dy));
		pLayoutLine_C_6 = new QgsLayoutItemPolyline(pPolygon_C_6, pPrintLayout);
		pLayoutLine_C_6->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_C_6);

		// 绘制D-(7)短线
		QgsLayoutItemPolyline* pLayoutLine_D_7 = nullptr;
		QPolygonF pPolygon_D_7;
		pPolygon_D_7.append(QPointF(interiorMapLayoutPoint_D.dx, interiorMapLayoutPoint_D.dy));
		pPolygon_D_7.append(QPointF(dEightCornerLayoutPoints[7].dx, dEightCornerLayoutPoints[7].dy));
		pLayoutLine_D_7 = new QgsLayoutItemPolyline(pPolygon_D_7, pPrintLayout);
		pLayoutLine_D_7->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_D_7);

		// 绘制D-(8)短线
		QgsLayoutItemPolyline* pLayoutLine_D_8 = nullptr;
		QPolygonF pPolygon_D_8;
		pPolygon_D_8.append(QPointF(interiorMapLayoutPoint_D.dx, interiorMapLayoutPoint_D.dy));
		pPolygon_D_8.append(QPointF(dEightCornerLayoutPoints[8].dx, dEightCornerLayoutPoints[8].dy));
		pLayoutLine_D_8 = new QgsLayoutItemPolyline(pPolygon_D_8, pPrintLayout);
		pLayoutLine_D_8->setSymbol(interiorMapLineSymbol);
		pPrintLayout->addLayoutItem(pLayoutLine_D_8);
...
//#pragma region "释放内图廓"
//		
//		if (pInteriorMapBorder_A_B)
//		{
//			delete pInteriorMapBorder_A_B;
//		}
//
//		if (pInteriorMapBorder_B_C)
//		{
//			delete pInteriorMapBorder_B_C;
//		}
//
//		if (pInteriorMapBorder_C_D)
//		{
//			delete pInteriorMapBorder_C_D;
//		}
//
//		if (pInteriorMapBorder_A_D)
//		{
//			delete pInteriorMapBorder_A_D;
//		}
//		
//
//#pragma endregion
...
//#pragma region "释放外图廓"
//
//		if (pExteriorMapBorder_a_b)
//		{
//			delete pExteriorMapBorder_a_b;
//		}
//
//		if (pExteriorMapBorder_b_c)
//		{
//			delete pExteriorMapBorder_b_c;
//		}
//
//		if (pExteriorMapBorder_c_d)
//		{
//			delete pExteriorMapBorder_c_d;
//		}
//
//		if (pExteriorMapBorder_a_d)
//		{
//			delete pExteriorMapBorder_a_d;
//		}
//
//
//#pragma endregion
...
#pragma region "释放布局对象"

		/*QList<QgsLayoutItem*> qPrintfLayoutItemList;
		
		pPrintLayout->layoutItems(qPrintfLayoutItemList);
		for (int iLayoutItemIndex = 0; iLayoutItemIndex < qPrintfLayoutItemList.size(); iLayoutItemIndex++)
		{
			delete qPrintfLayoutItemList[iLayoutItemIndex];
			qPrintfLayoutItemList[iLayoutItemIndex] = nullptr;
		}*/
...
void QgsImageMappingDialog::LoadSheetListFile()
{
	// 打开txt文件
	// 选择文件夹
...
void QgsImageMappingDialog::LoadMappingSchema()
{
	// 配置文件默认从运行环境加载
	QString curPath = QCoreApplication::applicationDirPath();
...
void QgsImageMappingDialog::SaveMappingSchema()
{
	// 获取当前选择的制图方案
...
/*根据两点确定的直线计算点坐标Y*/
void QgsImageMappingDialog::CalCoordYByTwoPoint(SE_DPoint dPoint1, SE_DPoint dPoint2, double dInputX, double& dOutputY)
{
	// 如果两点Y坐标相同
...
/*根据两点确定的直线计算点坐标X*/
void QgsImageMappingDialog::CalCoordXByTwoPoint(SE_DPoint dPoint1, SE_DPoint dPoint2, double dInputY, double& dOutputX)
{
	// 如果两点Y坐标相同
	if (fabs(dPoint1.dy - dPoint2.dy) <= IMAGE_MAPPING_ZERO)
	{
...
void QgsImageMappingDialog::CalGaussBorderCoords_A_B(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
...
void QgsImageMappingDialog::CalGaussBorderCoords_C_B(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
void QgsImageMappingDialog::CalGaussBorderCoords_D_C(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
void QgsImageMappingDialog::CalGaussBorderCoords_D_A(SE_DPoint dBeginPoint,
	SE_DPoint dEndPoint,
	vector<SE_DPoint>& vGaussBorderCoords)
{
	vGaussBorderCoords.clear();
...
int QgsImageMappingDialog::ClockWise(vector<SE_DPoint>& vPoints)
{
	int i, j, k;
	int count = 0;
	double z;
...
SE_DPoint QgsImageMappingDialog::LineIntersect(SE_DPoint p0,SE_DPoint p1,SE_DPoint p2,SE_DPoint p3)
{
...
int QgsImageMappingDialog::CalIndexInVector_XCoord(double dx, vector<SE_DPoint>& vPoints)
{
...
int QgsImageMappingDialog::CalIndexInVector_YCoord(double dy, vector<SE_DPoint>& vPoints)
{
	for (int i = 0; i < vPoints.size(); i++)
	{
...
void QgsImageMappingDialog::TransformLayoutToGauss(SE_DPoint dRefGaussPoint, 
	SE_DPoint dRefLayoutPoint, 
	SE_DPoint dLayoutPoint, 
	double dMapUnitToLayoutUnit, 
	SE_DPoint& dGaussPoint)
...
void QgsImageMappingDialog::CalEndPointByStartPointAndAngle(
...
void QgsImageMappingDialog::ConvertMilFromIntToString(int iMil, QString& qstrMil)
{
	// 如果密位数为个位数
	if (iMil < 10)
	{
...
QList<QgsStringMap> QgsImageMappingDialog::GetAttributeValue(QgsVectorLayer* layer)
{
...
void QgsImageMappingDialog::GetAcqDataAndSensorByCateName(QList<QgsStringMap>& qAttributeList, 
...
// 判断目录是否存在
bool QgsImageMappingDialog::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool QgsImageMappingDialog::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}


void QgsImageMappingDialog::InitLineEdit()
{
	// 上边界纬度
...
bool QgsImageMappingDialog::TranslateFromTif2Jpg(QString qstrFilePath, QString qstrSheetNumber)
{
...
void QgsImageMappingDialog::GetFeatureClassesByLayerType(
...
void QgsImageMappingDialog::ImageMapping_LayoutVectorization()
{
...
#pragma region "外图廓整饰点要素图层【生成地图标题（图幅号、地图名、副标题）、地图制图单位矢量点要素】"
		...
		bool bCreateLayoutDecorationPointLayer = true;
		if (bCreateLayoutDecorationPointLayer)
		{
			CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");	// 支持中文路径
			CPLSetConfigOption("SHAPE_ENCODING", "");			// 属性表支持中文字段
			GDALAllRegister();
...
#define MODE (S_IRWXU | S_IRWXG | S_IRWXO)
			...
			if (!CreateShapefileCPG(strCPGFilePath, "System"))
			{
				continue;
			}
		}
...
int QgsImageMappingDialog::SetFieldDefn(OGRLayer* poLayer, 
...
bool QgsImageMappingDialog::CreateShapefileCPG(string strCPGFilePath, string strEncoding /*= "System"*/)
{
...
int QgsImageMappingDialog::Set_Point(OGRLayer* poLayer, double x, double y, double z, vector<FieldNameAndValue_Imp>& vFieldAndValue)
{
...
int QgsImageMappingDialog::Set_LineString(OGRLayer* poLayer, vector<SE_DPoint> Line, vector<FieldNameAndValue_Imp>& vFieldAndValue)
{
	OGRFeature* poFeature;
	if (poLayer == NULL)
		return -1;
