...
QgsSetMultiScaleDataMatchParams::QgsSetMultiScaleDataMatchParams(QWidget* parent, Qt::WindowFlags fl)
	: QDialog(parent, fl)
{
	ui.setupUi(this);
	QgsGui::enableAutoGeometryRestore(this);
	ui.tabWidget_LayerType->setCurrentIndex(0);
	connect(ui.pushButton_LoadMatchParams, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::pushButton_LoadMatchParams_clicked);
	connect(ui.pushButton_SaveMatchParams, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::pushButton_SaveMatchParams_clicked);
	connect(ui.pushButton_OpenBigScaleData, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::Button_OpenBigScale_clicked);
	connect(ui.pushButton_OpenSmallScaleData, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::Button_OpenSmallScale_clicked);
	connect(ui.pushButton_Save, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::Button_MatchDBSave_clicked);
	connect(ui.Button_OK, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::Button_OK_accepted);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &QgsSetMultiScaleDataMatchParams::Button_Cancel_rejected);
	...
	// 设置默认规则，设置浮点数的编辑框不能输入文字、字母等字符
	InitLineEdit();


	// 恢复上次运行保存参数
	restoreState();
	ui.lineEdit_InputBigScaleDataPath->setText(m_qstrBigScaleDataPath);
	ui.lineEdit_InputSmallScaleDataPath->setText(m_qstrSmallScaleDataPath);
	ui.lineEdit_SavePath->setText(m_qstrMultiScaleDBPath);
}

QgsSetMultiScaleDataMatchParams::~QgsSetMultiScaleDataMatchParams()
{
	// 设置当前保存路径
	QgsSettings settings;
	settings.setValue(QStringLiteral("MultiScaleMatch/BigScaleDataPath"), m_qstrBigScaleDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("MultiScaleMatch/SmallScaleDataPath"), m_qstrSmallScaleDataPath, QgsSettings::Section::Plugins);
	settings.setValue(QStringLiteral("MultiScaleMatch/MultiScaleDBPath"), m_qstrMultiScaleDBPath, QgsSettings::Section::Plugins);
}

void QgsSetMultiScaleDataMatchParams::GetLayerMatchParams(vector<LayerMatchParam>& vLayerMatchParam)
{
...
void QgsSetMultiScaleDataMatchParams::GetLayerFieldsChecked(string strLayerType, vector<string>& vFields)
{
	vFields.clear();
...
void QgsSetMultiScaleDataMatchParams::GetLayerGeoChecked(string strLayerType, bool& bPointChecked, bool& bLineChecked, bool& bPolygonChecked)
{
	if (strLayerType == "A")
	{
...
void QgsSetMultiScaleDataMatchParams::pushButton_LoadMatchParams_clicked()
{
...
void QgsSetMultiScaleDataMatchParams::SetCheckBoxStatus(LayerMatchParam& param)
{
	// 按图层依次进行设置
	// 测量控制点
...
void QgsSetMultiScaleDataMatchParams::GetFolderNameFromPath(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
...
void QgsSetMultiScaleDataMatchParams::restoreState()
{
	const QgsSettings settings;
	m_qstrBigScaleDataPath = settings.value(QStringLiteral("MultiScaleMatch/BigScaleDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrSmallScaleDataPath = settings.value(QStringLiteral("MultiScaleMatch/SmallScaleDataPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
	m_qstrMultiScaleDBPath = settings.value(QStringLiteral("MultiScaleMatch/MultiScaleDBPath"), QDir::homePath(), QgsSettings::Section::Plugins).toString();
}

bool QgsSetMultiScaleDataMatchParams::bRectInOtherRect(SE_DRect dInputRect, SE_DRect dRefRect)
{
	if (dInputRect.dright <= dRefRect.dleft
		|| dInputRect.dleft >= dRefRect.dright
		|| dInputRect.dbottom >= dRefRect.dtop
		|| dInputRect.dtop <= dRefRect.dbottom)
	{
		return false;
	}
...
void QgsSetMultiScaleDataMatchParams::InitLineEdit()
{
	// 点匹配阈值设置输入限制，非负浮点数
...
QStringList QgsSetMultiScaleDataMatchParams::GetDirPathOfSplDir(QString dirPath)
{
...
/*生成界面配置文件*/
void QgsSetMultiScaleDataMatchParams::pushButton_SaveMatchParams_clicked()
{
	// QDomDocument类
	QDomDocument doc;
...
void QgsSetMultiScaleDataMatchParams::Button_OK_accepted()
{
	/*判断目录是否存在*/
	//------------------------------------//
	if (!FilePathIsExisted(ui.lineEdit_InputBigScaleDataPath->text()))
	{
		QMessageBox msgBox;
		msgBox.setWindowTitle(tr("地理提取与加工"));
		msgBox.setText(tr("大比例尺数据目录不存在，请重新输入！"));
	...
	if (!FilePathIsExisted(ui.lineEdit_InputSmallScaleDataPath->text()))
	{
		QMessageBox msgBox;
		msgBox.setWindowTitle(tr("地理提取与加工"));
		msgBox.setText(tr("小比例尺数据目录不存在，请重新输入！"));
		msgBox.setStandardButtons(QMessageBox::Yes);
		msgBox.setDefaultButton(QMessageBox::Yes);
		msgBox.exec();
		return;
	...
	if (!FilePathIsExisted(ui.lineEdit_SavePath->text()))
	{
		QMessageBox msgBox;
		msgBox.setWindowTitle(tr("地理提取与加工"));
	...
	// 判断大比例尺地图图幅在小比例尺图幅范围内的个数，至少有一个重叠时才能进行多尺度匹配，否则提示，然后退出
	// 获取输入大比例尺数据路径
	string strBigScaleDataPath = GetBigScaleDataPath().toLocal8Bit();

	vector<string> vBigScaleDataPath;
	vBigScaleDataPath.clear();

	// 获取大比例尺数据下所有图幅目录
	QStringList qstrPathList = GetDirPathOfSplDir(GetBigScaleDataPath());
	if (qstrPathList.size() == 0)
	{
		// 说明当前是单图幅目录
		string strTemp = GetBigScaleDataPath().toLocal8Bit();
		vBigScaleDataPath.push_back(strTemp);
	}
	// 如果是多图幅所在目录
	else
	{
		for (int i = 0; i < qstrPathList.size(); i++)
		{
			...
			vBigScaleDataPath.push_back(strPathTemp);
		}
	}

	// 获取输入小比例尺数据路径
	string strSmallScaleDataPath = GetSmallScaleDataPath().toLocal8Bit();

	// 获取小比例尺图幅号
	string strSmallScaleSheet;
	GetFolderNameFromPath(strSmallScaleDataPath, strSmallScaleSheet);

	// 获取大比例尺图幅号列表
	vector<string> vBigScaleSheet;
	vBigScaleSheet.clear();

	for (int i = 0; i < vBigScaleDataPath.size(); i++)
	{
		string strTemp;
		GetFolderNameFromPath(vBigScaleDataPath[i], strTemp);
		vBigScaleSheet.push_back(strTemp);
	}

	/*// 判断大比例尺图幅的前n（n为小比例尺图幅号字符长度）位是否和小比例尺图幅号一致
	// 小比例尺图幅号字符长度
	int iLength = strSmallScaleSheet.length();*/


	// 获取小比例尺图幅范围
	SE_DRect dSmallScaleSheetRect;
	mapSheet.get_box(strSmallScaleSheet,
		dSmallScaleSheetRect.dleft,
		dSmallScaleSheetRect.dtop,
		dSmallScaleSheetRect.dright,
		dSmallScaleSheetRect.dbottom);


	// 大比例尺图幅在小比例尺图幅范围中的个数
	int iCount = 0;
	for (int i = 0; i < vBigScaleSheet.size(); i++)
	{
		// 获取大比例尺图幅范围
		SE_DRect dBigScaleSheetRect;
		mapSheet.get_box(vBigScaleSheet[i],
			dBigScaleSheetRect.dleft,
			dBigScaleSheetRect.dtop,
			dBigScaleSheetRect.dright,
			dBigScaleSheetRect.dbottom);
		
		// 如果大比例尺与小比例尺图幅有交集
		if (bRectInOtherRect(dBigScaleSheetRect, dSmallScaleSheetRect))
		{
			iCount++;
		}
	}
	...
	{
		QMessageBox msgBox1;
		msgBox1.setWindowTitle(tr("多尺度数据匹配"));
		msgBox1.setText(tr("大比例尺图幅和小比例尺图幅无重叠，请重新选择数据！"));
	...
	// 获取输出多尺度匹配数据库存储路径
	QString qstrMultiScaleDBSavePath = GetMultiScaleDBSavePath();
	string strMultiScaleDBSavePath = qstrMultiScaleDBSavePath.toLocal8Bit();

	// 获取点匹配阈值、线面匹配阈值
	double dPointDistance = 0;
	double dLineAndPolygonDistance = 0;
	GetMatchDistance(dPointDistance, dLineAndPolygonDistance);

	// 获取匹配参数
	vector<LayerMatchParam> vMatchParam;
	vMatchParam.clear();

	GetLayerMatchParams(vMatchParam);
	...
	iResult = CSE_GeoExtractAndProcess::MultiScaleDataMatch(vBigScaleDataPath,
		strSmallScaleDataPath,
		strMultiScaleDBSavePath,
		dPointDistance,
...
void QgsSetMultiScaleDataMatchParams::Button_Cancel_rejected()
{
	reject();
}

void QgsSetMultiScaleDataMatchParams::Button_OpenSmallScale_clicked()
{
	// 选择文件夹
	QString curPath = m_qstrSmallScaleDataPath; 
	QString dlgTile = QObject::tr("请选择小比例尺数据存储目录");
	...
	{
		ui.lineEdit_InputSmallScaleDataPath->setText(selectedDir);
		m_qstrSmallScaleDataPath = selectedDir;
	}
}

void QgsSetMultiScaleDataMatchParams::Button_OpenBigScale_clicked()
{
	// 选择文件夹
	QString curPath = m_qstrBigScaleDataPath; 
	QString dlgTile = QObject::tr("请选择大比例尺数据存储目录");
	QString selectedDir = QFileDialog::getExistingDirectory(
		this,
		dlgTile,
		curPath,
	...
	if (!selectedDir.isEmpty())
	{
		ui.lineEdit_InputBigScaleDataPath->setText(selectedDir);
		m_qstrBigScaleDataPath = selectedDir;
	}
}

void QgsSetMultiScaleDataMatchParams::Button_MatchDBSave_clicked()
{
	// 选择文件夹
	QString curPath = m_qstrMultiScaleDBPath;
	QString dlgTile = QObject::tr("请选择多尺度匹配数据库存储目录");
	QString selectedDir = QFileDialog::getExistingDirectory(
		this,
		dlgTile,
		curPath,
	...
	if (!selectedDir.isEmpty())
	{
		...
		m_qstrMultiScaleDBPath = selectedDir;
	}
}

void QgsSetMultiScaleDataMatchParams::GetMatchDistance(double& dPointDistance, double& dLineAndPolygonDistance)
{
...
QString QgsSetMultiScaleDataMatchParams::GetBigScaleDataPath()
{
	return m_qstrBigScaleDataPath;
}

QString QgsSetMultiScaleDataMatchParams::GetSmallScaleDataPath()
{
	return m_qstrSmallScaleDataPath;
}

QString QgsSetMultiScaleDataMatchParams::GetMultiScaleDBSavePath()
{
	return m_qstrMultiScaleDBPath;
}


// 判断目录是否存在
bool QgsSetMultiScaleDataMatchParams::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool QgsSetMultiScaleDataMatchParams::FileIsExisted(QString qstrFilePath)
{
