...
/*----------------自定义--------------*/
#include "tool_DZB2Shapefile_thread.h"
#include "base_vector_odata2shapefile.h"
/*----------------自定义--------------*/
#pragma endregion

TOOLDZB2ShapefileThread::TOOLDZB2ShapefileThread(QObject *parent)
    : QThread(parent)
{
    restart = false;
    abort = false;
...
TOOLDZB2ShapefileThread::~TOOLDZB2ShapefileThread()
{
    mutex.lock();
    abort = true;
    condition.wakeOne();
...
void TOOLDZB2ShapefileThread::SetThreadParams(
...
void TOOLDZB2ShapefileThread::run()
{
...
#pragma region "根据不同的输出数据空间参考系分类处理"
		...
		if (iOutputSRS == 1)
		{
			err = BaseVectorOdata2Shapefile::DZB2Shapefile_OriginSRS(
		...
		{
			err = BaseVectorOdata2Shapefile::DZB2Shapefile_GeoSRS(
				strInputDataPath.c_str(),
				strOutputDataPath.c_str(),
				dOffsetX,
				dOffsetY,
		...
		{
			err = BaseVectorOdata2Shapefile::DZB2Shapefile_ProjSRS(
				strInputDataPath.c_str(),
				strOutputDataPath.c_str(),
				dOffsetX,
				dOffsetY,
...
//  判断目录是否存在
bool TOOLDZB2ShapefileThread::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
//  判断文件是否存在
bool TOOLDZB2ShapefileThread::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}
//  获取在指定目录下的目录的路径
QStringList TOOLDZB2ShapefileThread::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
//  获取指定目录的最后一级目录
void TOOLDZB2ShapefileThread::GetFolderNameFromPath_string(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
