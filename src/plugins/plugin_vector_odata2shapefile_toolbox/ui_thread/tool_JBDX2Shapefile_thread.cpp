...
/*----------------自定义--------------*/
#include "tool_JBDX2Shapefile_thread.h"
#include "base_vector_odata2shapefile.h"
/*----------------自定义--------------*/
#pragma endregion

TOOLJBDX2ShapefileThread::TOOLJBDX2ShapefileThread(QObject *parent)
    : QThread(parent)
{
    restart = false;
    abort = false;
...
TOOLJBDX2ShapefileThread::~TOOLJBDX2ShapefileThread()
{
    mutex.lock();
    abort = true;
    condition.wakeOne();
...
void TOOLJBDX2ShapefileThread::SetThreadParams(
...
void TOOLJBDX2ShapefileThread::run()
{
...
#pragma region "根据不同的输出数据空间参考系分类处理"
		...
		if (iOutputSRS == 1)
		{
			err = BaseVectorOdata2Shapefile::JBDX2Shapefile_OriginSRS(
		...
		{
			err = BaseVectorOdata2Shapefile::JBDX2Shapefile_GeoSRS(
				strInputDataPath.c_str(),
				strOutputDataPath.c_str(),
				dOffsetX,
				dOffsetY,
		...
		{
			err = BaseVectorOdata2Shapefile::JBDX2Shapefile_ProjSRS(
				strInputDataPath.c_str(),
				strOutputDataPath.c_str(),
				dOffsetX,
				dOffsetY,
...
//  判断目录是否存在
bool TOOLJBDX2ShapefileThread::FilePathIsExisted(QString qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
//  判断文件是否存在
bool TOOLJBDX2ShapefileThread::FileIsExisted(QString qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}
//  获取在指定目录下的目录的路径
QStringList TOOLJBDX2ShapefileThread::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
//  获取指定目录的最后一级目录
void TOOLJBDX2ShapefileThread::GetFolderNameFromPath_string(string strPath, string& strFolderName)
{
	// 获取图幅文件夹名称
