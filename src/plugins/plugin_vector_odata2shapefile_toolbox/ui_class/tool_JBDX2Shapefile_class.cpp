...
/*--------------自定义---------------*/
#include "tool_JBDX2Shapefile_class.h"
/*--------------自定义---------------*/
...
#pragma region "JBDX2Shapefile 转化对话类相关的函数"
ToolJBDX2ShapefileDialog::ToolJBDX2ShapefileDialog(
  ...
  m_isHandlingResults(false)
{
	...
	connect(ui.Button_Save, &QPushButton::clicked, this, &ToolJBDX2ShapefileDialog::Button_Save_clicked);
	connect(ui.Button_Open, &QPushButton::clicked, this, &ToolJBDX2ShapefileDialog::Button_Open_clicked);
	connect(ui.Button_OK, &QPushButton::clicked, this, &ToolJBDX2ShapefileDialog::Button_OK_accepted);
	connect(ui.Button_Cancel, &QPushButton::clicked, this, &ToolJBDX2ShapefileDialog::Button_Cancel_rejected);
	connect(ui.Button_summary_log_saving_path, &QPushButton::clicked, this, &ToolJBDX2ShapefileDialog::Button_summary_log_saving_path_clicked);

	// 界面输入信息状态改变，需要重置进度条
	connect(ui.lineEdit_InputDataPath, &QLineEdit::textChanged,this, &ToolJBDX2ShapefileDialog::on_InputDataPath_TextChanged);
	connect(ui.lineEdit_OutputDataPath, &QLineEdit::textChanged, this, &ToolJBDX2ShapefileDialog::on_lineEdit_OutputDataPath_TextChanged);
	connect(ui.radioButton_OriginSRS, &QRadioButton::clicked, this, &ToolJBDX2ShapefileDialog::on_radioButton_OriginSRS_clicked);
	connect(ui.radioButton_GeoSRS, &QRadioButton::clicked, this, &ToolJBDX2ShapefileDialog::on_radioButton_GeoSRS_clicked);
	connect(ui.radioButton_ProjSRS, &QRadioButton::clicked, this, &ToolJBDX2ShapefileDialog::on_radioButton_ProjSRS_clicked);
	connect(ui.lineEdit_OffsetX, &QLineEdit::textChanged, this, &ToolJBDX2ShapefileDialog::on_lineEdit_OffsetX_TextChanged);
	connect(ui.lineEdit_OffsetY, &QLineEdit::textChanged, this, &ToolJBDX2ShapefileDialog::on_lineEdit_OffsetY_TextChanged);
	// 获取汇总日志保存路径
	connect(ui.lineEdit_summary_log_saving_path, &QLineEdit::textChanged, this, &ToolJBDX2ShapefileDialog::on_summary_log_saving_path_TextChanged);
...
int ToolJBDX2ShapefileDialog::GetOutputSRS()
{
...
void ToolJBDX2ShapefileDialog::Button_Open_clicked()
{
	// 选择文件夹（使用上次使用保存下来的路径）
...
void ToolJBDX2ShapefileDialog::Button_Save_clicked()
{
	// 选择文件夹（使用上次使用保存下来的路径）
...
void ToolJBDX2ShapefileDialog::Button_summary_log_saving_path_clicked()
{
...
/*获取在指定目录下的目录的路径*/
QStringList ToolJBDX2ShapefileDialog::GetSubDirPathOfCurrentDir(QString dirPath)
{
...
void ToolJBDX2ShapefileDialog::Button_OK_accepted()
{
  ...
  //  将获取得到的子文件夹数量赋值给类数据成员
	...
	if (iSheetCountPerCpuKernal < 1)
	{
		for (int i = 0; i < iSheetCount; ++i)
		{
			...
			//  关联线程进度信号槽
			connect(&m_pThread[i], &SE_Odata2shpThread::resultProcess, this, &ToolJBDX2ShapefileDialog::handleResults);
			//
...
#endif
  //  创建多个线程
  m_pThread = new TOOLJBDX2ShapefileThread[iNumberOfProcessors];
  //  如果没有创建成功，则需要进行提示
  if (!m_pThread)
  {
    QString qstrTitle = tr("线划图数据格式转换");
...
#pragma region "2、循环处理每一个分幅odata数据"
  ...
  for (int i = 0; i < iSheetCount; ++i)
  {
    ...
    TOOLJBDX2ShapefileThread thread;
    // 关联线程进度信号槽
    connect(&thread, &TOOLJBDX2ShapefileThread::resultProcess, this, &ToolJBDX2ShapefileDialog::handleResults);
...
void ToolJBDX2ShapefileDialog::Button_Cancel_rejected()
{
  //	有可能设置了用户界面参数
  saveState();
	reject();
...
void ToolJBDX2ShapefileDialog::on_InputDataPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_summary_log_saving_path_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_lineEdit_OutputDataPath_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_lineEdit_OffsetX_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_lineEdit_OffsetY_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_radioButton_OriginSRS_clicked(bool checked)
{
  ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_radioButton_GeoSRS_clicked(bool checked)
{
  ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::on_radioButton_ProjSRS_clicked(bool checked)
{
  ui.progressBar->reset();
}

void ToolJBDX2ShapefileDialog::handleResults(
...
void ToolJBDX2ShapefileDialog::mergeLogFiles()
{
  // 获取输入子目录列表
  QStringList qstrSubDir = GetSubDirPathOfCurrentDir(m_qstrInputDataPath);
...
void ToolJBDX2ShapefileDialog::onAllProcessingCompleted()
{
  // 合并日志文件
  mergeLogFiles();
...
void ToolJBDX2ShapefileDialog::restoreState()
{
  QgsSettings settings;
  // 指定从Plugins节读取设置
  settings.beginGroup("JBDX2Shapefile");
  m_qstrInputDataPath = settings.value(QStringLiteral("JBDX2Shapefile/InputDataPath"), QDir::homePath()).toString();
  m_qstrSaveDataPath = settings.value(QStringLiteral("JBDX2Shapefile/SaveDataPath"), QDir::homePath()).toString();
  m_summary_log_saving_path = settings.value(QStringLiteral("JBDX2Shapefile/summary_log_saving_path"), QDir::homePath()).toString();
}

void ToolJBDX2ShapefileDialog::saveState()
{
  QgsSettings settings;
  // 指定设置保存到Plugins节
  settings.beginGroup("JBDX2Shapefile");
  settings.setValue(QStringLiteral("JBDX2Shapefile/InputDataPath"), m_qstrInputDataPath);
  settings.setValue(QStringLiteral("JBDX2Shapefile/SaveDataPath"), m_qstrSaveDataPath);
  settings.setValue(QStringLiteral("JBDX2Shapefile/summary_log_saving_path"), m_summary_log_saving_path);
...
QStringList ToolJBDX2ShapefileDialog::GetShpFileNames(const QString& path)
{
	QDir dir(path);
...
//  判断目录是否存在
bool ToolJBDX2ShapefileDialog::FilePathIsExisted(const QString& qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
//  判断文件是否存在
bool ToolJBDX2ShapefileDialog::FileIsExisted(const QString& qstrFilePath)
{
	return QFile::exists(qstrFilePath);
}

QString ToolJBDX2ShapefileDialog::GetInputDataPath()
{
	return ui.lineEdit_InputDataPath->text();
}

QString ToolJBDX2ShapefileDialog::GetOutputDataPath()
{
	return ui.lineEdit_OutputDataPath->text();
}


QString ToolJBDX2ShapefileDialog::GetScale()
{
	return ui.lineEdit_Scale->text();
}

double ToolJBDX2ShapefileDialog::GetOffsetX()
{
	return ui.lineEdit_OffsetX->text().toDouble();
}

double ToolJBDX2ShapefileDialog::GetOffsetY()
{
	return ui.lineEdit_OffsetY->text().toDouble();
}

//	获得是否选择手动输入放大系数
int ToolJBDX2ShapefileDialog::get_setting_zoomscale_method()
{
	//	如果点击了从SMS文件中读取选项
...
double ToolJBDX2ShapefileDialog::get_dzoomscale()
{
	return ui.lineEdit_zoom_scale->text().toDouble();
}

//  获取图层信息方式：1——从*.SMS文件中获取图层信息；2——从实际odata数据目录中获取图层信息
int ToolJBDX2ShapefileDialog::get_the_method_of_obtaining_layer_information()
{
...
spdlog::level::level_enum ToolJBDX2ShapefileDialog::get_log_level()
{
	//	设置输入的日志等级
...
//	获取汇总日志保存路径
std::string ToolJBDX2ShapefileDialog::get_summary_log_saving_path_str()
{
...
//	获取汇总日志保存路径
QString ToolJBDX2ShapefileDialog::get_summary_log_saving_path_qstr()
{
...
//  获取指定目录的最后一级目录
void ToolJBDX2ShapefileDialog::GetFolderNameFromPath(QString qstrPath, QString& qstrFolderName)
{
...
int ToolJBDX2ShapefileDialog::GetScale(string strCode)
{
...
void ToolJBDX2ShapefileDialog::InitLineEdit()
{
	// X偏移量非负浮点数
