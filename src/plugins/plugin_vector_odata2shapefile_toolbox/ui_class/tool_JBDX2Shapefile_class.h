#ifndef TOOL_JBDX2SHAPEFILE_CLASS_H
#define TOOL_JBDX2SHAPEFILE_CLASS_H
...
/*----------------自定义--------------*/
#include "ui_tool_JBDX2Shapefile.h"
#include "base_vector_odata2shapefile.h"
#include "../ui_thread/tool_JBDX2Shapefile_thread.h"
/*----------------自定义--------------*/
...
class ToolJBDX2ShapefileDialog : public QDialog
{
	Q_OBJECT

public:
	ToolJBDX2ShapefileDialog(QgisInterface* qgisInterface, QWidget* parent = nullptr, Qt::WindowFlags fl = Qt::WindowFlags());
	~ToolJBDX2ShapefileDialog() override;
...
private:
  ...
  //  odata格式转化对话框
	Ui_ToolJBDX2ShapefileDialog ui;
	//  恢复用户界面参数
	void restoreState();
	//	保存用户界面参数
	void saveState();
  ...
  TOOLJBDX2ShapefileThread* m_pThread;
...
#pragma endregion
#endif // TOOL_JBDX2SHAPEFILE_CLASS_H
