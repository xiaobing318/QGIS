...
int base64_decode_value(char value_in)
{
	...
	int value = (int)(unsigned char)value_in - 43;
	if (value < 0 || (size_t)value >= sizeof(decoding)) return -1;
...
uint32_t base64_decode_block(const char* code_in, uint32_t length_in, char* plaintext_out, base64_decodestate* state_in)
{
	...
	case step_a:
			do {
				if (codechar == code_in+length_in)
				{
					...
					return (uint32_t)(plainchar - plaintext_out);
				}
