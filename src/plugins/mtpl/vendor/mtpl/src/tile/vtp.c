...
static const char mtpl_vtp_magic[4] = {'V', 'T', 'P', '\0'};

mtpl_status_t mtpl_vtp_reader_open(
    const char *path,
    ...
    status = mtpl_tile_core_reader_open(path, mtpl_vtp_magic, crypto, &core);
    if (status == MTPL_STATUS_OK) {
        *reader = (mtpl_vtp_reader_t *)core;
    }
    return status;
}

mtpl_status_t mtpl_vtp_reader_read_tile(
    mtpl_vtp_reader_t *reader,
...
mtpl_status_t mtpl_vtp_reader_read_tile_raw(
    mtpl_vtp_reader_t *reader,
    const mtpl_tile_coordinate_t *coordinate,
    mtpl_buffer_t *tile) {
...
mtpl_status_t mtpl_vtp_reader_read_metadata(
    mtpl_vtp_reader_t *reader,
...
mtpl_status_t mtpl_vtp_reader_check_edge(
    mtpl_vtp_reader_t *reader,
    const mtpl_tile_coordinate_t *coordinate,
    uint8_t *edge) {
...
mtpl_status_t mtpl_vtp_reader_get_tile_size(
    mtpl_vtp_reader_t *reader,
    uint32_t *tile_size) {
...
mtpl_status_t mtpl_vtp_reader_get_storage_mode(
    mtpl_vtp_reader_t *reader,
...
mtpl_status_t mtpl_vtp_reader_get_range_count(
...
mtpl_status_t mtpl_vtp_reader_get_range_info(
    const mtpl_vtp_reader_t *reader,
    size_t range_index,
...
mtpl_status_t mtpl_vtp_reader_get_entry_info(
    const mtpl_vtp_reader_t *reader,
    size_t range_index,
    size_t slot_index,
...
mtpl_status_t mtpl_vtp_reader_close(mtpl_vtp_reader_t *reader) {
    return mtpl_tile_core_reader_close((mtpl_tile_core_reader_t *)reader);
}

mtpl_status_t mtpl_vtp_writer_create(
    const char *path,
    uint32_t tile_size,
    ...
    status = mtpl_tile_core_writer_create(
        path, mtpl_vtp_magic, tile_size, metadata, mode, crypto, &core);
    if (status == MTPL_STATUS_OK) {
        *writer = (mtpl_vtp_writer_t *)core;
    }
    return status;
}

mtpl_status_t mtpl_vtp_writer_open(
    const char *path,
    const mtpl_crypto_options_t *crypto,
    mtpl_vtp_writer_t **writer) {
    mtpl_tile_core_writer_t *core = NULL;
    ...
    status = mtpl_tile_core_writer_open(path, mtpl_vtp_magic, crypto, &core);
    if (status == MTPL_STATUS_OK) {
        *writer = (mtpl_vtp_writer_t *)core;
    }
    return status;
...
mtpl_status_t mtpl_vtp_writer_add_range(
    mtpl_vtp_writer_t *writer,
...
mtpl_status_t mtpl_vtp_writer_add_tile_file(
    mtpl_vtp_writer_t *writer,
    const mtpl_tile_coordinate_t *coordinate,
    const char *path) {
...
mtpl_status_t mtpl_vtp_writer_add_tile_data(
    mtpl_vtp_writer_t *writer,
    const mtpl_tile_coordinate_t *coordinate,
...
mtpl_status_t mtpl_vtp_writer_has_tile(
    mtpl_vtp_writer_t *writer,
    const mtpl_tile_coordinate_t *coordinate,
    bool *exists) {
...
mtpl_status_t mtpl_vtp_writer_close(mtpl_vtp_writer_t *writer) {
    return mtpl_tile_core_writer_close((mtpl_tile_core_writer_t *)writer);
}

mtpl_status_t mtpl_vtp_merge(
    const char *source_path,
    const char *destination_path,
    const mtpl_crypto_options_t *crypto) {
    return mtpl_tile_core_merge(
        source_path, destination_path, mtpl_vtp_magic, crypto);
}
