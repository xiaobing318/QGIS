  ...
  m_isHandlingResults(false)
{
	...
	connect(ui.Button_summary_log_saving_path, &QPushButton::clicked, this, &CSE_VectorFormatConversionDialog::Button_summary_log_saving_path_clicked);
...
#pragma region "计算比例尺并且自动填充比例尺分母文本框"
	...
	{
		// 获取图幅名称（选取其中的一个子文件夹即分幅名称，从而计算比例尺）
		// 获取最后一级文件夹目录
		QString qstrFolderName;
...
void CSE_VectorFormatConversionDialog::Button_summary_log_saving_path_clicked()
{
...
#pragma region "二、单线程并行处理1、宏定义控制编译；2、手动控制编译；3、交互界面控制）"
  ...
  for (int i = 0; i < iSheetCount; ++i)
  {
    ...
    SE_Odata2shpThread thread;
    // 关联线程进度信号槽
    connect(&thread, &SE_Odata2shpThread::resultProcess, this, &CSE_VectorFormatConversionDialog::handleResults);
...
void CSE_VectorFormatConversionDialog::on_summary_log_saving_path_TextChanged(const QString& qstr)
{
	ui.progressBar->reset();
}
...
void CSE_VectorFormatConversionDialog::handleResults(
...
void CSE_VectorFormatConversionDialog::mergeLogFiles()
{
  // 获取输入子目录列表
  QStringList qstrSubDir = GetSubDirPathOfCurrentDir(m_qstrInputDataPath);
...
void CSE_VectorFormatConversionDialog::onAllProcessingCompleted()
{
  // 合并日志文件
  mergeLogFiles();
...
void CSE_VectorFormatConversionDialog::restoreState()
{
  ...
  // 指定从Plugins节读取设置
  settings.beginGroup("Odata2Shp_tool");
  m_qstrInputDataPath = settings.value(QStringLiteral("Odata2Shp/InputDataPath"), QDir::homePath()).toString();
  m_qstrSaveDataPath = settings.value(QStringLiteral("Odata2Shp/SaveDataPath"), QDir::homePath()).toString();
  m_summary_log_saving_path = settings.value(QStringLiteral("Odata2Shp/summary_log_saving_path"), QDir::homePath()).toString();
}


void CSE_VectorFormatConversionDialog::saveState()
{
  QgsSettings settings;
  // 指定设置保存到Plugins节
  settings.beginGroup("Odata2Shp_tool");
  settings.setValue(QStringLiteral("Odata2Shp/InputDataPath"), m_qstrInputDataPath);
  settings.setValue(QStringLiteral("Odata2Shp/SaveDataPath"), m_qstrSaveDataPath);
  settings.setValue(QStringLiteral("Odata2Shp/summary_log_saving_path"), m_summary_log_saving_path);
...
// 判断目录是否存在
bool CSE_VectorFormatConversionDialog::FilePathIsExisted(const QString& qstrPath)
{
	QDir dir(qstrPath);
	return dir.exists();
}
...
// 判断文件是否存在
bool CSE_VectorFormatConversionDialog::FileIsExisted(const QString& qstrFilePath)
{
...
int CSE_VectorFormatConversionDialog::get_the_method_of_obtaining_layer_information()
{
