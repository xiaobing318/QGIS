...
/*
* （杨小兵-2023-12-11）这个头文件是 Qt 框架中的一部分。它用于包含 `QStringList` 类的定义
    说明：`QStringList` 是 Qt 框架中一个非常有用的类，它提供了一个字符串列表的容器，可以用来存储和操作字符串集合。
这个类提供了各种操作字符串列表的方法，例如添加、删除字符串，排序列表，搜索特定字符串等
*/
...
/*
    设置线程运行输入参数
    1、qstrInputOdataDataPathList：       输入odata数据路径（杨小兵 - 2023 - 12 - 11：存储多个 `QString` 对象的容器，其中`QString` 对象通常用来表示一个单独的文件路径或目录路径，也就是说qstrInputOdataDataPathList保存多个文件路径或者目录）
...
private:
    ...
    //	杨小兵-2023-12-11：添加多线程并行，增加速度；代码由杨志龙杨老师提供，杨小兵进行集成；变量：输入odata数据图幅路径列表
