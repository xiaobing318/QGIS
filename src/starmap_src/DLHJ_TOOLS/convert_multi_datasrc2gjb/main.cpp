...
/*概括：一共四个参数
* 第一个参数：命令行工具名称（DLHJ_TOOLS_convert_multi_datasrc2gjb.exe）
* 第二个参数： 数据源类型
* 第三个参数：输入数据路径
* 第四个参数：输出数据路径
*/
	//	如果使用DLHJ_TOOLS_convert_multi_datasrc2gjb这个命令行工具的时候，其传入的参数不是4个，输出提示信息并且结束程序
	if (argc != 4)
	{
		return_code = 1;
...
#pragma region "2、输入数据路径、输出数据路径"
  data_source_type = std::stoi(argv[1]);
	input_data_path = std::string(argv[2]);
	output_data_path = std::string(argv[3]);
#pragma endregion
...
#pragma region "5、验证输入数据源、输入目录的路径，输出目录路径是否有效"
  if (!((0 <= data_source_type) && (data_source_type <= 4)))
  {
...
#pragma region "7、执行数据结构一体化处理流程"
  convert_multi_datasrc2gjb object(data_source_type,input_data_path_utf8, output_data_path_utf8);
	object.perform_convert_multi_datasrc2gjb();
