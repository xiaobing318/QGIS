...
#pragma region "语义融合工具"

#pragma region "（1）JBDX---->一体化"
int CSE_GeoExtractAndProcess::DLHJ_convert_JBDXDATA2YTHData(
  const std::vector<std::string>& JBDX_vinput_data_path_GBK,
  const std::string& YTH_output_data_path_GBK,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_layer_mapping_table_t& YTH_JBDX_layer_mapping_table,
  const YTH_JBDX_code_mapping_table_t& YTH_JBDX_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  /*
  * 杨小兵-2024-03-15
  一、分析
    这里假设输入数据路径、输出数据路径、配置文件信息都是有效的，输入数据路径和输出数据路径的有效性是在插件的交互层进行验证，底层算法这个层面不进行验证，这里的错误处理
  是针对对数据的一些处理可能遇到的问题。

  二、错误处理
  返回值为整数0：正确处理
  返回值为整数1：输入数据路径或者输出数据路径字符集编码转化失败
  返回值为整数2：数据源类型不合法
  返回值为整数3：一体化的所有图层字段配置信息为空
  返回值为整数4：一体化同JBDX图层映射配置信息为空
  返回值为整数5：一体化同JBDX编码映射配置信息为空
  返回值为整数6：JBDX到一体化矢量数据语义融合失败

  */
#pragma region "（第一步）将输入数据路径和输出数据路径的编码统一转化为utf-8字符集编码"
  //  创建一个进行字符集编码转化的对象
  character_set_encoding_conversion character_set_encoding_conversion_instance("UTF-8", "GBK");
  //  用来保存字符集编码转化后的输入数据路径
  std::vector<std::string> JBDX_vinput_data_path_UTF8;
  JBDX_vinput_data_path_UTF8.clear();
  //  对每个分幅数据输入路径进行转化
  for (size_t index = 0; index < JBDX_vinput_data_path_GBK.size(); index++)
  {
    std::string temp = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(JBDX_vinput_data_path_GBK[index]);
    JBDX_vinput_data_path_UTF8.push_back(temp);
  }
  //  对输出路径进行转化
  std::string YTH_output_data_path_UTF8 = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(YTH_output_data_path_GBK);
  //  检查输入数据路径和输出数据路径字符集编码转化是否成功
  int character_set_encoding_conversion_error_code = character_set_encoding_conversion_instance.get_error_code();
  if (character_set_encoding_conversion_error_code != 0)
  {
    return 1;
  }
  // 数据源类型不合法
  ...
  // 一体化所有图层字段信息有效性判断
  if (YTH_fields4all_layers.vYTH_single_layer.size() == 0)
  {
    return 3;
  }

  //	一体化同JBDX图层映射配置信息为空
  if (YTH_JBDX_layer_mapping_table.vYTH_JBDX_layer_mapping.size() == 0)
  {
    return 4;
  }

  //	一体化同JBDX编码映射配置信息为空
  if (YTH_JBDX_code_mapping_table.vYTH_JBDX_code_mapping_single_layer.size() == 0)
  {
    return 5;
  }
#pragma endregion

#pragma region "（第二步）JBDX数据到一体化数据转化"
  int iResult;
  //	DLHJ_convert_JBDXDATA2YTHData是被包装的函数，_DLHJ_convert_JBDXData2YTHData为实际执行数据转化的函数
  iResult = _DLHJ_convert_JBDXData2YTHData(
    JBDX_vinput_data_path_UTF8,
    YTH_output_data_path_UTF8,
    iDataSourceType,
...
int CSE_GeoExtractAndProcess::_DLHJ_convert_JBDXData2YTHData(
  const std::vector<std::string>& JBDX_vinput_data_path_UTF8,
  const std::string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_layer_mapping_table_t& YTH_JBDX_layer_mapping_table,
  const YTH_JBDX_code_mapping_table_t& YTH_JBDX_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  ...
  （一）参数说明：
      ...
      4. **`YTH_fields4all_layers`**: 类型为 `YTH_fields4all_layers_t&`（引用）。
         - **描述**: 包含 JBDX 层信息的列表，用于存储和管理 JBDX 层的相关数据。
      5. **`YTH_JBDX_layer_mapping_table`**: 类型为 `YTH_JBDX_layer_mapping_table_t&`（引用）。
         - **描述**: 包含 GJB 层字段信息的列表，用于存储和管理 GJB 层字段的相关数据。
      6. **`YTH_JBDX_code_mapping_table`**: 类型为 `YTH_JBDX_code_mapping_table_t&`（引用）。
         - **描述**: 用于存储 JBDX 到 GJB 的层映射信息。
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
  ...
  一体化数据格式。
  （三）算法流程
    1、整体上将会对分幅数据循环操作，执行JBDX--->一体化数据的转化
    2、对单个分幅数据进行处理
    3、处理结果处理
  */

  size_t size_framing_data = JBDX_vinput_data_path_UTF8.size();
  for (size_t index = 0; index < size_framing_data; index++)
  {
    int iresult_single_framing_data;
    iresult_single_framing_data = convert_single_frameJBDXData2YTHData(
      JBDX_vinput_data_path_UTF8[index],
      YTH_output_data_path_UTF8,
      iDataSourceType,
      YTH_fields4all_layers,
      YTH_JBDX_layer_mapping_table,
    ...
    //	对单个JBDX分幅数据转化成一体化数据结果进行判断
    if (iresult_single_framing_data != 0)
    {
      //	返回整数不为0，说明发生了某种错误，记录日志
      string strMsg1 = "分幅数据：" + JBDX_vinput_data_path_UTF8[index] + ",JBDX数据到一体化矢量数据语义融合失败！";
      QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
      string strTag1 = "（JBDX）矢量数据语义融合处理日志";
      QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
      QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
      //  继续处理其他分幅数据
      continue;
    }
...
int CSE_GeoExtractAndProcess::convert_single_frameJBDXData2YTHData(
  const string& JBDX_single_frame_data_path_UTF8,
  const string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_layer_mapping_table_t& YTH_JBDX_layer_mapping_table,
  const YTH_JBDX_code_mapping_table_t& YTH_JBDX_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  /*
  （一）参数说明：
      1. **`single_frame_JBDX_data_path`**: 类型为 `std::string&`。
         - **描述**: 用于存储单个输入数据文件路径（单个分幅数据目录路径）。
      2. **`strOutputDataPath`**: 类型为 `std::string&`。
         - **描述**: 输出数据的目标存储路径。
      3. **`iDataSourceType`**: 类型为 `int&`。
      ...
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    single_frame_JBDX_data_path中存储的是单个分幅数据目录路径，这个函数将会把single_frame_JBDX_data_path中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、将单个shp分幅数据目录路径下的所有图层读取到内存当中
    2、将所有图层的相关信息读取到自定义数据结构中（图层名称、图层类型、图层指针等等）
...
#pragma region "（第一步）将单个shp分幅数据目录路径下的所有图层读取到内存当中（打开资源）"
  //	对GDAL库的一些全局的设置（在整个项目中只需要设置一次就可以了）
  CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "YES"); //GDAL中默认的就是YES
  CPLSetConfigOption("SHAPE_ENCODING", "");			// 属性表支持中文字段
  //	注册所有可用的驱动器
  GDALAllRegister();
  //	利用所有可用驱动器中的矢量驱动器
  ...
  {
    // 错误提示
    string strMsg1 = "分幅数据（" + JBDX_single_frame_data_path_UTF8 + "）" + "获取ESRI Shapefile驱动失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（JBDX）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
#pragma region "（第二步）将JBDX数据所有图层的相关信息读取到自定义数据结构中（single_frame_JBDXLayer_infos_t）"
  //	利用矢量驱动器打开单个shp分幅数据目录路径下的数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameJBDXDataSet = (GDALDataset*)GDALOpenEx(JBDX_single_frame_data_path_UTF8.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  // 文件不存在或打开失败
  if (poSingleFrameJBDXDataSet == NULL)
  {
    // 说明打开这个分幅数据目录路径下的文件出现了错误
    // 错误提示
    string strMsg1 = "分幅数据（" + JBDX_single_frame_data_path_UTF8 + "）" + "文件不存在或打开失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（JBDX）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  //	一体化分幅数据相关信息（对整个分幅数据记录有：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考）
  JBDX_single_frame_data_all_opened_layers_info_t JBDX_single_frame_data_all_opened_layers_info;
  use_JBDXData_create_single_frame_JBDXLayer_infos(poSingleFrameJBDXDataSet, JBDX_single_frame_data_all_opened_layers_info);

#pragma endregion

#pragma region "（第三步）根据JBDX数据和图层映射关系将一体化数据所有图层的相关信息读取到自定义数据结构中（YTH_single_frame_data_all_opened_layers_info）"
  YTH_single_frame_data_all_opened_layers_info_t YTH_single_frame_data_all_opened_layers_info;
  use_JBDXData_create_single_frame_YTHLayer_infos(
    JBDX_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_JBDX_layer_mapping_table,
    YTH_output_data_path_UTF8,
    YTH_single_frame_data_all_opened_layers_info);
#pragma endregion

#pragma region "（第四步）对当前分幅数据中的图层进行转化操作"

  single_JBDX_frame_data2single_YTH_frame_data(
    JBDX_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_JBDX_code_mapping_table,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第五步）关闭资源"
  ...
  GDALClose(JBDX_single_frame_data_all_opened_layers_info.JBDX_single_frame_data_poDS);
  GDALClose(YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS);
#pragma endregion

  return 0;
}

void CSE_GeoExtractAndProcess::use_JBDXData_create_single_frame_JBDXLayer_infos(
  GDALDataset* poSingleFrameShpDataSet,
  JBDX_single_frame_data_all_opened_layers_info_t& JBDX_single_frame_data_all_opened_layers_info)
{
  /*
  ...
  */
  ...
  //	获取当前分幅数据的路径
  JBDX_single_frame_data_all_opened_layers_info.JBDX_single_frame_data_input_path = poSingleFrameShpDataSet->GetDescription();
  //	获取当前分幅数据的数据集指针
  JBDX_single_frame_data_all_opened_layers_info.JBDX_single_frame_data_poDS = poSingleFrameShpDataSet;
  ...
  for (int index = 0; index < size_shp_layer; index++)
  {
    JBDX_single_opened_layer_info_t temp_single_JBDXLayer_info_t;
    ...
    temp_single_JBDXLayer_info_t.layer_name = temp_poLayer->GetName();
    OGRwkbGeometryType eType = temp_poLayer->GetGeomType();
    temp_single_JBDXLayer_info_t.layer_geo_type = OGRGeometryTypeToName(eType);
    temp_single_JBDXLayer_info_t.polayer = temp_poLayer;
    temp_single_JBDXLayer_info_t.poSRS = temp_poLayer->GetSpatialRef();

    JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info.push_back(temp_single_JBDXLayer_info_t);
...
void CSE_GeoExtractAndProcess::use_JBDXData_create_single_frame_YTHLayer_infos(
  const JBDX_single_frame_data_all_opened_layers_info_t& JBDX_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_layer_mapping_table_t& YTH_JBDX_layer_mapping_table,
  const string& YTH_output_data_path_UTF8,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  /*
  （一）函数功能：
    根据单个分幅数据中所有图层的名称（JBDX数据格式中的图层名称分类标准，例如BOUP）创建一体化数据格式中的名称（例如BOUP_point），从数学的角度来说就是
  建议一个从N个图层名称到M个图层名称的映射关系，还有图层指针信息，这个函数中还没有执行具体的转化，只是确定了要将分幅数据中的JBDX图层转化成一体化的哪些图层
  （二）算法流程
    1、循环处理一个分幅数据中的所有shp图层数据
    2、获取当前图层的名称（在JBDX格式中图层的名称）
    3、在JBDXLayer.xml配置文件中寻找到对应的关系映射
    4、根据关系映射创建一体化数据格式中图层的名称
  */
...
#pragma region "（第一步）根据输出路径构建分幅数据目录路径、在此路径下创建数据集GDALDataset"
  //	构建分幅数据路径
  //	分幅数据名称
  size_t starting_position_1 = JBDX_single_frame_data_all_opened_layers_info.JBDX_single_frame_data_input_path.find_last_of("/");
  string frame_data_name = JBDX_single_frame_data_all_opened_layers_info.JBDX_single_frame_data_input_path.substr(starting_position_1 + 1);
  string frame_data_path = YTH_output_data_path_UTF8 + "/" + frame_data_name;
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_output_path = frame_data_path;
  // 获取驱动并创建新的Shapefile图层
...
#pragma region "创建YTH数据输出数据源"
  ...
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS = poDS;
#pragma endregion 

  //	循环对当前分幅数据目录路径下的每一个JBDX数据中的shp图层进行处理
  for (size_t index = 0; index < JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info.size(); index++)
  {
#pragma region "（第二步）获取当前图层在JBDX中图层的名称"
    //	获取当前图层相关信息(JBDX当前图层名称、JBDX当前图层几何类型、JBDX当前图层的指针、JBDX当前图层的坐标参考系统指针)
    const JBDX_single_opened_layer_info_t& current_JBDX_single_layer_info = JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[index];
#pragma endregion

#pragma region "（第三步）在JBDX图层映射表中找到当前图层名称的映射关系，例如pois映射到YTH数据中的哪些图层"
    //	创建一个临时的YTH_JBDX_single_layer_mapping_t自定义数据结构，用来标识JBDX图层同YTH图层之间的映射关系
    YTH_JBDX_single_layer_mapping_t find_JBDX_single_layer_mapping;
    for (size_t index_vYTH_layer_list = 0; index_vYTH_layer_list < YTH_JBDX_layer_mapping_table.vYTH_JBDX_layer_mapping.size(); index_vYTH_layer_list++)
    {
      if (YTH_JBDX_layer_mapping_table.vYTH_JBDX_layer_mapping[index_vYTH_layer_list].JBDX_layer_name == current_JBDX_single_layer_info.layer_name)
      {
        find_JBDX_single_layer_mapping = YTH_JBDX_layer_mapping_table.vYTH_JBDX_layer_mapping[index_vYTH_layer_list];
        break;
      }
    }
#pragma endregion

#pragma region "（第四步）将JBDX图层名称对应一体化图层名称存放到自定义数据结构中（得到一体化图层对应的名字，例如：ORIT_point）"
    //	如果图层映射关系是多对一的，那么则循环逐个判断是否重复，如果没有重复则将其保存在一体化数据结构中
    for (size_t index_vGJBLayer = 0; index_vGJBLayer < find_JBDX_single_layer_mapping.vYTH_layer_list.size(); index_vGJBLayer++)
    {
      //	在将JBDX图层名称对应的一体化数据图层名称存放到映射数据结构中之前需要检查是否已经存在（循环查找）
      int flag_exist = 0;
      for (size_t j = 0; j < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); j++)
      {
        if (YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[j].layer_name == find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer])
        {
          //	如果条件成立，说明JBDX图层名称对应的一体化数据图层名称已经存在了
          flag_exist = 1;
          break;
        }
      }

      //	如果JBDX图层名称对应的一体化数据图层名称不存在，那么将其存放到映射数据结构中（存放名称和存放创建好的图层指针）
      if (flag_exist == 0)
      {
        //	创建一个临时的存储单个一体化图层相关信息的数据结构
        YTH_single_opened_layer_info_t temp;

        //	现在只是将layer_name先添加到数据结构中（layer_geo_type、polayer、poSRS将会在后面进行赋值）
        temp.layer_name = find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer];
        //	获取图层的类型（点图层、线图层、面图层等等）
        //	从GJB图层名称中找到图层的类别（例如ORIT_point找到point）
        size_t starting_position_3 = find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].find_last_of("_");
        string layer_type = find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].substr(starting_position_3 + 1);
        temp.layer_geo_type = layer_type;
        //	相同分幅数据目录中所有的图层空间参考都是一样的，选取JBDX图层其中一个图层的空间参考就可以了（获取JBDX图层的空间参考）
        temp.poSRS = JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[0].poSRS;


        //	需要根据不同的类型来创建图层
        if (temp.layer_geo_type == "point")
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPoint, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "点图层（" + find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（JBDX）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
          ...
          //	创建CPG文件，使得图层属性表可以在GIS软件中显示出本地环境的字符编码（也就是设置图层属性表的字符编码）
          string cpg_file_path = frame_data_path + "/" + temp.layer_name + ".cpg";
          create_shapefile_cpg_UTF8(cpg_file_path, "UTF-8");
        }
        else if (temp.layer_geo_type == "line")
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbLineString, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "线图层（" + find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（JBDX）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPolygon, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "面图层（" + find_JBDX_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（JBDX）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        //	在对所有信息读取完成之后，再将单个一体化图层信息存储到single_frame_GJBLayer_infos
        YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.push_back(temp);
      }
...
void CSE_GeoExtractAndProcess::single_JBDX_frame_data2single_YTH_frame_data(
  const JBDX_single_frame_data_all_opened_layers_info_t& JBDX_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_code_mapping_table_t& YTH_JBDX_code_mapping_table,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  //	循环对YTH_JBDX_code_mapping_table中的编码映射表进行处理
  for (size_t index_layer = 0; index_layer < YTH_JBDX_code_mapping_table.vYTH_JBDX_code_mapping_single_layer.size(); index_layer++)
  {
    const YTH_JBDX_code_mapping_single_layer_t& current_YTH_JBDX_code_mapping_layer = YTH_JBDX_code_mapping_table.vYTH_JBDX_code_mapping_single_layer[index_layer];
    //	对每种YTH图层中的各个要素进行循环处理
    for (size_t index_feature = 0; index_feature < current_YTH_JBDX_code_mapping_layer.vYTH_single_feature.size(); index_feature++)
    {
      const YTH_JBDX_single_feature_t& current_YTH_single_feature = current_YTH_JBDX_code_mapping_layer.vYTH_single_feature[index_feature];
      //	判断当前要素的JBDX筛选条件是否为空，如果不是空的，那么需要得到相对应的JBDX源图层进行筛选并且进行语义融合处理
      if (current_YTH_single_feature.SQL != "")
      {
        //	获得需要进行筛选进行语义融合的JBDX源图层
        std::string JBDX_origion_layer_name = current_YTH_single_feature.JBDX_origin_layer_name;
        if (JBDX_origion_layer_name != "")
        {
          //	从JBDX_single_frame_data_all_opened_layers_info中寻找同JBDX_origion_layer_name名称相同的图层
          int index_flag = -1;
          for (size_t index_JBDX_layers_info = 0; index_JBDX_layers_info < JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info.size(); index_JBDX_layers_info++)
          {
            if (JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[index_JBDX_layers_info].layer_name == JBDX_origion_layer_name)
            {
              index_flag = index_JBDX_layers_info;
              break;
            }
          }
          ...
          if (index_flag != -1)
          {
            ...
            //	2、对找到的图层进行筛选
            JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[index_flag].polayer->SetAttributeFilter(pszSQLCommand);
            OGRFeature* poFeature;
            // 重置图层的读取状态，以便进行下一次查询
            JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[index_flag].polayer->ResetReading();
            while ((poFeature = JBDX_single_frame_data_all_opened_layers_info.vJBDX_single_opened_layer_info[index_flag].polayer->GetNextFeature()) != NULL)
            {

              /*
                利用当前JBDX图层中的当前要素对一体化中的对应图层中的要素进行设置（创建一个YTH新的要素，其中几何信息和属性信息从JBDX要素中获得，
              但是几何信息不发生变化）（应该将新的要素创建到YTH中的哪一个图层中，这是需要的参数信息）
              */
              use_JBDX_layer_feature_set_YTH_layer_feature(
                poFeature,
                YTH_fields4all_layers,
                current_YTH_JBDX_code_mapping_layer,
                index_feature,
                YTH_single_frame_data_all_opened_layers_info);
      ...
      //	如果当前要素的JBDX筛选条件为空，那么跳过当前要素，继续处理下一个要素
      else
      {
        continue;
      }
...
void CSE_GeoExtractAndProcess::use_JBDX_layer_feature_set_YTH_layer_feature(
  const OGRFeature* poFeature,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_JBDX_code_mapping_single_layer_t& YTH_JBDX_code_mapping_single_layer,
  ...
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  ...
  （四）说明
    其中index_feature参数用来指示YTH_JBDX_code_mapping_single_layer中的具体是哪一个feature
  */

#pragma region "获得YTH图层的大类标识"
  //	获得YTH图层的大类标识
  const std::string& strYTHLayerID = YTH_JBDX_code_mapping_single_layer.YTHLayerID;
...
#pragma region "向YTH图层中添加要素"
  //	在YTH_single_frame_data_all_opened_layers_info中查找是否存在YTH_layer_full_name的图层
  int is_find_YTH_layer_flag = 0;
  for (size_t index = 0; index < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); index++)
  {

    if (YTH_layer_full_name == YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index].layer_name)
    {
      ...
      create_new_feature_and_add2YTH_layer(
        poFeature,
        iYTHLayerID,
        YTH_JBDX_code_mapping_single_layer,
        index_feature,
        YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index]);
      break;
    }
  }
...
void CSE_GeoExtractAndProcess::create_new_feature_and_add2YTH_layer(
  ...
  const YTH_JBDX_code_mapping_single_layer_t& YTH_JBDX_code_mapping_single_layer,
  const int& index_feature,
  YTH_single_opened_layer_info_t& YTH_single_opened_layer_info)
{

  //	1、创建一个新的要素，该要素的定义与传入的图层（poLayer）一致
  OGRFeature* poNewFeature = OGRFeature::CreateFeature(YTH_single_opened_layer_info.polayer->GetLayerDefn());
  //	2、设置新要素的几何信息
  poNewFeature->SetGeometry(poFeature->GetGeometryRef());
  ...
  {
    //	设置一体化13图层的“ID”字段
    std::string pszFName = "FID";
    std::string pszValue = poFeature->GetFieldAsString("编号");
    ...
    pszFName = "CODE";
    pszValue = YTH_JBDX_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_code_field;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "NAME";
    pszValue = poFeature->GetFieldAsString("名称");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "TYPE";
    pszValue = poFeature->GetFieldAsString("类型");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "NUM";
    pszValue = poFeature->GetFieldAsString("编号");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "RTEG";
    pszValue = poFeature->GetFieldAsString("等级");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "WIDTH";
    pszValue = poFeature->GetFieldAsString("宽度");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "PAVWIDTH";
    pszValue = poFeature->GetFieldAsString("铺宽");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "BRILEN";
    pszValue = poFeature->GetFieldAsString("桥长");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "HEADROOM";
    pszValue = poFeature->GetFieldAsString("净空高");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "LOAD";
    pszValue = poFeature->GetFieldAsString("载重吨数");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "MILEAGE";
    pszValue = poFeature->GetFieldAsString("里程");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "RELAHEIT";
    pszValue = poFeature->GetFieldAsString("比高");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "VALPRD";
    pszValue = poFeature->GetFieldAsString("通行月份");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "WATDEPTH";
    pszValue = poFeature->GetFieldAsString("水深");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "BOTMSUBS";
    pszValue = poFeature->GetFieldAsString("底质");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "MINCURVR";
    pszValue = poFeature->GetFieldAsString("最小曲率");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "MAXSLOPE";
    pszValue = poFeature->GetFieldAsString("最大纵坡");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "SHPTYPE";
    pszValue = poFeature->GetFieldAsString("图形特征");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
  ...
  //	4、将新要素添加到图层中
  YTH_single_opened_layer_info.polayer->CreateFeature(poNewFeature);
  //	5、将内存中修改好的结果写入到disk中（这里会有性能问题，可以考虑把一个图层中的feature全部转化完之后再写入到disk中，这里是对每一个feature进行的操作，效率不高）
  YTH_single_opened_layer_info.polayer->SyncToDisk();
  //	6、销毁要素
  OGRFeature::DestroyFeature(poNewFeature);
...
#pragma region "（2）OSM---->一体化"
int CSE_GeoExtractAndProcess::DLHJ_convert_OSMDATA2YTHData(
  const std::vector<std::string>& OSM_vinput_data_path_GBK,
  const std::string& YTH_output_data_path_GBK,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  ...
  DLHJProgressFunc pfnProgress)
{
  ...
  * 杨小兵-2024-03-15
  ...
  返回值为整数2：数据源类型不合法
  返回值为整数3：一体化的所有图层字段配置信息为空
  返回值为整数4：一体化同OSM图层映射配置信息为空
  返回值为整数5：一体化同OSM编码映射配置信息为空
  返回值为整数6：OSM到一体化矢量数据语义融合失败
...
#pragma region "（第一步）将输入数据路径和输出数据路径的编码统一转化为utf-8字符集编码"
  ...
  character_set_encoding_conversion character_set_encoding_conversion_instance("UTF-8", "GBK");
  //  用来保存字符集编码转化后的输入数据路径
  std::vector<std::string> OSM_vinput_data_path_UTF8;
  OSM_vinput_data_path_UTF8.clear();
  //  对每个分幅数据输入路径进行转化
  for (size_t index = 0; index < OSM_vinput_data_path_GBK.size(); index++)
  {
    std::string temp = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(OSM_vinput_data_path_GBK[index]);
    OSM_vinput_data_path_UTF8.push_back(temp);
  }
  //  对输出路径进行转化
  std::string YTH_output_data_path_UTF8 = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(YTH_output_data_path_GBK);
  //  检查输入数据路径和输出数据路径字符集编码转化是否成功
  ...
  //	一体化同OSM编码映射配置信息为空
  if (YTH_OSM_code_mapping_table.vYTH_OSM_code_mapping_single_layer.size() == 0)
  {
    return 5;
  }
#pragma endregion
...
int CSE_GeoExtractAndProcess::_DLHJ_convert_OSMData2YTHData(
  const std::vector<std::string>& OSM_vinput_data_path_UTF8,
  const std::string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_OSM_layer_mapping_table_t& YTH_OSM_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  ...
  */
  ...
  size_t size_framing_data = OSM_vinput_data_path_UTF8.size();
  for (size_t index = 0; index < size_framing_data; index++)
  {
    int iresult_single_framing_data;
    iresult_single_framing_data = convert_single_frameOSMData2YTHData(
      OSM_vinput_data_path_UTF8[index],
      YTH_output_data_path_UTF8,
      iDataSourceType,
      YTH_fields4all_layers,
      YTH_OSM_layer_mapping_table,
    ...
    {
      //	返回整数不为0，说明发生了某种错误，记录日志
      string strMsg1 = "分幅数据：" + OSM_vinput_data_path_UTF8[index] + ",OSM数据到一体化矢量数据语义融合失败！";
      QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
      string strTag1 = "（OSM）矢量数据语义融合处理日志";
      QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
      QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
      //  继续处理其他分幅数据
      continue;
...
int CSE_GeoExtractAndProcess::convert_single_frameOSMData2YTHData(
  const string& OSM_single_frame_data_path_UTF8,
  const string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_OSM_layer_mapping_table_t& YTH_OSM_layer_mapping_table,
...
#pragma region "（第一步）将单个shp分幅数据目录路径下的所有图层读取到内存当中（打开资源）"
  ...
  {
    // 错误提示
    string strMsg1 = "分幅数据（" + OSM_single_frame_data_path_UTF8 + "）" + "获取ESRI Shapefile驱动失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（OSM）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
#pragma region "（第二步）将OSM数据所有图层的相关信息读取到自定义数据结构中（single_frame_OSMLayer_infos_t）"
  //	利用矢量驱动器打开单个shp分幅数据目录路径下的数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameOSMDataSet = (GDALDataset*)GDALOpenEx(OSM_single_frame_data_path_UTF8.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  // 文件不存在或打开失败
  if (poSingleFrameOSMDataSet == NULL)
  {
    // 说明打开这个分幅数据目录路径下的文件出现了错误
    // 错误提示
    string strMsg1 = "分幅数据（" + OSM_single_frame_data_path_UTF8 + "）" + "文件不存在或打开失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（OSM）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  //	一体化分幅数据相关信息（对整个分幅数据记录有：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考）
  OSM_single_frame_data_all_opened_layers_info_t OSM_single_frame_data_all_opened_layers_info;
  use_OSMData_create_single_frame_OSMLayer_infos(poSingleFrameOSMDataSet, OSM_single_frame_data_all_opened_layers_info);

#pragma endregion

#pragma region "（第三步）根据OSM数据和图层映射关系将一体化数据所有图层的相关信息读取到自定义数据结构中（YTH_single_frame_data_all_opened_layers_info）"
  YTH_single_frame_data_all_opened_layers_info_t YTH_single_frame_data_all_opened_layers_info;
  use_OSMData_create_single_frame_YTHLayer_infos(
    OSM_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_OSM_layer_mapping_table,
    YTH_output_data_path_UTF8,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第五步）关闭资源"
  ...
  GDALClose(OSM_single_frame_data_all_opened_layers_info.OSM_single_frame_data_poDS);
  GDALClose(YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS);
#pragma endregion
  ...
  OSM_single_frame_data_all_opened_layers_info_t& OSM_single_frame_data_all_opened_layers_info)
{
  ...
  */
  ...
  //	获取当前分幅数据的路径
  OSM_single_frame_data_all_opened_layers_info.OSM_single_frame_data_input_path = poSingleFrameShpDataSet->GetDescription();
  //	获取当前分幅数据的数据集指针
  OSM_single_frame_data_all_opened_layers_info.OSM_single_frame_data_poDS = poSingleFrameShpDataSet;
  ...
  for (int index = 0; index < size_shp_layer; index++)
  {
    OSM_single_opened_layer_info_t temp_single_OSMLayer_info_t;
    ...
    OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info.push_back(temp_single_OSMLayer_info_t);
...
void CSE_GeoExtractAndProcess::use_OSMData_create_single_frame_YTHLayer_infos(
  const OSM_single_frame_data_all_opened_layers_info_t& OSM_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_OSM_layer_mapping_table_t& YTH_OSM_layer_mapping_table,
  const string& YTH_output_data_path_UTF8,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
...
#pragma region "（第一步）根据输出路径构建分幅数据目录路径、在此路径下创建数据集GDALDataset"
  //	构建分幅数据路径
  //	分幅数据名称
  size_t starting_position_1 = OSM_single_frame_data_all_opened_layers_info.OSM_single_frame_data_input_path.find_last_of("/");
  string frame_data_name = OSM_single_frame_data_all_opened_layers_info.OSM_single_frame_data_input_path.substr(starting_position_1 + 1);
  string frame_data_path = YTH_output_data_path_UTF8 + "/" + frame_data_name;
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_output_path = frame_data_path;
  // 获取驱动并创建新的Shapefile图层
  CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "YES");
...
#pragma endregion 
  ...
  for (size_t index = 0; index < OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info.size(); index++)
  {
#pragma region "（第二步）获取当前图层在OSM中图层的名称"
    //	获取当前图层相关信息(OSM当前图层名称、OSM当前图层几何类型、OSM当前图层的指针、OSM当前图层的坐标参考系统指针)
    const OSM_single_opened_layer_info_t& current_OSM_single_layer_info = OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[index];
#pragma endregion
...
#pragma region "（第四步）将OSM图层名称对应一体化图层名称存放到自定义数据结构中（得到一体化图层对应的名字，例如：ORIT_point）"
    ...
    for (size_t index_vGJBLayer = 0; index_vGJBLayer < find_OSM_single_layer_mapping.vYTH_layer_list.size(); index_vGJBLayer++)
    {
      ...
      for (size_t j = 0; j < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); j++)
      {
        if (YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[j].layer_name == find_OSM_single_layer_mapping.vYTH_layer_list[index_vGJBLayer])
        {
          //	如果条件成立，说明OSM图层名称对应的一体化数据图层名称已经存在了
          flag_exist = 1;
          break;
      ...
      if (flag_exist == 0)
      {
        ...
        temp.layer_geo_type = layer_type;
        //	相同分幅数据目录中所有的图层空间参考都是一样的，选取OSM图层其中一个图层的空间参考就可以了（获取OSM图层的空间参考）
        temp.poSRS = OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[0].poSRS;
  ...
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  ...
  for (size_t index_layer = 0; index_layer < YTH_OSM_code_mapping_table.vYTH_OSM_code_mapping_single_layer.size(); index_layer++)
  {
    const YTH_OSM_code_mapping_single_layer_t& current_YTH_OSM_code_mapping_layer = YTH_OSM_code_mapping_table.vYTH_OSM_code_mapping_single_layer[index_layer];
    //	对每种YTH图层中的各个要素进行循环处理
    for (size_t index_feature = 0; index_feature < current_YTH_OSM_code_mapping_layer.vYTH_single_feature.size(); index_feature++)
    {
      const YTH_OSM_single_feature_t& current_YTH_single_feature = current_YTH_OSM_code_mapping_layer.vYTH_single_feature[index_feature];
      ...
      if (current_YTH_single_feature.SQL != "")
      {
        ...
        {
          //	从OSM_single_frame_data_all_opened_layers_info中寻找同OSM_origion_layer_name名称相同的图层
          int index_flag = -1;
          for (size_t index_OSM_layers_info = 0; index_OSM_layers_info < OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info.size(); index_OSM_layers_info++)
          {
            if (OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[index_OSM_layers_info].layer_name == OSM_origion_layer_name)
            {
          ...
          if (index_flag != -1)
          {
            ...
            const char* pszSQLCommand = current_YTH_single_feature.SQL.c_str();
            //	2、对找到的图层进行筛选
            OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[index_flag].polayer->SetAttributeFilter(pszSQLCommand);
            OGRFeature* poFeature;
            // 重置图层的读取状态，以便进行下一次查询
            OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[index_flag].polayer->ResetReading();
            while ((poFeature = OSM_single_frame_data_all_opened_layers_info.vOSM_single_opened_layer_info[index_flag].polayer->GetNextFeature()) != NULL)
            {
  ...
  YTH_single_opened_layer_info_t& YTH_single_opened_layer_info)
{
  ...
  case 13:
  {
    ...
    pszFName = "CODE";
    pszValue = YTH_OSM_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_code_field;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "FGUID";
    pszValue = YTH_OSM_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_classification_name;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    else if(OSM_origin_feature_value == "B")
    {
      pszValue = "双";
    }
    else
...
#pragma region "（3）S57---->一体化"
int CSE_GeoExtractAndProcess::DLHJ_convert_S57DATA2YTHData(
  const std::vector<std::string>& S57_vinput_data_path_GBK,
  const std::string& YTH_output_data_path_GBK,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_layer_mapping_table_t& YTH_S57_layer_mapping_table,
  const YTH_S57_code_mapping_table_t& YTH_S57_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  /*
  * 杨小兵-2024-03-15
  ...
  返回值为整数2：数据源类型不合法
  返回值为整数3：一体化的所有图层字段配置信息为空
  返回值为整数4：一体化同S57图层映射配置信息为空
  返回值为整数5：一体化同S57编码映射配置信息为空
  返回值为整数6：S57到一体化矢量数据语义融合失败
...
#pragma region "（第一步）将输入数据路径和输出数据路径的编码统一转化为utf-8字符集编码"
  ...
  character_set_encoding_conversion character_set_encoding_conversion_instance("UTF-8", "GBK");
  //  用来保存字符集编码转化后的输入数据路径
  std::vector<std::string> S57_vinput_data_path_UTF8;
  S57_vinput_data_path_UTF8.clear();
  //  对每个分幅数据输入路径进行转化
  for (size_t index = 0; index < S57_vinput_data_path_GBK.size(); index++)
  {
    std::string temp = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(S57_vinput_data_path_GBK[index]);
    S57_vinput_data_path_UTF8.push_back(temp);
  }
  //  对输出路径进行转化
  std::string YTH_output_data_path_UTF8 = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(YTH_output_data_path_GBK);
  //  检查输入数据路径和输出数据路径字符集编码转化是否成功
  ...
  //	一体化同S57图层映射配置信息为空
  if (YTH_S57_layer_mapping_table.vYTH_S57_layer_mapping.size() == 0)
  {
    return 4;
  }

  //	一体化同S57编码映射配置信息为空
  if (YTH_S57_code_mapping_table.vYTH_S57_code_mapping_single_layer.size() == 0)
  {
    return 5;
  }
#pragma endregion

#pragma region "（第二步）S57数据到一体化数据转化"
  int iResult;
  //	DLHJ_convert_S57DATA2YTHData是被包装的函数，_DLHJ_convert_S57Data2YTHData为实际执行数据转化的函数
  iResult = _DLHJ_convert_S57Data2YTHData(
    S57_vinput_data_path_UTF8,
    YTH_output_data_path_UTF8,
    iDataSourceType,
    YTH_fields4all_layers,
...
int CSE_GeoExtractAndProcess::_DLHJ_convert_S57Data2YTHData(
  const std::vector<std::string>& S57_vinput_data_path_UTF8,
  const std::string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_layer_mapping_table_t& YTH_S57_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  ...
  （一）参数说明：
      ...
      3. **`iDataSourceType`**: 类型为 `int`。
         - **描述**: 数据源类型的标识符，用于区分不同种类或格式的地理数据。
      4. **`YTH_fields4all_layers`**: 类型为 `YTH_fields4all_layers_t&`（引用）。
         - **描述**: 包含 S57 层信息的列表，用于存储和管理 S57 层的相关数据。
      5. **`YTH_S57_layer_mapping_table`**: 类型为 `YTH_S57_layer_mapping_table_t&`（引用）。
         - **描述**: 包含 GJB 层字段信息的列表，用于存储和管理 GJB 层字段的相关数据。
      6. **`YTH_S57_code_mapping_table`**: 类型为 `YTH_S57_code_mapping_table_t&`（引用）。
         - **描述**: 用于存储 S57 到 GJB 的层映射信息。
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    vstrInputDataPath中存储的是多个分幅数据，每一个分幅数据中是由多个图层构成的，这个函数将会把vstrInputDataPath中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、整体上将会对分幅数据循环操作，执行S57--->一体化数据的转化
    2、对单个分幅数据进行处理
    3、处理结果处理
  */

  size_t size_framing_data = S57_vinput_data_path_UTF8.size();
  for (size_t index = 0; index < size_framing_data; index++)
  {
    int iresult_single_framing_data;
    iresult_single_framing_data = convert_single_frameS57Data2YTHData(
      S57_vinput_data_path_UTF8[index],
      YTH_output_data_path_UTF8,
      iDataSourceType,
      YTH_fields4all_layers,
      YTH_S57_layer_mapping_table,
    ...
    //	对单个S57分幅数据转化成一体化数据结果进行判断
    if (iresult_single_framing_data != 0)
    {
      //	返回整数不为0，说明发生了某种错误，记录日志
      string strMsg1 = "分幅数据：" + S57_vinput_data_path_UTF8[index] + ",S57数据到一体化矢量数据语义融合失败！";
      QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
      string strTag1 = "（S57）矢量数据语义融合处理日志";
      QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
      QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
      //  继续处理其他分幅数据
      continue;
...
int CSE_GeoExtractAndProcess::convert_single_frameS57Data2YTHData(
  const string& S57_single_frame_data_path_UTF8,
  const string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_layer_mapping_table_t& YTH_S57_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  /*
  （一）参数说明：
      1. **`single_frame_S57_data_path`**: 类型为 `std::string&`。
         - **描述**: 用于存储单个输入数据文件路径（单个分幅数据目录路径）。
      2. **`strOutputDataPath`**: 类型为 `std::string&`。
         - **描述**: 输出数据的目标存储路径。
      3. **`iDataSourceType`**: 类型为 `int&`。
      ...
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    single_frame_S57_data_path中存储的是单个分幅数据目录路径，这个函数将会把single_frame_S57_data_path中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、将单个shp分幅数据目录路径下的所有图层读取到内存当中
    2、将所有图层的相关信息读取到自定义数据结构中（图层名称、图层类型、图层指针等等）
...
#pragma region "（第一步）将单个shp分幅数据目录路径下的所有图层读取到内存当中（打开资源）"
  ...
  {
    // 错误提示
    string strMsg1 = "分幅数据（" + S57_single_frame_data_path_UTF8 + "）" + "获取ESRI Shapefile驱动失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（S57）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
#pragma region "（第二步）将S57数据所有图层的相关信息读取到自定义数据结构中（single_frame_S57Layer_infos_t）"
  //	利用矢量驱动器打开单个shp分幅数据目录路径下的数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameS57DataSet = (GDALDataset*)GDALOpenEx(S57_single_frame_data_path_UTF8.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  // 文件不存在或打开失败
  if (poSingleFrameS57DataSet == NULL)
  {
    // 说明打开这个分幅数据目录路径下的文件出现了错误
    // 错误提示
    string strMsg1 = "分幅数据（" + S57_single_frame_data_path_UTF8 + "）" + "文件不存在或打开失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（S57）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  //	一体化分幅数据相关信息（对整个分幅数据记录有：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考）
  S57_single_frame_data_all_opened_layers_info_t S57_single_frame_data_all_opened_layers_info;
  use_S57Data_create_single_frame_S57Layer_infos(poSingleFrameS57DataSet, S57_single_frame_data_all_opened_layers_info);

#pragma endregion

#pragma region "（第三步）根据S57数据和图层映射关系将一体化数据所有图层的相关信息读取到自定义数据结构中（YTH_single_frame_data_all_opened_layers_info）"
  YTH_single_frame_data_all_opened_layers_info_t YTH_single_frame_data_all_opened_layers_info;
  use_S57Data_create_single_frame_YTHLayer_infos(
    S57_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_S57_layer_mapping_table,
    YTH_output_data_path_UTF8,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第四步）对当前分幅数据中的图层进行转化操作"
  ...
  single_S57_frame_data2single_YTH_frame_data(
    S57_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_S57_code_mapping_table,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第五步）关闭资源"
  ...
  GDALClose(S57_single_frame_data_all_opened_layers_info.S57_single_frame_data_poDS);
  GDALClose(YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS);
#pragma endregion
...
void CSE_GeoExtractAndProcess::use_S57Data_create_single_frame_S57Layer_infos(
  GDALDataset* poSingleFrameShpDataSet,
  S57_single_frame_data_all_opened_layers_info_t& S57_single_frame_data_all_opened_layers_info)
{
  /*
  ...
  */
  ...
  //	获取当前分幅数据的路径
  S57_single_frame_data_all_opened_layers_info.S57_single_frame_data_input_path = poSingleFrameShpDataSet->GetDescription();
  //	获取当前分幅数据的数据集指针
  S57_single_frame_data_all_opened_layers_info.S57_single_frame_data_poDS = poSingleFrameShpDataSet;
  ...
  for (int index = 0; index < size_shp_layer; index++)
  {
    S57_single_opened_layer_info_t temp_single_S57Layer_info_t;
    ...
    temp_single_S57Layer_info_t.layer_name = temp_poLayer->GetName();
    OGRwkbGeometryType eType = temp_poLayer->GetGeomType();
    temp_single_S57Layer_info_t.layer_geo_type = OGRGeometryTypeToName(eType);
    temp_single_S57Layer_info_t.polayer = temp_poLayer;
    temp_single_S57Layer_info_t.poSRS = temp_poLayer->GetSpatialRef();

    S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info.push_back(temp_single_S57Layer_info_t);
...
void CSE_GeoExtractAndProcess::use_S57Data_create_single_frame_YTHLayer_infos(
  const S57_single_frame_data_all_opened_layers_info_t& S57_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_layer_mapping_table_t& YTH_S57_layer_mapping_table,
  const string& YTH_output_data_path_UTF8,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  /*
  （一）函数功能：
    根据单个分幅数据中所有图层的名称（S57数据格式中的图层名称分类标准，例如BOUP）创建一体化数据格式中的名称（例如BOUP_point），从数学的角度来说就是
  建议一个从N个图层名称到M个图层名称的映射关系，还有图层指针信息，这个函数中还没有执行具体的转化，只是确定了要将分幅数据中的S57图层转化成一体化的哪些图层
  （二）算法流程
    1、循环处理一个分幅数据中的所有shp图层数据
    2、获取当前图层的名称（在S57格式中图层的名称）
    3、在S57Layer.xml配置文件中寻找到对应的关系映射
    4、根据关系映射创建一体化数据格式中图层的名称
  */
...
#pragma region "（第一步）根据输出路径构建分幅数据目录路径、在此路径下创建数据集GDALDataset"
  //	构建分幅数据路径
  //	分幅数据名称
  size_t starting_position_1 = S57_single_frame_data_all_opened_layers_info.S57_single_frame_data_input_path.find_last_of("/");
  string frame_data_name = S57_single_frame_data_all_opened_layers_info.S57_single_frame_data_input_path.substr(starting_position_1 + 1);
  string frame_data_path = YTH_output_data_path_UTF8 + "/" + frame_data_name;
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_output_path = frame_data_path;
  // 获取驱动并创建新的Shapefile图层
  CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "YES");
...
#pragma endregion 
  ...
  //	循环对当前分幅数据目录路径下的每一个S57数据中的shp图层进行处理
  for (size_t index = 0; index < S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info.size(); index++)
  {
#pragma region "（第二步）获取当前图层在S57中图层的名称"
    //	获取当前图层相关信息(S57当前图层名称、S57当前图层几何类型、S57当前图层的指针、S57当前图层的坐标参考系统指针)
    const S57_single_opened_layer_info_t& current_S57_single_layer_info = S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[index];
#pragma endregion

#pragma region "（第三步）在S57图层映射表中找到当前图层名称的映射关系，例如pois映射到YTH数据中的哪些图层"
    //	创建一个临时的YTH_S57_single_layer_mapping_t自定义数据结构，用来标识S57图层同YTH图层之间的映射关系
    YTH_S57_single_layer_mapping_t find_S57_single_layer_mapping;
    for (size_t index_vYTH_layer_list = 0; index_vYTH_layer_list < YTH_S57_layer_mapping_table.vYTH_S57_layer_mapping.size(); index_vYTH_layer_list++)
    {
      if (YTH_S57_layer_mapping_table.vYTH_S57_layer_mapping[index_vYTH_layer_list].S57_layer_name == current_S57_single_layer_info.layer_name)
      {
        find_S57_single_layer_mapping = YTH_S57_layer_mapping_table.vYTH_S57_layer_mapping[index_vYTH_layer_list];
        break;
      }
    }
#pragma endregion

#pragma region "（第四步）将S57图层名称对应一体化图层名称存放到自定义数据结构中（得到一体化图层对应的名字，例如：ORIT_point）"
    //	如果图层映射关系是多对一的，那么则循环逐个判断是否重复，如果没有重复则将其保存在一体化数据结构中
    for (size_t index_vGJBLayer = 0; index_vGJBLayer < find_S57_single_layer_mapping.vYTH_layer_list.size(); index_vGJBLayer++)
    {
      //	在将S57图层名称对应的一体化数据图层名称存放到映射数据结构中之前需要检查是否已经存在（循环查找）
      int flag_exist = 0;
      for (size_t j = 0; j < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); j++)
      {
        if (YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[j].layer_name == find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer])
        {
          //	如果条件成立，说明S57图层名称对应的一体化数据图层名称已经存在了
          flag_exist = 1;
          break;
        }
      }

      //	如果S57图层名称对应的一体化数据图层名称不存在，那么将其存放到映射数据结构中（存放名称和存放创建好的图层指针）
      if (flag_exist == 0)
      {
        //	创建一个临时的存储单个一体化图层相关信息的数据结构
        YTH_single_opened_layer_info_t temp;
        ...
        //	现在只是将layer_name先添加到数据结构中（layer_geo_type、polayer、poSRS将会在后面进行赋值）
        temp.layer_name = find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer];
        //	获取图层的类型（点图层、线图层、面图层等等）
        //	从GJB图层名称中找到图层的类别（例如ORIT_point找到point）
        size_t starting_position_3 = find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].find_last_of("_");
        string layer_type = find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].substr(starting_position_3 + 1);
        temp.layer_geo_type = layer_type;
        //	相同分幅数据目录中所有的图层空间参考都是一样的，选取S57图层其中一个图层的空间参考就可以了（获取S57图层的空间参考）
        temp.poSRS = S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[0].poSRS;


        //	需要根据不同的类型来创建图层
        if (temp.layer_geo_type == "point")
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPoint, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "点图层（" + find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（S57）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbLineString, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "线图层（" + find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（S57）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPolygon, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "面图层（" + find_S57_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（S57）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
void CSE_GeoExtractAndProcess::single_S57_frame_data2single_YTH_frame_data(
  const S57_single_frame_data_all_opened_layers_info_t& S57_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_code_mapping_table_t& YTH_S57_code_mapping_table,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  //	循环对YTH_S57_code_mapping_table中的编码映射表进行处理
  for (size_t index_layer = 0; index_layer < YTH_S57_code_mapping_table.vYTH_S57_code_mapping_single_layer.size(); index_layer++)
  {
    const YTH_S57_code_mapping_single_layer_t& current_YTH_S57_code_mapping_layer = YTH_S57_code_mapping_table.vYTH_S57_code_mapping_single_layer[index_layer];
    //	对每种YTH图层中的各个要素进行循环处理
    for (size_t index_feature = 0; index_feature < current_YTH_S57_code_mapping_layer.vYTH_single_feature.size(); index_feature++)
    {
      const YTH_S57_single_feature_t& current_YTH_single_feature = current_YTH_S57_code_mapping_layer.vYTH_single_feature[index_feature];
      //	判断当前要素的S57筛选条件是否为空，如果不是空的，那么需要得到相对应的S57源图层进行筛选并且进行语义融合处理
      if (current_YTH_single_feature.SQL != "")
      {
        //	获得需要进行筛选进行语义融合的S57源图层
        std::string S57_origion_layer_name = current_YTH_single_feature.S57_origin_layer_name;
        if (S57_origion_layer_name != "")
        {
          //	从S57_single_frame_data_all_opened_layers_info中寻找同S57_origion_layer_name名称相同的图层
          int index_flag = -1;
          for (size_t index_S57_layers_info = 0; index_S57_layers_info < S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info.size(); index_S57_layers_info++)
          {
            if (S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[index_S57_layers_info].layer_name == S57_origion_layer_name)
            {
              index_flag = index_S57_layers_info;
              break;
            }
          }
          //	判断是否找到了想要进行筛选的图层（不是-1的话说明找到了）
          if (index_flag != -1)
          {
            ...
            const char* pszSQLCommand = current_YTH_single_feature.SQL.c_str();
            //	2、对找到的图层进行筛选
            S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[index_flag].polayer->SetAttributeFilter(pszSQLCommand);
            OGRFeature* poFeature;
            // 重置图层的读取状态，以便进行下一次查询
            S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[index_flag].polayer->ResetReading();
            while ((poFeature = S57_single_frame_data_all_opened_layers_info.vS57_single_opened_layer_info[index_flag].polayer->GetNextFeature()) != NULL)
            {

              /*
                利用当前S57图层中的当前要素对一体化中的对应图层中的要素进行设置（创建一个YTH新的要素，其中几何信息和属性信息从S57要素中获得，
              但是几何信息不发生变化）（应该将新的要素创建到YTH中的哪一个图层中，这是需要的参数信息）
              */
              use_S57_layer_feature_set_YTH_layer_feature(
                poFeature,
                YTH_fields4all_layers,
                current_YTH_S57_code_mapping_layer,
                index_feature,
                YTH_single_frame_data_all_opened_layers_info);
      ...
      //	如果当前要素的S57筛选条件为空，那么跳过当前要素，继续处理下一个要素
      else
      {
        continue;
      }
...
void CSE_GeoExtractAndProcess::use_S57_layer_feature_set_YTH_layer_feature(
  const OGRFeature* poFeature,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_S57_code_mapping_single_layer_t& YTH_S57_code_mapping_single_layer,
  const int& index_feature,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  ...
  （三）几何类型
    ...
    - `wkbGeometryCollection`: 几何集合
  （四）说明
    其中index_feature参数用来指示YTH_S57_code_mapping_single_layer中的具体是哪一个feature
  */

#pragma region "获得YTH图层的大类标识"
  //	获得YTH图层的大类标识
  const std::string& strYTHLayerID = YTH_S57_code_mapping_single_layer.YTHLayerID;
  const int iYTHLayerID = std::stoi(strYTHLayerID);
  std::string YTH_layer_full_name;
  switch (iYTHLayerID)
  {
  ...
  for (size_t index = 0; index < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); index++)
  {
    ...
    if (YTH_layer_full_name == YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index].layer_name)
    {
      ...
      create_new_feature_and_add2YTH_layer(
        poFeature,
        iYTHLayerID,
        YTH_S57_code_mapping_single_layer,
        index_feature,
        YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index]);
      break;
    }
  ...
  YTH_single_opened_layer_info_t& YTH_single_opened_layer_info)
{
  ...
  case 25:
  {
    ...
    pszFName = "CODE";
    pszValue = YTH_S57_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_code_field;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "NAME";
    pszValue = poFeature->GetFieldAsString("OBJNAM");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "TYPE";
    if (isFieldPresent(poFeature, "CATDIS"))
    {
      pszValue = poFeature->GetFieldAsString("CATDIS");
    }
    else if (isFieldPresent(poFeature, "CATFRY"))
    {
      pszValue = poFeature->GetFieldAsString("CATFRY");
    }
    else if (isFieldPresent(poFeature, "CATTRK"))
    {
      pszValue = poFeature->GetFieldAsString("CATTRK");
    }
    else if (isFieldPresent(poFeature, "CATTSS"))
    {
      pszValue = poFeature->GetFieldAsString("CATTSS");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "COLOR";
    pszValue = poFeature->GetFieldAsString("COLOUR");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "COLRPATN";
    pszValue = poFeature->GetFieldAsString("COLPAT");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "HEIGHT";
    if (isFieldPresent(poFeature, "VERLEN"))
    {
      pszValue = poFeature->GetFieldAsString("VERLEN");
    }
    else if (isFieldPresent(poFeature, "HEIGHT"))
    {
      pszValue = poFeature->GetFieldAsString("HEIGHT");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "SIGNLSET"; 
    pszValue = poFeature->GetFieldAsString("SIGGRP");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "SIGNLCYC";
    pszValue = poFeature->GetFieldAsString("SIGPER");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());


    pszFName = "ACTDIS"; 
    if (isFieldPresent(poFeature, "VALMXR"))
    {
      pszValue = poFeature->GetFieldAsString("VALMXR");
    }
    else if (isFieldPresent(poFeature, "ESTRNG"))
    {
      pszValue = poFeature->GetFieldAsString("ESTRNG");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "AZIMUTH"; 
    pszValue = poFeature->GetFieldAsString("ORIENT");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "RADARVIS"; 
    pszValue = poFeature->GetFieldAsString("CONRAD");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());


    pszFName = "WATDEPTH";
    if (isFieldPresent(poFeature, "DRVAL1"))
    {
      pszValue = poFeature->GetFieldAsString("DRVAL1");
    }
    else if(isFieldPresent(poFeature, "DRVAL2"))
    {
      pszValue = poFeature->GetFieldAsString("DRVAL2");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "ACTMODE";
    pszValue = poFeature->GetFieldAsString("STATUS");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "PRODATE";
    pszValue = poFeature->GetFieldAsString("RECDAT");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "SCALE";
    if (isFieldPresent(poFeature, "SCAMAX"))
    {
      pszValue = poFeature->GetFieldAsString("SCAMAX");
    }
    else if (isFieldPresent(poFeature, "SCAMIN"))
    {
      pszValue = poFeature->GetFieldAsString("SCAMIN");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "DATASRC";
    pszValue = poFeature->GetFieldAsString("SORDAT");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
  ...
  case 28:
  {
    ...
    pszFName = "TYPE";
    if (isFieldPresent(poFeature, "CATACH"))
    {
      pszValue = poFeature->GetFieldAsString("CATACH");
    }
    else if (isFieldPresent(poFeature, "CATCBL"))
    {
      pszValue = poFeature->GetFieldAsString("CATCBL");
    }
    else if (isFieldPresent(poFeature, "CATPIP"))
    {
      pszValue = poFeature->GetFieldAsString("CATPIP");
    }
    else if (isFieldPresent(poFeature, "CATMFA"))
    {
      pszValue = poFeature->GetFieldAsString("CATMFA");
    }
    else if (isFieldPresent(poFeature, "CATICE"))
    {
      pszValue = poFeature->GetFieldAsString("CATICE");
    }
    else if (isFieldPresent(poFeature, "CATVEG"))
    {
      pszValue = poFeature->GetFieldAsString("CATVEG");
    }
    else
    {
      pszValue = "";
    ...
    pszFName = "EMBDEPTH";
    pszValue = poFeature->GetFieldAsString("BURDEP");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "RADIUS";
    pszValue = poFeature->GetFieldAsString("RADIUS");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
...
#pragma region "（4）SYDH---->一体化"
int CSE_GeoExtractAndProcess::DLHJ_convert_SYDHDATA2YTHData(
  const std::vector<std::string>& SYDH_vinput_data_path_GBK,
  const std::string& YTH_output_data_path_GBK,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_layer_mapping_table_t& YTH_SYDH_layer_mapping_table,
  const YTH_SYDH_code_mapping_table_t& YTH_SYDH_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  /*
  * 杨小兵-2024-03-15
  ...
  返回值为整数2：数据源类型不合法
  返回值为整数3：一体化的所有图层字段配置信息为空
  返回值为整数4：一体化同SYDH图层映射配置信息为空
  返回值为整数5：一体化同SYDH编码映射配置信息为空
  返回值为整数6：SYDH到一体化矢量数据语义融合失败
...
#pragma region "（第一步）将输入数据路径和输出数据路径的编码统一转化为utf-8字符集编码"
  ...
  character_set_encoding_conversion character_set_encoding_conversion_instance("UTF-8", "GBK");
  //  用来保存字符集编码转化后的输入数据路径
  std::vector<std::string> SYDH_vinput_data_path_UTF8;
  SYDH_vinput_data_path_UTF8.clear();
  //  对每个分幅数据输入路径进行转化
  for (size_t index = 0; index < SYDH_vinput_data_path_GBK.size(); index++)
  {
    std::string temp = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(SYDH_vinput_data_path_GBK[index]);
    SYDH_vinput_data_path_UTF8.push_back(temp);
  }
  //  对输出路径进行转化
  std::string YTH_output_data_path_UTF8 = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(YTH_output_data_path_GBK);
  //  检查输入数据路径和输出数据路径字符集编码转化是否成功
  ...
  //	一体化同SYDH图层映射配置信息为空
  if (YTH_SYDH_layer_mapping_table.vYTH_SYDH_layer_mapping.size() == 0)
  {
    return 4;
  }

  //	一体化同SYDH编码映射配置信息为空
  if (YTH_SYDH_code_mapping_table.vYTH_SYDH_code_mapping_single_layer.size() == 0)
  {
    return 5;
  }
#pragma endregion

#pragma region "（第二步）SYDH数据到一体化数据转化"
  int iResult;
  //	DLHJ_convert_SYDHDATA2YTHData是被包装的函数，_DLHJ_convert_SYDHData2YTHData为实际执行数据转化的函数
  iResult = _DLHJ_convert_SYDHData2YTHData(
    SYDH_vinput_data_path_UTF8,
    YTH_output_data_path_UTF8,
    iDataSourceType,
    YTH_fields4all_layers,
...
int CSE_GeoExtractAndProcess::_DLHJ_convert_SYDHData2YTHData(
  const std::vector<std::string>& SYDH_vinput_data_path_UTF8,
  const std::string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_layer_mapping_table_t& YTH_SYDH_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  ...
  （一）参数说明：
      ...
      3. **`iDataSourceType`**: 类型为 `int`。
         - **描述**: 数据源类型的标识符，用于区分不同种类或格式的地理数据。
      4. **`YTH_fields4all_layers`**: 类型为 `YTH_fields4all_layers_t&`（引用）。
         - **描述**: 包含 SYDH 层信息的列表，用于存储和管理 SYDH 层的相关数据。
      5. **`YTH_SYDH_layer_mapping_table`**: 类型为 `YTH_SYDH_layer_mapping_table_t&`（引用）。
         - **描述**: 包含 GJB 层字段信息的列表，用于存储和管理 GJB 层字段的相关数据。
      6. **`YTH_SYDH_code_mapping_table`**: 类型为 `YTH_SYDH_code_mapping_table_t&`（引用）。
         - **描述**: 用于存储 SYDH 到 GJB 的层映射信息。
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    vstrInputDataPath中存储的是多个分幅数据，每一个分幅数据中是由多个图层构成的，这个函数将会把vstrInputDataPath中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、整体上将会对分幅数据循环操作，执行SYDH--->一体化数据的转化
    2、对单个分幅数据进行处理
    3、处理结果处理
  */

  size_t size_framing_data = SYDH_vinput_data_path_UTF8.size();
  for (size_t index = 0; index < size_framing_data; index++)
  {
    int iresult_single_framing_data;
    iresult_single_framing_data = convert_single_frameSYDHData2YTHData(
      SYDH_vinput_data_path_UTF8[index],
      YTH_output_data_path_UTF8,
      iDataSourceType,
      YTH_fields4all_layers,
      YTH_SYDH_layer_mapping_table,
    ...
    //	对单个SYDH分幅数据转化成一体化数据结果进行判断
    if (iresult_single_framing_data != 0)
    {
      //	返回整数不为0，说明发生了某种错误，记录日志
      string strMsg1 = "分幅数据：" + SYDH_vinput_data_path_UTF8[index] + ",SYDH数据到一体化矢量数据语义融合失败！";
      QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
      string strTag1 = "（SYDH）矢量数据语义融合处理日志";
      QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
      QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
      //  继续处理其他分幅数据
      continue;
...
int CSE_GeoExtractAndProcess::convert_single_frameSYDHData2YTHData(
  const string& SYDH_single_frame_data_path_UTF8,
  const string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_layer_mapping_table_t& YTH_SYDH_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  /*
  （一）参数说明：
      1. **`single_frame_SYDH_data_path`**: 类型为 `std::string&`。
         - **描述**: 用于存储单个输入数据文件路径（单个分幅数据目录路径）。
      2. **`strOutputDataPath`**: 类型为 `std::string&`。
         - **描述**: 输出数据的目标存储路径。
      3. **`iDataSourceType`**: 类型为 `int&`。
      ...
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    single_frame_SYDH_data_path中存储的是单个分幅数据目录路径，这个函数将会把single_frame_SYDH_data_path中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、将单个shp分幅数据目录路径下的所有图层读取到内存当中
    2、将所有图层的相关信息读取到自定义数据结构中（图层名称、图层类型、图层指针等等）
...
#pragma region "（第一步）将单个shp分幅数据目录路径下的所有图层读取到内存当中（打开资源）"
  ...
  {
    // 错误提示
    string strMsg1 = "分幅数据（" + SYDH_single_frame_data_path_UTF8 + "）" + "获取ESRI Shapefile驱动失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（SYDH）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
#pragma region "（第二步）将SYDH数据所有图层的相关信息读取到自定义数据结构中（single_frame_SYDHLayer_infos_t）"
  //	利用矢量驱动器打开单个shp分幅数据目录路径下的数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameSYDHDataSet = (GDALDataset*)GDALOpenEx(SYDH_single_frame_data_path_UTF8.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  // 文件不存在或打开失败
  if (poSingleFrameSYDHDataSet == NULL)
  {
    // 说明打开这个分幅数据目录路径下的文件出现了错误
    // 错误提示
    string strMsg1 = "分幅数据（" + SYDH_single_frame_data_path_UTF8 + "）" + "文件不存在或打开失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（SYDH）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  //	一体化分幅数据相关信息（对整个分幅数据记录有：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考）
  SYDH_single_frame_data_all_opened_layers_info_t SYDH_single_frame_data_all_opened_layers_info;
  use_SYDHData_create_single_frame_SYDHLayer_infos(poSingleFrameSYDHDataSet, SYDH_single_frame_data_all_opened_layers_info);

#pragma endregion

#pragma region "（第三步）根据SYDH数据和图层映射关系将一体化数据所有图层的相关信息读取到自定义数据结构中（YTH_single_frame_data_all_opened_layers_info）"
  YTH_single_frame_data_all_opened_layers_info_t YTH_single_frame_data_all_opened_layers_info;
  use_SYDHData_create_single_frame_YTHLayer_infos(
    SYDH_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_SYDH_layer_mapping_table,
    YTH_output_data_path_UTF8,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第四步）对当前分幅数据中的图层进行转化操作"
  ...
  single_SYDH_frame_data2single_YTH_frame_data(
    SYDH_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_SYDH_code_mapping_table,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第五步）关闭资源"
  ...
  GDALClose(SYDH_single_frame_data_all_opened_layers_info.SYDH_single_frame_data_poDS);
  GDALClose(YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS);
#pragma endregion
...
void CSE_GeoExtractAndProcess::use_SYDHData_create_single_frame_SYDHLayer_infos(
  GDALDataset* poSingleFrameShpDataSet,
  SYDH_single_frame_data_all_opened_layers_info_t& SYDH_single_frame_data_all_opened_layers_info)
{
  /*
  ...
  */
  ...
  //	获取当前分幅数据的路径
  SYDH_single_frame_data_all_opened_layers_info.SYDH_single_frame_data_input_path = poSingleFrameShpDataSet->GetDescription();
  //	获取当前分幅数据的数据集指针
  SYDH_single_frame_data_all_opened_layers_info.SYDH_single_frame_data_poDS = poSingleFrameShpDataSet;
  ...
  for (int index = 0; index < size_shp_layer; index++)
  {
    SYDH_single_opened_layer_info_t temp_single_SYDHLayer_info_t;
    ...
    temp_single_SYDHLayer_info_t.layer_name = temp_poLayer->GetName();
    OGRwkbGeometryType eType = temp_poLayer->GetGeomType();
    temp_single_SYDHLayer_info_t.layer_geo_type = OGRGeometryTypeToName(eType);
    temp_single_SYDHLayer_info_t.polayer = temp_poLayer;
    temp_single_SYDHLayer_info_t.poSRS = temp_poLayer->GetSpatialRef();

    SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info.push_back(temp_single_SYDHLayer_info_t);
...
void CSE_GeoExtractAndProcess::use_SYDHData_create_single_frame_YTHLayer_infos(
  const SYDH_single_frame_data_all_opened_layers_info_t& SYDH_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_layer_mapping_table_t& YTH_SYDH_layer_mapping_table,
  const string& YTH_output_data_path_UTF8,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  /*
  （一）函数功能：
    根据单个分幅数据中所有图层的名称（SYDH数据格式中的图层名称分类标准，例如BOUP）创建一体化数据格式中的名称（例如BOUP_point），从数学的角度来说就是
  建议一个从N个图层名称到M个图层名称的映射关系，还有图层指针信息，这个函数中还没有执行具体的转化，只是确定了要将分幅数据中的SYDH图层转化成一体化的哪些图层
  （二）算法流程
    1、循环处理一个分幅数据中的所有shp图层数据
    2、获取当前图层的名称（在SYDH格式中图层的名称）
    3、在SYDHLayer.xml配置文件中寻找到对应的关系映射
    4、根据关系映射创建一体化数据格式中图层的名称
  */
...
#pragma region "（第一步）根据输出路径构建分幅数据目录路径、在此路径下创建数据集GDALDataset"
  //	构建分幅数据路径
  //	分幅数据名称
  size_t starting_position_1 = SYDH_single_frame_data_all_opened_layers_info.SYDH_single_frame_data_input_path.find_last_of("/");
  string frame_data_name = SYDH_single_frame_data_all_opened_layers_info.SYDH_single_frame_data_input_path.substr(starting_position_1 + 1);
  string frame_data_path = YTH_output_data_path_UTF8 + "/" + frame_data_name;
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_output_path = frame_data_path;
  // 获取驱动并创建新的Shapefile图层
  CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "YES");
...
#pragma endregion 
  ...
  //	循环对当前分幅数据目录路径下的每一个SYDH数据中的shp图层进行处理
  for (size_t index = 0; index < SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info.size(); index++)
  {
#pragma region "（第二步）获取当前图层在SYDH中图层的名称"
    //	获取当前图层相关信息(SYDH当前图层名称、SYDH当前图层几何类型、SYDH当前图层的指针、SYDH当前图层的坐标参考系统指针)
    const SYDH_single_opened_layer_info_t& current_SYDH_single_layer_info = SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[index];
#pragma endregion

#pragma region "（第三步）在SYDH图层映射表中找到当前图层名称的映射关系，例如pois映射到YTH数据中的哪些图层"
    //	创建一个临时的YTH_SYDH_single_layer_mapping_t自定义数据结构，用来标识SYDH图层同YTH图层之间的映射关系
    YTH_SYDH_single_layer_mapping_t find_SYDH_single_layer_mapping;
    for (size_t index_vYTH_layer_list = 0; index_vYTH_layer_list < YTH_SYDH_layer_mapping_table.vYTH_SYDH_layer_mapping.size(); index_vYTH_layer_list++)
    {
      if (YTH_SYDH_layer_mapping_table.vYTH_SYDH_layer_mapping[index_vYTH_layer_list].SYDH_layer_name == current_SYDH_single_layer_info.layer_name)
      {
        find_SYDH_single_layer_mapping = YTH_SYDH_layer_mapping_table.vYTH_SYDH_layer_mapping[index_vYTH_layer_list];
        break;
      }
    }
#pragma endregion

#pragma region "（第四步）将SYDH图层名称对应一体化图层名称存放到自定义数据结构中（得到一体化图层对应的名字，例如：ORIT_point）"
    //	如果图层映射关系是多对一的，那么则循环逐个判断是否重复，如果没有重复则将其保存在一体化数据结构中
    for (size_t index_vGJBLayer = 0; index_vGJBLayer < find_SYDH_single_layer_mapping.vYTH_layer_list.size(); index_vGJBLayer++)
    {
      //	在将SYDH图层名称对应的一体化数据图层名称存放到映射数据结构中之前需要检查是否已经存在（循环查找）
      int flag_exist = 0;
      for (size_t j = 0; j < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); j++)
      {
        if (YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[j].layer_name == find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer])
        {
          //	如果条件成立，说明SYDH图层名称对应的一体化数据图层名称已经存在了
          flag_exist = 1;
          break;
        }
      }

      //	如果SYDH图层名称对应的一体化数据图层名称不存在，那么将其存放到映射数据结构中（存放名称和存放创建好的图层指针）
      if (flag_exist == 0)
      {
        //	创建一个临时的存储单个一体化图层相关信息的数据结构
        YTH_single_opened_layer_info_t temp;
        ...
        //	现在只是将layer_name先添加到数据结构中（layer_geo_type、polayer、poSRS将会在后面进行赋值）
        temp.layer_name = find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer];
        //	获取图层的类型（点图层、线图层、面图层等等）
        //	从GJB图层名称中找到图层的类别（例如ORIT_point找到point）
        size_t starting_position_3 = find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].find_last_of("_");
        string layer_type = find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].substr(starting_position_3 + 1);
        temp.layer_geo_type = layer_type;
        //	相同分幅数据目录中所有的图层空间参考都是一样的，选取SYDH图层其中一个图层的空间参考就可以了（获取SYDH图层的空间参考）
        temp.poSRS = SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[0].poSRS;


        //	需要根据不同的类型来创建图层
        if (temp.layer_geo_type == "point")
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPoint, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "点图层（" + find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（SYDH）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbLineString, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "线图层（" + find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（SYDH）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPolygon, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "面图层（" + find_SYDH_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（SYDH）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
void CSE_GeoExtractAndProcess::single_SYDH_frame_data2single_YTH_frame_data(
  const SYDH_single_frame_data_all_opened_layers_info_t& SYDH_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_code_mapping_table_t& YTH_SYDH_code_mapping_table,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  //	循环对YTH_SYDH_code_mapping_table中的编码映射表进行处理
  for (size_t index_layer = 0; index_layer < YTH_SYDH_code_mapping_table.vYTH_SYDH_code_mapping_single_layer.size(); index_layer++)
  {
    const YTH_SYDH_code_mapping_single_layer_t& current_YTH_SYDH_code_mapping_layer = YTH_SYDH_code_mapping_table.vYTH_SYDH_code_mapping_single_layer[index_layer];
    //	对每种YTH图层中的各个要素进行循环处理
    for (size_t index_feature = 0; index_feature < current_YTH_SYDH_code_mapping_layer.vYTH_single_feature.size(); index_feature++)
    {
      const YTH_SYDH_single_feature_t& current_YTH_single_feature = current_YTH_SYDH_code_mapping_layer.vYTH_single_feature[index_feature];
      //	判断当前要素的SYDH筛选条件是否为空，如果不是空的，那么需要得到相对应的SYDH源图层进行筛选并且进行语义融合处理
      if (current_YTH_single_feature.SQL != "")
      {
        //	获得需要进行筛选进行语义融合的SYDH源图层
        std::string SYDH_origion_layer_name = current_YTH_single_feature.SYDH_origin_layer_name;
        if (SYDH_origion_layer_name != "")
        {
          //	从SYDH_single_frame_data_all_opened_layers_info中寻找同SYDH_origion_layer_name名称相同的图层
          int index_flag = -1;
          for (size_t index_SYDH_layers_info = 0; index_SYDH_layers_info < SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info.size(); index_SYDH_layers_info++)
          {
            if (SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[index_SYDH_layers_info].layer_name == SYDH_origion_layer_name)
            {
              index_flag = index_SYDH_layers_info;
              break;
            }
          }
          //	判断是否找到了想要进行筛选的图层（不是-1的话说明找到了）
          if (index_flag != -1)
          {
            ...
            const char* pszSQLCommand = current_YTH_single_feature.SQL.c_str();
            //	2、对找到的图层进行筛选
            SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[index_flag].polayer->SetAttributeFilter(pszSQLCommand);
            OGRFeature* poFeature;
            // 重置图层的读取状态，以便进行下一次查询
            SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[index_flag].polayer->ResetReading();
            while ((poFeature = SYDH_single_frame_data_all_opened_layers_info.vSYDH_single_opened_layer_info[index_flag].polayer->GetNextFeature()) != NULL)
            {

              /*
                利用当前SYDH图层中的当前要素对一体化中的对应图层中的要素进行设置（创建一个YTH新的要素，其中几何信息和属性信息从SYDH要素中获得，
              但是几何信息不发生变化）（应该将新的要素创建到YTH中的哪一个图层中，这是需要的参数信息）
              */
              use_SYDH_layer_feature_set_YTH_layer_feature(
                poFeature,
                YTH_fields4all_layers,
                current_YTH_SYDH_code_mapping_layer,
                index_feature,
                YTH_single_frame_data_all_opened_layers_info);
      ...
      //	如果当前要素的SYDH筛选条件为空，那么跳过当前要素，继续处理下一个要素
      else
      {
        continue;
      }
...
void CSE_GeoExtractAndProcess::use_SYDH_layer_feature_set_YTH_layer_feature(
  const OGRFeature* poFeature,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_SYDH_code_mapping_single_layer_t& YTH_SYDH_code_mapping_single_layer,
  const int& index_feature,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  ...
  （三）几何类型
    ...
    - `wkbGeometryCollection`: 几何集合
  （四）说明
    其中index_feature参数用来指示YTH_SYDH_code_mapping_single_layer中的具体是哪一个feature
  */

#pragma region "获得YTH图层的大类标识"
  //	获得YTH图层的大类标识
  const std::string& strYTHLayerID = YTH_SYDH_code_mapping_single_layer.YTHLayerID;
  const int iYTHLayerID = std::stoi(strYTHLayerID);
  std::string YTH_layer_full_name;
  switch (iYTHLayerID)
  {
  ...
  for (size_t index = 0; index < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); index++)
  {
    ...
    if (YTH_layer_full_name == YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index].layer_name)
    {
      ...
      create_new_feature_and_add2YTH_layer(
        poFeature,
        iYTHLayerID,
        YTH_SYDH_code_mapping_single_layer,
        index_feature,
        YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index]);
      break;
    }
  ...
  YTH_single_opened_layer_info_t& YTH_single_opened_layer_info)
{
  ...
  case 13:
  {
    //	设置一体化13图层的“ID”字段
    std::string pszFName = "FID";
    std::string pszValue = poFeature->GetFieldAsString("ID");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "MAPNO";
    pszValue = poFeature->GetFieldAsString("MapID");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "CODE";
    pszValue = YTH_SYDH_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_code_field;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "SRCCODE";
    pszValue = poFeature->GetFieldAsString("Kind");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "NAME";
    bool exist_flag = isFieldPresent(poFeature, "RoadName");
    if (exist_flag)
    {
      pszValue = poFeature->GetFieldAsString("RoadName");
    }
    else
    {
      pszValue = poFeature->GetFieldAsString("Name");
    ...
    pszFName = "NUM";
    pszValue = poFeature->GetFieldAsString("RoadNo");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "PAVWIDTH";
    pszValue = poFeature->GetFieldAsString("Width");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "LANENUM";
    pszValue = poFeature->GetFieldAsString("LaneNumC");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "LENGTH";
    pszValue = poFeature->GetFieldAsString("Length");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "DIRECTION";
    pszValue = poFeature->GetFieldAsString("Direction");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "FUNCCLASS";
    pszValue = poFeature->GetFieldAsString("FuncClass");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "ELEVATED";
    pszValue = poFeature->GetFieldAsString("Elevated");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "ZLEVEL";
    pszValue = poFeature->GetFieldAsString("Zlevel");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "TOLL";
    pszValue = poFeature->GetFieldAsString("Toll");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "OWNERSHIP";
    pszValue = poFeature->GetFieldAsString("Ownership");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());

    pszFName = "SPEEDCLASS";
    pszValue = poFeature->GetFieldAsString("SpeedClass");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "LEVEL";
    pszValue = poFeature->GetFieldAsString("Level1");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
...
#pragma region "（5）QQCT---->一体化"
int CSE_GeoExtractAndProcess::DLHJ_convert_QQCTDATA2YTHData(
  const std::vector<std::string>& QQCT_vinput_data_path_GBK,
  const std::string& YTH_output_data_path_GBK,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_layer_mapping_table_t& YTH_QQCT_layer_mapping_table,
  const YTH_QQCT_code_mapping_table_t& YTH_QQCT_code_mapping_table,
  DLHJProgressFunc pfnProgress)
{
  /*
  * 杨小兵-2024-03-15
  ...
  返回值为整数2：数据源类型不合法
  返回值为整数3：一体化的所有图层字段配置信息为空
  返回值为整数4：一体化同QQCT图层映射配置信息为空
  返回值为整数5：一体化同QQCT编码映射配置信息为空
  返回值为整数6：QQCT到一体化矢量数据语义融合失败
...
#pragma region "（第一步）将输入数据路径和输出数据路径的编码统一转化为utf-8字符集编码"
  ...
  character_set_encoding_conversion character_set_encoding_conversion_instance("UTF-8", "GBK");
  //  用来保存字符集编码转化后的输入数据路径
  std::vector<std::string> QQCT_vinput_data_path_UTF8;
  QQCT_vinput_data_path_UTF8.clear();
  //  对每个分幅数据输入路径进行转化
  for (size_t index = 0; index < QQCT_vinput_data_path_GBK.size(); index++)
  {
    std::string temp = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(QQCT_vinput_data_path_GBK[index]);
    QQCT_vinput_data_path_UTF8.push_back(temp);
  }
  //  对输出路径进行转化
  std::string YTH_output_data_path_UTF8 = character_set_encoding_conversion_instance.execute_character_set_encoding_conversion(YTH_output_data_path_GBK);
  //  检查输入数据路径和输出数据路径字符集编码转化是否成功
  ...
  //	一体化同QQCT图层映射配置信息为空
  if (YTH_QQCT_layer_mapping_table.vYTH_QQCT_layer_mapping.size() == 0)
  {
    return 4;
  }

  //	一体化同QQCT编码映射配置信息为空
  if (YTH_QQCT_code_mapping_table.vYTH_QQCT_code_mapping_single_layer.size() == 0)
  {
    return 5;
  }
#pragma endregion

#pragma region "（第二步）QQCT数据到一体化数据转化"
  int iResult;
  //	DLHJ_convert_QQCTDATA2YTHData是被包装的函数，_DLHJ_convert_QQCTData2YTHData为实际执行数据转化的函数
  iResult = _DLHJ_convert_QQCTData2YTHData(
    QQCT_vinput_data_path_UTF8,
    YTH_output_data_path_UTF8,
    iDataSourceType,
    YTH_fields4all_layers,
...
int CSE_GeoExtractAndProcess::_DLHJ_convert_QQCTData2YTHData(
  const std::vector<std::string>& QQCT_vinput_data_path_UTF8,
  const std::string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_layer_mapping_table_t& YTH_QQCT_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  ...
  （一）参数说明：
      ...
      3. **`iDataSourceType`**: 类型为 `int`。
         - **描述**: 数据源类型的标识符，用于区分不同种类或格式的地理数据。
      4. **`YTH_fields4all_layers`**: 类型为 `YTH_fields4all_layers_t&`（引用）。
         - **描述**: 包含 QQCT 层信息的列表，用于存储和管理 QQCT 层的相关数据。
      5. **`YTH_QQCT_layer_mapping_table`**: 类型为 `YTH_QQCT_layer_mapping_table_t&`（引用）。
         - **描述**: 包含 GJB 层字段信息的列表，用于存储和管理 GJB 层字段的相关数据。
      6. **`YTH_QQCT_code_mapping_table`**: 类型为 `YTH_QQCT_code_mapping_table_t&`（引用）。
         - **描述**: 用于存储 QQCT 到 GJB 的层映射信息。
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    vstrInputDataPath中存储的是多个分幅数据，每一个分幅数据中是由多个图层构成的，这个函数将会把vstrInputDataPath中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、整体上将会对分幅数据循环操作，执行QQCT--->一体化数据的转化
    2、对单个分幅数据进行处理
    3、处理结果处理
  */

  size_t size_framing_data = QQCT_vinput_data_path_UTF8.size();
  for (size_t index = 0; index < size_framing_data; index++)
  {
    int iresult_single_framing_data;
    iresult_single_framing_data = convert_single_frameQQCTData2YTHData(
      QQCT_vinput_data_path_UTF8[index],
      YTH_output_data_path_UTF8,
      iDataSourceType,
      YTH_fields4all_layers,
      YTH_QQCT_layer_mapping_table,
    ...
    //	对单个QQCT分幅数据转化成一体化数据结果进行判断
    if (iresult_single_framing_data != 0)
    {
      //	返回整数不为0，说明发生了某种错误，记录日志
      string strMsg1 = "分幅数据：" + QQCT_vinput_data_path_UTF8[index] + ",QQCT数据到一体化矢量数据语义融合失败！";
      QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
      string strTag1 = "（QQCT）矢量数据语义融合处理日志";
      QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
      QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
      //  继续处理其他分幅数据
      continue;
...
int CSE_GeoExtractAndProcess::convert_single_frameQQCTData2YTHData(
  const string& QQCT_single_frame_data_path_UTF8,
  const string& YTH_output_data_path_UTF8,
  const int& iDataSourceType,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_layer_mapping_table_t& YTH_QQCT_layer_mapping_table,
  ...
  DLHJProgressFunc pfnProgress)
{
  /*
  （一）参数说明：
      1. **`single_frame_QQCT_data_path`**: 类型为 `std::string&`。
         - **描述**: 用于存储单个输入数据文件路径（单个分幅数据目录路径）。
      2. **`strOutputDataPath`**: 类型为 `std::string&`。
         - **描述**: 输出数据的目标存储路径。
      3. **`iDataSourceType`**: 类型为 `int&`。
      ...
      7. **`pfnProgress`**: 类型为 `DLHJProgressFunc`。
         - **描述**: 进度回调函数，用于在数据处理过程中提供进度更新。
  （二）函数功能：
    single_frame_QQCT_data_path中存储的是单个分幅数据目录路径，这个函数将会把single_frame_QQCT_data_path中的数据根据其他的参数转化成
  一体化数据格式。
  （三）算法流程
    1、将单个shp分幅数据目录路径下的所有图层读取到内存当中
    2、将所有图层的相关信息读取到自定义数据结构中（图层名称、图层类型、图层指针等等）
...
#pragma region "（第一步）将单个shp分幅数据目录路径下的所有图层读取到内存当中（打开资源）"
  ...
  {
    // 错误提示
    string strMsg1 = "分幅数据（" + QQCT_single_frame_data_path_UTF8 + "）" + "获取ESRI Shapefile驱动失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（QQCT）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
#pragma region "（第二步）将QQCT数据所有图层的相关信息读取到自定义数据结构中（single_frame_QQCTLayer_infos_t）"
  //	利用矢量驱动器打开单个shp分幅数据目录路径下的数据,以只读数据的方式打开数据
  GDALDataset* poSingleFrameQQCTDataSet = (GDALDataset*)GDALOpenEx(QQCT_single_frame_data_path_UTF8.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  // 文件不存在或打开失败
  if (poSingleFrameQQCTDataSet == NULL)
  {
    // 说明打开这个分幅数据目录路径下的文件出现了错误
    // 错误提示
    string strMsg1 = "分幅数据（" + QQCT_single_frame_data_path_UTF8 + "）" + "文件不存在或打开失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "（QQCT）矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  //	一体化分幅数据相关信息（对整个分幅数据记录有：分幅数据路径、数据集指针；对每个图层而言记录信息有：图层名字、图层类型、图层指针、图层空间参考）
  QQCT_single_frame_data_all_opened_layers_info_t QQCT_single_frame_data_all_opened_layers_info;
  use_QQCTData_create_single_frame_QQCTLayer_infos(poSingleFrameQQCTDataSet, QQCT_single_frame_data_all_opened_layers_info);

#pragma endregion

#pragma region "（第三步）根据QQCT数据和图层映射关系将一体化数据所有图层的相关信息读取到自定义数据结构中（YTH_single_frame_data_all_opened_layers_info）"
  YTH_single_frame_data_all_opened_layers_info_t YTH_single_frame_data_all_opened_layers_info;
  use_QQCTData_create_single_frame_YTHLayer_infos(
    QQCT_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_QQCT_layer_mapping_table,
    YTH_output_data_path_UTF8,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第四步）对当前分幅数据中的图层进行转化操作"
  ...
  single_QQCT_frame_data2single_YTH_frame_data(
    QQCT_single_frame_data_all_opened_layers_info,
    YTH_fields4all_layers,
    YTH_QQCT_code_mapping_table,
    YTH_single_frame_data_all_opened_layers_info);
...
#pragma region "（第五步）关闭资源"
  ...
  GDALClose(QQCT_single_frame_data_all_opened_layers_info.QQCT_single_frame_data_poDS);
  GDALClose(YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_poDS);
#pragma endregion
...
void CSE_GeoExtractAndProcess::use_QQCTData_create_single_frame_QQCTLayer_infos(
  GDALDataset* poSingleFrameShpDataSet,
  QQCT_single_frame_data_all_opened_layers_info_t& QQCT_single_frame_data_all_opened_layers_info)
{
  /*
  ...
  */
  ...
  //	获取当前分幅数据的路径
  QQCT_single_frame_data_all_opened_layers_info.QQCT_single_frame_data_input_path = poSingleFrameShpDataSet->GetDescription();
  //	获取当前分幅数据的数据集指针
  QQCT_single_frame_data_all_opened_layers_info.QQCT_single_frame_data_poDS = poSingleFrameShpDataSet;
  ...
  for (int index = 0; index < size_shp_layer; index++)
  {
    QQCT_single_opened_layer_info_t temp_single_QQCTLayer_info_t;
    ...
    temp_single_QQCTLayer_info_t.layer_name = temp_poLayer->GetName();
    OGRwkbGeometryType eType = temp_poLayer->GetGeomType();
    temp_single_QQCTLayer_info_t.layer_geo_type = OGRGeometryTypeToName(eType);
    temp_single_QQCTLayer_info_t.polayer = temp_poLayer;
    temp_single_QQCTLayer_info_t.poSRS = temp_poLayer->GetSpatialRef();

    QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info.push_back(temp_single_QQCTLayer_info_t);
...
void CSE_GeoExtractAndProcess::use_QQCTData_create_single_frame_YTHLayer_infos(
  const QQCT_single_frame_data_all_opened_layers_info_t& QQCT_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_layer_mapping_table_t& YTH_QQCT_layer_mapping_table,
  const string& YTH_output_data_path_UTF8,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  /*
  （一）函数功能：
    根据单个分幅数据中所有图层的名称（QQCT数据格式中的图层名称分类标准，例如BOUP）创建一体化数据格式中的名称（例如BOUP_point），从数学的角度来说就是
  建议一个从N个图层名称到M个图层名称的映射关系，还有图层指针信息，这个函数中还没有执行具体的转化，只是确定了要将分幅数据中的QQCT图层转化成一体化的哪些图层
  （二）算法流程
    1、循环处理一个分幅数据中的所有shp图层数据
    2、获取当前图层的名称（在QQCT格式中图层的名称）
    3、在QQCTLayer.xml配置文件中寻找到对应的关系映射
    4、根据关系映射创建一体化数据格式中图层的名称
  */
...
#pragma region "（第一步）根据输出路径构建分幅数据目录路径、在此路径下创建数据集GDALDataset"
  //	构建分幅数据路径
  //	分幅数据名称
  size_t starting_position_1 = QQCT_single_frame_data_all_opened_layers_info.QQCT_single_frame_data_input_path.find_last_of("/");
  string frame_data_name = QQCT_single_frame_data_all_opened_layers_info.QQCT_single_frame_data_input_path.substr(starting_position_1 + 1);
  string frame_data_path = YTH_output_data_path_UTF8 + "/" + frame_data_name;
  YTH_single_frame_data_all_opened_layers_info.YTH_single_frame_data_output_path = frame_data_path;
  // 获取驱动并创建新的Shapefile图层
  CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "YES");
...
#pragma endregion 
  ...
  //	循环对当前分幅数据目录路径下的每一个QQCT数据中的shp图层进行处理
  for (size_t index = 0; index < QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info.size(); index++)
  {
#pragma region "（第二步）获取当前图层在QQCT中图层的名称"
    //	获取当前图层相关信息(QQCT当前图层名称、QQCT当前图层几何类型、QQCT当前图层的指针、QQCT当前图层的坐标参考系统指针)
    const QQCT_single_opened_layer_info_t& current_QQCT_single_layer_info = QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[index];
#pragma endregion

#pragma region "（第三步）在QQCT图层映射表中找到当前图层名称的映射关系，例如pois映射到YTH数据中的哪些图层"
    //	创建一个临时的YTH_QQCT_single_layer_mapping_t自定义数据结构，用来标识QQCT图层同YTH图层之间的映射关系
    YTH_QQCT_single_layer_mapping_t find_QQCT_single_layer_mapping;
    for (size_t index_vYTH_layer_list = 0; index_vYTH_layer_list < YTH_QQCT_layer_mapping_table.vYTH_QQCT_layer_mapping.size(); index_vYTH_layer_list++)
    {
      if (YTH_QQCT_layer_mapping_table.vYTH_QQCT_layer_mapping[index_vYTH_layer_list].QQCT_layer_name == current_QQCT_single_layer_info.layer_name)
      {
        find_QQCT_single_layer_mapping = YTH_QQCT_layer_mapping_table.vYTH_QQCT_layer_mapping[index_vYTH_layer_list];
        break;
      }
    }
#pragma endregion

#pragma region "（第四步）将QQCT图层名称对应一体化图层名称存放到自定义数据结构中（得到一体化图层对应的名字，例如：ORIT_point）"
    //	如果图层映射关系是多对一的，那么则循环逐个判断是否重复，如果没有重复则将其保存在一体化数据结构中
    for (size_t index_vGJBLayer = 0; index_vGJBLayer < find_QQCT_single_layer_mapping.vYTH_layer_list.size(); index_vGJBLayer++)
    {
      //	在将QQCT图层名称对应的一体化数据图层名称存放到映射数据结构中之前需要检查是否已经存在（循环查找）
      int flag_exist = 0;
      for (size_t j = 0; j < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); j++)
      {
        if (YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[j].layer_name == find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer])
        {
          //	如果条件成立，说明QQCT图层名称对应的一体化数据图层名称已经存在了
          flag_exist = 1;
          break;
        }
      }

      //	如果QQCT图层名称对应的一体化数据图层名称不存在，那么将其存放到映射数据结构中（存放名称和存放创建好的图层指针）
      if (flag_exist == 0)
      {
        //	创建一个临时的存储单个一体化图层相关信息的数据结构
        YTH_single_opened_layer_info_t temp;
        ...
        //	现在只是将layer_name先添加到数据结构中（layer_geo_type、polayer、poSRS将会在后面进行赋值）
        temp.layer_name = find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer];
        //	获取图层的类型（点图层、线图层、面图层等等）
        //	从GJB图层名称中找到图层的类别（例如ORIT_point找到point）
        size_t starting_position_3 = find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].find_last_of("_");
        string layer_type = find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].substr(starting_position_3 + 1);
        temp.layer_geo_type = layer_type;
        //	相同分幅数据目录中所有的图层空间参考都是一样的，选取QQCT图层其中一个图层的空间参考就可以了（获取QQCT图层的空间参考）
        temp.poSRS = QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[0].poSRS;


        //	需要根据不同的类型来创建图层
        if (temp.layer_geo_type == "point")
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPoint, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "点图层（" + find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（QQCT）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbLineString, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "线图层（" + find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（QQCT）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
        ...
        {
          OGRLayer* poLayer = poDS->CreateLayer(find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer].c_str(), temp.poSRS, wkbPolygon, NULL);
          if (poLayer == NULL)
          {
            // 错误提示
            string strMsg1 = "面图层（" + find_QQCT_single_layer_mapping.vYTH_layer_list[index_vGJBLayer] + "）" + "创建失败！";
            QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
            string strTag1 = "（QQCT）矢量数据语义融合处理日志";
            QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
            QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
...
void CSE_GeoExtractAndProcess::single_QQCT_frame_data2single_YTH_frame_data(
  const QQCT_single_frame_data_all_opened_layers_info_t& QQCT_single_frame_data_all_opened_layers_info,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_code_mapping_table_t& YTH_QQCT_code_mapping_table,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  //	循环对YTH_QQCT_code_mapping_table中的编码映射表进行处理
  for (size_t index_layer = 0; index_layer < YTH_QQCT_code_mapping_table.vYTH_QQCT_code_mapping_single_layer.size(); index_layer++)
  {
    const YTH_QQCT_code_mapping_single_layer_t& current_YTH_QQCT_code_mapping_layer = YTH_QQCT_code_mapping_table.vYTH_QQCT_code_mapping_single_layer[index_layer];
    //	对每种YTH图层中的各个要素进行循环处理
    for (size_t index_feature = 0; index_feature < current_YTH_QQCT_code_mapping_layer.vYTH_single_feature.size(); index_feature++)
    {
      const YTH_QQCT_single_feature_t& current_YTH_single_feature = current_YTH_QQCT_code_mapping_layer.vYTH_single_feature[index_feature];
      //	判断当前要素的QQCT筛选条件是否为空，如果不是空的，那么需要得到相对应的QQCT源图层进行筛选并且进行语义融合处理
      if (current_YTH_single_feature.SQL != "")
      {
        //	获得需要进行筛选进行语义融合的QQCT源图层
        std::string QQCT_origion_layer_name = current_YTH_single_feature.QQCT_origin_layer_name;
        if (QQCT_origion_layer_name != "")
        {
          //	从QQCT_single_frame_data_all_opened_layers_info中寻找同QQCT_origion_layer_name名称相同的图层
          int index_flag = -1;
          for (size_t index_QQCT_layers_info = 0; index_QQCT_layers_info < QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info.size(); index_QQCT_layers_info++)
          {
            if (QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[index_QQCT_layers_info].layer_name == QQCT_origion_layer_name)
            {
              index_flag = index_QQCT_layers_info;
              break;
            }
          }
          //	判断是否找到了想要进行筛选的图层（不是-1的话说明找到了）
          if (index_flag != -1)
          {
            ...
            const char* pszSQLCommand = current_YTH_single_feature.SQL.c_str();
            //	2、对找到的图层进行筛选
            QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[index_flag].polayer->SetAttributeFilter(pszSQLCommand);
            OGRFeature* poFeature;
            // 重置图层的读取状态，以便进行下一次查询
            QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[index_flag].polayer->ResetReading();
            while ((poFeature = QQCT_single_frame_data_all_opened_layers_info.vQQCT_single_opened_layer_info[index_flag].polayer->GetNextFeature()) != NULL)
            {

              /*
                利用当前QQCT图层中的当前要素对一体化中的对应图层中的要素进行设置（创建一个YTH新的要素，其中几何信息和属性信息从QQCT要素中获得，
              但是几何信息不发生变化）（应该将新的要素创建到YTH中的哪一个图层中，这是需要的参数信息）
              */
              use_QQCT_layer_feature_set_YTH_layer_feature(
                poFeature,
                YTH_fields4all_layers,
                current_YTH_QQCT_code_mapping_layer,
                index_feature,
                YTH_single_frame_data_all_opened_layers_info);
      ...
      //	如果当前要素的QQCT筛选条件为空，那么跳过当前要素，继续处理下一个要素
      else
      {
        continue;
      }
...
void CSE_GeoExtractAndProcess::use_QQCT_layer_feature_set_YTH_layer_feature(
  const OGRFeature* poFeature,
  const YTH_fields4all_layers_t& YTH_fields4all_layers,
  const YTH_QQCT_code_mapping_single_layer_t& YTH_QQCT_code_mapping_single_layer,
  const int& index_feature,
  YTH_single_frame_data_all_opened_layers_info_t& YTH_single_frame_data_all_opened_layers_info)
{
  ...
  （三）几何类型
    ...
    - `wkbGeometryCollection`: 几何集合
  （四）说明
    其中index_feature参数用来指示YTH_QQCT_code_mapping_single_layer中的具体是哪一个feature
  */

#pragma region "获得YTH图层的大类标识"
  //	获得YTH图层的大类标识
  const std::string& strYTHLayerID = YTH_QQCT_code_mapping_single_layer.YTHLayerID;
  const int iYTHLayerID = std::stoi(strYTHLayerID);
  std::string YTH_layer_full_name;
  switch (iYTHLayerID)
  {
  ...
  for (size_t index = 0; index < YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info.size(); index++)
  {
    ...
    if (YTH_layer_full_name == YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index].layer_name)
    {
      ...
      create_new_feature_and_add2YTH_layer(
        poFeature,
        iYTHLayerID,
        YTH_QQCT_code_mapping_single_layer,
        index_feature,
        YTH_single_frame_data_all_opened_layers_info.vYTH_single_opened_layer_info[index]);
      break;
    }
  ...
  YTH_single_opened_layer_info_t& YTH_single_opened_layer_info)
{
  ...
  case 13:
  {
    //	设置一体化13图层的“ID”字段
    std::string pszFName = "FID";
    std::string pszValue = poFeature->GetFieldAsString("QQCT_id");
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "CODE";
    pszValue = YTH_QQCT_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_code_field;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "FGUID";
    pszValue = YTH_QQCT_code_mapping_single_layer.vYTH_single_feature[index_feature].YTH_feature_classification_name;
    poNewFeature->SetField(pszFName.c_str(), pszValue.c_str());
    ...
    pszFName = "ONETWOWY";
    std::string QQCT_origin_feature_value = poFeature->GetFieldAsString("oneway");
    if ((QQCT_origin_feature_value == "T") || (QQCT_origin_feature_value == "F"))
    {
      pszValue = "单";
    }
    else if (QQCT_origin_feature_value == "B")
    {
      pszValue = "双";
    }
    else
...
#pragma region "(*)公用函数"
bool CSE_GeoExtractAndProcess::create_shapefile_cpg_UTF8(const string& strCPGFilePath, const string& strEncoding)
{
  //  创建一个文件对象
  const std::filesystem::path cpg_file_path(strCPGFilePath);

  std::ofstream file_stream(cpg_file_path, std::ios::out);
  ...
  {
    // 错误提示
    string strMsg1 = "文件（" + strCPGFilePath + "）" + "打开文件失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
    return false;
  }
  ...
  // 将字符编码写入到文件中
  if (!(file_stream << strEncoding))
  {
    // 错误提示
    string strMsg1 = "文件（" + strCPGFilePath + "）" + "写入失败！";
    QString qstrMsg1 = QString::fromUtf8(strMsg1.c_str());
    string strTag1 = "矢量数据语义融合处理日志";
    QString qstrTag1 = QString::fromUtf8(strTag1.c_str());
    QgsMessageLog::logMessage(qstrMsg1, qstrTag1, Qgis::Critical);
  ...
  const YTH_fields4all_layers_t& YTH_fields4all_layers)
{
  ...
  for (const auto& current_layer : YTH_fields4all_layers.vYTH_single_layer)
  {
  ...
  //	根据target_layer中的字段信息对图层指针添加字段定义
  for (const auto& current_field : target_layer.vfield)
  {
    //	字段类型
    OGRFieldType fieldType;
...
bool CSE_GeoExtractAndProcess::isFieldPresent(const OGRFeature* feature, const char* fieldName)
{
  if (feature == nullptr || fieldName == nullptr) return false;

  // 获取要素的字段定义
  const OGRFeatureDefn* defn = feature->GetDefnRef();
  if (defn == nullptr) return false;
  ...
  for (int i = 0; i < fieldCount; i++)
  {
    ...
    if (fieldDefn == nullptr) continue;

    // 比较字段名
    if (strcmp(fieldDefn->GetNameRef(), fieldName) == 0)
    {
      // 找到指定的字段
      return true;
    }
