#ifndef CZCHJ_DATA_ENGINE_H
#define CZCHJ_DATA_ENGINE_H
...
extern "C"
{
    ...
    @return: 0：成功，1：失败
    */
    EXPORT_DECL int LCGetRasterAnalysisReadyData(const char* szPackagePath,
        const char* szType,
        int iLevel,
        double& dLeft,
        double& dTop,
    ...
    @return: 0：成功，1：失败
    */
    EXPORT_DECL int LCGetVectorAnalysisReadyData(
    ...
    /*从分析就绪数据包中获取指定位置的栅格数据像素值
    @param：szPackagePath               分析就绪型数据包所在的路径，如：/data
    @param：szType                      分析就绪数据产品标识
    @param: iLevel                      分析就绪数据级别
    @param：dX                          指定点位置的经度，单位：度
    @param：dY                          指定点位置的纬度，单位：度
    @param：fValue                      指定点位置像素值               
    @
    @return: 0：成功，1：失败
    */
    EXPORT_DECL int LCGetRasterAnalysisReadyData4Point(const char* szPackagePath,
        const char* szType,
        int iLevel,
        double dX,
        double dY,
