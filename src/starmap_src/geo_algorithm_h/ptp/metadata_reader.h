...
HANDLE64_PTP metadata_load(const char *map_path, const char *map_id,
		const char* private_key, uint32 private_key_len,
		const char* dev_key, uint32 dev_key_len,
		uint32 *status);

char *get_first_single_map_guid(HANDLE64_PTP handle);

uint32 metadata_unload(HANDLE64_PTP handle);
