...
// 浮点数判断相等容差
const double DOUBLE_EQUAL_ZERO = 1e-6;
    ...
    double* pazi2)
{
    ...
    if (fabs(lon1 - lon2) < DOUBLE_EQUAL_ZERO
        && fabs(lat1 - lat2) < DOUBLE_EQUAL_ZERO)
    {
        *pazi1 = 0;
        *pazi2 = 0;
    }
    ...
    // 【3】水平向右
    if (fabs(lat1 - lat2) < DOUBLE_EQUAL_ZERO
        && lon1 < lon2)
    {
        *pazi2 += 180;
    }
...
void CSE_GeoArithmetic::CalOneThirdPointByMapPointsAndRotationAngle(
...
{
    /*计算思路：
    1、计算屏幕中心点到右上角坐标的距离及方位角；
    2、计算对角线与地理坐标轴正北方向的夹角；
    3、计算旋转后的1/n距离
    4、大地线正算公式计算1/n点的地理坐标
    */

    /*【1】计算屏幕中心点到屏幕右上角点的距离及方位角*/
    double dDistance = 0;
    double dAzimuth1 = 0;
    double dAzimuth2 = 0;
    CalDistanceAndAzimuthByTwoPoints(
        dSemiMajorAxis,
        dSemiMinorAxis,
        dCenterY,
        dCenterX,
    ...
    // 如果旋转角度位于[0,90°] 区间
    if (dRotationAngle >= 0 && dRotationAngle <= 90)
    {
        // 屏幕中心点与右上角点与屏幕正上方y坐标轴的夹角
        double dAngleOfScreen = 0;
        // 如果方位角小于等于0
        if (dAzimuth1 <= 0)
        {
            dAngleOfScreen = dRotationAngle - fabs(dAzimuth1);
        }
        else
        {
            dAngleOfScreen = dRotationAngle + dAzimuth1;
        }

        // 计算1/n距离
        dAngleOfScreen = dAngleOfScreen * DEGREE_2_RADIAN;  // 转换为弧度
        double dOneThirdDistance = 2 * dDistance * cos(dAngleOfScreen) * dScale;

        // 如果是上部点
        if (iFlag == 1)
        {
            CalPointByDistanceAndAzimuth(dSemiMajorAxis,
                dSemiMinorAxis,
                dCenterY,
                dCenterX,
                -dRotationAngle,
    ...
    // 如果旋转角度位于(90°,180°] 区间
    else if (dRotationAngle > 90 && dRotationAngle <= 180)
    {
        // 屏幕中心点与右上角点与屏幕正上方y坐标轴的夹角
        double dAngleOfScreen = 0;
    ...
    // 如果旋转角度位于(180°,270°] 区间
    else if (dRotationAngle > 180 && dRotationAngle <= 270)
    {
        // 屏幕中心点与右上角点与屏幕正上方y坐标轴的夹角
        double dAngleOfScreen = 0;
        // 如果方位角在[0，180°]
        if (dAzimuth1 >= 0 && dAzimuth1 <= 180)
        {
            dAngleOfScreen = dRotationAngle - (360 - dAzimuth1);
        }
        else
        {
            dAngleOfScreen = dRotationAngle - fabs(dAzimuth1);
    ...
    // 如果旋转角度位于(270°,360°) 区间
    else if (dRotationAngle > 270  && dRotationAngle < 360)
    {
        // 屏幕中心点与右上角点与屏幕正上方y坐标轴的夹角
        double dAngleOfScreen = 0;
        dAngleOfScreen = dAzimuth1 - (360 - dRotationAngle);
