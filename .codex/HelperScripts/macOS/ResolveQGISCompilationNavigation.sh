...
resolve_qgis_compilation_navigation() {
  ...
  if [[ ${#search_roots[@]} -eq 0 ]]; then
    case "$default_search_mode" in
      ...
      AllFileSystemsAndVolumes) search_roots=("/" "/Volumes") ;;
      AllFileSystems) search_roots=("/") ;;
      *)
        messages+=("{\"Level\":\"Warning\",\"Code\":\"UnknownDefaultSearchMode\",\"Text\":$(json_string "未知 DefaultSearchMode '$default_search_mode'，回退到 AllFileSystemsAndVolumes。")}")
