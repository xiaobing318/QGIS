"""Command line entry point for the QCopilots QGIS binary MCP service.
...
from qcopilots_common.qgis_binary_tools import build_qgis_binary_tools
...
def main() -> None:
    ...
    run_mcp_server(
        name="QCopilots MCP Server QGIS Binary",
        version="0.1.0",
        tools=build_qgis_binary_tools(),
        default_port=DEFAULT_SERVICE_PORTS[SERVICE_ID],
        description=(
            "Run explicitly configured QGIS package command line programs "
            "through persistent asynchronous jobs."
        ),
        logger=logger,
    )
