...
def vector_table_calculate_field(self, layer_ref: str, field_name: str, expression: str, field_type: str = "string", where: str | None = None, in_place: bool = False) -> dict[str, Any]:
    """
    作用：处理 `vector_table_calculate_field` 相关逻辑，完成当前函数负责的处理步骤并产出结果。
    用途：处理 `vector_table_calculate_field` 相关逻辑，完成当前函数负责的处理步骤并产出结果。
    ...
    - 参数 `expression`（`str`）：业务输入参数，由调用方提供以驱动当前函数逻辑。
...
def _vector_table_calculate_field_impl(self, layer_ref: str, field_name: str, expression: str, field_type: str, where: str | None, in_place: bool) -> dict[str, Any]:
    """
    作用：实现 `_vector_table_calculate_field_impl` 对应的核心处理逻辑，承担实际数据处理与结果组织。
    用途：实现 `_vector_table_calculate_field_impl` 对应的核心处理逻辑，承担实际数据处理与结果组织。
    使用场景：在 MCP 工具内部处理链路中被同模块函数串联调用，用于完成分步业务处理。
    参数与返回：
    - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
    - 参数 `layer_ref`（`str`）：QGIS 数据对象相关参数，用于定位图层、要素或空间参考上下文。
