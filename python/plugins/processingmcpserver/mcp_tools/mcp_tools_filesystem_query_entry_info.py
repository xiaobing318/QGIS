...
def filesystem_query_entry_info(self, path: str) -> dict[str, Any]:
    """
    作用：处理 `filesystem_query_entry_info` 相关逻辑，完成当前函数负责的处理步骤并产出结果。
    用途：处理 `filesystem_query_entry_info` 相关逻辑，完成当前函数负责的处理步骤并产出结果。
...
def _filesystem_query_entry_info_impl(self, path: str) -> dict[str, Any]:
    """
    作用：实现 `_filesystem_query_entry_info_impl` 对应的核心处理逻辑，承担实际数据处理与结果组织。
    用途：实现 `_filesystem_query_entry_info_impl` 对应的核心处理逻辑，承担实际数据处理与结果组织。
    使用场景：在 MCP 工具内部处理链路中被同模块函数串联调用，用于完成分步业务处理。
    参数与返回：
    - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
    - 参数 `path`（`str`）：路径类参数，用于定位输入或输出文件系统位置。
