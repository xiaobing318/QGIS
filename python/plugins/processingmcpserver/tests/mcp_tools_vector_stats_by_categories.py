...
class ToolsVectorStatsByCategoriesTest(ProcessingMCPTestBase):
    def test_registered(self):
        ...
        返回结果：无返回值。
        """
        assert_tool_registered(self, "mcp_tools_vector_stats_by_categories")
    ...
    def test_success_vector_stats_by_categories(self):
        ...
        result = tools.mcp_tools_vector_stats_by_categories(
            layer_ref=layer.id(),
            category_fields=["category"],
            values_field="value",
        )
        self.assertTrue(result["ok"])
        self.assertEqual(result["tool"], "mcp_tools_vector_stats_by_categories")
    ...
    def test_failure_empty_categories(self):
        ...
        with self.assertRaises(Exception) as ctx:
            tools.mcp_tools_vector_stats_by_categories(layer_ref=layer.id(), category_fields=[])
