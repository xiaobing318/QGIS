...
class ToolsVectorAddLayersTest(ProcessingMCPTestBase):
    def test_removed_vector_add_layers_not_registered(self):
        mcp = DummyMcp()
        register_tools(mcp, self.build_tools(), enable_execute_code=False)
        self.assertNotIn("vector_add_layers", set(mcp.tool_names))

    def test_dataset_load_layers_registered(self):
        assert_tool_registered(self, "mcp_tools_dataset_load_layers")

    def test_dataset_load_layers_can_load_vector_path(self):
        tools = self.build_tools()
        geojson = self.sample_vector_path()

        result = tools.mcp_tools_dataset_load_layers(paths=[str(geojson)], skip_invalid=True)
