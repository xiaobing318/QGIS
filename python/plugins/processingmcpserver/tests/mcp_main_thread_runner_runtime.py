...
class McpMainThreadRunnerRuntimeTest(ProcessingMCPTestBase):
    def _pump_events_until(
        ...
        """Process Qt events until the predicate succeeds or the timeout expires."""
    ...
    def test_constructor_rejects_non_positive_timeout(self):
        """Verify constructor rejects non positive timeout."""
    ...
    def test_run_executes_directly_on_main_thread(self):
        """Verify run executes directly on main thread."""
    ...
    def test_qt_event_loop_executes_queued_callback_when_invoke_method_returns_none(self):
        """Verify Qt event loop executes queued callback when invoke method returns none."""
        runner = McpMainThreadRunner()
        state = {"called": False}

        def wrapper() -> None:
            """Mark the callback as executed for the event-loop assertion."""
            state["called"] = True
    ...
    def test_run_executes_queued_callback_for_worker_thread(self):
        """Verify run executes queued callback for worker thread."""
        runner = McpMainThreadRunner()
        fake_app = MagicMock()
    ...
    def test_run_executes_queued_callback_for_real_worker_thread(self):
        """Verify run executes queued callback for real worker thread."""
        runner = McpMainThreadRunner()
        result: dict[str, object] = {}
        finished = threading.Event()

        def worker() -> None:
            """Execute the runner from a background thread and capture the outcome."""
            try:
    ...
    def test_run_propagates_worker_thread_callback_error(self):
        """Verify run propagates worker thread callback error."""
        runner = McpMainThreadRunner()
        fake_app = MagicMock()
        fake_app.thread.return_value = object()
    ...
    def test_run_raises_when_invoke_method_fails(self):
        """Verify run raises when invoke method fails."""
        runner = McpMainThreadRunner()
        fake_app = MagicMock()
        fake_app.thread.return_value = object()
    ...
    def test_run_raises_when_worker_thread_times_out(self):
        """Verify run raises when worker thread times out."""
