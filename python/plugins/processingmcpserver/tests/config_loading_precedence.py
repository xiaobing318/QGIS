...
class ConfigLoadingPrecedenceTest(ProcessingMCPTestBase):
    def test_create_default_json_when_missing(self):
        """
        作用：执行测试用例 `create default json when missing`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `create default json when missing`，验证目标行为在回归场景下是否符合预期。
    ...
    def test_json_overrides_settings_host(self):
        """
        作用：执行测试用例 `json overrides settings host`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `json overrides settings host`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_json_missing_key_fallback_to_settings(self):
        """
        作用：执行测试用例 `json missing key fallback to settings`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `json missing key fallback to settings`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_processing_mcp_section_must_be_object(self):
        """
        作用：执行测试用例 `processing mcp section must be object`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `processing mcp section must be object`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_dependencies_section_must_be_object(self):
        """
        作用：执行测试用例 `dependencies section must be object`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `dependencies section must be object`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_invalid_json_value_fallback_to_settings(self):
        """
        作用：执行测试用例 `invalid json value fallback to settings`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `invalid json value fallback to settings`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_invalid_json_and_settings_fallback_to_default(self):
        """
        作用：执行测试用例 `invalid json and settings fallback to default`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `invalid json and settings fallback to default`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_transport_aliases_normalized_to_streamable_http(self):
        """
        作用：执行测试用例 `transport aliases normalized to streamable http`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `transport aliases normalized to streamable http`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_cors_values_support_string_list_and_empty_disable(self):
        """
        作用：执行测试用例 `cors values support string list and empty disable`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `cors values support string list and empty disable`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
    ...
    def test_legacy_limits_json_is_ignored(self):
        """
        作用：执行测试用例 `legacy limits json is ignored`，验证目标行为在回归场景下是否符合预期。
        用途：执行测试用例 `legacy limits json is ignored`，验证目标行为在回归场景下是否符合预期。
        使用场景：在 processingmcpserver 自动化测试套件执行阶段由 unittest 运行器调用，用于回归验证。
        参数与返回：
        - 参数 `self`：实例或类上下文对象，用于访问当前方法所在对象状态。
        - 返回：无返回值。
