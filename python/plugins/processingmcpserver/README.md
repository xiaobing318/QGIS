...
说明：`enable_execute_code` 目前仅作为兼容字段保留，当前版本不会注册任意代码执行类工具。
...
- `common_get_qgis_info`：读取 QGIS 运行环境、配置目录与插件状态，用于快速定位环境问题。
...
- `layer_get_panel_tree`：返回 Layers 面板组树与扁平层序，便于模型理解分组结构。
- `layer_get_details`：按图层 ID/名称读取详情（字段、几何类型、范围、CRS、数据源）。
- `layer_remove`：删除单个图层。
- `layer_remove_batch`：批量删除多个图层并返回缺失项。
- `layer_resolve_references`：把名称/ID 引用解析为唯一 layer id，支持严格模式。
...
- `vector_table_insert_records`：新增记录（API 兜底，默认安全副本）。
...
- `filesystem_query_list_entries`：按目录、递归、名称模式列举文件/目录条目（通用文件系统视角）。
- `filesystem_query_entry_info`：查询单个路径的元信息（文件/目录、大小、时间戳等）。
- `filesystem_query_read_text`：读取 UTF-8 文本文件（支持 `max_chars` 上限与截断标记）。
- `filesystem_edit_write_text`：写入 UTF-8 文本（覆盖场景需要 `overwrite=true` + `confirm_destructive=true`）。
- `filesystem_edit_append_text`：向 UTF-8 文本追加内容。
- `filesystem_edit_copy_entry`：复制文件或目录（覆盖场景需要显式确认）。
- `filesystem_edit_move_entry`：移动/重命名文件或目录（覆盖场景需要显式确认）。
- `filesystem_edit_delete_entry`：删除文件或目录（必须 `confirm_destructive=true`）。
- `filesystem_stats_directory`：统计目录文件数/目录数/总大小与扩展名分布。
...
- `vector_table_insert_records`：批量新增记录；关键输入 `records`（可带 `geometry_wkt`）；返回插入计数与未知字段告警。
...
- `vector_stats_basic`：单字段基础统计（count/min/max/mean 等）。
...
| 典型用途 | 批量发现并加载 shp/gpkg/tif | 管理配置、日志、脚本、临时文件 |
...
```json
{
  ...
  "arguments": {
    "directory": "C:/OSGeo4W64/QGIS/python/plugins/processingmcpserver",
    "recursive": true,
    "name_glob": "*.py",
    "limit": 200
  }
...
- 配置文件只影响 `dependencies.auto_install`。
...
```python
from processingmcpserver.tests.processing_mcp_server_tests import run_from_qgis_console
...
<OSGEO4W_ROOT>\apps\Python312\python.exe -m processingmcpserver.tests.processing_mcp_server_tests
```
