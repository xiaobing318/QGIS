...
- 执行要求：
  ...
  - 必须调用 `InvokeQGISBuild.ps1` 执行编译验证，并输出详细日志分析；不允许仅描述手动回归流程而不实际调用脚本。
  - 必须将测试/回归结果与自动创建的验收标准逐项严格对比；任一必选项不满足时，结论必须为“未通过”，并继续优化。
...
6. 按 `4.5` 自动创建本次 `R2` 的验收标准，并回填所有必选验收项的实际结果与证据。
7. 将测试/回归结果与自动创建的验收标准逐项做阻断式对比：任一必选项 `FAIL`、`BLOCKED` 或缺失证据，必须判定为“未通过”并继续优化。
8. 记录证据：执行环境、真实路径、执行命令、关键日志、逐项对比结果、最终结论、未覆盖项。
9. 明确说明本次为 `R2`，且已实际调用 `InvokeQGISBuild.ps1`；不得仅描述手动回归流程。

### 4.5 R2 自动验收标准与严格对比

1. 进入 `R2` 后必须先自动创建“验收标准”，并在实现与验证过程中持续回填，不得在结论阶段临时补写。
2. 验收标准必须使用统一字段，至少包含：`AC-ID`、`验收项`、`验证方式/命令`、`预期结果`、`实际结果`、`证据`、`结论(PASS/FAIL/BLOCKED)`。
3. 必选验收项最小集合如下，且不可删减：
   - `AC-FUNC-*`：每个需求点至少 1 项功能验收项。
   - `AC-REG-*`：至少 1 项回归验收项，覆盖本次改动影响面。
   - `AC-BUILD-001`：`InvokeQGISBuild.ps1` 成功完成且退出码符合预期。
   - `AC-LOG-001`：日志中无未闭环的 `fatal`、`CMake Error`、`MSB`、`LNK`、`ninja: build stopped`。
   - `AC-TEST-001`：当测试启用时，测试阶段结果满足预期（通过率、失败用例与退出码需可追溯）。
4. 严格对比规则（阻断式准入）：
   - 任一必选验收项为 `FAIL`、`BLOCKED` 或证据缺失，整体结论必须为“未通过”。
   - 结论为“未通过”时，不得宣称“已完成”或“已验收通过”，必须继续修复并重新执行对比，直到全部必选项为 `PASS`。
5. 受控例外机制（仅限客观不可测场景）：
   - 仅当环境、依赖或基础设施阻塞导致无法完成完整验证时，允许标记“受控例外（未通过）”。
   - 启用受控例外时必须同时给出：阻塞证据、已执行的替代核验、风险影响、补测触发条件与补测计划。
   - “受控例外（未通过）”不得等同于“通过”，禁止作为完成验收的替代结论。
...
- 涉及 `R2` 时，回答中必须包含“自动创建的验收标准”与“验收对比结果”两个独立小节。
- `R2` 的最终结论必须显式给出逐项对比表与总判定：`通过` / `未通过` / `受控例外（未通过）`。
- 当总判定为 `未通过` 或 `受控例外（未通过）` 时，必须给出下一轮优化目标，不得直接结束任务。
...
4. 若为 `R2`，先自动创建验收标准（含 `AC-ID`、验收项、验证方式/命令、预期结果）再实施验证。
5. 给出可执行命令、预期输出、校验点，并回填验收标准中的实际结果与证据。
6. 输出“验收对比结果”：逐项判定 `PASS/FAIL/BLOCKED`，并给出总判定（`通过` / `未通过` / `受控例外（未通过）`）。
7. 当总判定为 `未通过` 或 `受控例外（未通过）`，必须列出下一轮优化目标与补测计划。
8. 补充失败场景与排障命令，并标注结论来源（本地文件或官方资料）。
