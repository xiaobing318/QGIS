...
- 修改、完善、修复或补充内容时，若用户未明确要求大范围改写或重构，应优先做满足目标的最小充分改动，并保留既有结构和表达风格。
...
- 主代理启动子代理之后等待时间必须设置为至少 20 分钟，即 `1200000` 毫秒或者更长，曾触发 120 秒主代理等待超时，因此不能使用默认等待时间或 120 秒超时作为主代理等待子代理完成的判断状态。
...
- QGIS 版本、Qt 版本、工具链、构建/运行配置方案、平台配置文件、依赖根、发行版、glibc 系统库、部署目标、用户指定脚本目录等不属于所有任务都必须由用户预先提供的基础上下文。只有当当前任务涉及编译、测试、安装、打包、插件调试、运行时验证，或平台分册明确声明某个动作依赖这些配置时，才必须按当前平台分册继续核验并在缺失时询问用户。不得用平台分册示例、脚本隐式行为或历史资料替代用户选择。
...
- 子代理任务包必须是自包含输入，包含已选平台分册路径、基础平台信息、占位符真实值与 `Test-Path` 结果、任务等级、允许读写范围、关键路径、已核验证据、需要子代理独立确认的事项、期望输出、停止条件和回补方式。若任务依赖 `QGIS/Qt/工具链/构建/运行配置方案/平台配置文件/依赖根/发行版/glibc/部署目标` 等额外平台上下文，也必须一并提供其来源和核验结果；不依赖时必须明确写“不适用”。
...
- 仓库外写入例外：
  - 仅可在已确认有效的 `__QGISCompilationNavigation__` 占位符、当前平台分册声明的必需外部根路径及其编译输出目录下创建/修改/删除临时产物（如日志、缓存、中间文件）用于验证。执行前必须声明影响范围，验证完成后必须清理。
  - `session_state_maintainer` 子代理仅可在主代理任务包显式提供并核验的 Codex Home 会话状态路径及其父目录下创建或更新 `SessionState.md` 文件，默认路径协议为 `$CODEX_HOME/session-state/QGIS/<MAIN_CODEX_THREAD_ID>/SessionState.md` 文件，不得把该例外扩展为任意仓库外写入。
...
- Codex 指令/说明配置文件如 `AGENTS.md`、`.codex/agents/README.md`、`.codex/AgentsDocs/*.md`、`.codex/agents/*.toml` 若不被 QGIS 构建链消费，按本等级执行静态核验，不触发 QGIS 自动化编译。
...
- 只有当前任务实际依赖编译、测试、安装、打包、发布、插件调试或运行时验证时，才继续锁定 `QGIS/Qt/工具链/构建/运行配置方案/平台配置文件/依赖根/发行版/glibc/部署目标` 等额外上下文。若用户未明确提供且当前上下文也无法确认这些必需字段，必须先按当前平台分册向用户请求补充。用户无法提供、拒绝提供或补充后仍无法核验时，才按当前平台分册报告 `BLOCKED` 标签。
...
3. 按当前平台分册锁定并核验完整验证入口、配置文件、构建/运行配置方案或等效上下文、依赖根、构建/安装/发布目录、日志目录、测试启用状态和目标阶段。任一必需项未由用户明确提供且当前上下文无法确认时，必须先向用户请求补充。用户无法提供、拒绝提供或补充后仍无法核验时，回填验收项为 `BLOCKED` 并停止完整验证流程。
...
- 子代理配置位于 .codex/agents 目录中，用于辅助主代理处理大型、跨模块、多阶段或需要并行审查的 QGIS 任务。在用户显式授权且当前环境可实际调用子代理时，简单 `R0/R1` 任务等级的问答、小范围文档修改或风险很低的静态核验也必须按 `5.3` 和 `5.4` 的轻量固定编排派发职责匹配的子代理。若工具不可用、前置条件不满足或存在阻塞项，主代理必须按对应子代理检查口径完成等效审查并说明原因。
...
- 本轮用户显式授权使用子代理后，主代理的规划、派发、审查、恢复和最终验收总结都必须以 `5.3 代理编排流程图` 为流程契约，并按 `5.4` 记录轻量路径或完整路径、是否并行、适用子代理、未派发原因和偏离流程图的理由。严格按 `5.3` 执行不等于所有任务都进入完整多子代理流水线，简单 `R0/R1` 或低风险静态任务仍按轻量固定路由处理。
- `session_state_maintainer` 是会话级状态维护子代理。当前会话开始后、长任务前置检查完成后、关键节点完成或失败后、压缩前、压缩恢复后和最终汇总前，主代理必须优先尝试以 `fork_context=false` 的自包含任务包启动或复用该子代理，并让它维护 Codex Home 下的仓库外状态文件。
- `session_state_maintainer` 的“常驻”和“轮询”含义是主代理在会话生命周期内主动发送状态包并等待写入结果，不表示后台进程、自动 hook 或子代理能读取主代理未提供的隐含上下文。自动压缩不保证存在可执行的运行时前置 hook，因此主代理应在可预期的上下文压力、长任务续跑或多子代理等待前提前发送 `pre_compaction` 状态包。
- 主代理每次发送 `session_start`、`progress_checkpoint`、`pre_compaction`、`post_compaction_resume` 或 `final_summary` 状态包后，必须等待 `session_state_maintainer` 返回明确的成功写入结果。已发送请求、等待超时、字段缺失、路径未核验、写入失败或返回非成功均不算状态维护完成；可修正时必须补齐任务包后重试，无法成功时停止主线并记录阻塞。
- 发生主动压缩、自动压缩、上下文恢复或长任务续跑后，主代理恢复执行的第一项主线前置动作必须是发送 `post_compaction_resume` 状态包并等待成功写入；第二项主线前置动作必须读取并详细分析刚更新成功的 Codex Home `SessionState.md`，再结合压缩摘要、`AGENTS.md`、`.codex/agents/README.md`、相关 TOML 和当前差别重建状态。第一动作未成功、第二动作未完成前，不得继续主线编辑、测试、验证、最终答复或派发非恢复用途子代理。
- 主代理必须在派发 `session_state_maintainer` 前解析并核验状态文件绝对路径、`CODEX_HOME`、主会话 ID 和路径来源。不得要求该子代理根据它自己的 `CODEX_THREAD_ID` 推导主会话路径；子代理环境中的 `CODEX_THREAD_ID` 可能不是主会话 ID。当前会话无法热加载或调用该子代理时，主代理必须读取 `.codex/agents/SessionStateMaintainer.toml` 的 `developer_instructions` 做等效维护，并说明验证边界；等效维护只算替代核验，真实 `spawn_agent(agent_type="session_state_maintainer", fork_context=false)` 必须在 agent registry 刷新后的下一会话补测，补测前不得宣称真实派发通过。
- `SessionState.md` 的完整状态文件格式以 `.codex/agents/SessionStateMaintainer.toml` 的 `状态文件格式` 小节为准。`AGENTS.md` 只维护主代理恢复流程、成功门槛和必须覆盖的状态类别，不重复复制完整模板，避免多处模板漂移。
- `SessionState.md` 中的“测试记录”是独立状态类别，不得只用“文件和验证证据”中的通用命令记录代替。每个状态包都必须明确测试是否适用；适用时应覆盖测试目标与范围、测试矩阵、测试实现状态、执行批次、发现/筛选/通过/失败/跳过/未执行结果、日志证据、失败或未执行原因、覆盖缺口和补测计划；不适用时必须记录理由和证据。
- 测试矩阵只表示计划覆盖，不等于测试已经执行。测试通过结论必须绑定可追溯的运行批次、完整命令、工作目录、退出码、数量统计和日志证据；未执行、无匹配、未发现、跳过、超时、阻塞或证据缺失不得写成通过，也不得用估算值补齐测试数量或覆盖率。
...
- 子代理不得覆盖本 AGENTS.md 的任何约束。若子代理说明、`.codex/agents/README.md` 或平台分册与本文件冲突，必须以本文件为准。
...
| `session_state_maintainer` | 按主代理提供的自包含状态包维护 Codex Home 下的仓库外 `SessionState.md` 文件，辅助长任务、压缩前检查点和压缩恢复后重新聚焦。 | 可写主代理任务包指定的状态文件及其父目录。 |
...
下面流程图同时给出轻量固定编排路径和完整多子代理协作路径。简单 `R0/R1` 任务等级的问答、小范围文档修改或风险很低的静态核验，在满足 `5.1` 显式授权和工具可用条件时，必须先进入轻量固定路由，派发职责匹配的只读、配置一致性或风格审查代理并汇总到最终验收总结。轻量路径不进入实现、测试实现、完整测试验证或整体编译验证节点。其它可实现、可测试、跨模块、多阶段或高风险任务进入完整协作路径。一旦触发多子代理协作、任务等级 `R2` 验证、一致性审查或完整测试/构建结论，必须保留图中适用的顺序、阻断节点和失败回退关系。图中的必要平台上下文表示当前任务实际依赖且已核验的平台、工具链、构建/运行配置方案、依赖根或等效配置，不代表所有任务都必须锁定构建/运行配置方案。
...
flowchart TD
    ...
    B --> B0{主代理：是否为简单 R0/R1<br/>或低风险静态任务?}

    B0 -- 是 --> Z[主代理：轻量固定路由<br/>选择子代理 1/2/7/9/10]
    Z --> Z1[子代理 1/2/7/9/10：轻量审查或定位<br/>按 5.4 轻量路由选择其一或串行组合]
    Z1 --> W

    B0 -- 否，进入完整路径 --> B1{主代理：是否可并行拆分<br/>角色或任务分片?}
...
轻量路径和完整路径是互斥入口，其中 `B0` 判断为“是”时，轻量路径中的 Codex 一致性表示简单 `R0/R1` 或低风险静态任务直接按 `5.4` 派发 `codex_conformity_reviewer` 子代理，如果 `B0` 判断为“否”并进入完整路径后，`Q -> R` 表示实现、测试和审查链路之后，对涉及 Codex 配置域同步的任务派发同一个子代理。同一任务不会同时走两条 Codex 一致性路线。若任务升级为 `R2` 或进入完整路径，必须按重新判定后的路径执行。

`session_state_maintainer` 是会话级状态代理，不替代下图中的需求、实现、测试、审查或构建验证节点，也不改变主代理对最终结论的责任。主代理必须在进入轻量路径或完整路径前优先启动或等效维护它，并在每个适用节点完成、失败、裁剪、阻塞、压缩前或恢复后发送状态包，使 Codex Home 状态文件持续反映最新事实。
...
- 子代理派发以 `5.3 代理编排流程图` 为基线，分为轻量固定编排路径和完整多子代理协作路径。简单 `R0/R1` 任务等级的问答、小范围文档修改或风险很低的静态核验不进入完整流水线，但在满足 `5.1` 显式授权和工具可用条件时，必须按轻量固定路径派发职责匹配的子代理。完整路径必须保留适用的阻断节点、失败回退关系和第 4 章要求的验证闭环。
- 当前会话开始后或前置检查完成后，主代理必须优先尝试启动或复用 `session_state_maintainer` 子代理。长任务、多子代理协作、R2 验证、上下文压力明显升高、压缩前、恢复后和最终汇总前，必须发送对应状态包并等待成功写入；当前会话无法热加载时，按该 TOML 做等效维护并记录未覆盖风险，同时把真实 `agent_type` 派发留到 agent registry 刷新后的下一会话补测。
...
- 项目级子代理派发时，任务拆分依据必须写入自包含任务包。不得把“见上文”“按完整历史上下文处理”或类似引用当作子代理输入，也不得通过完整历史 fork 规避任务包字段要求。
...
- 下列情况不应并行派发，但仍应在满足 `5.1` 条件时按轻量固定路径派发单个或串行只读子代理：任务很小、下一步被单个阻塞问题卡住、子任务之间存在未解决的顺序依赖、写范围重叠或可能互相覆盖、并行输出难以合并为可复核结论。基础平台、路径或必要上下文缺失时，必须按第 1-3 章先停止并报告或补齐前置条件。当前环境无法实际调用对应子代理时，主代理不得实际 spawn 子代理，必须记录未派发原因，并仅在不依赖缺失子代理能力的范围内完成等效审查。
- 同一子代理类型可以按任务分片启动多个并行实例，例如多个只读仓库分析分片、多个互不重叠实现分片或多个独立审查分片。主代理必须为每个实例指定唯一分片范围和禁止触碰范围，不得让多个写权限子代理修改同一文件、同一测试数据或同一构建配置入口。
- 需求或验收口径不清且任务不宜由主代理直接澄清时，应自动派发 `qgis_acceptance_analyst` 子代理。模块、测试入口或构建入口不清且可独立定位时，应自动派发或并行派发 `qgis_repository_cartographer` 子代理。
- 简单 `R0` 只读问答的轻量固定路由：需求、范围、验收口径类问题派发 `qgis_acceptance_analyst` 子代理。仓库结构、模块入口、资料证据定位类问题派发 `qgis_repository_cartographer` 子代理。如果是 `AGENTS.md`、`.codex/**` 或 Codex 配置规则一致性类问题派发 `codex_conformity_reviewer` 子代理。主代理整合子代理证据后进入最终验收总结，不进入实现、测试实现、完整测试验证或整体编译验证节点。
- 风险很低的静态核验轻量固定路由：按核验对象派发职责最匹配的只读定位或审查代理。普通 QGIS 仓库文件一致性核验使用 `qgis_conformity_reviewer` 子代理，而 Codex 配置域一致性核验使用 `codex_conformity_reviewer` 子代理，格式和相邻风格核验使用 `qgis_code_style_reviewer` 子代理。
- 单点 `R1` 文档调整轻量固定路由：修改前可派发 `qgis_acceptance_analyst` 子代理固定范围和验收标准。主代理完成文档修改后，按文件域派发 `codex_conformity_reviewer` 或 `qgis_conformity_reviewer` 子代理做一致性审查，必要时再派发 `qgis_code_style_reviewer` 子代理做文档风格静态审查。此路径只做静态核验，不执行自动化编译或完整验证入口。
- 若 `R0/R1` 任务要求测试、运行、构建、安装、打包、发布或运行时验证结论，必须立即升级为 `R2`，按第 4 章创建验收标准并进入完整验证闭环，不得继续使用轻量固定路径替代。
...
- 修改、新增或验证 `session_state_maintainer` 子代理、状态文件路径协议、状态文件格式、常驻轮询口径、状态包成功门槛、压缩前检查点或压缩恢复读取规则后，应自动派发 `codex_conformity_reviewer` 做 Codex 配置域一致性检查。该检查不得替代状态代理自身写入成功证据。
...
- 主代理派发子代理时必须给出自包含任务包，至少包含：目标、背景摘要、范围、禁止事项、当前平台分册路径、基础平台信息、占位符真实路径与 `Test-Path` 结果、任务等级、允许读写范围、关键路径、已核验证据、需要子代理独立确认的事项、期望输出、停止条件、回补方式和需要回填的证据。
- 按 `5.3` 流程派发时，任务包还必须标注当前流程编号、上游证据、允许读写范围、失败回退目标和需要回填给主代理的证据。并行派发时还必须标注并行组 ID、分片名称、分片范围、排除范围、上游证据、输出模板、合并口径、冲突处理方式和失败回退目标。每个并行任务包都必须能脱离完整历史独立理解和执行。
- 按 `5.3` 流程派发时，任务包默认必须包含“任务状态快照”。若主代理判断为单轮短任务且不会发生上下文压缩、自动压缩、中断、恢复或多轮延续，也必须在任务包中显式写明 `任务状态快照：不适用` 及理由。
- 主代理在可能发生上下文压缩、自动压缩、中断、恢复或多轮延续的任务中，必须维护“任务状态快照”。快照至少包含：本轮目标、R 等级、平台信息、占位符真实路径与核验结果、当前 `5.3` 流程节点、轻量路径或完整路径判定、已派发/待派发/待回收子代理、并行组与分片状态、未回填证据、失败回退目标、下一步动作和验证边界。上下文压缩、中断或恢复后，主代理继续执行或最终答复前，必须先对照该快照核对当前 `5.3` 节点、未回收子代理、未回填证据、下一步动作和验证边界。存在缺口时必须先补齐、等待、回收或报告阻塞，不得跳过未闭环步骤。
- 任务状态快照只用于主代理在上下文压缩或恢复后继续保持目标、步骤和 `5.3` 编排状态，不得替代项目级子代理自包含任务包，不得作为完整历史 fork 传给项目级 `agent_type` 子代理，也不得替代第 4 章要求的验收标准、测试证据或构建证据。
- 平台分册可以补充当前平台特有的子代理任务包字段、外部根路径和阻塞条件。平台分册未声明额外子代理任务包字段时，默认继承本节和 `.codex/agents/README.md` 的通用要求，不得让子代理自行猜测平台特有上下文。
...
- 若子代理任务不依赖上述额外平台上下文，任务包必须明确写“不适用”。不得让子代理用平台分册示例、历史构建配置、完整历史上下文或猜测值补齐这些字段。
...
- `session_state_maintainer` 状态包至少包括 `session_start`、`progress_checkpoint`、`pre_compaction`、`post_compaction_resume` 和 `final_summary`。其中 `pre_compaction` 必须覆盖最新用户目标、已核验条件、编排审计表、已派发/待派发子代理、子代理结论、已修改/待修改文件、验证证据、测试适用性与适用时的测试矩阵/执行结果/异常及补测计划、计划描述、已完成/正在/未完成事项、剩余风险、禁止事项和下一步停止条件。`post_compaction_resume` 必须写明压缩摘要来源、压缩后重新核验的证据、缺失字段、状态文件路径、主会话 ID、状态文件绝对路径、路径来源、测试记录的最新有效状态和恢复后第一动作执行结果。
...
- 子代理收到按 `5.3` 派发的任务包后，还必须核对流程编号、上游证据、失败回退目标、需要回填证据、并行派发字段和任务状态快照。缺少这些字段且影响判断时，应停止对应判断并请求主代理回补，不得自行猜测流程节点、合并口径或压缩后的任务状态。
- 子代理应独立核验自己职责内的关键细节，例如用户需求和 AC 是否一致、路径和模块入口是否存在、写范围是否明确、diff 和测试矩阵是否可追溯、允许命令是否真实存在、日志和临时产物边界是否受控。
...
- 主代理在主动压缩、准备请求压缩、长任务续跑前、多子代理等待前、上下文压力明显升高或预期可能自动压缩时，必须先发送 `pre_compaction` 状态包并等待成功写入。自动压缩不保证主代理有机会在触发瞬间执行前置 hook，因此该检查点应在可预期风险出现时提前完成。
...
- 若压缩摘要没有保留 `session_state_maintainer` 的 agent id、状态文件路径或最近一次写入结果，主代理必须按缺失字段处理：重新核验 `CODEX_HOME`、会话 ID 和状态文件路径，重新启动或等效维护状态文件，并在 `post_compaction_resume` 状态包中说明缺失来源。
...
- 压缩和恢复状态摘要至少包含：状态包类型、最新用户目标、状态文件路径和来源、当前 OS 和 ISA、任务等级、允许读写范围、已核验条件、编排审计表、已派发和待派发子代理、各子代理结论、计划描述、任务进度、已修改和待修改文件、已执行验证证据、测试记录（适用性、矩阵、执行批次、异常、覆盖缺口和补测计划）、剩余风险、禁止事项和下一步停止条件。旧版独立字段如“等待状态”“无关脏改动处理方式”不再作为单独必填字段，相关信息应并入“子代理状态”“文件和验证证据”“任务进度”或“剩余风险”。恢复后缺少任一关键字段时，必须先补读 `AGENTS.md`、`.codex/agents/README.md`、相关 TOML 和当前差别，补齐后再继续实现、验证或汇总。
- 具体 `Codex Session State` 模板以 `.codex/agents/SessionStateMaintainer.toml` 的 `状态文件格式` 小节为准。修改 `SessionStateMaintainer.toml` 的状态文件格式时，应同步核验本节的状态类别和恢复契约是否仍一致，但不得把完整模板复制回 `AGENTS.md` 文件。

### 5.8 子代理 TOML 维护规则

子代理的目录级维护细则位于 `.codex/agents/README.md` 文件中。修改 `.codex/agents/*.toml` 前必须先读取该文件，并按其中字段规则、禁止事项和同步要求执行。需要注意的是 `.codex/agents/README.md` 是说明文档，不是子代理配置文件。若 `.codex/agents/README.md` 或子代理文件与 `AGENTS.md` 冲突，必须以 `AGENTS.md` 为准。
