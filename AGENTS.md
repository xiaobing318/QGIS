...
- `session_state_maintainer` 状态包至少包括 `session_start`、`progress_checkpoint`、`pre_compaction`、`post_compaction_resume` 和 `final_summary`。其中 `pre_compaction` 必须覆盖最新用户目标、已核验条件、编排审计表、已派发/待派发子代理、子代理结论、已修改/待修改文件、验证证据、计划描述、已完成/正在/未完成事项、剩余风险、禁止事项和下一步停止条件。`post_compaction_resume` 必须写明压缩摘要来源、压缩后重新核验的证据、缺失字段、状态文件路径、主会话 ID、状态文件绝对路径、路径来源和恢复后第一动作执行结果。
...
- 压缩和恢复状态摘要至少包含：状态包类型、最新用户目标、状态文件路径和来源、当前 OS 和 ISA、任务等级、允许读写范围、已核验条件、编排审计表、已派发和待派发子代理、各子代理结论、计划描述、任务进度、已修改和待修改文件、已执行验证证据、剩余风险、禁止事项和下一步停止条件。旧版独立字段如“等待状态”“无关脏改动处理方式”不再作为单独必填字段，相关信息应并入“子代理状态”“文件和验证证据”“任务进度”或“剩余风险”。恢复后缺少任一关键字段时，必须先补读 `AGENTS.md`、`.codex/agents/README.md`、相关 TOML 和当前差别，补齐后再继续实现、验证或汇总。
