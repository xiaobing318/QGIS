...
- 修改、新增或验证 `session_state_maintainer` 子代理、状态文件路径协议、状态 Markdown 格式、常驻轮询口径、状态包成功门槛、压缩前检查点或压缩恢复读取规则后，应自动派发 `codex_conformity_reviewer` 做 Codex 配置域一致性检查。该检查不得替代状态代理自身写入成功证据。
...
- `session_state_maintainer` 状态包至少包括 `session_start`、`progress_checkpoint`、`pre_compaction`、`post_compaction_resume` 和 `final_summary`。其中 `pre_compaction` 必须覆盖最新用户目标、已核验条件、5.3 编排审计表、子代理状态、文件改动、验证证据、已完成/正在/未完成事项、剩余风险、禁止事项和下一步停止条件；`post_compaction_resume` 必须写明压缩摘要来源、压缩后重新核验的证据、缺失字段、状态文件路径和恢复后第一动作执行结果。
...
- 压缩和恢复状态摘要至少包含：状态包类型、最新用户目标、状态文件路径和来源、当前 OS 和 ISA、任务等级、允许读写范围、5.3 编排审计表、已派发和待派发子代理、各子代理结论、已修改和待修改文件、已执行验证证据、已核验条件、已完成/正在进行/未完成事项、剩余风险、禁止事项和下一步停止条件。恢复后缺少任一关键字段时，必须先补读 `AGENTS.md`、`.codex/agents/README.md`、相关 TOML 和当前差别，补齐后再继续实现、验证或汇总。
